...
function applyLandingStyles() {
  ...
  style.textContent = `
    ...
    .form-container {
      ...
      padding: clamp(18px, 3.5vh, 40px) clamp(18px, 4vw, 38px) clamp(14px, 2.5vh, 32px);
    ...
    .axp-input-wrap select {
      ...
      border-radius: 4px;
      padding: clamp(8px, 1.3vh, 12px) 42px clamp(8px, 1.3vh, 12px) 13px;
...
function submitForm(action) {
  ...
  if (button) {
    ...
    button.innerHTML = `<span class="axp-spin-ring"></span> Processing...`;
    _ensureSpinnerStyle();
  }
  ...
  .then(data => {
    ...
    if (isSuccess) {
      if (action === "login") {
        ...
        const vid = document.getElementById("axp-video-bg");
        if (vid) vid.style.display = "none";
        const vidOv = document.getElementById("axp-video-overlay");
        if (vidOv) vidOv.style.display = "none";
        const cvs = document.getElementById("axp-canvas-bg");
...
function _ensureSpinnerStyle() {
  if (document.getElementById("axp-spinner-style")) return;
  const s = document.createElement("style");
  s.id = "axp-spinner-style";
  s.textContent = `
    .axp-spin-ring {
      display:inline-block;width:15px;height:15px;
      border:2px solid rgba(6,12,28,0.22);
      border-top-color:#060c1c;border-radius:50%;
      animation:axpSpin 0.7s linear infinite;flex-shrink:0;
    }
