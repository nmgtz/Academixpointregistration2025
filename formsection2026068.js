...
function renderResultsReportsSection(containerEl) {
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        // A4 landscape 297mm, margins 6mm each side = 285mm content = 1077px at 96dpi
        const _PW = 1077;
        cont.style.width    = _PW + 'px';
        cont.style.maxWidth = _PW + 'px';
        cont.style.minWidth = _PW + 'px';
        cont.style.overflow = 'hidden';   // clip nothing — tables use width:100% so they fit

        // Force every result table inside to exactly _PW wide so nothing overflows right
        cont.querySelectorAll('table.axp-results-table, table.axp-rr-table, table.summary-table').forEach(t => {
            t.style.width    = '100%';
        ...
        const opt = {
            ...
            html2canvas: {
                ...
                windowWidth: _PW,   // MUST match cont width exactly — no right-side bleed
                scrollX: 0,
        ...
        const _hdrL    = _pdfSi.displayName || _pdfSi.rawName || '';
        const _hdrR    = `${_axpExamLabel(examType,'en')} | ${_axpClassLabel(cls,'en')} | ${_pdfYear}`;
        const _MARGIN  = 8;  // matches opt.margin[0] top

        const worker = html2pdf().set(opt).from(cont);
        const p = worker.toPdf().get('pdf').then(pdf => {
            const nPages = pdf.internal.getNumberOfPages();
            const pgW    = pdf.internal.pageSize.getWidth();
            const pgH    = pdf.internal.pageSize.getHeight();
            for (let pg = 1; pg <= nPages; pg++) {
                pdf.setPage(pg);
                // ── HEADER: pages 2+ (page 1 has full school header in content) ──
                if (pg > 1) {
                    pdf.setDrawColor(51,51,51); pdf.setLineWidth(0.3);
                    pdf.line(6, _MARGIN-1, pgW-6, _MARGIN-1);
                    pdf.setFont('helvetica','bold'); pdf.setFontSize(8); pdf.setTextColor(26,58,92);
                    pdf.text(_hdrL, 6, _MARGIN-3);
                    pdf.setFont('helvetica','normal'); pdf.setFontSize(6.5); pdf.setTextColor(85,85,85);
                    pdf.text(_hdrR, pgW-6, _MARGIN-3, {align:'right'});
                }
                // ── FOOTER: all pages ──
                pdf.setDrawColor(180,180,180); pdf.setLineWidth(0.2);
                pdf.line(6, pgH-6, pgW-6, pgH-6);
                pdf.setFont('helvetica','normal'); pdf.setFontSize(7.5); pdf.setTextColor(120,120,120);
                pdf.text(`Page ${pg} of ${nPages}`, pgW/2, pgH-3.5, {align:'center'});
            }
            if (action === 'download') {
                pdf.save(filename);
            } else {
...
function buildDivisionTable(students) {
    ...
    outer.style.cssText = 'width:40%;margin:0 auto 10px;page-break-inside:avoid;break-inside:avoid;page-break-before:avoid;break-before:avoid;';
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const tdStyle    = 'border:0.5px solid #000;padding:3px 4px;text-align:center;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdNameStyle= 'border:0.5px solid #000;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdLastStyle= 'border:0.5px solid #000;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:normal;word-break:break-word;vertical-align:middle;';
    ...
    tbl.appendChild(tbody);
    // Force page break after student table so analysis tables start fresh
...
function buildStudentTableB(students, displayMode, showPosition) {
    ...
    tbl.style.cssText = 'width:100%;border-collapse:collapse;font-size:clamp(6px,0.7vw,10px);table-layout:auto;font-family:Arial,sans-serif;page-break-inside:auto;';
    ...
    // tbody
    const tbody = document.createElement('tbody');
    const tdSt  = 'border:0.5px solid #000;padding:2px 3px;text-align:center;background:#fff;color:#000;vertical-align:middle;white-space:nowrap;overflow:hidden;';
...
function buildSubjectTable(students) {
    ...
    lastPage.style.cssText = 'page-break-before:always;break-before:page;padding-top:4px;';
