...
// ─────────────────────────────────────────────────────────────────────────────
// FIXED _repCapturePdf — Clean, vertically centred, no repetition
// ─────────────────────────────────────────────────────────────────────────────
...
async function _repCapturePdf(filename, action) {
    try {
        ...
        const closingDate  = gv('closingDateInput')  || new Date().toLocaleDateString('en-GB');
        const openingDate  = gv('openingDateInput')  || '';
        const classTeacher = gv('classTeacherInput') || (currentLang === 'sw' ? 'Mwalimu wa Darasa' : 'Class Teacher');
        const headmaster   = gv('headmasterInput')   || (currentLang === 'sw' ? 'Mkuu wa Shule'     : 'Head of School');
        const reqs         = [gv('requirement1'), gv('requirement2'), gv('requirement3'),
                              ...(_saved._extraReqs || [])].filter(Boolean);
        ...
        const candNo  = ci ? `${ci}-${String(currentIndex + 1).padStart(4, '0')}` : `S000-${String(currentIndex + 1).padStart(4, '0')}`;

        // Behaviour grades
        let disc='C', coop='C', learn='C', sports='B', part='C',
            punct='C', lead='C', neat='C', resp='C', creat='C', remark=lang.remarksList.followup;
        if (pt<=17)  { disc='A';coop='A';learn='A';sports='C';part='A';punct='A';lead='A';neat='A';resp='A';creat='B';remark=lang.remarksList.outstanding; }
        else if(pt<=21){ disc='B';coop='B';learn='B';sports='C';part='B';punct='B';lead='B';neat='B';resp='B';creat='B';remark=lang.remarksList.commendable; }
        else if(pt<=30){ remark=lang.remarksList.satisfactory; }

        // Auto comments (single sentence each)
        ...
        const hosComment = (pt<=13 ? lang.remarksList.outstanding
            ...
            : lang.remarksList.followup) + ' ' + (lang.headSupportNote||'');

        // ── jsPDF setup ───────────────────────────────────────────────────
        const doc = new jsPDFCtor({ unit:'mm', format:'a4', orientation:'portrait', compress:false });
        const PW=210, PH=297, ML=10, MR=10, CW=190;

        // ── Colours ───────────────────────────────────────────────────────
        const NAVY=[26,58,92], TEAL=[78,204,163], WHITE=[255,255,255], BLACK=[0,0,0];
        const RED=[183,28,28], LGREY=[235,241,248], DGREY=[80,80,80];
        ...
        // ── Vertically centred cell text ──────────────────────────────────
        // Draws a single line of text perfectly centred vertically in a row
        const cellText = (text, x, y, w, h, opts={}) => {
            ...
            st(col);
            const ascent = fs * 0.352; // mm approx
            const cy = y + (h / 2) + (ascent / 2);
            const maxW = w - (align==='center' ? 2 : 3);
            const str  = doc.splitTextToSize(T(text), maxW)[0]; // single line only
            const tx   = align==='left' ? x+2 : align==='right' ? x+w-2 : x+w/2;
            doc.text(str, tx, cy, { align });
        };

        // ── Draw bordered row with cells ──────────────────────────────────
        const drawRow = (cols, rowY, rowH, opts={}) => {
            const { bg=null, borderCol=NAVY, lw=0.2 } = opts;
            if (bg) { sf(bg); doc.rect(cols[0].x, rowY, cols.reduce((s,c)=>s+c.w,0), rowH, 'F'); }
            sd(borderCol); doc.setLineWidth(lw);
            cols.forEach(col => {
                doc.rect(col.x, rowY, col.w, rowH, 'S');
                if (col.text !== undefined && col.text !== null) {
                    cellText(col.text, col.x, rowY, col.w, rowH, col);
                }
            });
        };
        ...
        const CONTENT_BOTTOM = DATES_TOP - 1;               // tables end here

        // ════════════════════════════════════════════════════════════════════
        // HEADER — matches results header exactly
        // ════════════════════════════════════════════════════════════════════
        let cy = 10;
        const HEADER_H = 34;
        ...
        const panel = document.getElementById('axpClassInfoPanel');
        let hLines = panel
            ? [...panel.querySelectorAll('[data-hdrline]')].map(i=>i.value.trim()).filter(Boolean)
            : SchoolInfoStorage.getHeaderLines(examClass).filter(Boolean);
        const _dd  = (typeof _dashboardData!=='undefined') ? _dashboardData : {};
        const dist = _dd.district ? _dd.district.toUpperCase()+' DISTRICT COUNCIL' : (hLines[2]||'DISTRICT COUNCIL');
        hLines[0] = currentLang==='sw' ? "OFISI YA RAIS" : "PRESIDENT'S OFFICE";
        hLines[1] = currentLang==='sw' ? "TAWALA ZA MIKOA NA SERIKALI ZA MITAA" : "REGIONAL ADMINISTRATION AND LOCAL GOVERNMENT";
        hLines[2] = dist;
        hLines[3] = (si.displayLine2 ? si.displayLine2.toUpperCase()+' — ' : '') + si.displayName.toUpperCase();
        ...
        let hy = cy+3;
        hLines.forEach((ln,i) => {
            doc.setFontSize(hFS[i]||8); doc.setFont('helvetica', hFW[i]||'normal');
            st(i>=3 ? NAVY : DGREY);
        ...
        // ════════════════════════════════════════════════════════════════════
        // MAIN TABLES — fill exactly from cy to CONTENT_BOTTOM
        // Academic (left 57%) | Behaviour (right 40%)
        // ════════════════════════════════════════════════════════════════════
        const TABLE_TOP = cy;
        ...
        // Academic column widths
        const SBJ_W = ACW * 0.35, MRK_W = ACW * 0.14, GR_W = ACW * 0.13;
        ...
        // Row 2: column labels
        drawRow([
            { x:ML,                           w:SBJ_W, text:lang.subject||'Subject', fs:7, bold:true, col:NAVY, align:'left'   },
            { x:ML+SBJ_W,                     w:MRK_W, text:lang.mark||'Mark',       fs:7, bold:true, col:NAVY, align:'center' },
            { x:ML+SBJ_W+MRK_W,              w:GR_W,  text:'Gr',                     fs:7, bold:true, col:NAVY, align:'center' },
            { x:ML+SBJ_W+MRK_W+GR_W,        w:CMT_W, text:lang.comment||'Remark',   fs:7, bold:true, col:NAVY, align:'center' }
        ], TABLE_TOP+HDR_H, HDR_H, { bg:LGREY, borderCol:[170,196,224], lw:0.2 });
        ...
        drawRow([
            { x:BHX,          w:BHW*0.76, text:lang.aspect||'Aspect', fs:7, bold:true, col:NAVY, align:'left'   },
            { x:BHX+BHW*0.76, w:BHW*0.24, text:lang.rating||'Rate',  fs:7, bold:true, col:NAVY, align:'center' }
        ], TABLE_TOP+HDR_H, HDR_H, { bg:LGREY, borderCol:[170,196,224], lw:0.2 });

        let bhRowY = TABLE_TOP + HDR_H*2;

        // ── Calculate row heights to fill space perfectly ─────────────────
        ...
        const behavItems = [
            ...
            ['leadership',lead],['neatness',neat],['respect',resp],['creativity',creat]
        ];
        const BH_REM_H = 6;
        const availBh  = TABLE_H - HDR_H*2 - BH_REM_H;
        const bhRowH   = Math.max(4, Math.min(6.5, availBh / behavItems.length));

        // ── Subject rows — vertically centred ────────────────────────────
        subjectEntries.forEach(([sub, sc], idx) => {
            const gBg   = gradeColor[sc.grade] || [120,120,120];
            const gFg   = gradeFg[sc.grade]    || WHITE;
            const rowBg = idx%2===0 ? WHITE : [250,252,255];

            // Subject, mark, comment cells
            drawRow([
                { x:ML,                       w:SBJ_W, text:sub,                            fs:7.5, col:BLACK,     align:'left'  },
                { x:ML+SBJ_W,                 w:MRK_W, text:sc.mark,                        fs:8.5, col:BLACK,     align:'center', bold:true   },
                { x:ML+SBJ_W+MRK_W+GR_W,    w:CMT_W, text:lang.comments?.[sc.grade]||'—', fs:7,   col:[90,90,90], align:'left',   italic:true }
            ], acRowY, acRowH, { bg:rowBg, borderCol:[200,210,220], lw:0.2 });

            // Grade cell — coloured fill, vertically centred grade letter
            sf(gBg); doc.rect(ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH, 'F');
            sd([200,210,220]); doc.setLineWidth(0.2); doc.rect(ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH, 'S');
            cellText(sc.grade||'—', ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH, {fs:9, bold:true, col:gFg, align:'center'});
        ...
        // ── Behaviour rows — vertically centred ───────────────────────────
        behavItems.forEach(([key, val], idx) => {
            ...
            cellText(lang.behaviorAspects?.[key]||key, BHX, bhRowY, BHW*0.76, bhRowH, {fs:7.5, col:BLACK, align:'left'});

            // Grade
            sf(bBg); doc.rect(BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, 'F');
            sd([200,210,220]); doc.rect(BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, 'S');
            cellText(val, BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, {fs:9, bold:true, col:bFg, align:'center'});

            bhRowY += bhRowH;
        });

        // Behaviour overall remark — single centred line
        ...
        cellText(remarkShort, BHX, bhRowY, BHW, BH_REM_H, {fs:7.5, col:[60,60,60], align:'center', italic:true});

        // ════════════════════════════════════════════════════════════════════
        // FIXED ZONE — always pinned to bottom
        // ════════════════════════════════════════════════════════════════════

        // Separator above fixed zone
        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, CONTENT_BOTTOM, ML+CW, CONTENT_BOTTOM);
        ...
        doc.setFont('helvetica','normal'); st(BLACK);
        doc.text(T(closingDate), ML+4+doc.getTextWidth(closesLbl), DATES_TOP+DATES_H/2+8*0.352/2);

        doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(reopensLbl, ML+CW/2+5, DATES_TOP+DATES_H/2+8*0.352/2);
        doc.setFont('helvetica','normal'); st(BLACK);
        doc.text(T(openingDate||'—'), ML+CW/2+5+doc.getTextWidth(reopensLbl), DATES_TOP+DATES_H/2+8*0.352/2);

        // ── REQUIREMENTS ─────────────────────────────────────────────────
        if (reqs.length) {
            sd(NAVY); doc.setLineWidth(0.3); doc.line(ML, REQS_TOP, ML+CW, REQS_TOP);
            sf([255,255,245]); sd([200,210,180]); doc.setLineWidth(0.2);
            doc.rect(ML, REQS_TOP, CW, REQS_H, 'FD');

            const reqLabel = currentLang==='sw' ? 'Mahitaji ya Muhula Ujao:' : 'Requirements for Next Term:';
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(reqLabel, ML+3, REQS_TOP+4.5);

            // Items on one row, spaced evenly
        ...
        // ════════════════════════════════════════════════════════════════════
        // CLASS TEACHER — PINNED, clean 3-column signature row
        // heading | comment | Name | Signature line | Date
        // ════════════════════════════════════════════════════════════════════
        {
            sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, CT_TOP, ML+CW, CT_TOP);
            sf([248,250,255]); doc.rect(ML, CT_TOP, CW, SIG_H, 'F');

            // Role label + comment on one line
            const ctRole  = currentLang==='sw' ? 'MWALIMU WA DARASA' : 'CLASS TEACHER';
            doc.setFontSize(8); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(ctRole+':', ML+2, CT_TOP+5.5);

            // Comment — fit on one line
            const ctMax   = CW - doc.getTextWidth(ctRole+': ') - 4;
            const ctShort = doc.splitTextToSize(ctComment, ctMax)[0];
            doc.setFont('helvetica','normal'); st([40,40,40]);
            ...
            // 3-column signature row: Name | Signature | Date
            const SY  = CT_TOP + 10;
            const NW  = CW*0.36, SW = CW*0.38, DW = CW*0.26;
            const NX  = ML, SX2 = ML+NW, DX2 = ML+NW+SW;
            const midSigY = SY + (SIG_H-10)/2;

            // Name col
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
            doc.text(currentLang==='sw'?'Jina:':'Name:', NX+2, SY+3);
            doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(T(classTeacher), NX+2, SY+9);

            // Signature col (blank line for pen)
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
            doc.text(currentLang==='sw'?'Sahihi:':'Signature:', SX2+2, SY+3);
            sd(NAVY); doc.setLineWidth(0.5);
            doc.line(SX2+2, SY+10, SX2+SW-4, SY+10);
        ...
        // ════════════════════════════════════════════════════════════════════
        // HEAD OF SCHOOL — PINNED, same clean layout
        // ════════════════════════════════════════════════════════════════════
        {
            ...
            const hosMax   = CW - doc.getTextWidth(hosRole+': ') - 4;
            const hosShort = doc.splitTextToSize(hosComment, hosMax)[0];
            doc.setFont('helvetica','normal'); st([40,40,40]);
            ...
            doc.text(currentLang==='sw'?'Jina:':'Name:', NX+2, SY+3);
            doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(T(headmaster), NX+2, SY+9);
        ...
        // Bottom line
        sd(NAVY); doc.setLineWidth(0.35); doc.line(ML, PH-BOT_MARGIN, ML+CW, PH-BOT_MARGIN);
