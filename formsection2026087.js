...
function _axpCandNo(idx) {
  const sid = String(_appScriptSchoolId||"0").padStart(4,"0");
...
window.axpFeedSubmitAll = function() {
  ...
  axpConfirm("Confirm Submission",
    ...
    async()=>{
      ...
      try{
        const res=await _apiPost({
          mode:"marks",adminEmail:_dashboardData.email,
          schoolId:_appScriptSchoolId,year:_schoolMeta.year,
          class:cls,examType,subject,students:JSON.stringify(payload)
        });
        if(res.status==="success"){
...
window.axpSubmitCorrection=function(){
  ...
  axpConfirm("Confirm Correction",
    ...
    async()=>{
      ...
      try{
        const res=await _apiPost({
          mode:"correct",adminEmail:_dashboardData.email,
          schoolId:_appScriptSchoolId,year:_schoolMeta.year,
          class:cls,examType,subject,
          studentName,   /* flat field — not nested */
          oldValue:nOld,newValue:nNew,reason
        });
        if(res.status==="success"){
...
var _PDF = {
  ...
  /* ── Page header ── */
  drawHeader(doc, { schoolName, examType, cls, year, title, subTitle }) {
    const x = this.ML;
    let y = this.MT;

    /* Top accent bar */
    doc.setFillColor(6, 12, 28);
    doc.rect(x, y, this.UW, 14, "F");

    /* School name */
    doc.setFont("helvetica","bold");
    doc.setFontSize(12);
    doc.setTextColor(78, 204, 163);
    doc.text((schoolName||"School").toUpperCase(), x+4, y+5.5);
    ...
    /* Title row */
    doc.setFillColor(15, 34, 72);
    doc.rect(x, y+14, this.UW, 9, "F");
    doc.setFont("helvetica","bold");
    doc.setFontSize(9);
  ...
  drawFooter(doc, pageNum, totalPages, generatedAt, pageH, pageW) {
    ...
    doc.text(`Generated: ${generatedAt}`, this.ML, y+4);

    doc.setFont("helvetica","italic");
    doc.setFontSize(6.5);
    doc.setTextColor(150,150,150);
    doc.text("AcademixPoint", this.ML + UW/2, y+4, {align:"center"});
  },
  ...
  drawRow(doc, cols, y, rowH, isHeader, isAlt) {
    ...
    doc.rect(this.ML, y, this.UW, rowH, "F");
    ...
    cols.forEach(({text, width, align, bold, color}) => {
      ...
      const r = color ? color[0] : isHeader ? 255 : 30;
      const g = color ? color[1] : isHeader ? 255 : 30;
      const b = color ? color[2] : isHeader ? 255 : 30;
      ...
      const ty = y + rowH/2 + 2.5;
      doc.text(String(text||""), tx, ty, { align: align||"left", maxWidth: width-4 });
      cx += width;
    });
...
window.axpPrintSubjectSheets = async function() {
  ...
  try {
    ...
    const schoolName = (_schoolMeta&&_schoolMeta.schoolName)||"School";
    ...
    for (const subjectName of checked) {
      ...
      students.forEach((s, idx) => {
        ...
        if (y + ROW_H > _PDF.H - _PDF.MB - 10) {
          _PDF.drawFooter(doc, pageNum, totalPages, generatedAt, _PDF.H, _PDF.W);
          doc.addPage();
          pageNum++;
          y = drawPageHeader();
          y = drawTableHeader(y);
  ...
  try {
    ...
    */
    ...
    const drawTableHeader = (y) => {
      ...
      /* Draw header bg manually across full UW */
      doc.setFillColor(6,12,28);
      doc.rect(_PDF.ML, y, UW, HEADER_ROW_H, "F");
      doc.setDrawColor(220,228,236);
      doc.setLineWidth(0.2);
      ...
      cols.forEach(col => {
        ...
        if (col.width <= 14 && col.text !== "#" && !["Cand. No.","Student Name","Sex","Signature"].includes(col.text)) {
          ...
          doc.text(col.text, cx + col.width/2, y + HEADER_ROW_H - 1, {
            angle: 50, align:"left", maxWidth: col.width + 4
          });
          doc.restoreGraphicsState();
        } else {
          const tx = col.align==="center" ? cx+col.width/2 : cx+2;
          doc.text(col.text, tx, y + HEADER_ROW_H/2 + 2.2,
    ...
    allStudents.forEach((student, idx) => {
      ...
      if (y + ROW_H > pageH - _PDF.MB - 10) {
        _PDF.drawFooter(doc, pageNum, totalPages, generatedAt, pageH, pageW);
        doc.addPage();
        pageNum++;
        y = drawPageHeader();
        y = drawTableHeader(y);
      ...
      fixedCols.forEach(col => {
        ...
        const tx = col.align==="center" ? cx+col.width/2 : cx+2;
        doc.text(String(col.text), tx, y+ROW_H/2+2.2,
