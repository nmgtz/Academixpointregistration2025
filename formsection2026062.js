...
function _axpNormalizeStudents(students) {
    ...
    students.forEach(s => {
        ...
        // Step 5 — uppercase student name (consistent across all outputs)
...
function renderResultsReportsSection(containerEl) {
    ...
    function _buildSchoolHeader() {
        ...
        lines[4] = _axpExamLabel(rawExam,'en').toUpperCase() + ' — ' + _axpClassLabel(cls,'en').toUpperCase() + ' — ' + year;
        ...
        // Inline page-break so html2canvas respects it (not just @media print)
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        const tabsEl    = document.getElementById('axpRRTabs');
        const actionsEl = document.getElementById('axpRRActions');
        const bannerEl  = document.getElementById('axpRRSelectedBanner');
        const folderEl  = document.getElementById('axpRRFolderGrid');
        // Hide all UI chrome so only result content appears in PDF
        [tabsEl, actionsEl, bannerEl, folderEl].forEach(el => { if (el) el.style.visibility = 'hidden'; });

        // A4 landscape: 297mm × 210mm → 1122 × 794px at 96dpi.
        // windowWidth = full A4 width (1122) so html2canvas renders at correct DPI.
        // margin 6mm each side ≈ 23px → content container = 1122 - 46 = 1076px.
        // Container is locked to 1076px so content never bleeds past the margin.
        const _PAGE_W   = 1122;  // full A4 landscape px at 96dpi
        const _MARGIN   = 6;     // mm each side
        const _FORCED_W = Math.round(_PAGE_W - 2 * _MARGIN * 96 / 25.4); // ≈1076
        const opt = {
            ...
            pagebreak: { mode: ['css','legacy'], before: '.page-break-before', avoid: ['tr','thead'] }
        };

        // Lock content to forced width — strip any padding/margin/shadow so
        // the results content fills the page edge-to-edge within the PDF margins.
        // overflow:visible so multi-page content isn't clipped at page boundaries.
        const _origWidth  = cont.style.width;
        const _origMaxW   = cont.style.maxWidth;
        const _origOflow  = cont.style.overflow;
        const _origBx     = cont.style.boxSizing;
        const _origPad    = cont.style.padding;
        const _origMar    = cont.style.margin;
        const _origBg     = cont.style.background;
        const _origShadow = cont.style.boxShadow;
        cont.style.width      = _FORCED_W + 'px';
        cont.style.maxWidth   = _FORCED_W + 'px';
        ...
        const restore = () => {
            [tabsEl, actionsEl, bannerEl, folderEl].forEach(el => { if (el) el.style.visibility = ''; });
            cont.style.width      = _origWidth;
            cont.style.maxWidth   = _origMaxW;
            cont.style.overflow   = _origOflow;
            cont.style.boxSizing  = _origBx;
            cont.style.padding    = _origPad;
            cont.style.margin     = _origMar;
            cont.style.background = _origBg;
            cont.style.boxShadow  = _origShadow;
        };
        let p;
        if (action === 'download') {
            p = html2pdf().set(opt).from(cont).save();
        } else {
            p = html2pdf().set(opt).from(cont).outputPdf('dataurlnewwindow');
        }
        return p.then(restore).catch(err => { restore(); console.error('[AXP PDF]', err); });
    }
...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        ...
        table.summary-table td { text-align: center; border: 0.75px solid #333; padding: 3px 5px; background: #fff; color: #000; }
        ...
        @media print {
            ...
            table.summary-table th, table.summary-table td {
                ...
                border: 0.75px solid #333 !important;
            }
...
function buildDivisionTable(students) {
    ...
    outer.style.cssText = 'width:40%;margin:0 auto 10px;page-break-inside:avoid;break-inside:avoid;';
    ...
    if (schoolGPA) {
        ...
        gpaCell.style.cssText = `text-align:center;font-weight:bold;padding:6px;border:1px solid #333;font-size:11px;letter-spacing:0.5px;background:${gpaBg};color:${gpaFg};-webkit-print-color-adjust:exact;print-color-adjust:exact;`;
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const thStyle = 'border:0.75px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    ...
    // Per-page runner: LEFT=school name, RIGHT=exam|class|year © school
    // Page footer: centred Page X of Y
    const _A_RPP  = 25;
    const _A_COLS = headers.length;
    const _siA2   = _axpSchoolInfo('en');
    const _rawExA2 = document.getElementById('axpRRExamSel') ? document.getElementById('axpRRExamSel').value : '';
    const _rawClA2 = document.getElementById('axpRRClassSel') ? document.getElementById('axpRRClassSel').value : '';
    const _yearA2  = (typeof _schoolMeta!=='undefined'&&_schoolMeta&&_schoolMeta.year) ? _schoolMeta.year : new Date().getFullYear();
    const _rRightA = `${_axpExamLabel(_rawExA2,'en')} | ${_axpClassLabel(_rawClA2,'en')} | ${_yearA2} © ${_siA2.displayName}`;
    const _rLeftA  = _siA2.displayName;
    let   _pgNumA  = 1;
    const _pgTotA  = Math.ceil(students.length / _A_RPP) + 1;

    function _insertAPageRunner() {
        // ── Bottom of outgoing page: centred page number ──
        const pgRow = document.createElement('tr');
        pgRow.style.cssText = 'page-break-inside:avoid;break-inside:avoid;';
        const pgTd = document.createElement('td');
        pgTd.colSpan = _A_COLS;
        pgTd.style.cssText = 'text-align:center;padding:4px 0 2px;font-size:8.5px;color:#666;border:none;border-top:0.5px solid #bbb;font-family:Arial,sans-serif;';
        pgTd.textContent = `Page ${_pgNumA} of ${_pgTotA}`;
        pgRow.appendChild(pgTd); tbody.appendChild(pgRow);
        _pgNumA++;
        // ── Top of new page: thin runner strip — LEFT school name | RIGHT exam·class·year © ──
        const rhr = document.createElement('tr');
        rhr.style.cssText = 'page-break-before:always;break-before:page;';
        const rTd = document.createElement('td');
        rTd.colSpan = _A_COLS;
        rTd.style.cssText = 'padding:3px 8px 3px 6px;border-bottom:1px solid #333;background:#f5f5f5;';
        rTd.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">'
            + '<span style="font-size:8px;font-weight:700;color:#1a3a5c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:45%;">' + escapeHtml(_rLeftA) + '</span>'
            + '<span style="font-size:7px;color:#555;white-space:nowrap;text-align:right;">' + escapeHtml(_rRightA) + '</span>'
            + '</div>';
        rhr.appendChild(rTd); tbody.appendChild(rhr);
        // No column header repeat — runner strip is sufficient
    }

    students.forEach((s, i) => {
        if (i > 0 && i % _A_RPP === 0) _insertAPageRunner();
        ...
        const divColourMap = { 'I':'#006400','II':'#00AA00','III':'#ADFF2F','IV':'#FFA500','O':'#FF0000' };
        const divFgMap     = { 'I':'#fff',   'II':'#fff',  'III':'#000',   'IV':'#000',   'O':'#fff'   };
        const divBg = divColourMap[s.division] || '#fff';
        const divFg = divFgMap[s.division]     || '#000';
        const cells = [
            ...
            `<td style="${tdStyle}background:${divBg};color:${divFg};-webkit-print-color-adjust:exact;print-color-adjust:exact;font-weight:bold;">${escapeHtml(s.division||'-')}</td>`,
    ...
    // Final page footer
    const _fpgRowA = document.createElement('tr');
    const _fpgTdA  = document.createElement('td');
    _fpgTdA.colSpan = _A_COLS;
    _fpgTdA.style.cssText = 'text-align:center;padding:4px 0 2px;font-size:8.5px;color:#666;border:none;border-top:0.5px solid #bbb;font-family:Arial,sans-serif;';
    _fpgTdA.textContent = `Page ${_pgNumA} of ${_pgTotA}`;
    _fpgRowA.appendChild(_fpgTdA); tbody.appendChild(_fpgRowA);
...
function buildStudentTableB(students, displayMode, showPosition) {
    ...
    if (subjects.length) {
        ...
        subjects.forEach(sub => {
            const th = document.createElement('th');
            th.textContent = sub;
            th.style.cssText = thSt + 'font-size:clamp(5px,0.65vw,9px);white-space:normal;word-break:break-word;font-weight:bold;color:#000;background:#fff;border:0.75px solid #333;';
            hr2.appendChild(th);
        });
    }
    ...
    const _B_RPP  = 25;
    const _B_COLS = fixedHeaders.length + subjects.length;
    const _siB2   = _axpSchoolInfo('en');
    const _rawExB2 = document.getElementById('axpRRExamSel') ? document.getElementById('axpRRExamSel').value : '';
    const _rawClB2 = document.getElementById('axpRRClassSel') ? document.getElementById('axpRRClassSel').value : '';
    const _yearB2  = (typeof _schoolMeta!=='undefined'&&_schoolMeta&&_schoolMeta.year) ? _schoolMeta.year : new Date().getFullYear();
    const _rRightB = `${_axpExamLabel(_rawExB2,'en')} | ${_axpClassLabel(_rawClB2,'en')} | ${_yearB2} © ${_siB2.displayName}`;
    const _rLeftB  = _siB2.displayName;
    let   _pgNumB  = 1;
    const _pgTotB  = Math.ceil(students.length / _B_RPP) + 1;

    function _insertBPageRunner() {
        // ── Bottom of outgoing page: centred page number ──
        const pgRow = document.createElement('tr');
        pgRow.style.cssText = 'page-break-inside:avoid;break-inside:avoid;';
        const pgTd = document.createElement('td');
        pgTd.colSpan = _B_COLS;
        pgTd.style.cssText = 'text-align:center;padding:4px 0 2px;font-size:8.5px;color:#666;border:none;border-top:0.5px solid #bbb;font-family:Arial,sans-serif;';
        pgTd.textContent = `Page ${_pgNumB} of ${_pgTotB}`;
        pgRow.appendChild(pgTd); tbody.appendChild(pgRow);
        _pgNumB++;
        // ── Top of new page: thin runner strip — LEFT school name | RIGHT exam·class·year © ──
        const rhr = document.createElement('tr');
        ...
        rTd.style.cssText = 'padding:3px 8px 3px 6px;border-bottom:1px solid #333;background:#f5f5f5;';
        rTd.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">'
            + '<span style="font-size:8px;font-weight:700;color:#1a3a5c;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:45%;">' + escapeHtml(_rLeftB) + '</span>'
            + '<span style="font-size:7px;color:#555;white-space:nowrap;text-align:right;">' + escapeHtml(_rRightB) + '</span>'
            + '</div>';
        rhr.appendChild(rTd); tbody.appendChild(rhr);
        // No column header repeat — runner strip is sufficient
    }
    ...
    students.forEach((s, i) => {
        if (i > 0 && i % _B_RPP === 0) _insertBPageRunner();
        const tr = document.createElement('tr');
        tr.style.cssText = 'page-break-inside:avoid;break-inside:avoid;';
        const cNum = schoolIdx ? `${schoolIdx}-${String(i+1).padStart(4,'0')}` : `S000-${String(i+1).padStart(4,'0')}`;
        ...
        subjects.forEach(sub => {
            ...
            if (sc && sc.mark != null && sc.mark !== '') {
                ...
                // Colour cell by grade
                const gMap = { A:'#006400', B:'#00AA00', C:'#ADFF2F', D:'#FFA500', F:'#FF0000' };
                const fMap = { A:'#fff',    B:'#fff',    C:'#000',    D:'#000',    F:'#fff'    };
                if (sc.grade && gMap[sc.grade]) { gradeBg = gMap[sc.grade]; gradeFg = fMap[sc.grade]; }
            }
            tr.insertAdjacentHTML('beforeend',
                `<td style="${tdSt}background:${gradeBg};color:${gradeFg};-webkit-print-color-adjust:exact;print-color-adjust:exact;">${escapeHtml(cell)}</td>`);
        });

        tbody.appendChild(tr);
    });
    // Final page footer
    const _fpgRowB = document.createElement('tr');
    const _fpgTdB  = document.createElement('td');
    _fpgTdB.colSpan = _B_COLS;
    _fpgTdB.style.cssText = 'text-align:center;padding:4px 0 2px;font-size:8.5px;color:#666;border:none;border-top:0.5px solid #bbb;font-family:Arial,sans-serif;';
    _fpgTdB.textContent = `Page ${_pgNumB} of ${_pgTotB}`;
    _fpgRowB.appendChild(_fpgTdB); tbody.appendChild(_fpgRowB);
...
function buildSubjectTable(students) {
    ...
    csT.style.cssText += 'width:100%;margin-left:0;';
    csT.innerHTML = `<thead><tr>
        <th colspan="2" style="text-align:center;background:#fff;color:#000;font-weight:bold;padding:5px 8px;border:1px solid #333;">EXAMINATION CENTRE OVERALL PERFORMANCE</th>
    </tr></thead>
        <tbody>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:1px solid #333;width:65%;">TOTAL PASSED CANDIDATES (DIV I–IV)</td>
                <td style="text-align:left;font-weight:bold;padding:4px 8px;border:1px solid #333;">${passed}</td>
            </tr>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:1px solid #333;">PERFORMANCE PERCENTAGE</td>
                <td style="text-align:left;font-weight:bold;padding:4px 8px;border:1px solid #333;">${perfPct}%</td>
            </tr>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:1px solid #333;">SCHOOL GPA</td>
                <td style="text-align:left;font-weight:bold;padding:5px 8px;border:1px solid #333;font-size:11px;background:${schoolGPABg||'#fff'};color:${schoolGPABg&&['#006400','#00AA00','#FF0000'].includes(schoolGPABg)?'#fff':'#000'};-webkit-print-color-adjust:exact;print-color-adjust:exact;">
    ...
    const thStyle = 'border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    const thFirst = document.createElement('th');
    thFirst.textContent = 'CANDIDATES'; thFirst.rowSpan=2;
    thFirst.style.cssText = thStyle + 'text-align:left;vertical-align:middle;';
    oHR1.appendChild(thFirst);
    ['REGISTERED','SAT','CLEAN','ABSENT','INC','DIV I','DIV II','DIV III','DIV IV','DIV 0']
        .forEach(h=>{ const th=document.createElement('th'); th.textContent=h; th.colSpan=3; th.style.cssText=thStyle; oHR1.appendChild(th); });
    ['F','M','T'].concat(...Array(9).fill(['F','M','T'])).forEach(l=>{
        const th=document.createElement('th'); th.textContent=l;
        th.style.cssText=thStyle+'font-size:9px;padding:2px 1px;';
        oHR2.appendChild(th);
    });
    ...
    catTd.style.cssText='border:1px solid #333;padding:3px 6px;font-weight:bold;text-align:left;background:#fff;color:#000;';
    oDR.appendChild(catTd);
    const tdSubStyle='border:1px solid #333;padding:2px 2px;text-align:center;vertical-align:middle;background:#fff;color:#000;font-size:10px;';
    ...
    const sTHStyle='border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    const thS=document.createElement('th'); thS.textContent='SUBJECT'; thS.rowSpan=2; thS.style.cssText=sTHStyle+'text-align:left;vertical-align:middle;'; sHR1.appendChild(thS);
    const satCols = [{t:'SAT-F',rs:2},{t:'SAT-M',rs:2},{t:'TOTAL',rs:2}];
    satCols.forEach(({t,rs})=>{ const th=document.createElement('th'); th.textContent=t; th.rowSpan=rs; th.style.cssText=sTHStyle+'font-size:9px;'; sHR1.appendChild(th); });
    ['A','B','C','D','F'].forEach(gr=>{ const th=document.createElement('th'); th.textContent=gr; th.colSpan=3; th.style.cssText=sTHStyle; sHR1.appendChild(th); });
    ['AVER','GPA','POS','GRADE'].forEach(h=>{ const th=document.createElement('th'); th.textContent=h; th.rowSpan=2; th.style.cssText=sTHStyle; sHR1.appendChild(th); });
    ['A','B','C','D','F'].forEach(()=>['F','M','T'].forEach(l=>{
        const th=document.createElement('th'); th.textContent=l; th.style.cssText=sTHStyle+'font-size:8px;padding:2px 1px;'; sHR2.appendChild(th);
    }));
    sThead.appendChild(sHR1); sThead.appendChild(sHR2); sT.appendChild(sThead);
    const sTbody=document.createElement('tbody');
    const sTDBase='border:1px solid #333;padding:2px 3px;text-align:center;vertical-align:middle;background:#fff;color:#000;';
    Object.keys(subjectStats).forEach(sub=>{
        ...
        const tdG=document.createElement('td'); tdG.textContent=finalGPAs[sub]; tdG.style.cssText=sTDBase; row.appendChild(tdG);
        // Position — centred
...
function renderStudentReports(students, container) {
    ...
    function render(index) {
        ...
        if (!eligStudent) {
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;height:297mm;margin:0 auto;background:#fff;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;overflow:hidden;page-break-after:always;page-break-inside:avoid;">

                    <!-- ░░ DECORATIONS (all position:absolute, pointer-events:none) ░░ -->

                    <!-- Double border frame -->
                    <div style="position:absolute;inset:5mm;border:2px solid #1a3a5c;pointer-events:none;z-index:0;"></div>
                    <div style="position:absolute;inset:7mm;border:0.5px solid #aac4e0;pointer-events:none;z-index:0;"></div>

                    <!-- Corner ornaments -->
                    <div style="position:absolute;top:5mm;left:5mm;width:8mm;height:8mm;border-top:3px solid #1a3a5c;border-left:3px solid #1a3a5c;pointer-events:none;z-index:1;"></div>
                    <div style="position:absolute;top:5mm;right:5mm;width:8mm;height:8mm;border-top:3px solid #1a3a5c;border-right:3px solid #1a3a5c;pointer-events:none;z-index:1;"></div>
                    <div style="position:absolute;bottom:5mm;left:5mm;width:8mm;height:8mm;border-bottom:3px solid #1a3a5c;border-left:3px solid #1a3a5c;pointer-events:none;z-index:1;"></div>
                    <div style="position:absolute;bottom:5mm;right:5mm;width:8mm;height:8mm;border-bottom:3px solid #1a3a5c;border-right:3px solid #1a3a5c;pointer-events:none;z-index:1;"></div>

                    <!-- Name watermarks -->
                    ${[{t:'50%',l:'50%',fs:'54px'},{t:'22%',l:'25%',fs:'26px'},{t:'75%',l:'25%',fs:'26px'},{t:'22%',l:'75%',fs:'26px'},{t:'75%',l:'75%',fs:'26px'}]
                        .map(p=>`<div style="position:absolute;top:${p.t};left:${p.l};transform:translate(-50%,-50%) rotate(-35deg);font-size:${p.fs};font-weight:bold;color:rgba(26,58,92,0.04);white-space:nowrap;pointer-events:none;z-index:0;">${escapeHtml(student.name.toUpperCase())}</div>`).join('')}

                    <!-- Cand number corners -->
                    <div style="position:absolute;top:9mm;left:9mm;font-size:7.5px;font-weight:bold;color:#777;z-index:2;">${escapeHtml(candNum)}</div>
                    <div style="position:absolute;top:9mm;right:9mm;font-size:7.5px;font-weight:bold;color:#777;z-index:2;">${escapeHtml(candNum)}</div>

                    <!-- ░░ CONTENT LAYER — flex column fills the full page ░░ -->
                    <div style="position:absolute;inset:0;z-index:2;display:flex;flex-direction:column;padding:12mm 12mm 10mm;box-sizing:border-box;gap:0;">

                        <!-- ── HEADER (fixed, ~18% of page height) ── -->
                        <div style="flex-shrink:0;text-align:center;border-bottom:1.5px solid #1a3a5c;padding-bottom:2.5mm;margin-bottom:2.5mm;">
                            <div style="font-size:8px;font-weight:700;letter-spacing:0.8px;text-transform:uppercase;color:#1a3a5c;">${lang.governmentHeader}</div>
                            <div style="font-size:7.5px;color:#555;margin:0.5mm 0;">${lang.localGov}</div>
                            <div style="display:flex;align-items:center;justify-content:center;gap:6px;margin:1.5mm 0;">
                                ${schoolLogo ? `<img src="${schoolLogo}" style="width:38px;height:38px;object-fit:contain;flex-shrink:0;-webkit-print-color-adjust:exact;print-color-adjust:exact;">` : ''}
                                <div>
                                    <div style="font-size:13px;font-weight:900;text-transform:uppercase;letter-spacing:0.5px;color:#1a3a5c;">${escapeHtml(si.displayLine1)}</div>
                                    <div style="font-size:7.5px;color:#555;">${escapeHtml(si.displayLine2)}</div>
                                </div>
                                ${schoolLogo ? `<img src="${schoolLogo}" style="width:38px;height:38px;object-fit:contain;flex-shrink:0;-webkit-print-color-adjust:exact;print-color-adjust:exact;">` : ''}
                            </div>
                            <div style="display:inline-block;border:1.5px solid #1a3a5c;padding:1.5px 12px;font-size:10px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;color:#1a3a5c;margin-top:1mm;">
                                ${lang.reportTitle} — ${escapeHtml(examLabel)} — ${escapeHtml(month.toUpperCase())}
                            </div>
                        </div>

                        <!-- ── STUDENT INFO ROW (fixed height) ── -->
                        <div style="flex-shrink:0;display:flex;border:1px solid #1a3a5c;font-size:8px;margin-bottom:2.5mm;">
                            <div style="flex:2.5;padding:2px 5px;border-right:0.5px solid #aac4e0;"><strong>${lang.studentName||'Name'}:</strong> ${escapeHtml(student.name)}</div>
                            <div style="flex:1;padding:2px 5px;border-right:0.5px solid #aac4e0;"><strong>${lang.class||'Class'}:</strong> ${escapeHtml(examClass)}</div>
                            <div style="padding:2px 5px;border-right:0.5px solid #aac4e0;"><strong>${lang.sex||'Sex'}:</strong> ${escapeHtml(student.gender||'-')}</div>
                            <div style="flex:1.3;padding:2px 5px;"><strong>Cand No:</strong> ${escapeHtml(candNum)}</div>
                        </div>

                        <!-- ── MAIN BODY (flex:1 — expands to fill all remaining space) ── -->
                        <div style="flex:1;min-height:0;display:flex;gap:5px;overflow:hidden;">

                            <!-- Academic Performance column (flex:2) -->
                            <div style="flex:2;min-width:0;display:flex;flex-direction:column;overflow:hidden;">
                                <div style="flex-shrink:0;font-size:8.5px;font-weight:bold;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1.5px solid #1a3a5c;padding-bottom:1.5px;margin-bottom:2px;color:#1a3a5c;">${lang.performanceTitle}</div>
                                <table style="width:100%;border-collapse:collapse;font-size:8px;table-layout:fixed;">
                                    <colgroup>
                                        <col style="width:38%;"><col style="width:14%;"><col style="width:10%;"><col style="width:38%;">
                                    </colgroup>
                                    <thead>
                                        <tr>
                                            <th style="border:0.5px solid #aac4e0;padding:2px 4px;text-align:left;background:#dce8f5;color:#1a3a5c;font-weight:bold;">${lang.subject}</th>
                                            <th style="border:0.5px solid #aac4e0;padding:2px 4px;background:#dce8f5;color:#1a3a5c;font-weight:bold;">${lang.mark}</th>
                                            <th style="border:0.5px solid #aac4e0;padding:2px 4px;background:#dce8f5;color:#1a3a5c;font-weight:bold;">Gr</th>
                                            <th style="border:0.5px solid #aac4e0;padding:2px 4px;background:#dce8f5;color:#1a3a5c;font-weight:bold;">${lang.comment}</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ${Object.entries(student.scores||{}).map(([sub,sc])=>{
                                            ...
                                            return `<tr>
                                                <td style="border:0.5px solid #ccd;padding:1.5px 4px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${escapeHtml(sub)}</td>
                                                <td style="border:0.5px solid #ccd;padding:1.5px 4px;text-align:center;font-weight:bold;">${sc.mark}</td>
                                                <td style="border:0.5px solid #ccd;padding:1.5px 4px;text-align:center;font-weight:bold;background:${bg};color:${fg};-webkit-print-color-adjust:exact;print-color-adjust:exact;">${sc.grade}</td>
                                                <td style="border:0.5px solid #ccd;padding:1.5px 4px;font-style:italic;color:#444;overflow:hidden;white-space:nowrap;font-size:7.5px;">${lang.comments[sc.grade]||'-'}</td>
                                            </tr>`;
                                        }).join('')}
                                    </tbody>
                                    <tfoot>
                                        <tr style="background:#eef3fc;">
                                            <td colspan="4" style="border:0.5px solid #aac4e0;padding:2px 4px;font-weight:bold;font-size:8px;">
                            ...
                            <!-- Right column: Behaviour + Requirements (flex:1) -->
                            <div style="flex:1;min-width:0;display:flex;flex-direction:column;overflow:hidden;gap:2.5mm;">

                                <!-- Behaviour table -->
                                <div style="flex-shrink:0;">
                                    <div style="font-size:8.5px;font-weight:bold;text-transform:uppercase;letter-spacing:0.5px;border-bottom:1.5px solid #1a3a5c;padding-bottom:1.5px;margin-bottom:2px;color:#1a3a5c;">${lang.behavior}</div>
                                    <table style="width:100%;border-collapse:collapse;font-size:7.5px;">
                                        <thead>
                                            <tr>
                                                <th style="border:0.5px solid #aac4e0;padding:2px 3px;text-align:left;background:#dce8f5;color:#1a3a5c;">${lang.aspect}</th>
                                                <th style="border:0.5px solid #aac4e0;padding:2px 3px;background:#dce8f5;color:#1a3a5c;width:22px;">${lang.rating}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ${Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                                                .map(([key,val])=>`<tr>
                                                    <td style="border:0.5px solid #ccd;padding:1.5px 3px;">${lang.behaviorAspects[key]}</td>
                                                    <td style="border:0.5px solid #ccd;padding:1.5px 3px;text-align:center;font-weight:bold;">${val}</td>
                                                </tr>`).join('')}
                                            <tr>
                                                <td colspan="2" style="border:0.5px solid #ccd;padding:2px 3px;font-style:italic;font-size:7px;">${escapeHtml(remark)}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>

                                <!-- Term dates + Requirements -->
                                <div style="flex-shrink:0;font-size:7.5px;border-top:0.5px solid #aac4e0;padding-top:2mm;">
                                    <div style="margin-bottom:1.5mm;">
                                        <strong>${lang.termDates}:</strong>
                                        ${lang.closes} <strong>${escapeHtml(closingDate)}</strong> &nbsp;
                                        ${lang.reopens} <strong>${escapeHtml(openingDate)}</strong>
                                    </div>
                                    ${reqs ? `<div><strong>${lang.requirements}:</strong>
                                        <ul style="margin:1mm 0 0 8px;padding:0;line-height:1.5;">${reqs}</ul>
                                    </div>` : ''}
                                </div>
                        ...
                        <!-- ── CLASS TEACHER COMMENT + NAME ── -->
                        <div style="flex-shrink:0;border-top:1px solid #aac4e0;margin-top:2mm;padding-top:2mm;">
                            <div style="font-size:8.5px;color:#222;margin-bottom:1.5mm;line-height:1.5;">
                                <strong style="color:#1a3a5c;font-size:9px;">${lang.classComment}:</strong>
                            ...
                            <div style="display:flex;align-items:center;gap:4mm;font-size:8.5px;border-top:0.5px dashed #aac4e0;padding-top:1.5mm;">
                                <span style="color:#555;">${lang.classComment.replace(/:.*/,'')}:</span>
                                <strong style="color:#000;font-size:9px;">${escapeHtml(classTeacher||'—')}</strong>
                                <span style="margin-left:auto;color:#555;">${lang.date}: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>

                        <!-- ── HEADMASTER COMMENT + NAME ── -->
                        <div style="flex-shrink:0;border-top:1px solid #aac4e0;margin-top:2mm;padding-top:2mm;">
                            <div style="font-size:8.5px;color:#222;margin-bottom:1.5mm;line-height:1.5;">
                                <strong style="color:#1a3a5c;font-size:9px;">${lang.headComment}:</strong>
                            ...
                            <div style="display:flex;align-items:center;gap:4mm;font-size:8.5px;border-top:0.5px dashed #aac4e0;padding-top:1.5mm;">
                                <span style="color:#555;">${lang.headComment.replace(/:.*/,'')}:</span>
                                <strong style="color:#000;font-size:9px;">${escapeHtml(headmaster||'—')}</strong>
                                <span style="margin-left:auto;color:#555;">${lang.date}: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>
    ...
    function renderTemplateB(index) {
        ...
        if (!eligStudent) {
            ...
            const behavRows = Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                .map(([key,val]) => `<tr style="background:#fff;">
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;height:297mm;margin:0 auto;background:#fff;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;overflow:hidden;page-break-after:always;page-break-inside:avoid;">
                    ...
                    <!-- ░░ CONTENT LAYER — flex column fills full A4 page ░░ -->
                    <div style="position:absolute;inset:0;display:flex;flex-direction:column;padding:8mm 10mm 7mm;box-sizing:border-box;gap:0;">

                        <!-- ── HEADER (fixed) ── -->
                        <div style="flex-shrink:0;display:flex;align-items:center;gap:6px;border-bottom:2.5px solid #000;padding-bottom:3mm;margin-bottom:3mm;">
                            ${schoolLogo ? `<img src="${schoolLogo}" style="width:42px;height:42px;object-fit:contain;flex-shrink:0;">` : `<div style="width:42px;height:42px;border:1px solid #ccc;display:flex;align-items:center;justify-content:center;font-size:7px;color:#999;flex-shrink:0;">LOGO</div>`}
                            <div style="flex:1;text-align:center;">
                                <div style="font-size:8px;font-weight:bold;letter-spacing:0.4px;">${lang.governmentHeader}</div>
                                <div style="font-size:7.5px;color:#555;margin:0.5mm 0;">${lang.localGov}</div>
                                <div style="font-size:13px;font-weight:bold;text-transform:uppercase;margin:1.5mm 0;">${escapeHtml(si.displayLine1)}</div>
                                <div style="font-size:8px;color:#444;margin-bottom:2mm;">${escapeHtml(si.displayLine2)}</div>
                                <div style="background:#000;color:#fff;font-size:10px;font-weight:bold;letter-spacing:1px;padding:2px 10px;display:inline-block;">
                                    ${lang.reportTitle.toUpperCase()} — ${escapeHtml(examLabel.toUpperCase())}
                                </div>
                            </div>
                            ${schoolLogo ? `<img src="${schoolLogo}" style="width:42px;height:42px;object-fit:contain;flex-shrink:0;">` : `<div style="width:42px;height:42px;flex-shrink:0;"></div>`}
                        </div>

                        <!-- ── STUDENT INFO (fixed) ── -->
                        <div style="flex-shrink:0;display:flex;border:1px solid #333;font-size:8px;margin-bottom:3mm;">
                            <div style="flex:2.5;padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Name:</strong> ${escapeHtml(student.name)}</div>
                            <div style="flex:1;padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Class:</strong> ${escapeHtml(examClass)}</div>
                            <div style="padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Sex:</strong> ${escapeHtml(student.gender||'-')}</div>
                            <div style="flex:1.2;padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Cand No:</strong> ${escapeHtml(candNum)}</div>
                            <div style="padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Pts:</strong> <strong style="color:#b71c1c;">${student.point}</strong></div>
                            <div style="padding:2px 5px;border-right:0.5px solid #bbb;"><strong>Div:</strong> <strong style="color:#b71c1c;">${escapeHtml(student.division||'-')}</strong></div>
                            <div style="padding:2px 5px;"><strong>Pos:</strong> ${posInfo}</div>
                        </div>

                        <!-- ── MAIN BODY (flex:1 fills all remaining space) ── -->
                        <div style="flex:1;min-height:0;display:flex;gap:5px;overflow:hidden;">

                            <!-- Academic results (flex:2.2) -->
                            <div style="flex:2.2;min-width:0;display:flex;flex-direction:column;overflow:hidden;">
                                <div style="flex-shrink:0;font-size:8.5px;font-weight:bold;border-bottom:1.5px solid #000;padding-bottom:1.5px;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.5px;">${lang.performanceTitle}</div>
                                <table style="width:100%;border-collapse:collapse;font-size:8px;table-layout:fixed;">
                                    <colgroup><col style="width:35%;"><col style="width:13%;"><col style="width:12%;"><col style="width:40%;"></colgroup>
                                    <thead>
                                        <tr style="background:#f0f0f0;">
                                            <th style="border:0.5px solid #bbb;padding:2px 4px;text-align:left;font-weight:bold;">Subject</th>
                                            <th style="border:0.5px solid #bbb;padding:2px 4px;font-weight:bold;">Mark</th>
                                            <th style="border:0.5px solid #bbb;padding:2px 4px;font-weight:bold;">Gr</th>
                                            <th style="border:0.5px solid #bbb;padding:2px 4px;font-weight:bold;">Remark</th>
                                        </tr>
                                    </thead>
                                    <tbody>${subjectRows}</tbody>
                                    <tfoot>
                                        <tr style="background:#f5f5f5;">
                                            <td colspan="4" style="border:0.5px solid #bbb;padding:2px 5px;font-size:8px;font-weight:bold;">
                            ...
                            <!-- Right column: Behaviour + Reqs + Remark (flex:1.1) -->
                            <div style="flex:1.1;min-width:0;display:flex;flex-direction:column;gap:3px;overflow:hidden;">
                                <div style="flex-shrink:0;font-size:8.5px;font-weight:bold;border-bottom:1.5px solid #000;padding-bottom:1.5px;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.5px;">${lang.behavior}</div>
                                <table style="width:100%;border-collapse:collapse;font-size:7.5px;flex-shrink:0;">
                                    <thead><tr style="background:#f0f0f0;">
                                        <th style="border:0.5px solid #bbb;padding:2px 4px;text-align:left;">Aspect</th>
                                        <th style="border:0.5px solid #bbb;padding:2px 4px;width:22px;">Rating</th>
                                    </tr></thead>
                                    <tbody>${behavRows}</tbody>
                                </table>
                                <div style="border:0.5px solid #bbb;padding:3px 5px;font-size:7.5px;flex-shrink:0;">
                                    <strong>Overall:</strong> <span style="font-style:italic;">${escapeHtml(remark)}</span>
                                </div>
                                ${reqs ? `<div style="font-size:7.5px;flex-shrink:0;border-top:0.5px solid #ddd;padding-top:2px;">
                        ...
                        <!-- ── SIGNATURE STRIP (fixed, at bottom) ── -->
                        <div style="flex-shrink:0;border-top:1.5px solid #000;margin-top:3mm;padding-top:2.5mm;">
                            <div style="font-size:8px;margin-bottom:2mm;">
                                <span>School Closes: <strong>${escapeHtml(closingDate)}</strong></span>
                                &nbsp;&nbsp;&nbsp;
                                <span>School Opens: <strong>${escapeHtml(openingDate)}</strong></span>
                            </div>
                            <!-- Class Teacher row -->
                            <div style="display:flex;align-items:center;gap:4mm;font-size:8.5px;border-top:0.5px dashed #ccc;padding-top:2mm;margin-bottom:2mm;">
                                <span style="color:#555;white-space:nowrap;">Class Teacher:</span>
                                <strong style="color:#000;font-size:9px;">${escapeHtml(classTeacher||'—')}</strong>
                                <span style="margin-left:auto;color:#555;white-space:nowrap;">Date: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                            <!-- Head Teacher row -->
                            <div style="display:flex;align-items:center;gap:4mm;font-size:8.5px;border-top:0.5px dashed #ccc;padding-top:2mm;">
                                <span style="color:#555;white-space:nowrap;">Head Teacher:</span>
                                <strong style="color:#000;font-size:9px;">${escapeHtml(headmaster||'—')}</strong>
                                <span style="margin-left:auto;color:#555;white-space:nowrap;">Date: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>
    ...
    // windowWidth=794 (210mm at 96dpi). reportDiv is locked to 794px before capture.
    const _REP_W = 794;
    ...
    // Lock reportDiv before capture so html2canvas sees exact A4 portrait width.
    // Also strip axp-section-card padding/shadow so the 210mm card fills edge-to-edge.
    function _repPreCapture() {
        reportDiv.style.width        = _REP_W + 'px';
        reportDiv.style.maxWidth     = _REP_W + 'px';
        reportDiv.style.minWidth     = _REP_W + 'px';
        ...
        reportDiv.style.borderRadius = '0';
    }
    function _repPostCapture() {
        reportDiv.style.width        = '';
        reportDiv.style.maxWidth     = '';
        reportDiv.style.minWidth     = '';
        reportDiv.style.overflow     = '';
        reportDiv.style.padding      = '';
        reportDiv.style.margin       = '';
        reportDiv.style.background   = '';
        reportDiv.style.boxShadow    = '';
        reportDiv.style.border       = '';
        reportDiv.style.borderRadius = '';
    }
    ...
    document.getElementById('axpRepPreview').onclick = () => {
        ...
        html2pdf().set(_pdfOpt(fn)).from(reportDiv).outputPdf('dataurlnewwindow')
            .then(()=>{ _repPostCapture(); restore(); })
            .catch(()=>{ _repPostCapture(); restore(); });
    };
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        try {
            for (let i=0; i<eligible.length; i++) {
                ...
                count++;
                prog.update(`Collecting report ${count} of ${eligible.length}…`);
            }
            ...
            if (count === 0) {
                ...
                prog.update(`Generating merged PDF for ${count} students…`);
                const fn = `${_sanitize(_schoolIdx())}_${_sanitize(_examClass())}_${_sanitize(_examType())}_AllReports.pdf`;
                // Lock mergeContainer width to portrait A4 before capture
                mergeContainer.style.width    = '794px';
                mergeContainer.style.maxWidth = '794px';
                await html2pdf().set({
...
function renderStudentCertificates(students) {
    ...
    function renderCert(index) {
        ...
        const closingDate  = _getVal('closingDateInput');
        const schoolLogo   = window.getSchoolLogo ? window.getSchoolLogo() : null;
        // Exam label (language-aware) for recognition text
        const _certExamRaw = (document.getElementById('axpRRExamSel')||{}).value || '';
        const examLabel    = _axpExamLabel(_certExamRaw, currentLang);
                ...
                <div style="position:absolute;inset:0;z-index:2;display:flex;flex-direction:column;padding:10mm 17mm 8mm 17mm;box-sizing:border-box;gap:0;">
                    ...
                    <div style="flex:1;min-height:0;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:0 10mm;">
                        ...
                        <div style="font-size:9.5px;color:#444;font-style:italic;line-height:1.9;max-width:150mm;font-family:Arial,sans-serif;">
                            ${lang.recognition} — ${escapeHtml(examLabel)}<br>
    ...
    // Before capture: lock certDiv to exact A4 landscape width, strip any container styles
    function _certPreCapture() {
        certDiv.style.width        = _CERT_W + 'px';
        certDiv.style.maxWidth     = _CERT_W + 'px';
        certDiv.style.minWidth     = _CERT_W + 'px';
        certDiv.style.overflow     = 'visible';
        certDiv.style.lineHeight   = 'normal';
        certDiv.style.padding      = '0';
        certDiv.style.margin       = '0';
        certDiv.style.background   = '#fff';
        certDiv.style.boxShadow    = 'none';
        certDiv.style.border       = 'none';
        certDiv.style.borderRadius = '0';
    }
    function _certPostCapture() {
        certDiv.style.width        = '';
        certDiv.style.maxWidth     = '';
        certDiv.style.minWidth     = '';
        certDiv.style.overflow     = 'hidden';
        certDiv.style.lineHeight   = '0';
        certDiv.style.padding      = '';
        certDiv.style.margin       = '';
        certDiv.style.background   = '';
        certDiv.style.boxShadow    = '';
        certDiv.style.border       = '';
        certDiv.style.borderRadius = '';
    }
    ...
    document.getElementById('axpCertPreview').onclick = () => {
        ...
        _certPreCapture();
        html2pdf().set(_pdfOpt(`Certificate_${currentIndex+1}.pdf`)).from(certDiv).outputPdf('dataurlnewwindow')
            .then(()=>{ _certPostCapture(); restore(); })
            .catch(e=>{ _certPostCapture(); restore(); console.error('[AXP cert]',e); });
    };
    document.getElementById('axpCertDownload').onclick = () => {
        const restore = _certBtnLoad('axpCertDownload','Downloading…');
        _certPreCapture();
        html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_Certificate_${_sanitize(allStudents[currentIndex].name)}.pdf`)).from(certDiv).save()
            .then(()=>{ _certPostCapture(); restore(); })
            .catch(e=>{ _certPostCapture(); restore(); console.error('[AXP cert]',e); _axpToast('Certificate download failed','error'); });
    };
...
function _axpExamLabel(examType, lang) {
    ...
    const t = (examType || '').trim().toLowerCase().replace(/[-_\s]+/g,' ');

    // Map: [regex pattern, en label, sw label]
    const MAP = [
        [/^(mid[\s-]?term|midterm|mid)$/,         'Midterm Examination',     'Mtihani wa Robo Muhula'],
        [/^(terminal|end[\s-]?term|end[\s-]?of[\s-]?term|final)$/,
                                                    'Terminal Examination',    'Mtihani wa Nusu Muhula'],
        [/^(robo|quarter[\s-]?term|quarter)$/,     'Quarter-Term Examination','Mtihani wa Robo Muhula'],
        [/^(weekly|week[\s-]?test|week)$/,         'Weekly Test',             'Jaribio la Wiki'],
        [/^(monthly|month[\s-]?test|month)$/,      'Monthly Test',            'Jaribio la Mwezi'],
        [/^(mock|pre[\s-]?exam|pre[\s-]?necta)$/, 'Mock Examination',        'Mtihani wa Dhihaka'],
        [/^(annual|yearly|year[\s-]?end)$/,        'Annual Examination',      'Mtihani wa Muhula'],
        [/^(opener|opening[\s-]?exam|first[\s-]?test)$/,
                                                    'Opener Test',             'Jaribio la Kwanza'],
        [/^(cat|continuous[\s-]?assessment|class[\s-]?test)$/,
                                                    'Class Assessment Test',   'Tathmini ya Darasani'],
        [/^(necta|national|national[\s-]?exam)$/,  'National Examination',    'Mtihani wa Taifa'],
    ];

    for (const [re, en, sw] of MAP) {
        if (re.test(t)) return L === 'sw' ? sw : en;
    }
    // Fallback: capitalise first letter of each word in the original string
    return (examType||'').replace(/\b\w/g, c => c.toUpperCase());
}

// ─────────────────────────────────────────────────────────────────────────────
// SECTION 2b: Class label translator
// Converts raw class strings (e.g. "form 1", "Standard 4", "Form IV")
// into language-aware labels.
// ─────────────────────────────────────────────────────────────────────────────
function _axpClassLabel(cls, lang) {
    ...
    const t = cls.trim().toUpperCase()
        ...
        .replace(/\bSTD[\s-]?/,'STANDARD ')
        .replace(/\b(I{1,3}V?|VI?)\b/, m => ({I:'1',II:'2',III:'3',IV:'4',V:'5',VI:'6'}[m]||m));
    // Swahili class names
    const SW = {
        'FORM 1':'Kidato cha Kwanza','FORM 2':'Kidato cha Pili',
        'FORM 3':'Kidato cha Tatu',  'FORM 4':'Kidato cha Nne',
        'FORM 5':'Kidato cha Tano',  'FORM 6':'Kidato cha Sita',
        'STANDARD 1':'Darasa la Kwanza','STANDARD 2':'Darasa la Pili',
        'STANDARD 3':'Darasa la Tatu',  'STANDARD 4':'Darasa la Nne',
        'STANDARD 5':'Darasa la Tano',  'STANDARD 6':'Darasa la Sita',
        'STANDARD 7':'Darasa la Saba',
    };
    if (L === 'sw' && SW[t]) return SW[t];
