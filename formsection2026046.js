        ...
        <tbody>
          ${students.map((s, i) => {
            ...
            return `<tr>
              ...
              ${subjects.map(sub => {
                const mark = (s.marks || s.scores || {})[sub];
...
function _axpRRRenderAnalysis(data, cls, examType, cont) {
  ...
  const subjectAverages = analysis.subjectAverages || subjects.map(sub => {
    const marks = students.map(s => parseFloat((s.marks||s.scores||{})[sub])).filter(n=>!isNaN(n));
    return {
      subject:  sub,
...
function _axpRRRenderSubjectRanking(data, cls, examType, cont) {
  ...
  const ranking = subjects.map(sub => {
    const marks = students
      .map(s => ({ name: s.name||"—", examNo: s.examNo||s.exam_no||"—", score: parseFloat((s.marks||s.scores||{})[sub]) }))
      ...
      <table class="axp-ranking-table">
        <thead>
          <tr>
            ...
            <th><i class="bi bi-trophy-fill" style="color:#f59e0b;"></i> Best</th><th><i class="bi bi-arrow-down-circle-fill" style="color:#ef4444;"></i> Least</th>
          </tr>
        </thead>
        <tbody>
          ${ranking.map((s, i) => `
 ...
 try {
    if (!_appScriptSchoolId) {
      cont.innerHTML = `<div class="axp-alert axp-alert-warning">
        <i class="bi bi-exclamation-triangle"></i>
        <span>School not configured yet. Complete school setup first to enable audit logs.</span>
      </div>`;
      return;
    }
