...
async function loadDashboardData(username, password) {
    try {
        ...
        console.log('Global variables set:', { schoolindex, schoolname, globalUsername });
...
window.loadUserData = function() {
    ...
    if (typeof globalUsername !== 'undefined' && globalUsername && 
        ...
        console.log('Using global data:', { globalUsername, schoolindex, schoolname });
...
function updateUIWithGlobalData() {
    const userData = {
        ...
        notifications: Array.isArray(notification) ? notification.length : 1,
        messages: Array.isArray(usermessage) ? usermessage.length : 1,
    ...
    if (schoolNameEl) {
        schoolNameEl.textContent = userData.schoolName;
    ...
    if (shareLinkTextEl) shareLinkTextEl.textContent = `${userData.schoolIndex}-TEACHER'S FEEDING AREA`;
...
window.showModernDashboard = function() {
    ...
    if (dashboardEl) {
        ...
        setTimeout(() => window.loadUserData(), 500);
    }
};
