...
const SchoolInfoStorage = {
    ...
    getClassData(className) {
        return this.getAllData()[className] || null;
    },
...
function _showRestorePrompt(className, existingData) {
    ...
    modal.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(6,12,28,0.85);display:flex;justify-content:center;align-items:center;z-index:9999;';
    ...
    box.querySelector('#_axpRestoreFresh').onclick = () => {
        if (confirm(`Clear all saved data for ${className} and start fresh?`)) {
...
(function _axpLogoSystem() {
    ...
    function _updatePreview() {
        ...
        if (saved) {
            ...
            container.querySelector('#_logoDel').onclick = () => {
                if (confirm('Delete the school logo?')) {
...
//
// PURPOSE:
//   The server sends raw student records:
//     { name, gender, scores: { SUBJECT: { mark } } }
//   This function derives every downstream field — grade per subject, total
//   grade-points (point), and division — entirely on the frontend.
//   It OVERWRITES any server-sent point/division/grade values so the display
//   is always consistent with File 1's own _axpGrade() grading scale.
//
// Call this immediately after _apiGet resolves, before caching or rendering.
// ─────────────────────────────────────────────────────────────────────────────

function _axpNormalizeStudents(students) {
    if (!Array.isArray(students)) return students;

    // Division thresholds (NECTA-style: sum of best 7 subject grade-points)
    //  I  = 7–17   II = 18–21   III = 22–25   IV = 26–33   0 = 34–35 / missing
    function _calcDivision(point) {
        const p = Number(point);
        if (isNaN(p) || p < 7 || p > 35) return '0';
        if (p <= 17) return 'I';
        if (p <= 21) return 'II';
        if (p <= 25) return 'III';
        if (p <= 33) return 'IV';
    ...
    students.forEach(s => {
        ...
        // Step 1 — derive grade for every subject from raw mark
        Object.entries(s.scores).forEach(([sub, sc]) => {
            if (sc && sc.mark != null && sc.mark !== '') {
                const derived = _axpGrade(Number(sc.mark));   // File 1 helper
                sc.grade = derived.grade;
            }
        });

        // Step 2 — collect grade-points for subjects that have a valid mark
        const gradePointMap = { A:1, B:2, C:3, D:4, E:5, S:6, F:7 };
        const allPoints = Object.values(s.scores)
            .filter(sc => sc && sc.grade && gradePointMap[sc.grade] !== undefined)
            .map(sc => gradePointMap[sc.grade]);

        // Step 3 — best 7 subjects (lowest points = best grades)
        allPoints.sort((a, b) => a - b);
        const best7 = allPoints.slice(0, 7);
        const total = best7.length > 0 ? best7.reduce((a, v) => a + v, 0) : 35;

        // Step 4 — write back computed values (overwrite server values)
        s.point    = total;
        s.division = _calcDivision(total);
    });
...
function renderResultsReportsSection(containerEl) {
    ...
    let cont = containerEl
            || document.getElementById('section-results-reports')
            || document.querySelector('[data-section="results-reports"]');

    // If still not found, search among section panels for the active one
    if (!cont) {
        const panels = document.querySelectorAll(
            '.section-panel, .section-content, [data-section], .axp-section-panel'
        );
        for (const p of panels) {
            if (p.style.display !== 'none' && !p.classList.contains('hidden') &&
                (p.dataset.section === 'results-reports' ||
                 p.id === 'section-results-reports')) {
                cont = p; break;
            }
        }
    }

    // Last resort: find the innermost visible content area that is NOT the
    // page/app shell (header, nav, sidebar must be ancestors, not the element itself)
    if (!cont) {
        const candidates = [
            document.getElementById('sectionContent'),
            document.getElementById('section-content'),
            document.getElementById('pageContent'),
            document.querySelector('.section-panel.active'),
            document.querySelector('.section-content.active'),
        ];
        cont = candidates.find(el => el &&
            !el.tagName.match(/^(HEADER|NAV|ASIDE|FOOTER)$/) &&
            el.offsetParent !== null   // must be visible
        ) || null;
    }

    if (!cont) {
        console.error('[AXP] renderResultsSection: could not find a section container');
        return;
    }

    // Safety guard: never render into body, html, header, nav or sidebar
    const forbidden = ['BODY','HTML','HEADER','NAV','ASIDE','FOOTER'];
    if (forbidden.includes(cont.tagName) ||
        cont.id.match(/sidebar|topbar|navbar|header/i) ||
        cont.classList.contains('sidebar') ||
        cont.classList.contains('navbar')) {
        console.error('[AXP] renderResultsSection: resolved container is a shell element — aborting');
        return;
    }
    ...
    cont.innerHTML = `
        <div class="axp-section-card">
            ...
            <p style="color:#888;margin-bottom:1.5em;">Select a class to view its results and generate reports.</p>

            <div class="axp-form-row" id="axpRRControls" style="flex-wrap:wrap;gap:0.8em;margin-bottom:1.5em;">
                <div class="axp-field-group" style="min-width:160px;">
                    <label class="axp-label"><i class="bi bi-collection"></i> Class</label>
                    <select id="axpRRClassSel" class="axp-select">
                        <option value="">— Select Class —</option>
                ...
                <div class="axp-field-group" style="min-width:160px;">
                    ...
                    <select id="axpRRExamSel" class="axp-select">
                        <option value="">— Select Exam —</option>
                        ${(meta.examTypes || []).map(e => `<option value="${escapeHtml(e)}">${escapeHtml(e)}</option>`).join('')}
                    </select>
                </div>
                <div class="axp-field-group" style="min-width:140px;">
                    <label class="axp-label"><i class="bi bi-list-ol"></i> Show Marks As</label>
                    <select id="axpRRDisplaySel" class="axp-select">
                        <option value="both">Mark + Grade</option>
                        <option value="raw">Raw Marks Only</option>
                        <option value="grade">Grades Only</option>
                    </select>
                </div>
                <div class="axp-field-group" style="min-width:130px;">
                    <label class="axp-label"><i class="bi bi-sort-numeric-down"></i> Position</label>
                    <select id="axpRRPosSel" class="axp-select">
                        <option value="show">Show Position</option>
                        <option value="hide">Hide Position</option>
                    </select>
                </div>
                <div class="axp-field-group" style="min-width:160px;">
                    <label class="axp-label"><i class="bi bi-layout-text-window"></i> Results Template</label>
                    <select id="axpRRResultTplSel" class="axp-select">
                        <option value="A">Template A — Standard (default)</option>
                        <option value="B">Template B — Compact Grid</option>
                    </select>
                </div>
                <div style="display:flex;align-items:flex-end;gap:0.6em;flex-wrap:wrap;">
                    <button id="axpRRFetchBtn" class="axp-btn-primary" style="display:flex;align-items:center;gap:0.5em;">
                        <i class="bi bi-search"></i> Load Results
                    </button>
                    <button id="axpRRRefreshBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
                        <i class="bi bi-arrow-clockwise"></i> Refresh
                    </button>
                </div>
            </div>
            ...
            <div id="axpRRActions" style="display:none;margin:0 0 1em;padding:6px 8px;background:#f8f9fa;border:1px solid #dee2e6;border-top:none;border-radius:0 0 4px 4px;flex-wrap:wrap;gap:0.5em;align-items:center;">
                ...
                <span class="axp-act-results axp-act-analysis" style="display:none;">
                ...
                <span class="axp-act-report" style="display:none;">
    ...
    document.getElementById('axpRRFetchBtn').onclick = () => _axpRRLoad(false);
    ...
    function _axpRRLoad(forceRefresh) {
        ...
        if (!cls || !examType) {
            _showSectionMsg('axpRRMsg', 'Please select both a class and an exam type.', 'warning');
            return;
        }
        ...
        document.getElementById('axpRRContent').innerHTML = `
            <div style="text-align:center;padding:2em;color:#888;">
                <i class="bi bi-hourglass-split" style="font-size:2em;display:block;margin-bottom:0.5em;"></i>
                Loading results…
            </div>`;
    ...
    function _axpRRShowTabs() {
        ...
        document.getElementById('axpRRExcelBtn').onclick   = () => createExcelFile(_currentStudents, examType, cls);
        document.getElementById('axpRRPdfBtn').onclick     = () => _axpRRGeneratePDF('download', cls, examType);
        document.getElementById('axpRRPreviewBtn').onclick = () => _axpRRGeneratePDF('preview', cls, examType);
        document.getElementById('axpRRCertBtn').onclick    = () => _axpRRStartCerts(_currentStudents);
    ...
    function _axpRRUpdateActionBar(tab) {
        ...
        // Hide the whole toolbar if no actions apply to this tab
        const anyVisible = tab === 'results' || tab === 'student-report';
        ...
        if (schoolLogo) {
            div.innerHTML = `
                <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:0 1rem;">
                    <img src="${schoolLogo}" style="width:60px;height:60px;object-fit:contain;flex-shrink:0;">
                    <div style="text-align:center;flex:1;">
                        <h1>${escapeHtml(si.displayName)}</h1>
                        <p style="font-size:11px;color:#555;">${escapeHtml(si.displayLine2)}</p>
                        <p>Academic Year: ${_schoolMeta.year || '—'} &nbsp;|&nbsp; Class: ${escapeHtml(cls)} &nbsp;|&nbsp; Exam: ${escapeHtml(examType)}</p>
                    </div>
                    <img src="${schoolLogo}" style="width:80px;height:80px;object-fit:contain;flex-shrink:0;">
                </div>`;
        } else {
            div.innerHTML = `
                <h1>${escapeHtml(si.displayName)}</h1>
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        const _si     = _axpSchoolInfo('en');
        const sName   = _si.placeName || _si.rawName || 'School';
        ...
        // Temporarily hide the nav bar and action buttons
        const tabsEl    = document.getElementById('axpRRTabs');
        const actionsEl = document.getElementById('axpRRActions');
        ...
        const opt = {
            margin: [4, 3, 4, 3],
            filename: `${sanitize(sName)}_${sanitize(cls)}_${sanitize(examType)}.pdf`,
            ...
            html2canvas: {
                ...
                allowTaint: true,
                logging: false,
                // Force borders to render sharply
                imageTimeout: 0,
        ...
        const worker = html2pdf().set(opt).from(cont);
        const restore = () => {
            if (tabsEl)    tabsEl.style.display    = 'block';
            if (actionsEl) actionsEl.style.display = 'flex';
        };
        if (action === 'download') {
            worker.save().then(restore).catch(restore);
        } else {
            worker.outputPdf('dataurlnewwindow').then(restore).catch(restore);
        }
    }
...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        ...
        /* Header rows — dark navy for readability in print */
        ...
        /* No background colors on result table rows — clean white for PDF */
        table.axp-results-table tbody tr,
        table.axp-rr-table tbody tr {
            background: #fff;
        }
        /* Division summary table header */
        table.summary-table thead tr {
            background: #fff !important;
            color: #000 !important;
        }
        table.summary-table th,
        table.summary-table td { text-align: center; border: 1px solid #333; padding: 3px 5px; }
...
function buildDivisionTable(students) {
    ...
    // Outer wrapper — centers the 40% table
    const outer = document.createElement('div');
    outer.style.cssText = 'width:100%;display:flex;justify-content:center;margin-bottom:10px;';

    const tbl = document.createElement('table');
    tbl.className = 'axp-rr-table summary-table';
    // 40% width, centered, no background color on any row
    tbl.style.cssText = 'width:40%;border-collapse:collapse;font-size:clamp(8px,0.85vw,12px);table-layout:auto;';
    ...
    [document.createElement('th'), ...Object.keys(divCount).map(d => {
        ...
        th.style.cssText = 'border:1px solid #333;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
        hRow.appendChild(th);
    });
    ...
    ['F','M'].forEach(g => {
        ...
        td0.textContent = g;
        td0.style.cssText = 'border:1px solid #333;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
        row.appendChild(td0);
        Object.keys(divCount).forEach(d => {
            const td = document.createElement('td');
            td.textContent = divGender[d][g];
            td.style.cssText = 'border:1px solid #333;padding:4px 6px;text-align:center;background:#fff;color:#000;';
            row.appendChild(td);
        });
    ...
    td0t.textContent = 'T';
    td0t.style.cssText = 'border:1px solid #333;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    totRow.appendChild(td0t);
    Object.keys(divCount).forEach(d => {
        const td = document.createElement('td');
        td.textContent = divCount[d];
        td.style.cssText = 'border:1px solid #333;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    ...
    if (schoolGPA) {
        let grade='F', comment='Fail';
        if (schoolGPA <= 1.6) { grade='A'; comment='Excellent'; }
        else if (schoolGPA <= 2.6) { grade='B'; comment='Very Good'; }
        else if (schoolGPA <= 3.6) { grade='C'; comment='Good'; }
        else if (schoolGPA <= 4.6) { grade='D'; comment='Satisfactory'; }
        const gpaRow = document.createElement('tr');
        const gpaCell = document.createElement('td');
        gpaCell.colSpan = Object.keys(divCount).length + 1;
        gpaCell.style.cssText = 'text-align:center;font-weight:bold;padding:4px 6px;border:1px solid #333;background:#fff;color:#000;';
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const thStyle = 'border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
    ...
    const tdStyle    = 'border:1px solid #333;padding:3px 4px;text-align:center;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdNameStyle= 'border:1px solid #333;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdLastStyle= 'border:1px solid #333;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:normal;word-break:break-word;vertical-align:middle;';

    const tbody = document.createElement('tbody');
    students.forEach((s, i) => {
        const tr = document.createElement('tr');
        tr.style.cssText = 'page-break-inside:avoid;break-inside:avoid;';
        const cNum = schoolIdx ? `${schoolIdx}-${String(i+1).padStart(4,'0')}` : String(i+1).padStart(4,'0');
        ...
        const cells = [
            ...
            `<td style="${tdStyle}">${escapeHtml(s.division||'-')}</td>`,
...
function buildStudentTableB(students, displayMode, showPosition) {
    ...
    tbl.style.cssText = 'width:100%;border-collapse:collapse;font-size:clamp(6px,0.7vw,10px);table-layout:fixed;font-family:Arial,sans-serif;page-break-inside:auto;';
    ...
    const thSt = 'border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;vertical-align:middle;';
    const fixedHeaders = ["CAND'S NO","CAND'S NAME","SEX","AGG","DIV",...(showPosition?["POS"]:[])];
    fixedHeaders.forEach((h,i) => {
        const th = document.createElement('th');
        th.textContent = h;
        th.style.cssText = thSt + (i===1?'text-align:left;':'');
        th.rowSpan = 2;
        hr1.appendChild(th);
    });
    ...
    if (subjects.length) {
        ...
        subGrpTh.style.cssText = thSt;
        hr1.appendChild(subGrpTh);

        subjects.forEach(sub => {
            const th = document.createElement('th');
            th.textContent = sub;
            th.style.cssText = thSt + 'font-size:clamp(5px,0.65vw,9px);white-space:normal;word-break:break-word;';
            hr2.appendChild(th);
        });
    }
    ...
    // tbody
    const tbody = document.createElement('tbody');
    const tdSt  = 'border:1px solid #333;padding:2px 3px;text-align:center;background:#fff;color:#000;vertical-align:middle;white-space:nowrap;overflow:hidden;';
    ...
    students.forEach((s, i) => {
        ...
        subjects.forEach(sub => {
            ...
            tr.insertAdjacentHTML('beforeend', `<td style="${tdSt}">${escapeHtml(cell)}</td>`);
        });
...
function buildSubjectTable(students) {
    ...
    if (schoolGPAVal!==null) {
        ...
        else if(schoolGPAVal<2.6){schoolGradeComment='B (V.Good)';schoolGPABg='#00FF00';}
    ...
    function mkTbl(className) {
        ...
        t.style.cssText = 'width:100%;border-collapse:collapse;font-size:clamp(7px,0.80vw,12px);margin-top:8px;table-layout:fixed;background:#fff;color:#000;';
        return t;
    }

    // Centre Summary
    const csT = mkTbl('center-summary-table');
    csT.innerHTML = `<thead><tr><th colspan="2" style="text-align:center;">EXAMINATION CENTRE OVERALL PERFORMANCE</th></tr></thead>
        <tbody>
            <tr><td style="font-weight:bold;">TOTAL PASSED CANDIDATES (DIV I–IV)</td><td style="text-align:center;font-weight:bold;">${passed}</td></tr>
            <tr><td style="font-weight:bold;">PERFORMANCE PERCENTAGE</td><td style="text-align:center;font-weight:bold;">${perfPct}%</td></tr>
            <tr><td style="font-weight:bold;">SCHOOL GPA</td>
                <td style="text-align:center;font-weight:bold;color:#000;">${schoolGPA} - ${schoolGradeComment}</td></tr>
        </tbody>`;
    ...
    ['CANDIDATES','REGISTERED','SAT','CLEAN','ABSENT','INC','DIV I','DIV II','DIV III','DIV IV','DIV 0']
        .forEach((h,i)=>{ const th=document.createElement('th'); th.textContent=h; th.colSpan=i===0?1:3; oHR1.appendChild(th); });
    const gTh = document.createElement('th'); gTh.textContent='GENDER'; oHR2.appendChild(gTh);
    for(let i=0;i<10;i++) ['F','M','T'].forEach(l=>{ const th=document.createElement('th'); th.textContent=l; oHR2.appendChild(th); });
    ...
    const catTd=document.createElement('td'); catTd.textContent='No. of Candidates'; catTd.style.fontWeight='bold'; oDR.appendChild(catTd);
    ['registered','sat','clean','absent','INC'].forEach(key=>{
        const f=overall[key].F, m=overall[key].M;
        [f,m,f+m].forEach(v=>{ const td=document.createElement('td'); td.textContent=v; oDR.appendChild(td); });
    });
    ...
    const thS=document.createElement('th'); thS.textContent='SUBJECT'; thS.rowSpan=2; sHR1.appendChild(thS);
    ['SAT-F','SAT-M','TOTAL'].forEach(h=>{ const th=document.createElement('th'); th.textContent=h; th.rowSpan=2; sHR1.appendChild(th); });
    ['A','B','C','D','F'].forEach(gr=>{ const th=document.createElement('th'); th.textContent=gr; th.colSpan=3; sHR1.appendChild(th); });
    ['AVER','GPA','POS','GRADE'].forEach(h=>{ const th=document.createElement('th'); th.textContent=h; th.rowSpan=2; sHR1.appendChild(th); });
    ['A','B','C','D','F'].forEach(()=>['F','M','T'].forEach(l=>{ const th=document.createElement('th'); th.textContent=l; sHR2.appendChild(th); }));
    sThead.appendChild(sHR1); sThead.appendChild(sHR2); sT.appendChild(sThead);
    const sTbody=document.createElement('tbody');
    Object.keys(subjectStats).forEach(sub=>{
        const row=document.createElement('tr');
        const sTd=document.createElement('td'); sTd.textContent=sub; sTd.style.fontWeight='bold'; row.appendChild(sTd);
        const satF=subjectCounts[sub]?.F||0, satM=subjectCounts[sub]?.M||0;
        [satF,satM,satF+satM].forEach(v=>{ const td=document.createElement('td'); td.textContent=v; row.appendChild(td); });
        ['A','B','C','D','F'].forEach(gr=>{
            const gF=subjectStats[sub][gr]['F']||0, gM=subjectStats[sub][gr]['M']||0;
            [gF,gM,gF+gM].forEach(v=>{ const td=document.createElement('td'); td.textContent=v; row.appendChild(td); });
        });
        const tdA=document.createElement('td'); tdA.textContent=finalAvgs[sub]; row.appendChild(tdA);
        const tdG=document.createElement('td'); tdG.textContent=finalGPAs[sub]; row.appendChild(tdG);
        const tdP=document.createElement('td'); tdP.textContent=positions[sub]||''; row.appendChild(tdP);
        const tdC=document.createElement('td');
        const gt=finalGrades[sub]; tdC.textContent=gt;
        const grBg=gt.startsWith('A')?'#006400':gt.startsWith('B')?'#00FF00':gt.startsWith('C')?'#ADFF2F':gt.startsWith('D')?'#FFA500':'#FF0000';
        tdC.style.cssText='border:1px solid #333;font-weight:bold;color:#000;text-align:center;'; row.appendChild(tdC);
        sTbody.appendChild(row);
    });
    sT.appendChild(sTbody);

    // ── EXAMINATION CENTRE OVERALL PERFORMANCE goes on its own forced page ──
    // Wrap it in a page-break div so it always starts a new standalone page in the PDF
    const lastPageDiv = document.createElement('div');
    lastPageDiv.style.cssText = 'page-break-before:always;break-before:page;padding-top:4px;';
    lastPageDiv.appendChild(csT);
    lastPageDiv.appendChild(oT);
    lastPageDiv.appendChild(sT);

    const wrapper=document.createElement('div');
    wrapper.appendChild(lastPageDiv);
    return wrapper;
}
...
function _axpFormalDoc(titleText) {
    const si       = _axpSchoolInfo('en');  // formal docs always in English
    ...
    hdr.innerHTML = logo
        ? `<div style="display:flex;align-items:center;justify-content:space-between;padding:0 8px;">
               <img src="${logo}" style="width:55px;height:55px;object-fit:contain;">
               <div style="flex:1;text-align:center;">
                   <div style="font-size:15px;font-weight:bold;text-transform:uppercase;">${escapeHtml(si.displayName)}</div>
                   <div style="font-size:11px;color:#444;margin-top:1px;">${escapeHtml(si.displayLine2)}</div>
                   <div style="font-size:11px;margin-top:2px;">${escapeHtml(cls)} &nbsp;|&nbsp; ${escapeHtml(examType)}</div>
               </div>
               <img src="${logo}" style="width:55px;height:55px;object-fit:contain;">
           </div>`
        : `<div style="font-size:15px;font-weight:bold;text-transform:uppercase;">${escapeHtml(si.displayName)}</div>
    ...
    ttl.style.cssText = 'background:#1a3a5c;color:#fff;font-size:12px;font-weight:bold;text-align:center;padding:5px 0;letter-spacing:1px;text-transform:uppercase;margin-bottom:10px;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
...
function _axpTH(text, opts) {
    ...
    const th = document.createElement('th');
    th.textContent = text;
    th.style.cssText = `border:1px solid #333;padding:4px 6px;text-align:${opts.align||'center'};font-weight:bold;background:#fff;color:#000;-webkit-print-color-adjust:exact;print-color-adjust:exact;${opts.extra||''}`;
...
function _axpTD(text, opts) {
    ...
    td.style.cssText = `border:1px solid #333;padding:3px 5px;text-align:${opts.align||'center'};background:#fff;color:#000;${opts.extra||''}`;
...
function _buildAnalysisView(students) {
    ...
    const wrap = _axpFormalDoc('ACADEMIC PERFORMANCE ANALYSIS REPORT');
...
function _buildRankingView(students, direction, limit) {
    ...
    const title = direction==='top'
        ? `TOP ${limit} PERFORMING STUDENTS`
        : `LEAST ${limit} PERFORMING STUDENTS`;

    const _siR = _axpSchoolInfo('en');
    const schoolIdx = _siR.rawIndex || '';
    ...
    ranked.forEach((s, i) => {
        const tr = document.createElement('tr');
        tr.style.cssText = 'page-break-inside:avoid;break-inside:avoid;';
        const candNum = schoolIdx ? `${schoolIdx}-${String(students.indexOf(s)+1).padStart(4,'0')}` : String(i+1).padStart(4,'0');
...
function _buildGeneralView(students) {
    ...
    const wrap = _axpFormalDoc('GENERAL EXAMINATION RESULTS SUMMARY');
...
function _buildSubjectWiseView(students) {
    ...
    const schoolIdx = _siSW.rawIndex || '';

    const wrap = _axpFormalDoc('SUBJECT-WISE STUDENT PERFORMANCE');
    ...
    subjects.forEach(sub => {
        ...
        const subHdr = document.createElement('div');
        subHdr.style.cssText = 'background:#1a3a5c;color:#fff;font-weight:bold;font-size:11px;padding:4px 8px;margin-bottom:0;text-transform:uppercase;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
...
function _buildSubjectRankingView(students) {
    ...
    const wrap = _axpFormalDoc('SUBJECT RANKING BY ACADEMIC PERFORMANCE');
...
function renderStudentsReportSection(containerEl) {
    ...
    // Scoped resolver — never touches page shell (header/nav/sidebar).
    let cont = containerEl
            || document.getElementById('section-students-report')
            || document.querySelector('[data-section="students-report"]');
    ...
    if (!cont) {
        ...
        for (const p of panels) {
            if (p.style.display !== 'none' && !p.classList.contains('hidden') &&
                (p.dataset.section === 'students-report' ||
                 p.id === 'section-students-report')) {
                cont = p; break;
            }
        }
    }
    ...
    if (forbidden.includes(cont.tagName) ||
        cont.id.match(/sidebar|topbar|navbar|header/i) ||
        cont.classList.contains('sidebar') ||
        cont.classList.contains('navbar')) return;
...
function renderStudentReports(students, container) {
    ...
    function _getVal(fieldId) {
        // Try live DOM first, then SchoolInfoStorage, then empty
    ...
    function render(index) {
        ...
        const reqs         = [_getVal('requirement1'),_getVal('requirement2'),_getVal('requirement3')].filter(Boolean).map(r=>`<li>${escapeHtml(r)}</li>`).join('');

        const candNum = si.rawIndex
            ? `${si.rawIndex.toUpperCase()}-${String(index+1).padStart(4,'0')}`
            : String(index+1).padStart(4,'0');

        const reportTitle = currentLang==='en'
            ? `${translClass} ${lang.reportTitle} - ${month.toUpperCase()}`
            : `${lang.reportTitle} - ${translClass} - ${month.toUpperCase()}`;
        ...
        if (!eligStudent) {
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;min-height:297mm;margin:0 auto;background:#fff;padding:10mm 12mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;">

                    <!-- Outer border frame -->
                    <div style="position:absolute;top:5mm;left:5mm;right:5mm;bottom:5mm;border:2px solid #1a3a5c;pointer-events:none;"></div>
                    <div style="position:absolute;top:7mm;left:7mm;right:7mm;bottom:7mm;border:0.5px solid #aac4e0;pointer-events:none;"></div>

                    <!-- Name watermarks -->
                    ${[{t:'50%',l:'50%',fs:'58px'},{t:'22%',l:'22%',fs:'28px'},{t:'78%',l:'22%',fs:'28px'},{t:'22%',l:'78%',fs:'28px'},{t:'78%',l:'78%',fs:'28px'}]
                        .map(p=>`<div style="position:absolute;top:${p.t};left:${p.l};transform:translate(-50%,-50%) rotate(-40deg);font-size:${p.fs};font-weight:bold;color:rgba(0,0,0,0.025);white-space:nowrap;pointer-events:none;">${escapeHtml(student.name.toUpperCase())}</div>`).join('')}

                    <!-- Cand number top corners -->
                    <div style="position:absolute;top:10mm;left:10mm;font-size:9px;font-weight:bold;color:#555;">${escapeHtml(candNum)}</div>
                    <div style="position:absolute;top:10mm;right:10mm;font-size:9px;font-weight:bold;color:#555;">${escapeHtml(candNum)}</div>

                    <!-- Government header -->
                    <div style="text-align:center;padding-top:6mm;margin-bottom:3mm;">
                        <p style="margin:0;font-size:11px;font-weight:bold;color:#1a3a5c;letter-spacing:0.5px;">${lang.governmentHeader}</p>
                        <p style="margin:1px 0 4px;font-size:10px;color:#1a3a5c;">${lang.localGov}</p>
                        <div style="width:100%;height:1.5px;background:linear-gradient(90deg,transparent,#1a3a5c 20%,#1a3a5c 80%,transparent);margin-bottom:3px;"></div>
                        ${schoolLogo ? `<img src="${schoolLogo}" style="width:55px;height:55px;object-fit:contain;display:block;margin:4px auto;">` : ''}
                        <h2 style="margin:2px 0;font-size:13px;font-weight:bold;color:#1a3a5c;text-transform:uppercase;">${escapeHtml(si.displayLine1)}</h2>
                        <p style="margin:1px 0;font-size:9px;color:#555;">${escapeHtml(si.displayLine2)}</p>
                        <div style="width:100%;height:1.5px;background:linear-gradient(90deg,transparent,#1a3a5c 20%,#1a3a5c 80%,transparent);margin-top:3px;"></div>
                        <h3 style="margin:4px 0;font-size:12px;font-weight:bold;color:#fff;background:#1a3a5c;padding:3px 0;letter-spacing:1px;">${lang.reportTitle}</h3>
                    </div>

                    <!-- Student intro -->
                    <p style="font-size:10px;line-height:1.6;margin:2mm 0 3mm;color:#222;">
                        ${lang.examNotice} <strong style="color:#1a3a5c;">${escapeHtml(student.name)}</strong>
                        ${lang.satFor} <strong>${escapeHtml(examType.toUpperCase())}</strong> ${lang.at}
                        <strong style="color:#1a3a5c;">${escapeHtml(si.displayLine1)}</strong>
                        (${escapeHtml(si.displayLine2)}).
                        ${lang.academicStanding}
                    </p>

                    <!-- Main body: performance + behaviour -->
                    <div style="display:flex;gap:8px;">

                        <!-- Academic Performance -->
                        <div style="flex:2;min-width:0;">
                            <div style="background:#1a3a5c;color:#fff;text-align:center;font-size:10px;font-weight:bold;padding:4px 0;margin-bottom:0;">${lang.performanceTitle}</div>
                            <table style="width:100%;border-collapse:collapse;font-size:9.5px;">
                                <thead>
                                    <tr style="background:#dce8f5;color:#1a3a5c;">
                                        <th style="border:0.5px solid #aac;padding:4px 5px;text-align:left;">${lang.subject}</th>
                                        <th style="border:0.5px solid #aac;padding:4px 5px;">${lang.mark}</th>
                                        <th style="border:0.5px solid #aac;padding:4px 5px;">${lang.comment}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${Object.entries(student.scores||{}).map(([sub,sc],idx)=>{
                                        if(sc.mark==null||sc.mark==='') return '';
                                        const bg=idx%2===0?'#f7fafd':'#fff';
                                        const grColor=sc.grade==='A'?'#1b5e20':sc.grade==='B'?'#2e7d32':sc.grade==='C'?'#827717':sc.grade==='D'?'#e65100':'#b71c1c';
                                        return `<tr style="background:${bg};">
                                            <td style="border:0.5px solid #ccd;padding:3px 5px;">${escapeHtml(sub)}</td>
                                            <td style="border:0.5px solid #ccd;padding:3px 5px;text-align:center;font-weight:bold;color:${grColor};">${sc.mark} <span style="font-size:8.5px;">(${sc.grade})</span></td>
                                            <td style="border:0.5px solid #ccd;padding:3px 5px;font-style:italic;color:#444;">${lang.comments[sc.grade]||'-'}</td>
                                        </tr>`;
                                    }).join('')}
                                    <tr style="background:#e8f0fe;">
                                        <td colspan="3" style="border:0.5px solid #ccd;padding:4px 5px;font-size:10px;">
                                            <strong>${lang.points}:</strong> <span style="color:#b71c1c;font-weight:bold;">${student.point}</span>
                                            &nbsp;&nbsp;&nbsp;
                                            <strong>${lang.division}:</strong> <span style="color:#b71c1c;font-weight:bold;">${student.division}</span>
                                            &nbsp;&nbsp;&nbsp;
                                            <strong>${lang.position}:</strong> ${posInfo}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div style="flex:1;min-width:0;">
                            <div style="background:#1a3a5c;color:#fff;text-align:center;font-size:10px;font-weight:bold;padding:4px 0;margin-bottom:0;">${lang.behavior}</div>
                            <table style="width:100%;border-collapse:collapse;font-size:9px;">
                                <thead>
                                    <tr style="background:#dce8f5;color:#1a3a5c;">
                                        <th style="border:0.5px solid #aac;padding:3px 4px;text-align:left;">${lang.aspect}</th>
                                        <th style="border:0.5px solid #aac;padding:3px 4px;">${lang.rating}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    ${Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                                        ...
                                        <tr style="background:${idx%2===0?'#f7fafd':'#fff'};">
                                            <td style="border:0.5px solid #ccd;padding:3px 4px;">${lang.behaviorAspects[key]}</td>
                                            <td style="border:0.5px solid #ccd;padding:3px 4px;text-align:center;font-weight:bold;">${val}</td>
                                        </tr>`).join('')}
                                    <tr>
                                        <td colspan="2" style="border:0.5px solid #ccd;padding:4px 5px;font-size:9px;">
                                            <strong>${lang.remarks}</strong><br>${escapeHtml(remark)}
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <!-- Term info + comments -->
                    <div style="margin-top:4mm;font-size:9.5px;color:#222;line-height:1.7;">
                        <p style="margin:1mm 0;"><strong style="color:#1a3a5c;">${lang.termDates}:</strong>
                            ${lang.closes} <strong>${escapeHtml(closingDate)}</strong>
                            ${lang.reopens} <strong>${escapeHtml(openingDate)}</strong>.
                        </p>
                        <p style="margin:1mm 0;"><strong style="color:#1a3a5c;">${lang.requirements}:</strong></p>
                        <ul style="margin:-1mm 0 2mm 4mm;font-size:9px;">${reqs}</ul>
                        <div style="border-top:0.5px solid #ccc;padding-top:2mm;">
                            <p style="margin:1mm 0;">
                                <strong style="color:#1a3a5c;">${lang.classComment}:</strong>
                            ...
                            <div style="display:flex;gap:10mm;font-size:9px;margin:1mm 0 2mm;">
                                <span><strong>${lang.signature}:</strong> <span style="display:inline-block;width:35mm;border-bottom:0.5px solid #000;">&nbsp;</span></span>
                                <span><strong>${lang.date}:</strong> ${escapeHtml(closingDate)}</span>
                            </div>
                            <p style="margin:1mm 0;">
                                <strong style="color:#1a3a5c;">${lang.headComment}:</strong>
                            ...
                            <div style="display:flex;gap:10mm;font-size:9px;margin:1mm 0;">
                                <span><strong>${lang.signature}:</strong> <span style="display:inline-block;width:35mm;border-bottom:0.5px solid #000;">&nbsp;</span></span>
                                <span><strong>${lang.date}:</strong> ${escapeHtml(closingDate)}</span>
                            </div>
                        </div>
    ...
    function renderTemplateB(index) {
        ...
        if (!eligStudent) {
            ...
            const subjectRows = Object.entries(student.scores||{})
                .filter(([,sc]) => sc.mark!=null&&sc.mark!=='')
                .map(([sub,sc]) => {
                    const gc = sc.grade==='A'?'#1b5e20':sc.grade==='B'?'#2e7d32':sc.grade==='C'?'#f57f17':sc.grade==='D'?'#e65100':'#b71c1c';
                    return `<tr>
                        <td style="border:0.5px solid #bbb;padding:3px 6px;font-size:9.5px;">${escapeHtml(sub)}</td>
                        <td style="border:0.5px solid #bbb;padding:3px 6px;text-align:center;font-weight:bold;font-size:9.5px;color:${gc};">${sc.mark}</td>
                        <td style="border:0.5px solid #bbb;padding:3px 6px;text-align:center;font-weight:bold;font-size:9.5px;color:${gc};">${sc.grade}</td>
                        <td style="border:0.5px solid #bbb;padding:3px 6px;font-size:9px;color:#444;font-style:italic;">${lang.comments[sc.grade]||'-'}</td>
                    </tr>`;
                }).join('');
            ...
            const behavRows = Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                .map(([key,val]) => `<tr>
                    <td style="border:0.5px solid #bbb;padding:2px 5px;font-size:9px;">${lang.behaviorAspects[key]}</td>
                    <td style="border:0.5px solid #bbb;padding:2px 5px;text-align:center;font-weight:bold;font-size:9px;">${val}</td>
                </tr>`).join('');

            reportDiv.innerHTML = `
                <div style="width:210mm;min-height:297mm;margin:0 auto;background:#fff;padding:8mm 10mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;">

                    <!-- Header strip -->
                    <table style="width:100%;border-collapse:collapse;border-bottom:2px solid #000;padding-bottom:4px;margin-bottom:5px;">
                        <tr>
                            <td style="width:14%;text-align:center;vertical-align:middle;">
                                ${schoolLogo ? `<img src="${schoolLogo}" style="width:48px;height:48px;object-fit:contain;">` : `<div style="width:48px;height:48px;border:1px solid #ccc;display:flex;align-items:center;justify-content:center;font-size:9px;color:#999;">LOGO</div>`}
                            </td>
                            <td style="text-align:center;vertical-align:middle;padding:0 8px;">
                                <div style="font-size:8.5px;font-weight:bold;letter-spacing:0.3px;">${lang.governmentHeader}</div>
                                <div style="font-size:8.5px;color:#555;margin-bottom:1px;">${lang.localGov}</div>
                                <div style="font-size:13px;font-weight:bold;text-transform:uppercase;margin:2px 0;">${escapeHtml(si.displayLine1)}</div>
                                <div style="font-size:9px;color:#444;">${escapeHtml(si.displayLine2)}</div>
                            </td>
                            <td style="width:14%;text-align:center;vertical-align:middle;">
                                ${schoolLogo ? `<img src="${schoolLogo}" style="width:48px;height:48px;object-fit:contain;">` : ''}
                            </td>
                        </tr>
                    </table>

                    <!-- Report title band -->
                    <div style="background:#000;color:#fff;text-align:center;font-size:11px;font-weight:bold;letter-spacing:1px;padding:4px 0;margin-bottom:6px;">
                        ${lang.reportTitle.toUpperCase()} &mdash; ${escapeHtml(examType.toUpperCase())}
                    </div>

                    <!-- Student info strip -->
                    <table style="width:100%;border:1px solid #555;border-collapse:collapse;margin-bottom:6px;font-size:9.5px;">
                        <tr>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Name:</strong> ${escapeHtml(student.name)}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Class:</strong> ${escapeHtml(examClass)}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Sex:</strong> ${escapeHtml(student.gender||'-')}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Cand No:</strong> ${escapeHtml(candNum)}</td>
                        </tr>
                        <tr>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Points:</strong> ${student.point}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Division:</strong> ${escapeHtml(student.division||'-')}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Position:</strong> ${posInfo}</td>
                            <td style="padding:3px 6px;border:0.5px solid #bbb;"><strong>Month:</strong> ${escapeHtml(month)}</td>
                        </tr>
                    </table>
                    ...
                    <div style="display:flex;gap:6px;">
                        ...
                        <div style="flex:2.2;min-width:0;">
                            <div style="font-size:10px;font-weight:bold;border-bottom:1.5px solid #000;padding-bottom:2px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px;">${lang.performanceTitle}</div>
                            <table style="width:100%;border-collapse:collapse;">
                                <thead>
                                    <tr style="background:#eee;">
                                        <th style="border:0.5px solid #bbb;padding:3px 6px;text-align:left;font-size:9.5px;">Subject</th>
                                        <th style="border:0.5px solid #bbb;padding:3px 6px;font-size:9.5px;">Mark</th>
                                        <th style="border:0.5px solid #bbb;padding:3px 6px;font-size:9.5px;">Grade</th>
                                        <th style="border:0.5px solid #bbb;padding:3px 6px;font-size:9px;">Remark</th>
                                    </tr>
                                </thead>
                                <tbody>${subjectRows}</tbody>
                                <tfoot>
                                    <tr style="background:#f0f0f0;">
                                        <td colspan="4" style="border:0.5px solid #bbb;padding:4px 6px;font-size:9.5px;font-weight:bold;">
                                            Total Points: ${student.point} &nbsp;|&nbsp; Division: ${escapeHtml(student.division||'-')} &nbsp;|&nbsp; Position: ${posInfo}
                                        </td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        <div style="flex:1.1;min-width:0;">
                            <div style="font-size:10px;font-weight:bold;border-bottom:1.5px solid #000;padding-bottom:2px;margin-bottom:3px;text-transform:uppercase;letter-spacing:0.5px;">${lang.behavior}</div>
                            <table style="width:100%;border-collapse:collapse;">
                                <thead>
                                    <tr style="background:#eee;">
                                        <th style="border:0.5px solid #bbb;padding:2px 5px;text-align:left;font-size:9px;">Aspect</th>
                                        <th style="border:0.5px solid #bbb;padding:2px 5px;font-size:9px;">Rating</th>
                                    </tr>
                                </thead>
                            ...
                            <!-- Overall remark box -->
                            <div style="border:1px solid #bbb;padding:4px 6px;margin-top:5px;font-size:9px;">
                                <strong>Overall Remark:</strong><br>
                                <span style="font-style:italic;">${escapeHtml(remark)}</span>
                            </div>
                        </div>
                    </div>

                    <!-- Dates + signature strip -->
                    <div style="margin-top:5mm;border-top:1px solid #ccc;padding-top:3mm;font-size:9px;">
                        <table style="width:100%;border-collapse:collapse;">
                            <tr>
                                <td style="padding:2px 4px;">School Closes: <strong>${escapeHtml(closingDate)}</strong></td>
                                <td style="padding:2px 4px;">School Opens: <strong>${escapeHtml(openingDate)}</strong></td>
                            </tr>
                        </table>
                        <table style="width:100%;border-collapse:collapse;margin-top:4mm;">
                            <tr>
                                <td style="padding:2px;text-align:center;width:33%;">
                                    <div style="border-bottom:0.5px solid #000;margin:0 6px 2px;">&nbsp;</div>
                                    <div style="font-size:8.5px;">${escapeHtml(classTeacher)}</div>
                                    <div style="font-size:8px;color:#555;">Class Teacher</div>
                                </td>
                                <td style="padding:2px;text-align:center;width:33%;">
                                    <div style="width:40px;height:40px;border:1px solid #aaa;border-radius:50%;margin:0 auto 2px;display:flex;align-items:center;justify-content:center;font-size:7px;color:#aaa;">STAMP</div>
                                </td>
                                <td style="padding:2px;text-align:center;width:33%;">
                                    <div style="border-bottom:0.5px solid #000;margin:0 6px 2px;">&nbsp;</div>
                                    <div style="font-size:8.5px;">${escapeHtml(headmaster)}</div>
                                    <div style="font-size:8px;color:#555;">Head Teacher</div>
                                </td>
                            </tr>
                        </table>
                    </div>
    ...
    // Shared PDF options — portrait A4, renders the 210mm×297mm card exactly
    const _pdfOpt = (filename) => ({
        margin: 0,
        filename,
        image: { type:'jpeg', quality:0.98 },
        html2canvas: { scale:3, useCORS:true, allowTaint:true, logging:false, imageTimeout:0 },
    ...
    document.getElementById('axpRepPreview').onclick = () => {
        ...
        html2pdf().set(_pdfOpt(fn)).from(reportDiv).outputPdf('dataurlnewwindow');
    };
    ...
    document.getElementById('axpRepDownload').onclick = () => {
        ...
        html2pdf().set(_pdfOpt(fn)).from(reportDiv).save();
    };
    ...
    document.getElementById('axpRepDownAll').addEventListener('click', async () => {
        if (isDownloadingAll) {
            ...
            document.getElementById('axpRepDownAll').innerHTML = '<i class="bi bi-cloud-download"></i> Download All';
            document.getElementById('axpRepDownAll').style.cssText = '';
            return;
        }
        isDownloadingAll = true;
        const origIdx = currentIndex;
        document.getElementById('axpRepDownAll').innerHTML = '<i class="bi bi-stop-circle"></i> Stop';
        document.getElementById('axpRepDownAll').style.cssText = 'background:#ef4444;color:#fff;border-color:#ef4444;';
        ...
        try {
            for (let i=0; i<eligible.length; i++) {
                ...
                document.getElementById('axpRepDownAll').innerHTML = `<i class="bi bi-stop-circle"></i> ${downloaded+1}/${eligible.length} — Stop`;
                const repNum = String(i+1).padStart(4,'0');
                await html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_${_sanitize(_examClass())}_${_sanitize(_examType())}_Report_${repNum}.pdf`)).from(reportDiv).save();
                downloaded++;
                await new Promise(r=>setTimeout(r,900));
            }
            alert(isDownloadingAll ? `Done! Downloaded: ${downloaded}, Skipped: ${skipped}` : `Stopped. Downloaded: ${downloaded}`);
        } catch(e) { console.error('[AXP] Batch download error:',e); }
        finally {
            isDownloadingAll=false;
            document.getElementById('axpRepDownAll').innerHTML='<i class="bi bi-cloud-download"></i> Download All';
            document.getElementById('axpRepDownAll').style.cssText='';
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        mergeBtn.innerHTML = '<i class="bi bi-hourglass-split"></i> Building…';
        mergeBtn.disabled = true;

        // Build a combined container — one report per page (page-break-after each)
        const mergeContainer = document.createElement('div');
        mergeContainer.style.cssText = 'position:absolute;left:-9999px;top:0;width:210mm;background:#fff;';
        ...
        try {
            for (let i=0; i<eligible.length; i++) {
                ...
                if (reportDiv.querySelector('.axp-alert')) continue;

                // Clone the rendered report card and append with forced page break
                const clone = reportDiv.firstElementChild ? reportDiv.firstElementChild.cloneNode(true) : null;
                if (!clone) continue;
                clone.style.pageBreakAfter = 'always';
                clone.style.breakAfter = 'page';
                mergeContainer.appendChild(clone);
                count++;
                mergeBtn.innerHTML = `<i class="bi bi-hourglass-split"></i> ${count}/${eligible.length}…`;
            }

            if (count === 0) { alert('No valid reports found to merge.'); }
            else {
                ...
                await html2pdf().set({
                    ...
                    jsPDF: { unit:'mm', format:'a4', orientation:'portrait' },
                    pagebreak: { mode:['css','legacy'], after:['div[style*="page-break-after"]'] }
                }).from(mergeContainer).save();
                alert(`Merged PDF created with ${count} student reports.`);
            }
        } catch(e) {
            console.error('[AXP] Merge error:', e);
            alert('Error creating merged PDF. Try with fewer students.');
        } finally {
...
function renderStudentCertificates(students) {
    ...
    certDiv.style.cssText = 'padding:0;margin:0;background:#fff;line-height:0;overflow:hidden;';
    ...
    function renderCert(index) {
        ...
        certDiv.innerHTML = `
            <div style="width:297mm;height:210mm;margin:0;padding:0;background:#fff;position:relative;font-family:'Georgia',serif;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;">

                <!-- Decorative background gradients -->
                <div style="position:absolute;inset:0;background:radial-gradient(ellipse at 10% 50%,rgba(200,168,75,0.07) 0%,transparent 60%),radial-gradient(ellipse at 90% 50%,rgba(200,168,75,0.07) 0%,transparent 60%),radial-gradient(ellipse at 50% 5%,rgba(26,58,92,0.05) 0%,transparent 50%),radial-gradient(ellipse at 50% 95%,rgba(26,58,92,0.05) 0%,transparent 50%);pointer-events:none;"></div>

                <!-- Outer gold border -->
                <div style="position:absolute;top:5mm;left:5mm;right:5mm;bottom:5mm;border:3px solid #c8a84b;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
                <!-- Inner navy border -->
                <div style="position:absolute;top:8mm;left:8mm;right:8mm;bottom:8mm;border:1px solid #1a3a5c;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>

                <!-- Corner ornaments -->
                <div style="position:absolute;top:5mm;left:5mm;width:14mm;height:14mm;border-top:4px solid #c8a84b;border-left:4px solid #c8a84b;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
                <div style="position:absolute;top:5mm;right:5mm;width:14mm;height:14mm;border-top:4px solid #c8a84b;border-right:4px solid #c8a84b;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
                <div style="position:absolute;bottom:5mm;left:5mm;width:14mm;height:14mm;border-bottom:4px solid #c8a84b;border-left:4px solid #c8a84b;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
                <div style="position:absolute;bottom:5mm;right:5mm;width:14mm;height:14mm;border-bottom:4px solid #c8a84b;border-right:4px solid #c8a84b;pointer-events:none;-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>

                <!-- Left gold bar -->
                <div style="position:absolute;top:11mm;left:11mm;bottom:11mm;width:4mm;background:linear-gradient(180deg,#c8a84b,#1a3a5c,#c8a84b);-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>
                <!-- Right gold bar -->
                <div style="position:absolute;top:11mm;right:11mm;bottom:11mm;width:4mm;background:linear-gradient(180deg,#c8a84b,#1a3a5c,#c8a84b);-webkit-print-color-adjust:exact;print-color-adjust:exact;"></div>

                <!-- Content area -->
                <div style="position:relative;z-index:2;padding:11mm 22mm 9mm 22mm;flex:1;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:space-between;overflow:hidden;">

                    <!-- Top: school identity -->
                    <div style="text-align:center;width:100%;">
                        ${schoolLogo ? `<img src="${schoolLogo}" style="width:52px;height:52px;object-fit:contain;display:block;margin:0 auto 3px;">` : `<div style="width:52px;height:52px;border-radius:50%;background:#1a3a5c;margin:0 auto 3px;display:flex;align-items:center;justify-content:center;-webkit-print-color-adjust:exact;print-color-adjust:exact;"><span style="color:#c8a84b;font-size:22px;font-weight:bold;">${(schoolName||'S')[0]}</span></div>`}
                        <div style="font-size:7px;letter-spacing:3px;color:#999;text-transform:uppercase;margin-bottom:1px;">THE UNITED REPUBLIC OF TANZANIA</div>
                        <div style="font-size:12px;font-weight:bold;color:#1a3a5c;text-transform:uppercase;letter-spacing:1px;">${escapeHtml((schoolName||'').toUpperCase())}</div>
                        <div style="font-size:7.5px;color:#888;letter-spacing:0.5px;">${escapeHtml(schoolIndex.toUpperCase())}</div>
                        <div style="display:flex;align-items:center;gap:5px;margin:5px auto 0;width:55%;">
                            <div style="flex:1;height:1px;background:#c8a84b;"></div>
                            <span style="color:#c8a84b;font-size:10px;">❖</span>
                            <div style="flex:1;height:1px;background:#c8a84b;"></div>
                        </div>
                    </div>

                    <!-- Certificate heading -->
                    <div style="text-align:center;width:100%;margin:2mm 0;">
                        <div style="font-size:8px;letter-spacing:4px;color:#888;text-transform:uppercase;margin-bottom:2px;">– ${lang.presentedTo} –</div>
                        <div style="font-size:22px;font-weight:bold;color:#1a3a5c;letter-spacing:4px;text-transform:uppercase;">${lang.certificate}</div>
                        <div style="width:70px;height:3px;background:linear-gradient(90deg,transparent,#c8a84b,transparent);margin:3px auto 0;"></div>
                    </div>

                    <!-- Recipient name -->
                    <div style="text-align:center;width:100%;padding:3mm 0;">
                        <div style="font-size:8.5px;letter-spacing:2px;color:#aaa;font-style:italic;margin-bottom:2mm;">${lang.presentedTo}</div>
                        <div style="font-size:22px;font-weight:bold;color:#1a3a5c;letter-spacing:2px;text-transform:uppercase;">${escapeHtml(student.name)}</div>
                        <div style="display:flex;align-items:center;gap:8px;margin:2mm auto;width:50%;">
                            <div style="flex:1;height:1.5px;background:linear-gradient(90deg,transparent,#c8a84b);"></div>
                            <span style="color:#c8a84b;font-size:9px;">✦</span>
                            <div style="flex:1;height:1.5px;background:linear-gradient(90deg,#c8a84b,transparent);"></div>
                        </div>
                        <div style="font-size:9.5px;color:#444;font-style:italic;line-height:1.7;max-width:140mm;margin:0 auto;">
                            ${lang.recognition}<br>
                            <span style="color:#888;font-size:8.5px;">${lang.conducted} <strong style="color:#1a3a5c;">${escapeHtml((schoolName||'').toUpperCase())}</strong></span>
                        </div>
                    </div>

                    <!-- Signatures -->
                    <div style="display:flex;justify-content:space-between;align-items:flex-end;width:100%;padding-top:4mm;border-top:1px solid #e0d9c4;">

                        <div style="text-align:center;flex:1;">
                            <div style="height:9mm;border-bottom:1.5px solid #333;width:38mm;margin:0 auto 2mm;"></div>
                            <div style="font-size:9px;font-weight:bold;color:#222;">${escapeHtml(classTeacher||'___________')}</div>
                            <div style="font-size:7.5px;color:#777;letter-spacing:0.5px;">${lang.classTeacher}</div>
                            <div style="font-size:7px;color:#aaa;margin-top:1mm;">${lang.date}: ${escapeHtml(closingDate)}</div>
                        </div>

                        <div style="text-align:center;flex:1;">
                            <div style="width:20mm;height:20mm;border-radius:50%;border:2px solid #c8a84b;margin:0 auto;display:flex;align-items:center;justify-content:center;background:rgba(200,168,75,0.06);-webkit-print-color-adjust:exact;print-color-adjust:exact;">
                                <div style="text-align:center;line-height:1;">
                                    <div style="font-size:13px;color:#c8a84b;">⭐</div>
                                    <div style="font-size:5.5px;color:#1a3a5c;font-weight:bold;letter-spacing:0.5px;">${lang.seal||'OFFICIAL SEAL'}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="text-align:center;flex:1;">
                            <div style="height:9mm;border-bottom:1.5px solid #333;width:38mm;margin:0 auto 2mm;"></div>
                            <div style="font-size:9px;font-weight:bold;color:#222;">${escapeHtml(headmaster||'___________')}</div>
                            <div style="font-size:7.5px;color:#777;letter-spacing:0.5px;">${lang.headTeacher}</div>
                            <div style="font-size:7px;color:#aaa;margin-top:1mm;">${lang.date}: ${escapeHtml(closingDate)}</div>
                        </div>
    ...
    const _sanitize = s=>s.replace(/[^a-z0-9]/gi,'_').replace(/_+/g,'_');
    const _schoolIdx = ()=>(window.currentSchoolData?.indexNumber||'SCH').toUpperCase();
    const _pdfOpt = filename=>({ margin:0, filename, image:{type:'jpeg',quality:0.98}, html2canvas:{scale:3,useCORS:true,allowTaint:true,logging:false,imageTimeout:0}, jsPDF:{unit:'mm',format:[297,210],orientation:'landscape'} });

    document.getElementById('axpCertPreview').onclick  = ()=>html2pdf().set(_pdfOpt(`Certificate_${currentIndex+1}.pdf`)).from(certDiv).outputPdf('dataurlnewwindow');
    document.getElementById('axpCertDownload').onclick = ()=>html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_Certificate_${_sanitize(allStudents[currentIndex].name)}.pdf`)).from(certDiv).save();

    document.getElementById('axpCertDownAll').addEventListener('click', async ()=>{
        if (isDownloadingAll) {
            isDownloadingAll=false;
            document.getElementById('axpCertDownAll').innerHTML='<i class="bi bi-cloud-download"></i> Download All';
            document.getElementById('axpCertDownAll').style.background='';
            return;
        }
        isDownloadingAll=true;
        const origIdx=currentIndex;
        document.getElementById('axpCertDownAll').innerHTML='<i class="bi bi-stop-circle"></i> Stop Download';
        document.getElementById('axpCertDownAll').style.background='#ef4444';
        let downloaded=0;
        try {
            for(let i=0;i<allStudents.length;i++){
                ...
                if(!isDownloadingAll) break;
                document.getElementById('axpCertDownAll').innerHTML=`<i class="bi bi-stop-circle"></i> ${downloaded+1}/${allStudents.length} — Stop`;
                await html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_Certificate_${String(i+1).padStart(4,'0')}.pdf`)).from(certDiv).save();
                downloaded++;
                await new Promise(r=>setTimeout(r,1000));
            }
            if(isDownloadingAll) alert(`Done! Downloaded: ${downloaded} certificates`);
        } catch(e){ console.error('[AXP] Cert batch error:',e); }
        finally{
            isDownloadingAll=false;
            document.getElementById('axpCertDownAll').innerHTML='<i class="bi bi-cloud-download"></i> Download All';
            document.getElementById('axpCertDownAll').style.background='';
...
function _axpSchoolInfo(lang) {
    const sd  = window.currentSchoolData;
    const raw = (sd?.schoolName || '').trim() ||
                (document.getElementById('schoolNameInput') ? document.getElementById('schoolNameInput').value.trim() : '');
    const idx = (sd?.indexNumber || '').trim() ||
                (document.getElementById('schoolIndexInput') ? document.getElementById('schoolIndexInput').value.trim() : '');
