    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        return new Promise(resolve => {
            try {
                ...
                if (!jsPDFCtor) { _axpToast('jsPDF not available', 'error'); resolve(); return; }
                ...
                const drawHeaderFooter = (pg, total) => {
                    ...
                    if (pg > 1) {
                        ...
                        doc.text(`${examLabel} | ${classLabel} | ${year}`, PW-MR, MT+2, {align:'right'});
                    }
                };
                ...
                const hLines = [
                    ...
                    examLabel.toUpperCase()+' — '+classLabel.toUpperCase()+' — '+year
                ].filter(Boolean);
                const hFonts = [
                    {size:7,  bold:false},
                    {size:7,  bold:false},
                ...
                students.forEach((s,i)=>{
                    ...
                    const dBg = divCol[s.division]||[255,255,255];
                    const dFg = divFg[s.division]||[0,0,0];
                    ...
                    cell(x,curY,fixedCols[2].w,rowH,s.gender||'-',{fontSize:6.5}); x+=fixedCols[2].w;
                    cell(x,curY,fixedCols[3].w,rowH,s.point,{fontSize:6.5}); x+=fixedCols[3].w;
                    cell(x,curY,fixedCols[4].w,rowH,s.division||'-',{fontSize:6.5,bold:true,bg:dBg,fg:dFg}); x+=fixedCols[4].w;
                ...
                const csTW=120, csX=ML+(CW-csTW)/2, csRH=7;
                const csHdrH=8;

                // Title
                cell(csX,curY,csTW,csHdrH,'EXAMINATION CENTRE OVERALL PERFORMANCE',{bold:true,fontSize:8});
                curY+=csHdrH;
                const csRows=[
                    ['TOTAL PASSED CANDIDATES (DIV I–IV)', passed],
                    ['PERFORMANCE PERCENTAGE', perfPct+'%'],
                ];
                csRows.forEach(([lbl,val])=>{
                    cell(csX,curY,csTW*0.7,csRH,lbl,{bold:true,align:'left',fontSize:7.5});
                    cell(csX+csTW*0.7,curY,csTW*0.3,csRH,val,{bold:true,align:'left',fontSize:7.5});
                    curY+=csRH;
                });
                // GPA row coloured
                if (schoolGPA) {
                    const gc=gpaToColour(schoolGPA);
                    cell(csX,curY,csTW*0.7,csRH,'SCHOOL GPA',{bold:true,align:'left',fontSize:7.5});
                    cell(csX+csTW*0.7,curY,csTW*0.3,csRH,`${schoolGPA.toFixed(4)} — ${gc.grade} (${gc.label})`,{bold:true,align:'left',fontSize:7,bg:gc.bg,fg:gc.fg});
                    curY+=csRH;
                }
                curY+=6;
                ...
                const sSubW=30, sSatW=6, sGrpW=6*3, sAverW=8, sGpaW=10, sPosW=7, sGradeW=14;
                const sTotalW=sSubW+sSatW*3+sGrpW*5+sAverW+sGpaW+sPosW+sGradeW;
                const sX=ML+(CW-sTotalW)/2>0?ML+(CW-sTotalW)/2:ML;
                const sTH=4.5, sTR=4.5;
                ...
                sx=sX+sSubW+sSatW*3;
                ['A','B','C','D','F'].forEach(()=>['F','M','T'].forEach(l=>{ cell(sx,curY,sGrpW/3,sTH,l,{bold:true,fontSize:4.5}); sx+=sGrpW/3; }));
                curY+=sTH;
                ...
                subjects.forEach(sub=>{
                    ...
                    ['A','B','C','D','F'].forEach(gr=>{
                        ...
                        [f,m,f+m].forEach(v=>{ cell(sx,curY,sGrpW/3,sTR,v,{fontSize:5}); sx+=sGrpW/3; });
                    });
...
function renderStudentReports(students, container) {
    ...
    function _repCapturePdf(filename, action) {
        ...
        return html2canvas(reportDiv, {
            ...
            cleanup();
        }).catch(err => { cleanup(); console.error('[AXP report PDF]', err); _axpToast('Report PDF failed: '+err.message,'error'); });
    }
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        try {
            ...
            if (count === 0) {
                ...
                // Render each cloned report page individually → jsPDF
                const jsPDFCtor = (window.jspdf && window.jspdf.jsPDF) || window.jsPDF;
                const mergeDoc = new jsPDFCtor({ unit:'mm', format:[210,297], orientation:'portrait' });
                const pages = mergeContainer.children;
                for (let pi = 0; pi < pages.length; pi++) {
                    prog.update(`Rendering page ${pi+1} of ${pages.length}…`);
                    const pageEl = pages[pi];
                    pageEl.style.width = _REP_W + 'px';
                    pageEl.style.minWidth = _REP_W + 'px';
                    const cvs = await html2canvas(pageEl, {
                        scale: 3, useCORS: true, allowTaint: true,
                        logging: false, backgroundColor: '#ffffff',
                        windowWidth: _REP_W, scrollX: 0, scrollY: 0
                    });
                    const imgData = cvs.toDataURL('image/jpeg', 0.98);
                    if (pi > 0) mergeDoc.addPage([210,297], 'portrait');
                    mergeDoc.addImage(imgData, 'JPEG', 0, 0, 210, 297);
                }
                mergeDoc.save(fn);
...
function renderStudentCertificates(students) {
    ...
    function _certCapturePdf(filename, action) {
        ...
        return html2canvas(certDiv, {
            ...
            const jsPDFCtor = (window.jspdf && window.jspdf.jsPDF) || window.jsPDF;
            const doc = new jsPDFCtor({ unit:'mm', format:[297,210], orientation:'landscape' });
            const imgData = canvas.toDataURL('image/jpeg', 0.98);
            ...
            cleanup();
        }).catch(err => { cleanup(); console.error('[AXP cert PDF]', err); _axpToast('Certificate PDF failed: '+err.message,'error'); });
    }
