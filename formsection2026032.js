...
function renderAssignTeachersSection() {
  ...
  sw.innerHTML = `
    <div class="axp-section-card">
      ...
      <!-- Add Teacher Form -->
      <div style="background:#f8fafc;padding:18px;margin-bottom:20px;">
...
function _axpRenderSubjectMatrix() {
  ...
  const hdrStyle  = "width:34px;min-width:34px;text-align:center;font-size:10px;font-weight:700;color:#64748b;padding:0 2px;writing-mode:vertical-lr;transform:rotate(180deg);height:76px;display:flex;align-items:center;justify-content:center;border-right:1px solid #e2e8f0;";

  cont.innerHTML = `
    <p style="font-size:12px;color:#64748b;margin:0 0 8px;">Check subjects each student takes.</p>
    <div style="min-width:max-content;">
      <div style="display:flex;align-items:flex-end;border-bottom:2px solid #e2e8f0;padding-bottom:3px;">
        ...
        <div style="min-width:56px;font-size:11px;font-weight:700;color:#64748b;text-align:center;">Subjects</div>
      </div>
      ${names.map((name, ni) => {
      ...
      ${_announcements.length > 0
        ? `<div style="display:flex;flex-direction:column;gap:10px;">
            ${_announcements.map(ann=>{
              ...
              return `
                <div style="background:#f8fafc;border-left:3px solid ${c};padding:14px;">
                  <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:6px;">
                    <strong style="font-size:13.5px;color:#1e293b;">${escapeHtml(ann.title)}</strong>
                    <span style="background:${c};color:#fff;padding:2px 7px;font-size:10.5px;font-weight:600;">${escapeHtml(ann.priority||"Normal")}</span>
                  </div>
                  <p style="margin:0 0 7px;color:#475569;line-height:1.6;font-size:13px;">${escapeHtml(ann.message)}</p>
                  <span style="font-size:11px;color:#94a3b8;"><i class="bi bi-calendar3"></i> ${formatAnnouncementDate(ann.date)}</span>
                </div>`;
            }).join("")}
          </div>`
...
function renderMessagesSectionFull() {
  ...
  sw.innerHTML = `
    <div class="axp-section-card">
      ...
      ${_operatorMessage
        ? `<div style="background:linear-gradient(135deg,#667eea,#764ba2);padding:18px;color:#fff;">
            <div style="display:flex;align-items:center;gap:9px;margin-bottom:10px;">
              <i class="bi bi-chat-left-text-fill" style="font-size:17px;color:#ffd700;"></i>
              <strong style="font-size:14px;">Message from AcademixPoint</strong>
            </div>
            <div style="background:rgba(255,255,255,0.12);padding:12px;white-space:pre-wrap;line-height:1.6;font-size:13px;">
...
function renderHelpSection() {
  ...
  sw.innerHTML = `
    <div class="axp-section-card">
      ...
      <h4 style="font-size:13.5px;font-weight:700;color:#1e293b;margin:0 0 12px;">Getting Started</h4>
      <div style="display:flex;flex-direction:column;gap:10px;">
        ${[
          ["1","bi-building-gear","Setup School Structure","Go to Assign Tasks section. If your school isn't set up yet, you'll be prompted to configure your year, classes, exam types, and subjects."],
          ...
          <div style="display:flex;gap:12px;padding:12px;background:#f8fafc;">
            <div style="width:32px;height:32px;min-width:32px;background:#4ecca3;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;color:#060c1c;">${n}</div>
            <div>
              <strong style="font-size:13px;color:#1e293b;display:flex;align-items:center;gap:5px;"><i class="bi ${icon}" style="color:#4ecca3;"></i>${title}</strong>
              <p style="margin:3px 0 0;font-size:12.5px;color:#64748b;line-height:1.55;">${desc}</p>
            </div>
          </div>`).join("")}
      </div>
    </div>`;
...
function _axpDemoBuildTeachersPanel(demoClasses, demoSubjectsMap, initialTeachers) {
  return `
    ...
    <div class="axp-alert axp-alert-info" style="margin-bottom:16px;">
      <i class="bi bi-info-circle"></i>
      <span>Pre-loaded with demo teachers. Add new ones or modify assignments.</span>
    </div>
    ...
    <div style="background:#f8fafc;padding:16px;margin-bottom:18px;">
      <h4 style="font-size:13px;font-weight:700;color:#1a1a2e;margin:0 0 12px;display:flex;align-items:center;gap:6px;">
        <i class="bi bi-person-plus" style="color:#4ecca3;"></i> Add Demo Teacher
      </h4>
      <div class="axp-form-row">
        <div class="axp-field-group">
        ...
        <div class="axp-field-group">
          <label>Email</label>
          <input id="axpDemoTchEmail" class="axp-input" type="email" placeholder="teacher@gmail.com" />
        </div>
      </div>
...
window.axpDemoSaveTeacher = function() {
  ...
  const email   = (document.getElementById("axpDemoTchEmail")||{}).value.trim();
  const cls     = (document.getElementById("axpDemoTchClass")||{}).value;
  ...
  if (!email) { _showSectionMsg("axpDemoTchMsg","Email is required.","danger"); return; }
  if (!cls || !subject) { _showSectionMsg("axpDemoTchMsg","Select class and subject.","danger"); return; }

  window._axpDemo.teachers.push({ name, email, class:cls, subject });
  _showSectionMsg("axpDemoTchMsg", `Teacher "${name}" saved to demo!`, "success");
  document.getElementById("axpDemoTchName").value = "";
  document.getElementById("axpDemoTchEmail").value = "";
