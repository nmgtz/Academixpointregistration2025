...
const scriptURL = 'https://script.google.com/macros/s/AKfycbwmj_xT0NEnQZu_-6qTTXntwOE0PTaVV6w9hMjZPqdpvfHjWspiWaGdwfouPGbPBW3j/exec';
    ...
    <div class="axp-section-card" style="padding:0;">
      ...
      <div id="axpFeedPanelCorrection" style="display:none;padding:20px;">
        ...
        <div id="axpCorForm" style="display:none;background:#f8fafc;padding:16px;">
          <div class="axp-form-row">
            <div class="axp-field-group">
              <label>Current (Old) Score</label>
              <input id="axpCorOldValue" class="axp-input" type="number"
                min="0" max="100" placeholder="e.g. 45" />
            </div>
            <div class="axp-field-group">
...
window.axpFeedSubmitAll = function() {
  ...
  const payload=students.map(s=>({name:s.name,score:marks[s.name]!==undefined?marks[s.name]:""}));
  const filled=payload.filter(p=>p.score!=="").length;
  ...
  axpConfirm("Confirm Submission",
    ...
    async()=>{
      ...
      try{
        ...
        } else axpAlert("Failed",res.message||"Please try again.","danger");
      }catch(e){axpAlert("Network Error","Check connection and retry.","danger");}
  ...
  axpConfirm("Confirm Bulk Submission",
    ...
    async()=>{
      ...
      try{
        ...
        } else axpAlert("Failed",res.message||"Please retry.","danger");
      }catch(e){axpAlert("Network Error","Check connection and retry.","danger");}
...
window.axpCorSelectStudent=function(name, rowEl){
  ...
  if (list) {
    list.querySelectorAll("[data-name]").forEach(row => {
      const isThis = row.getAttribute("data-name") === name;
      row.style.background = isThis ? "#060c1c" : "#fff";
      row.classList.toggle("axp-cor-active", isThis);
      /* swap icon */
      const iconEl = row.querySelector(".bi-check-circle-fill, .bi-person-fill");
      /* re-render cleanly */
    });
    /* Cleanest approach — re-render the filtered list */
  ...
  document.getElementById("axpCorGradePreview").innerHTML="";
