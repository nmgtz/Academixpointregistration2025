...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isLoggedIn) {
    ...
    /* Only lock body scroll if the .login-form wrapper actually exists in the HTML
       (i.e. this is the dedicated auth page, not the public homepage) */
    const loginFormEl = document.querySelector(".login-form");
    if (loginFormEl) {
      _openAuthForm("login-form");
    }
  }
  ...
  if (resetEmail && resetToken) {
    _openAuthForm("new-password-form");
...
/* Opens an auth form AND locks body scroll */
function _openAuthForm(formId) {
  applyLandingStyles();            // ensure styles injected
  _injectVideoBackground();        // inject bg only when auth opens
  document.body.classList.add("axp-auth-open");
  const loginForm = document.querySelector(".login-form");
  if (loginForm) loginForm.style.display = "flex";
  toggleForms(formId);
}

/* Restores body scroll (called when leaving auth back to homepage) */
function _closeAuthForm() {
  document.body.classList.remove("axp-auth-open");
  const loginForm = document.querySelector(".login-form");
...
function applyLandingStyles() {
  ...
  style.textContent = `
    /* ── Base: font only, no layout interference on homepage ── */
    body {
      font-family: 'Roboto', Arial, sans-serif;
    }
    body::before { content: none; }

    /* ── Lock body ONLY when auth overlay is open ── */
    body.axp-auth-open {
      position: fixed;
      inset: 0;
      background: #060c1c;
    ...
    /* Video/canvas hidden by default; shown only when auth is open */
    #axp-video-bg, #axp-video-overlay, #axp-canvas-bg {
      display: none !important;
    }
    body.axp-auth-open #axp-video-bg {
    ...
    body.axp-auth-open #axp-video-bg.loaded { opacity: 1; }

    body.axp-auth-open #axp-video-overlay {
      display: block !important;
      position: fixed;
      inset: 0;
    ...
    body.axp-auth-open #axp-canvas-bg {
      display: block !important;
    }

    /* ── Auth form overlay: hidden by default, centered when open ── */
    .login-form {
      display: none !important;
    }
    body.axp-auth-open .login-form {
...
function showFormPage(formId) {
  /* Hide homepage chrome */
  ["#header","#dashboard","#main-id",".footer",".footer-bottom"].forEach(sel => {
