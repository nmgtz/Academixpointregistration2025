...
function updateDashboardUI(data) {
    ...
    // 📢 Display Announcements Section (after plan div)
    displayAnnouncements(data);
}

function displayAnnouncements(data) {
    // Find the plan div to insert announcements after it
    const planDiv = document.querySelector('.plan, [class*="plan"]');
    
    if (!planDiv) {
        console.warn('Plan div not found. Announcements cannot be displayed.');
        return;
    }
    ...
    // Create announcements section with simple styling
    ...
    `;
    header.innerHTML = `
        <i class="bi bi-megaphone" style="font-size: 20px; color: #4ecca3;"></i>
        <h3 style="margin: 0; font-size: 16px; font-weight: 600; color: #333;">Announcements</h3>
        <span style="margin-left: auto; font-size: 12px; color: #666;">${announcements.length} total</span>
    `;
    
    announcementsSection.appendChild(header);
    
    // Create simple announcements list
    ...
    // Add each announcement with simple styling
    announcements.forEach((announcement, index) => {
        ...
        const priorityColor = priorityColors[announcement.priority] || '#6c757d';
        ...
        announcementCard.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <i class="bi bi-info-circle" style="color: ${priorityColor}; font-size: 16px;"></i>
                    <strong style="font-size: 14px; color: #333;">${announcement.title}</strong>
                </div>
                <span style="font-size: 11px; color: white; background: ${priorityColor}; padding: 2px 8px; border-radius: 3px; font-weight: 500;">
                    ${announcement.priority}
                </span>
            </div>
            <p style="margin: 0 0 8px 24px; color: #555; font-size: 13px; line-height: 1.5;">
                ${announcement.message}
            </p>
            <div style="display: flex; align-items: center; gap: 15px; margin-left: 24px; font-size: 11px; color: #6c757d;">
    ...
    planDiv.insertAdjacentElement('afterend', announcementsSection);
}
