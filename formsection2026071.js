...
function renderStudentReports(students, container) {
    ...
    function render(index) {
        ...
        if (!eligStudent) {
            ...
            // Build division/points/position description (multilingual)
            const _divDesc = currentLang==='sw'
                ? `${escapeHtml(student.name)} amepata jumla ya pointi <strong>${student.point}</strong>, mgawanyo <strong>${student.division}</strong>${eligStudent?`, nafasi <strong>${eligStudent.position} kati ya ${eligStudent.total}</strong> darasani`:''}.`
                : `${escapeHtml(student.name)} scored a total of <strong>${student.point}</strong> points, attaining <strong>Division ${student.division}</strong>${eligStudent?`, ranked <strong>${eligStudent.position} out of ${eligStudent.total}</strong> in class`:''}.`;
            const _divGuide = currentLang==='sw'
                ? `Mgawanyo I (pointi 7–17): Bora Sana | Mgawanyo II (18–21): Nzuri | Mgawanyo III (22–25): Ya Kati | Mgawanyo IV (26–33): Hafifu | Mgawanyo 0 (34–35): Kushindwa`
                    ...
                    <div style="position:absolute;inset:0;z-index:2;display:flex;flex-direction:column;padding:11mm 11mm 8mm;box-sizing:border-box;">
                        ...
                        <div style="flex-shrink:0;text-align:center;border-bottom:2px solid #1a3a5c;padding-bottom:2mm;margin-bottom:2mm;">
                            ...
                            <div style="display:inline-block;border:2px solid #1a3a5c;padding:2px 14px;font-size:11px;font-weight:bold;letter-spacing:1px;text-transform:uppercase;color:#1a3a5c;margin-top:1mm;">
                                ${lang.reportTitle} — ${escapeHtml(examLabel)}${month?' — '+month.toUpperCase():''}  — ${year}
                            </div>
                        </div>
                        ...
                        <!-- ── DESCRIPTION ── -->
                        <div style="flex-shrink:0;font-size:9.5px;line-height:1.6;padding:2.5mm 3mm;border:1px solid #dce8f5;background:#f7faff;margin-bottom:2mm;border-radius:1px;">
                            ${_divDesc}
                            <span style="color:#666;font-size:8.5px;display:block;margin-top:1mm;">${_divGuide}</span>
                        </div>
                        ...
                        <div style="flex-shrink:0;border-top:1px solid #aac4e0;margin-top:2mm;padding-top:2mm;">
                            <div style="font-size:9.5px;color:#222;margin-bottom:1.5mm;line-height:1.6;">
                            ...
                            <div style="display:flex;align-items:center;gap:4mm;font-size:9.5px;border-top:0.5px dashed #aac4e0;padding-top:1.5mm;">
                                <span style="color:#555;white-space:nowrap;">${lang.classComment.replace(/:.*/,'')}:</span>
                                <strong style="color:#000;font-size:10px;">${escapeHtml(classTeacher||'—')}</strong>
                        ...
                        <div style="flex-shrink:0;border-top:1px solid #aac4e0;margin-top:2mm;padding-top:2mm;">
                            ...
                            <div style="display:flex;align-items:center;gap:4mm;font-size:9.5px;border-top:0.5px dashed #aac4e0;padding-top:1.5mm;">
                                <span style="color:#555;white-space:nowrap;">${lang.headComment.replace(/:.*/,'')}:</span>
                                <strong style="color:#000;font-size:10px;">${escapeHtml(headmaster||'—')}</strong>
                                <span style="margin-left:auto;color:#555;">${lang.date}: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>
    ...
    function renderTemplateB(index) {
        ...
        if (!eligStudent) {
            ...
            // Division description (Template B, multilingual)
            const _divDescB = currentLang==='sw'
                ? `${escapeHtml(student.name)} amepata pointi <strong>${student.point}</strong>, mgawanyo <strong>${student.division}</strong>${eligStudent?`, nafasi <strong>${posInfo}</strong> darasani`:''}.`
                : `${escapeHtml(student.name)} scored <strong>${student.point}</strong> points, <strong>Division ${student.division}</strong>${eligStudent?`, ranked <strong>${posInfo}</strong> in class`:''}.`;
            const _divGuideB = currentLang==='sw'
                ? `Div I (7–17): Bora Sana | Div II (18–21): Nzuri | Div III (22–25): Ya Kati | Div IV (26–33): Hafifu | Div 0: Kushindwa`
                : `Div I (7–17 pts): Excellent | Div II (18–21): Good | Div III (22–25): Average | Div IV (26–33): Poor | Div 0: Fail`;
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;height:297mm;margin:0 auto;background:#fff;box-sizing:border-box;font-family:Arial,sans-serif;font-size:10px;color:#000;position:relative;overflow:hidden;page-break-after:always;page-break-inside:avoid;">
                    <div style="position:absolute;inset:0;display:flex;flex-direction:column;padding:8mm 10mm 7mm;box-sizing:border-box;">
                        ...
                        <div style="flex-shrink:0;display:flex;align-items:center;gap:6px;border-bottom:2.5px solid #000;padding-bottom:3mm;margin-bottom:2.5mm;">
                            ...
                            <div style="flex:1;text-align:center;">
                                ...
                                <div style="background:#000;color:#fff;font-size:11px;font-weight:bold;letter-spacing:1px;padding:3px 12px;display:inline-block;">
                                    ${lang.reportTitle.toUpperCase()} — ${escapeHtml(examLabel.toUpperCase())} — ${yearB}
                                </div>
                            </div>
                            ${schoolLogo?`<img src="${schoolLogo}" style="width:44px;height:44px;object-fit:contain;flex-shrink:0;">`:`` }
                        </div>
                        ...
                        <!-- DESCRIPTION -->
                        <div style="flex-shrink:0;font-size:9.5px;line-height:1.6;padding:2.5mm 3mm;border:1px solid #ddd;background:#fafafa;margin-bottom:2mm;">
                            ${_divDescB}
                            <span style="color:#666;font-size:8.5px;display:block;margin-top:1mm;">${_divGuideB}</span>
                        </div>
                        ...
                        <!-- CLASS TEACHER -->
                        <div style="flex-shrink:0;border-top:1px solid #ccc;margin-top:2mm;padding-top:2mm;">
                            <div style="font-size:9.5px;color:#222;margin-bottom:1.5mm;line-height:1.6;">
                            ...
                            <div style="display:flex;align-items:center;gap:4mm;font-size:9.5px;border-top:0.5px dashed #ccc;padding-top:1.5mm;">
                                <span style="color:#555;white-space:nowrap;">${lang.classComment&&lang.classComment.replace(/:.*/,'')||'Class Teacher'}:</span>
                                <strong style="color:#000;font-size:10px;">${escapeHtml(classTeacher||'—')}</strong>
                                <span style="margin-left:auto;color:#555;">${lang.date||'Date'}: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>
                        ...
                        <div style="flex-shrink:0;border-top:1px solid #ccc;margin-top:2mm;padding-top:2mm;">
                            <div style="font-size:9.5px;color:#222;margin-bottom:1.5mm;line-height:1.6;">
                                <strong style="font-size:10px;">${lang.headComment||'Head Teacher Comment'}:</strong>
                                ${pt<=13?lang.remarksList.outstanding:pt<=17?lang.remarksList.commendable:pt<=24?lang.remarksList.satisfactory:lang.remarksList.followup}
                                ${lang.headSupportNote||''}
                            </div>
                            <div style="display:flex;align-items:center;gap:4mm;font-size:9.5px;border-top:0.5px dashed #ccc;padding-top:1.5mm;">
                                <span style="color:#555;white-space:nowrap;">${lang.headComment&&lang.headComment.replace(/:.*/,'')||'Head Teacher'}:</span>
                                <strong style="color:#000;font-size:10px;">${escapeHtml(headmaster||'—')}</strong>
                                <span style="margin-left:auto;color:#555;">${lang.date||'Date'}: <strong>${escapeHtml(closingDate)}</strong></span>
                            </div>
                        </div>
    ...
    async function _repCapturePdf(filename, action) {
        ...
        const html2canvas = await _axpGetHtml2Canvas();
...
function renderStudentCertificates(students) {
    ...
    async function _certCapturePdf(filename, action) {
        // Clone cert into body so html2canvas sees it fully outside any scroll container
        const inner = certDiv.firstElementChild;
        if (!inner) { _axpToast('Certificate not rendered yet','warning'); return; }
        const clone = inner.cloneNode(true);
        clone.style.cssText = `position:fixed;top:0;left:0;width:${_CERT_W}px;height:794px;z-index:99999;background:#fff;margin:0;padding:0;box-shadow:none;border:none;overflow:hidden;`;
        document.body.appendChild(clone);
        await new Promise(r=>setTimeout(r,120)); // let fonts/images settle
