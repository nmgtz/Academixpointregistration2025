const scriptURL = 'https://script.google.com/macros/s/AKfycbydHrH5UOu5VN85Y9V67uzzpZovAbj04mMgyYITCovxTqkzV3CB3sDY3EDVMDkQMFVo/exec';
...
window.axpRRRefreshClass = function() {
  if (window._axpRRActiveClass) axpRROpenClass(window._axpRRActiveClass);
};
...
window.axpRRSwitchTab = function(tabKey) {
  ...
  switch (tabKey) {
    case "results":     axpRRLoadResults(cls, examType, cont);     break;
    case "report":      axpRRLoadReport(cls, examType, cont);      break;
    case "analysis":    axpRRLoadAnalysis(cls, examType, cont);    break;
    case "top10":       axpRRLoadRanking(cls, examType, "top", 10, cont);    break;
    case "least10":     axpRRLoadRanking(cls, examType, "least", 10, cont);  break;
    case "general":     axpRRLoadGeneral(cls, examType, cont);     break;
    case "subjectwise": axpRRLoadSubjectWise(cls, examType, cont); break;
    case "subjectrank": axpRRLoadSubjectRanking(cls, examType, cont); break;
    default:
...
/* ── Tab: Results table ── */
async function axpRRLoadResults(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "classResults", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
    if (res.status === "success" && res.students && res.students.length > 0) {
      ...
      cont.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
          ...
          <span style="font-size:12px;color:#64748b;">${res.students.length} students</span>
        </div>
        <div style="overflow-x:auto;">
          <table class="axp-table" style="min-width:500px;">
            <thead>
              <tr>
                ...
                ${subjects.map(s => `<th style="max-width:80px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${escapeHtml(s)}">${escapeHtml(s.length>8?s.slice(0,8)+'…':s)}</th>`).join("")}
                <th>Total</th><th>Avg</th><th>Grade</th><th>Pos</th>
              </tr>
            </thead>
            <tbody>
              ${res.students.map((s, i) => {
                const { grade, color } = _axpGrade(s.average);
                return `<tr>
                  ...
                  ${subjects.map(sub => {
                    ...
                    return `<td style="color:${mc};font-weight:600;">${mark !== undefined ? mark : "—"}</td>`;
                  }).join("")}
                  <td style="font-weight:700;">${s.total||"—"}</td>
                  <td style="font-weight:700;">${s.average||"—"}</td>
                  <td><strong style="color:${color};font-size:14px;">${grade}</strong></td>
                  <td style="font-weight:700;color:#1a1a2e;">${s.position||i+1}</td>
                </tr>`;
              }).join("")}
            </tbody>
          </table>
      ...
      cont.innerHTML = `<div class="axp-empty-state"><i class="bi bi-inbox"></i><p>No results found for ${escapeHtml(cls)} — ${escapeHtml(examType)}.</p><p style="font-size:12px;color:#94a3b8;margin-top:6px;">Results are entered by teachers through their feeding link.</p></div>`;
    }
  } catch(e) {
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load results. Check connection.</span></div>`;
  }
}

/* ── Tab: Report summary ── */
async function axpRRLoadReport(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "classReport", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
    if (res.status === "success" && res.report) {
      ...
      cont.innerHTML = `
        <div style="background:#060c1c;color:#fff;padding:16px 20px;margin-bottom:16px;display:flex;flex-wrap:wrap;gap:12px;align-items:center;">
          ...
          <div style="margin-left:auto;background:rgba(78,204,163,.12);border:1px solid rgba(78,204,163,.3);padding:8px 16px;text-align:center;">
            <div style="font-size:24px;font-weight:900;color:#4ecca3;">${r.passRate||"—"}%</div>
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-bottom:18px;">
          ${[
            {l:"Total Students", v:r.total||0,       c:"#4ecca3"},
            {l:"Passed",         v:r.passed||0,      c:"#10b981"},
            {l:"Failed",         v:r.failed||0,      c:"#ef4444"},
            {l:"School Average", v:(r.average||"—")+"%", c:"#3b82f6"},
            {l:"Highest Score",  v:r.highest||"—",   c:"#f59e0b"},
            {l:"Lowest Score",   v:r.lowest||"—",    c:"#f97316"},
          ].map(s => `
        ...
        ${r.gradeDistribution ? `
          ...
          <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:8px;margin-bottom:16px;">
            ${["A","B","C","D","F"].map(g => {
              ...
              const count = (r.gradeDistribution||{})[g] || 0;
      ...
      cont.innerHTML = `<div class="axp-empty-state"><i class="bi bi-file-earmark-x"></i><p>No report data for ${escapeHtml(cls)} — ${escapeHtml(examType)}.</p></div>`;
    }
  } catch(e) {
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load report.</span></div>`;
  }
}

/* ── Tab: Analysis ── */
async function axpRRLoadAnalysis(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "classAnalysis", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
        ...
        ${a.genderBreakdown ? `
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;">
            <div style="background:#eff6ff;padding:14px;border-left:3px solid #3b82f6;">
              ...
              <div style="font-size:22px;font-weight:900;color:#1e40af;">${(a.genderBreakdown.M||{}).count||0}</div>
              <div style="font-size:12px;color:#3b82f6;">Avg: ${(a.genderBreakdown.M||{}).average||"—"} &nbsp; Pass: ${(a.genderBreakdown.M||{}).passRate||"—"}%</div>
            </div>
            <div style="background:#fce7f3;padding:14px;border-left:3px solid #db2777;">
              ...
              <div style="font-size:22px;font-weight:900;color:#831843;">${(a.genderBreakdown.F||{}).count||0}</div>
              <div style="font-size:12px;color:#db2777;">Avg: ${(a.genderBreakdown.F||{}).average||"—"} &nbsp; Pass: ${(a.genderBreakdown.F||{}).passRate||"—"}%</div>
            </div>
          </div>` : ""}
        ${a.subjectAverages && a.subjectAverages.length > 0 ? `
          <h4 style="font-size:12.5px;font-weight:700;color:#1a1a2e;margin:0 0 10px;">Subject Averages</h4>
          <div style="overflow-x:auto;margin-bottom:16px;">
            <table class="axp-table"><thead><tr><th>Subject</th><th>Average</th><th>Highest</th><th>Lowest</th><th>Pass Rate</th></tr></thead>
            <tbody>
              ${a.subjectAverages.map(s => `
                <tr>
                  <td style="font-weight:600;">${escapeHtml(s.subject||"")}</td>
                  <td><strong style="color:#4ecca3;">${s.average||"—"}</strong></td>
                  <td style="color:#10b981;">${s.highest||"—"}</td>
                  <td style="color:#ef4444;">${s.lowest||"—"}</td>
                  <td>${s.passRate||"—"}%</td>
                </tr>`).join("")}
            </tbody></table>
          </div>` : ""}`;
    } else {
      cont.innerHTML = `<div class="axp-empty-state"><i class="bi bi-bar-chart-line"></i><p>No analysis data yet for ${escapeHtml(cls)} — ${escapeHtml(examType)}.</p></div>`;
    }
  } catch(e) {
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load analysis.</span></div>`;
  }
}

/* ── Tab: Top / Least Ranking ── */
async function axpRRLoadRanking(cls, examType, direction, limit, cont) {
  try {
    const res = await _apiGet({
      mode: "classRanking", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType,
      direction, limit
    });
    ...
    if (res.status === "success" && res.students && res.students.length > 0) {
      cont.innerHTML = `
        <strong style="font-size:13.5px;color:#1a1a2e;display:block;margin-bottom:14px;">
          <i class="bi ${isTop ? 'bi-trophy-fill' : 'bi-arrow-down-circle-fill'}" style="color:${isTop?'#f59e0b':'#ef4444'};"></i>
          ${isTop ? "Top" : "Least"} ${limit} Students — ${escapeHtml(cls)} (${escapeHtml(examType)})
        </strong>
        <div style="overflow-x:auto;">
          <table class="axp-ranking-table">
            <thead><tr><th>Pos</th><th>Exam No</th><th>Name</th><th>Sex</th><th>Total</th><th>Avg</th><th>Grade</th></tr></thead>
            <tbody>
              ${res.students.map((s, i) => {
                const { grade, color } = _axpGrade(s.average);
                const posColor = i === 0 && isTop ? "#f59e0b" : i === 0 && !isTop ? "#ef4444" : "#475569";
                return `<tr>
                  <td style="font-size:16px;font-weight:900;color:${posColor};">${s.position||i+1}</td>
                  <td style="color:#4ecca3;font-weight:700;">${escapeHtml(s.examNo||"—")}</td>
    ...
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load ranking.</span></div>`;
  }
}

/* ── Tab: General Result summary ── */
async function axpRRLoadGeneral(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "generalResult", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
    if (res.status === "success" && res.general) {
      const g = res.general;
      cont.innerHTML = `
        <strong style="font-size:13.5px;color:#1a1a2e;display:block;margin-bottom:14px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:10px;margin-bottom:18px;">
          ${[
            {l:"Registered",   v:g.registered||0,   c:"#4ecca3"},
            {l:"Sat Exam",     v:g.sat||0,           c:"#3b82f6"},
            {l:"Absent",       v:g.absent||0,        c:"#f59e0b"},
            {l:"Qualified",    v:g.qualified||0,     c:"#10b981"},
            {l:"Disqualified", v:g.disqualified||0,  c:"#ef4444"},
            {l:"Pass Rate",    v:(g.passRate||"—")+"%", c:"#8b5cf6"},
          ].map(s => `
        ...
        ${g.remarks ? `<div class="axp-alert axp-alert-info"><i class="bi bi-info-circle"></i><span>${escapeHtml(g.remarks)}</span></div>` : ""}`;
    } else {
      cont.innerHTML = `<div class="axp-empty-state"><i class="bi bi-clipboard-x"></i><p>No general result data yet.</p></div>`;
    }
  } catch(e) {
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load general result.</span></div>`;
  }
}

/* ── Tab: Subject-wise Results ── */
async function axpRRLoadSubjectWise(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "subjectWise", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
      ...
      cont.innerHTML = `
        ...
        ${res.subjects.map(sub => `
          <div style="margin-bottom:18px;">
            <div style="background:#060c1c;color:#fff;padding:10px 14px;display:flex;justify-content:space-between;align-items:center;margin-bottom:0;">
              <strong style="font-size:13px;">${escapeHtml(sub.subject||"")}</strong>
              <div style="display:flex;gap:10px;font-size:12px;">
                <span>Avg: <strong style="color:#4ecca3;">${sub.average||"—"}</strong></span>
                <span>Pass: <strong style="color:#10b981;">${sub.passRate||"—"}%</strong></span>
              </div>
            </div>
            <div style="overflow-x:auto;">
              <table class="axp-table" style="min-width:300px;">
                ...
                <tbody>
                  ${(sub.students||[]).map(s => {
                    ...
                    return `<tr>
                      ...
                      <td style="font-weight:700;">${s.score !== undefined ? s.score : "—"}</td>
    ...
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load subject-wise results.</span></div>`;
  }
}

/* ── Tab: Subject Ranking & Analysis ── */
async function axpRRLoadSubjectRanking(cls, examType, cont) {
  try {
    const res = await _apiGet({
      mode: "subjectRanking", schoolId: _appScriptSchoolId,
      year: _schoolMeta.year, class: cls, examType
    });
    if (res.status === "success" && res.ranking && res.ranking.length > 0) {
      cont.innerHTML = `
        <strong style="font-size:13.5px;color:#1a1a2e;display:block;margin-bottom:14px;">
        ...
        <div style="overflow-x:auto;margin-bottom:18px;">
          <table class="axp-ranking-table">
            <thead>
              <tr>
                <th>Rank</th><th>Subject</th><th>Avg Score</th>
                <th>Highest</th><th>Lowest</th><th>Pass Rate</th>
                <th>Best Student</th><th>Least Student</th>
              </tr>
            </thead>
            <tbody>
              ${res.ranking.map((s, i) => `
                <tr>
                  <td style="font-size:16px;font-weight:900;color:${i===0?'#f59e0b':i===res.ranking.length-1?'#ef4444':'#475569'};">${i+1}</td>
                  <td style="font-weight:700;color:#1a1a2e;">${escapeHtml(s.subject||"")}</td>
                  <td><strong style="color:#4ecca3;">${s.average||"—"}</strong></td>
                  <td style="color:#10b981;">${s.highest||"—"}</td>
                  <td style="color:#ef4444;">${s.lowest||"—"}</td>
                  <td>${s.passRate||"—"}%</td>
                  <td style="font-weight:600;color:#065f46;">${escapeHtml((s.bestStudent||{}).name||"—")} ${(s.bestStudent||{}).score ? `(${s.bestStudent.score})` : ""}</td>
                  <td style="font-weight:600;color:#991b1b;">${escapeHtml((s.leastStudent||{}).name||"—")} ${(s.leastStudent||{}).score ? `(${s.leastStudent.score})` : ""}</td>
                </tr>`).join("")}
            </tbody>
          </table>
        </div>`;
    ...
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load subject ranking.</span></div>`;
  }
}
...
window.axpLoadAuditLog = async function() {
  ...
  const limit  = (document.getElementById("axpAuditFilterLimit")||{}).value  || "200";

  cont.innerHTML = `<div class="axp-empty-state"><div class="axp-spinner-sm" style="margin:0 auto 8px;"></div><p>Loading audit log...</p></div>`;
  ...
  try {
    ...
    if (_appScriptSchoolId) params.schoolId = _appScriptSchoolId;

    const res = await _apiGet(params);

    if (res.status === "success") {
      window._axpAuditLog      = res.log || [];
      window._axpAuditSelected = new Set();
      ...
      cont.innerHTML = `<div class="axp-alert axp-alert-warning"><i class="bi bi-info-circle"></i><span>${escapeHtml(res.message||"No audit data found.")}</span></div>`;
    }
  } catch(e) {
    cont.innerHTML = `<div class="axp-alert axp-alert-danger"><i class="bi bi-exclamation-circle"></i><span>Failed to load audit log.</span></div>`;
  }
};
...
function _axpRenderAuditTable() {
  ...
  if (log.length === 0) {
    cont.innerHTML = `<div class="axp-empty-state"><i class="bi bi-inbox"></i><p>No audit entries found.</p></div>`;
    return;
  }
  ...
  cont.innerHTML = `
    <div style="overflow-x:auto;">
      <table class="axp-table" style="min-width:700px;">
        <thead>
          <tr>
            <th style="width:32px;">
              <input type="checkbox" id="axpAuditCheckAll" onchange="axpAuditToggleAll(this.checked)"
        ...
        <tbody>
          ${log.map((entry, i) => {
            const color   = actionColors[entry.actionType] || "#94a3b8";
            const checked = window._axpAuditSelected.has(entry.auditId);
            return `<tr id="axpAuditRow_${i}" style="${checked ? 'background:#fffbeb;' : ''}">
              <td>
                <input type="checkbox" ${checked ? "checked" : ""}
                  onchange="axpAuditToggleRow('${entry.auditId}', ${i}, this.checked)"
                  style="width:14px;height:14px;accent-color:#4ecca3;cursor:pointer;" />
              </td>
              <td style="font-size:11px;color:#64748b;white-space:nowrap;">${escapeHtml(String(entry.timestamp||""))}</td>
              <td>
                <span style="background:${color}18;color:${color};padding:2px 7px;font-size:10.5px;font-weight:700;white-space:nowrap;">
                  ${escapeHtml(entry.actionType||"—")}
                </span>
              </td>
              <td style="font-weight:500;">${escapeHtml(entry.className||"—")}</td>
              <td>${escapeHtml(entry.examType||"—")}</td>
              <td>${escapeHtml(entry.subject||"—")}</td>
              <td style="font-weight:500;">${escapeHtml(entry.studentName||"—")}</td>
              <td style="font-size:11.5px;color:#64748b;">${escapeHtml((entry.performedBy||"").split("@")[0]||"—")}</td>
              <td style="font-size:12px;color:#475569;max-width:120px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"
                title="${escapeHtml(entry.newValue||"")}">
                ${escapeHtml((entry.newValue||"").slice(0,40))}
              </td>
              <td>
                <button onclick="axpAuditDeleteOne('${entry.auditId}', ${i})" class="axp-btn-danger"
...
window.axpAuditToggleAll = function(checked) {
  const log = window._axpAuditLog || [];
  if (checked) log.forEach(e => window._axpAuditSelected.add(e.auditId));
  else         window._axpAuditSelected.clear();
  _axpUpdateAuditSelectionUI();
  _axpRenderAuditTable();
};

window.axpAuditToggleRow = function(auditId, rowIdx, checked) {
  if (checked) window._axpAuditSelected.add(auditId);
  else         window._axpAuditSelected.delete(auditId);
  _axpUpdateAuditSelectionUI();
  const row = document.getElementById(`axpAuditRow_${rowIdx}`);
...
window.axpAuditDeleteOne = async function(auditId, rowIdx) {
  ...
  try {
    const res = await _apiPost({ mode:"deleteAuditEntry", adminEmail:_dashboardData.email, auditId });
    if (res.status === "success") {
  ...
  } catch(e) { _axpToast("Network error.", "danger"); }
};

window.axpDeleteSelectedAuditEntries = async function() {
  const ids = [...window._axpAuditSelected];
  if (ids.length === 0) return;
  if (!confirm(`Delete ${ids.length} selected audit entries? This cannot be undone.`)) return;
  let deleted = 0, failed = 0;
  for (const auditId of ids) {
    try {
      const res = await _apiPost({ mode:"deleteAuditEntry", adminEmail:_dashboardData.email, auditId });
      if (res.status === "success") { deleted++; window._axpAuditLog = window._axpAuditLog.filter(e => e.auditId !== auditId); }
      else failed++;
    } catch { failed++; }
  }
  window._axpAuditSelected.clear();
  _axpRenderAuditTable();
  _axpUpdateAuditSelectionUI();
  _axpToast(`${deleted} entries deleted${failed > 0 ? `, ${failed} failed` : ""}.`, deleted > 0 ? "success" : "danger");
};

window.axpExportAuditLog = function() {
  const log = window._axpAuditLog || [];
  if (log.length === 0) { _axpToast("No data to export.", "warning"); return; }
  const headers = ["Audit ID","Timestamp","Action","Class","Exam","Subject","Student","By","Old Value","New Value"];
  const rows    = log.map(e => [
    e.auditId, e.timestamp, e.actionType, e.className,
    e.examType, e.subject, e.studentName,
    e.performedBy, e.oldValue, e.newValue
