...
function renderTeacherFeedingSection() {
  ...
  /* Account / setup guards — same pattern as other sections */
  if (_dashboardData && _dashboardData.status !== "ACTIVE") {
    sw.innerHTML = _axpInactiveBlock();
    return;
  }
  if (!_appScriptSchoolId) {
    sw.innerHTML = `
      <div class="axp-section-card" style="text-align:center;padding:40px 24px;">
        ...
        <h3 style="font-size:16px;font-weight:800;color:#1a1a2e;margin:0 0 8px;">Feature Locked</h3>
        <p style="font-size:13px;color:#64748b;max-width:360px;margin:0 auto 20px;line-height:1.6;">
          Configure your school first to unlock Marks Feeding.
        </p>
        <button onclick="axpOpenSetupPopup()" class="axp-btn-primary" style="font-size:13.5px;padding:10px 24px;">
          <i class="bi bi-gear-fill"></i> Setup School Now
        </button>
  ...
  /* Reset all feeding state */
  window._axpFeed = {
    students    : [],   /* [{name, gender}] — from getFeedingRoster */
    teacher     : null, /* {id, name, email, phone} or null */
    marks       : {},   /* { "STUDENT NAME": score } */
    currentIdx  : 0,
    cls         : "",
    examType    : "",
    subject     : "",
  ...
  sw.innerHTML = `
    ...
    <div class="axp-section-card" style="padding:0;">
      ...
      <!-- ── Tab bar ── -->
      <div style="display:flex;background:#fff;border-bottom:2px solid #e2e8f0;">
        <button onclick="axpFeedTab('entry')" id="axpFeedTabEntry"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#4ecca3;border-bottom:3px solid #4ecca3;margin-bottom:-2px;cursor:pointer;">
          <i class="bi bi-pencil-square"></i> Enter Marks
        </button>
        <button onclick="axpFeedTab('correction')" id="axpFeedTabCorrection"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#94a3b8;border-bottom:3px solid transparent;margin-bottom:-2px;cursor:pointer;">
      ...
      <!-- ══════════════════════════════════════════
           TAB: ENTER MARKS (one-by-one card style)
      ══════════════════════════════════════════ -->
      <div id="axpFeedPanelEntry" style="padding:20px;">

        <!-- Filter / roster selector -->
        <div style="background:#f8fafc;padding:16px;margin-bottom:18px;">
          <h4 style="font-size:12.5px;font-weight:700;color:#1a1a2e;margin:0 0 12px;display:flex;align-items:center;gap:6px;">
            <i class="bi bi-funnel-fill" style="color:#4ecca3;"></i> Select Assignment
          </h4>
          <div class="axp-form-row">
            <div class="axp-field-group">
              <label>Exam Type</label>
              <select id="axpFeedExamType" class="axp-select" onchange="axpFeedPopulateSubjects()">
                <option value="">— Select Exam —</option>
                ${examTypes.map(et => `<option value="${escapeHtml(et)}">${escapeHtml(et)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Class</label>
              <select id="axpFeedClass" class="axp-select" onchange="axpFeedPopulateSubjects()">
                <option value="">— Select Class —</option>
                ${classes.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Subject</label>
              <select id="axpFeedSubject" class="axp-select">
                <option value="">— Select Subject —</option>
              </select>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            <button onclick="axpFeedLoadRoster()" class="axp-btn-primary" id="axpFeedLoadBtn">
              <i class="bi bi-cloud-download"></i> Load Roster
            </button>
            <span style="font-size:11.5px;color:#94a3b8;">
              Only students enrolled in the selected subject will appear.
            </span>
          </div>
        </div>

        <!-- Teacher info banner (shown after roster loads) -->
        <div id="axpFeedTeacherBanner" style="display:none;margin-bottom:14px;"></div>

        <!-- Progress bar (shown after roster loads) -->
        <div id="axpFeedProgressWrap" style="display:none;background:#060c1c;padding:12px 16px;margin-bottom:14px;color:#fff;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
          ...
          <div class="axp-progress-bar" style="height:6px;background:rgba(255,255,255,0.1);">
            <div class="axp-progress-fill" id="axpFeedProgressFill" style="width:0%;transition:width .4s;"></div>
          </div>
          <div style="font-size:11px;opacity:.4;margin-top:4px;" id="axpFeedProgressSublabel">
            Select all three fields and click Load Roster
          </div>
        </div>

        <!-- Student card (one at a time) -->
        <div id="axpFeedStudentCard" style="display:none;">

          <!-- Card -->
          <div style="border-left:4px solid #4ecca3;padding:16px 18px;background:#f0fdf9;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
              <div>
                <div style="font-size:10px;font-weight:700;color:#4ecca3;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">Student No.</div>
                <div style="font-size:15px;font-weight:800;color:#060c1c;letter-spacing:.5px;" id="axpFeedCardNo">—</div>
              </div>
              ...
              <div style="margin-left:auto;text-align:center;">
                <div style="font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">Grade</div>
                <div style="font-size:36px;font-weight:900;line-height:1;" id="axpFeedGradeBadge">—</div>
              </div>
            </div>
          </div>
          ...
          <div style="margin-bottom:16px;">
            ...
            <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
              <input id="axpFeedScoreInput" type="number" min="0" max="100"
                class="axp-input"
                style="font-size:30px;font-weight:900;text-align:center;letter-spacing:2px;width:150px;padding:12px;"
                placeholder="—"
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            ...
            <button onclick="axpFeedSkip()" class="axp-btn-secondary" style="color:#f59e0b;border-color:#f59e0b;">
              <i class="bi bi-skip-forward"></i> Skip
            </button>
        ...
        <div id="axpFeedEmptyState">
          <div class="axp-empty-state" style="padding:50px 16px;">
            ...
            <p>Select Exam Type, Class, and Subject — then click <strong>Load Roster</strong>.</p>
          </div>
        </div>
      </div>
      ...
      <div id="axpFeedPanelCorrection" style="display:none;padding:20px;">
        ...
        <div class="axp-alert axp-alert-warning" style="margin-bottom:18px;">
          <i class="bi bi-exclamation-triangle-fill"></i>
          <span>Corrections are limited to <strong>5 per teacher per day</strong>. Each correction is logged in the audit trail.
          Use this only to fix genuine data entry mistakes.</span>
        </div>

        <div style="background:#f8fafc;padding:16px;margin-bottom:18px;">
          <h4 style="font-size:12.5px;font-weight:700;color:#1a1a2e;margin:0 0 12px;">Select Record to Correct</h4>
          <div class="axp-form-row">
            <div class="axp-field-group">
              <label>Exam Type</label>
              <select id="axpCorExamType" class="axp-select">
                <option value="">— Select Exam —</option>
                ${examTypes.map(et => `<option value="${escapeHtml(et)}">${escapeHtml(et)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Class</label>
              <select id="axpCorClass" class="axp-select" onchange="axpCorPopulateSubjects()">
                <option value="">— Select Class —</option>
                ${classes.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Subject</label>
              <select id="axpCorSubject" class="axp-select">
                <option value="">— Select Subject —</option>
              </select>
            </div>
          </div>
          <button onclick="axpCorLoadStudents()" class="axp-btn-primary">
        ...
        <div id="axpCorStudentPicker" style="display:none;margin-bottom:16px;">
          <div class="axp-field-group">
            ...
            <select id="axpCorStudentSelect" class="axp-select" onchange="axpCorOnStudentSelect()">
        ...
        <div id="axpCorForm" style="display:none;background:#f8fafc;padding:16px;">
          <div class="axp-form-row">
            <div class="axp-field-group">
              ...
              <input id="axpCorOldValue" class="axp-input" type="number" min="0" max="100" placeholder="e.g. 45" />
            </div>
            <div class="axp-field-group">
              <label>New Correct Score</label>
              <input id="axpCorNewValue" class="axp-input" type="number" min="0" max="100" placeholder="e.g. 54"
          ...
          <div class="axp-field-group" style="margin-bottom:12px;">
            ...
            <input id="axpCorReason" class="axp-input" placeholder="e.g. Transposition error — 54 was entered as 45" />
          </div>
        ...
        <div id="axpCorEmptyState">
          <div class="axp-empty-state" style="padding:40px 16px;">
            ...
            <p>Select exam, class, and subject then load students to make a correction.</p>
          </div>
        </div>
      </div>

      <!-- ══════════════════════════════════════════
           TAB: BULK ENTRY (spreadsheet-style table)
      ══════════════════════════════════════════ -->
      <div id="axpFeedPanelBulk" style="display:none;padding:20px;">

        <div class="axp-alert axp-alert-info" style="margin-bottom:18px;">
          <i class="bi bi-info-circle"></i>
          <span>Enter all marks at once in the table below. Useful when you have all scores ready.
          Marks are only submitted when you click <strong>Submit All Marks</strong>.</span>
        </div>
        ...
        <div style="background:#f8fafc;padding:16px;margin-bottom:18px;">
          <div class="axp-form-row">
            <div class="axp-field-group">
              <label>Exam Type</label>
              <select id="axpBulkExamType" class="axp-select" onchange="axpBulkPopulateSubjects()">
                <option value="">— Select Exam —</option>
                ${examTypes.map(et => `<option value="${escapeHtml(et)}">${escapeHtml(et)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Class</label>
              <select id="axpBulkClass" class="axp-select" onchange="axpBulkPopulateSubjects()">
                <option value="">— Select Class —</option>
                ${classes.map(c => `<option value="${escapeHtml(c)}">${escapeHtml(c)}</option>`).join("")}
              </select>
            </div>
            <div class="axp-field-group">
              <label>Subject</label>
              <select id="axpBulkSubject" class="axp-select">
                <option value="">— Select Subject —</option>
              </select>
            </div>
          </div>
          <button onclick="axpBulkLoadRoster()" class="axp-btn-primary" id="axpBulkLoadBtn">
        ...
        <!-- Bulk table -->
        <div id="axpBulkTable" style="display:none;">

          <!-- Stats row -->
          <div style="display:flex;gap:12px;flex-wrap:wrap;background:#f8fafc;padding:10px 14px;margin-bottom:12px;align-items:center;">
            ...
            <div style="flex:1;min-width:100px;">
              <div class="axp-progress-bar" style="height:5px;">
                <div class="axp-progress-fill" id="axpBulkProgressFill" style="width:0%;transition:width .3s;"></div>
              </div>
            </div>
          ...
          <div style="overflow-x:auto;">
            <table class="axp-table" id="axpBulkMarksTable" style="min-width:420px;">
              <thead>
                <tr>
                  <th style="width:32px;">#</th>
                  <th>Student Name</th>
                  <th style="width:55px;text-align:center;">Sex</th>
                  <th style="width:110px;text-align:center;">Score (0–100)</th>
        ...
        <div id="axpBulkEmptyState">
          <div class="axp-empty-state" style="padding:40px 16px;">
            <i class="bi bi-table" style="font-size:38px;"></i>
            <p>Select Exam, Class, and Subject — then click <strong>Load Students</strong>.</p>
          </div>
        </div>
      </div>
  ...
  /* Auto-populate subjects if class is already known */
  axpFeedPopulateSubjects();
}
...
window.axpFeedTab = function(tab) {
  ["entry","correction","bulk"].forEach(t => {
    const panel = document.getElementById(`axpFeedPanel${t.charAt(0).toUpperCase()+t.slice(1)}`);
    const btn   = document.getElementById(`axpFeedTab${t.charAt(0).toUpperCase()+t.slice(1)}`);
    const active = t === tab;
...
/* ─────────────────────────────────────────────────────────────
   SUBJECT POPULATORS  (class → subjects from _schoolMeta)
─────────────────────────────────────────────────────────────── */
window.axpFeedPopulateSubjects = function() {
  const cls = (document.getElementById("axpFeedClass")||{}).value;
  _axpPopulateSubjectSelect("axpFeedSubject", cls);
};

window.axpCorPopulateSubjects = function() {
  const cls = (document.getElementById("axpCorClass")||{}).value;
  _axpPopulateSubjectSelect("axpCorSubject", cls);
};

window.axpBulkPopulateSubjects = function() {
  const cls = (document.getElementById("axpBulkClass")||{}).value;
  _axpPopulateSubjectSelect("axpBulkSubject", cls);
};

function _axpPopulateSubjectSelect(selectId, cls) {
  const sel = document.getElementById(selectId);
  if (!sel) return;
  sel.innerHTML = '<option value="">— Select Subject —</option>';
  const subjects = (_schoolMeta && _schoolMeta.subjects && cls && _schoolMeta.subjects[cls])
...
function _axpFeedRenderTeacherBanner(bannerId, teacher, cls, subject) {
  ...
  if (!teacher) {
    ...
    el.innerHTML = `
      <div class="axp-alert axp-alert-warning">
        ...
        <div>
          <strong>No teacher assigned</strong> to <em>${escapeHtml(cls)}</em> — <em>${escapeHtml(subject)}</em>.
          <br><span style="font-size:12px;opacity:.85;">
            You can still enter marks. Assign a teacher in the
            <a href="#" onclick="navigateToSection('assign-tasks');return false;"
               style="color:#f59e0b;font-weight:700;">Assign Tasks</a> section.
          </span>
        </div>
      </div>`;
    return;
  ...
  el.innerHTML = `
    <div style="background:#060c1c;color:#fff;padding:12px 16px;
      ...
      <div style="width:36px;height:36px;min-width:36px;background:rgba(78,204,163,0.15);
                  display:flex;align-items:center;justify-content:center;">
      ...
      <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <span style="background:rgba(78,204,163,0.12);color:#4ecca3;
...
/* ═══════════════════════════════════════════════════════════════
   ENTER MARKS TAB — one-by-one card feeding
═══════════════════════════════════════════════════════════════ */

window.axpFeedLoadRoster = async function() {
  const examType = (document.getElementById("axpFeedExamType")||{}).value;
  const cls      = (document.getElementById("axpFeedClass")||{}).value;
  const subject  = (document.getElementById("axpFeedSubject")||{}).value;

  if (!examType || !cls || !subject) {
    _showSectionMsg("axpFeedEntryMsg", "Please select Exam Type, Class, and Subject.", "warning");
    return;
  }
  ...
  try {
      ...
      document.getElementById("axpFeedEmptyState").innerHTML = `
        <div class="axp-empty-state" style="padding:40px 16px;">
          ...
          <p style="font-size:12px;color:#94a3b8;margin-top:6px;">
            Assign subjects to students in the
            <a href="#" onclick="navigateToSection('push-names');return false;"
               style="color:#4ecca3;font-weight:700;">Student Names</a> section.
          </p>
        </div>`;
      return;
    }
    ...
    /* Store state */
    window._axpFeed.students   = res.students;
    window._axpFeed.teacher    = res.teacher || null;
    window._axpFeed.cls        = cls;
    window._axpFeed.examType   = examType;
    window._axpFeed.subject    = subject;
    window._axpFeed.currentIdx = 0;
    window._axpFeed.marks      = {};
    window._axpFeed.submitted  = false;
...
function _axpFeedRenderCard() {
  ...
  const student    = students[currentIdx];
  const existing   = marks[student.name];

  document.getElementById("axpFeedCardNo").textContent     = `${currentIdx + 1} / ${total}`;
  document.getElementById("axpFeedCardName").textContent   = student.name;
  document.getElementById("axpFeedCardGender").textContent = student.gender === "F" ? "Female" : "Male";
...
window.axpFeedOnScoreInput = function(val) {
  const badge = document.getElementById("axpFeedGradeBadge");
  if (!badge) return;
  if (val === "" || val === null || val === undefined) {
    badge.textContent = "—"; badge.style.color = "#94a3b8"; return;
  }
...
function _axpFeedUpdateProgress() {
  ...
  const pctEl  = document.getElementById("axpFeedProgressPct");
  const lbl    = document.getElementById("axpFeedProgressLabel");
  const sublbl = document.getElementById("axpFeedProgressSublabel");

  if (fill)   { fill.style.width = pct + "%"; fill.style.background = pct === 100 ? "#10b981" : "#4ecca3"; }
  if (pctEl)  pctEl.textContent  = pct + "%";
  if (lbl)    lbl.textContent    = `${filled} of ${total} students`;
  if (sublbl) sublbl.textContent = pct === 100
...
window.axpFeedNext = function() {
  ...
  /* Validate only if something was typed */
  if (val !== "") {
    const n = parseInt(val);
    if (isNaN(n) || n < 0 || n > 100) {
      _showSectionMsg("axpFeedEntryMsg", "Score must be between 0 and 100.", "danger"); return;
    }
    window._axpFeed.marks[students[currentIdx].name] = n;
    _axpFeedUpdateProgress();
  }
...
/* Submit all marks to server */
window.axpFeedSubmitAll = async function() {
  _axpFeedSaveCurrentInput();
  const { students, marks, cls, examType, subject } = window._axpFeed;

  const payload = students.map(s => ({
    name : s.name,
    score: marks[s.name] !== undefined ? marks[s.name] : ""
  }));

  const filled = payload.filter(p => p.score !== "").length;
  if (filled === 0) {
    _showSectionMsg("axpFeedEntryMsg", "No marks entered yet. Enter at least one score.", "warning"); return;
  }

  if (!confirm(`Submit marks for ${filled} student${filled !== 1 ? "s" : ""} in ${subject} — ${cls} — ${examType}?`)) return;
  ...
  try {
    ...
    if (res.status === "success") {
      ...
      _showSectionMsg("axpFeedEntryMsg", res.message || "Submission failed. Please try again.", "danger");
    }
  } catch(err) {
    _showSectionMsg("axpFeedEntryMsg", "Network error. Check connection and retry.", "danger");
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-send-fill"></i> Submit All'; }
  }
};

/* Summary table */
function _axpFeedShowSummary(showSubmittedBadge = false) {
  const { students, marks, cls, examType, subject } = window._axpFeed;
  const summaryEl = document.getElementById("axpFeedSummary");
  if (!summaryEl) return;
  summaryEl.style.display = "block";

  const gradeCounts = { A:0, B:0, C:0, D:0, F:0, "—":0 };
  const rows = students.map(s => {
    const score = marks[s.name];
    const { grade, color } = _axpGrade(score);
    gradeCounts[grade] = (gradeCounts[grade]||0) + 1;
    return { name:s.name, gender:s.gender, score, grade, color };
  });

  const filled = rows.filter(r => r.score !== undefined).length;

  const gradeColors = { A:"#10b981", B:"#4ecca3", C:"#f59e0b", D:"#f97316", F:"#ef4444", "—":"#94a3b8" };
  ...
  summaryEl.innerHTML = `
    <div style="border-bottom:2px solid #4ecca3;padding-bottom:10px;margin-bottom:14px;
      ...
      ${showSubmittedBadge
        ? `<span style="background:#10b981;color:#fff;padding:3px 12px;font-size:11.5px;font-weight:800;">
             <i class="bi bi-check2-all"></i> SUBMITTED
           </span>`
    ...
    <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:7px;margin-bottom:16px;">
      ${["A","B","C","D","F","—"].map(g => `
        <div style="background:#f8fafc;padding:10px 6px;text-align:center;border-top:3px solid ${gradeColors[g]};">
          <div style="font-size:22px;font-weight:900;color:${gradeColors[g]};">${gradeCounts[g]||0}</div>
          <div style="font-size:10.5px;font-weight:700;color:${gradeColors[g]};margin-top:1px;">Grade ${g}</div>
        </div>`).join("")}
    </div>

    <!-- Full marks table -->
    <div style="overflow-x:auto;">
      <table class="axp-table" style="min-width:380px;">
        <thead>
          <tr>
            <th style="width:32px;">#</th>
            <th>Name</th>
          ...
          ${rows.map((r,i) => `
            <tr>
              ...
              <td style="text-align:center;">
                <strong style="font-size:15px;font-weight:900;color:${r.color};">${r.grade}</strong>
              </td>
            </tr>`).join("")}
        </tbody>
      </table>
    ...
    <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;">
      ${!showSubmittedBadge ? `
        <button onclick="axpFeedReenter()" class="axp-btn-primary">
          <i class="bi bi-send-fill" style="color:#10b981;"></i>
          <span style="color:#fff;" onclick="axpFeedSubmitAll()"> Submit Now</span>
        </button>` : ""}
      <button onclick="axpFeedReenter()" class="axp-btn-secondary">
        <i class="bi bi-arrow-counterclockwise"></i> Re-enter / Edit
      </button>
...
/* Go back to card view from summary */
...
window.axpFeedExportSummary = function() {
  const { students, marks, cls, examType, subject } = window._axpFeed;
  const lines = ["Name,Gender,Score,Grade"];
  students.forEach(s => {
    const score = marks[s.name];
    const { grade } = _axpGrade(score);
    lines.push(`"${s.name}","${s.gender}","${score !== undefined ? score : ""}","${grade}"`);
  });
  const blob = new Blob([lines.join("\n")], { type:"text/csv" });
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = `marks_${cls}_${subject}_${examType}.csv`;
  a.click();
};
...
window.axpCorLoadStudents = async function() {
  const examType = (document.getElementById("axpCorExamType")||{}).value;
  const cls      = (document.getElementById("axpCorClass")||{}).value;
  const subject  = (document.getElementById("axpCorSubject")||{}).value;

  if (!examType || !cls || !subject) {
    _showSectionMsg("axpCorMsg", "Please select all three fields.", "warning"); return;
  }
  ...
  try {
    ...
    if (!students.length) {
      document.getElementById("axpCorEmptyState").innerHTML = `
        <div class="axp-alert axp-alert-warning">
          <i class="bi bi-exclamation-triangle"></i>
          <span>No students found for this selection.</span>
        </div>`;
      return;
    }

    const picker = document.getElementById("axpCorStudentPicker");
    const sel    = document.getElementById("axpCorStudentSelect");
    ...
    _showSectionMsg("axpCorMsg", "Failed to load students. Try again.", "danger");
  }
};

window.axpCorOnStudentSelect = function() {
  const name = (document.getElementById("axpCorStudentSelect")||{}).value;
  const form = document.getElementById("axpCorForm");
  if (!name) { if (form) form.style.display = "none"; return; }
...
window.axpCorPreviewGrade = function() {
  ...
  const prev  = document.getElementById("axpCorGradePreview");
  if (!prev) return;
  if (!val) { prev.innerHTML = ""; return; }
  const { grade, color } = _axpGrade(parseInt(val));
  prev.innerHTML = `
    <span style="font-size:12.5px;color:#64748b;">New grade: </span>
    <strong style="font-size:18px;font-weight:900;color:${color};">${grade}</strong>`;
};

window.axpSubmitCorrection = async function() {
  const examType    = (document.getElementById("axpCorExamType")||{}).value;
  const cls         = (document.getElementById("axpCorClass")||{}).value;
  const subject     = (document.getElementById("axpCorSubject")||{}).value;
  const studentName = (document.getElementById("axpCorStudentSelect")||{}).value;
  ...
  if (!studentName) { _showSectionMsg("axpCorMsg","Select a student.","warning"); return; }
  if (!oldValue)    { _showSectionMsg("axpCorMsg","Enter the current (old) score.","warning"); return; }
  if (!newValue)    { _showSectionMsg("axpCorMsg","Enter the new correct score.","warning"); return; }
  if (!reason)      { _showSectionMsg("axpCorMsg","Reason for correction is required.","warning"); return; }

  const nOld = parseInt(oldValue), nNew = parseInt(newValue);
  if (isNaN(nOld)||nOld<0||nOld>100) { _showSectionMsg("axpCorMsg","Old score must be 0–100.","danger"); return; }
  if (isNaN(nNew)||nNew<0||nNew>100) { _showSectionMsg("axpCorMsg","New score must be 0–100.","danger"); return; }
  if (nOld === nNew) { _showSectionMsg("axpCorMsg","Old and new scores are the same. No correction needed.","warning"); return; }
  ...
  try {
    ...
    if (res.status === "success") {
      _showSectionMsg("axpCorMsg",
        `Correction saved for ${studentName}: ${nOld} → ${nNew}.`, "success");
      axpCorReset();
      _axpToast("Correction submitted successfully.", "success");
    } else {
      _showSectionMsg("axpCorMsg", res.message || "Correction failed. Limit may be reached.", "danger");
    }
  } catch(err) {
    _showSectionMsg("axpCorMsg", "Network error. Please try again.", "danger");
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-check2-circle"></i> Submit Correction'; }
  }
};

window.axpCorReset = function() {
  document.getElementById("axpCorForm").style.display = "none";
  document.getElementById("axpCorStudentPicker").style.display = "none";
  const sel = document.getElementById("axpCorStudentSelect");
  if (sel) sel.value = "";
  ["axpCorOldValue","axpCorNewValue","axpCorReason"].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = "";
  });
  ["axpCorExamType","axpCorClass","axpCorSubject"].forEach(id => {
    const el = document.getElementById(id); if (el) el.value = "";
  });
  document.getElementById("axpCorGradePreview").innerHTML = "";
  document.getElementById("axpCorMsg").innerHTML = "";
  document.getElementById("axpCorEmptyState").innerHTML = `
    <div class="axp-empty-state" style="padding:40px 16px;">
      <i class="bi bi-pencil-square" style="font-size:38px;"></i>
      <p>Select exam, class, and subject then load students to make a correction.</p>
...
/* ═══════════════════════════════════════════════════════════════
   BULK ENTRY TAB — spreadsheet-style table
═══════════════════════════════════════════════════════════════ */

window.axpBulkLoadRoster = async function() {
  const examType = (document.getElementById("axpBulkExamType")||{}).value;
  const cls      = (document.getElementById("axpBulkClass")||{}).value;
  const subject  = (document.getElementById("axpBulkSubject")||{}).value;

  if (!examType || !cls || !subject) {
    _showSectionMsg("axpBulkMsg","Please select Exam Type, Class, and Subject.","warning"); return;
  }

  const btn = document.getElementById("axpBulkLoadBtn");
  if (btn) { btn.disabled = true; btn.innerHTML = '<span class="axp-spinner-sm"></span> Loading…'; }

  document.getElementById("axpBulkTable").style.display = "none";
  ...
  try {
    ...
    if (!students.length) {
      document.getElementById("axpBulkEmptyState").innerHTML = `
        <div class="axp-empty-state" style="padding:30px;">
          <i class="bi bi-inbox"></i>
          <p>No students enrolled in <strong>${escapeHtml(subject)}</strong> for <strong>${escapeHtml(cls)}</strong>.</p>
        </div>`;
      return;
    }
    ...
    /* Build table */
    _axpBulkRenderTable(students);
    document.getElementById("axpBulkTable").style.display = "block";
    document.getElementById("axpBulkEmptyState").innerHTML = "";

    document.getElementById("axpBulkTotal").textContent = students.length;
    _axpBulkUpdateStats();
...
function _axpBulkRenderTable(students) {
  const tbody = document.getElementById("axpBulkTableBody");
  if (!tbody) return;
      ...
      <td style="text-align:center;padding:4px 6px;">
        <input
          ...
          onkeydown="if(event.key==='Enter'||event.key==='ArrowDown'){
            var next=document.getElementById('axpBulkScore_'+(${i}+1));
            if(next){next.focus();next.select();}
          } else if(event.key==='ArrowUp'){
            var prev=document.getElementById('axpBulkScore_'+(${i}-1));
...
window.axpBulkOnInput = function(input) {
  ...
  const val  = input.value.trim();
  const gradeEl = document.getElementById(`axpBulkGrade_${idx}`);

  if (val === "") {
    if (gradeEl) gradeEl.innerHTML = `<span style="font-size:15px;font-weight:900;color:#94a3b8;">—</span>`;
    ...
    if (!isNaN(n) && n >= 0 && n <= 100) {
      ...
      if (gradeEl) gradeEl.innerHTML = `<strong style="font-size:15px;font-weight:900;color:${color};">${grade}</strong>`;
    }
  }
  _axpBulkUpdateStats();
};
...
function _axpBulkUpdateStats() {
  ...
  _setText("axpBulkFilled", filled);
  _setText("axpBulkEmpty",  empty);
  _setText("axpBulkPct",    pct + "%");
}
...
window.axpBulkSubmitAll = async function() {
  if (!window._axpBulk) return;
  ...
  const filled = payload.filter(p => p.score !== "").length;
  if (filled === 0) {
    _showSectionMsg("axpBulkMsg","No marks entered yet.","warning"); return;
  }
  ...
  try {
    ...
    if (res.status === "success") {
      _showSectionMsg("axpBulkMsg", `${filled} marks submitted successfully!`, "success");
      _axpToast(`Marks saved for ${subject} — ${cls} — ${examType}!`, "success");
      /* Highlight saved rows green briefly */
      students.forEach((s, i) => {
        if (marks[s.name] !== undefined) {
          const row = document.getElementById(`axpBulkRow_${i}`);
          if (row) { row.style.background = "#ecfdf5"; setTimeout(()=>{ if(row) row.style.background=""; }, 2500); }
        }
      });
    } else {
      _showSectionMsg("axpBulkMsg", res.message || "Submission failed. Please retry.", "danger");
    }
  } catch(err) {
    _showSectionMsg("axpBulkMsg","Network error. Check connection and retry.","danger");
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-send-fill"></i> Submit All Marks'; }
  }
};
