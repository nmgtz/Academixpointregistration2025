...
const SchoolInfoStorage = {
    ...
    getHeaderLines(className) {
        ...
        const name  = (d && d.schoolNameInput)  || '';
        const idx   = (d && d.schoolIndexInput)  || '';
        const exam  = '';  // will be filled from examType pill at render time
        const year  = (typeof _schoolMeta !== 'undefined' && _schoolMeta && _schoolMeta.year) ? _schoolMeta.year : new Date().getFullYear();
        // Build smart defaults from any available school data
        const sd = (typeof window !== 'undefined' && window.currentSchoolData) ? window.currentSchoolData : {};
        const autoName  = name  || (sd.schoolName  || '').trim();
        const autoIdx   = idx   || (sd.indexNumber || '').trim();
        const autoExam  = (sd.examType || '').trim();
        const line4 = (autoIdx ? autoIdx.toUpperCase() + ' - ' : '') + (autoName ? autoName.toUpperCase() : 'ACADEMIXPOINT SECONDARY SCHOOL');
        const line5 = (autoExam ? autoExam.toUpperCase() : 'XXXX') + ' EXAMINATION - ' + year;
        return [
            "PRESIDENT'S OFFICE",
            "REGIONAL ADMINISTRATION AND LOCAL GOVERNMENT",
            "XXXXX DISTRICT COUNCIL",
            line4 || "SXXXX - ACADEMIXPOINT SECONDARY SCHOOL",
            line5
        ];
    },
...
function _axpRenderClassInfoPanel(className, container) {
    ...
    function _autosave() {
        ...
        if (!panel) return;
        [
            'schoolNameInput','schoolIndexInput','openingDateInput','monthInput',
    ...
    const fieldDefs = [
        { key:'schoolNameInput',  label:'School Name',    icon:'bi-building',        type:'text'  },
        { key:'schoolIndexInput', label:'School Index',   icon:'bi-hash',            type:'text'  },
...
function renderResultsReportsSection(containerEl) {
    ...
    function _axpRRLoad(forceRefresh) {
        ...
        if (lbl) lbl.textContent = `Class: ${cls}  |  Exam: ${examType}`;
...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        ...
        /* Header rows — dark navy, kept for both web and PDF */
...
function buildDivisionTable(students) {
    ...
    if (schoolGPA) {
        ...
        gpaCell.style.cssText = `text-align:center;font-weight:bold;padding:5px 6px;border:1px solid #333;background:${gpaBg};color:${gpaFg};-webkit-print-color-adjust:exact;print-color-adjust:exact;`;
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const thStyle = 'border:0.75px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#1a3a5c;color:#fff;overflow:hidden;white-space:nowrap;vertical-align:middle;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
...
function buildSubjectTable(students) {
    ...
    csT.style.cssText += 'width:55%;margin-left:0;';
    csT.innerHTML = `<thead><tr>
        <th colspan="2" style="text-align:center;background:#1a3a5c;color:#fff;padding:5px 8px;border:1px solid #333;-webkit-print-color-adjust:exact;print-color-adjust:exact;">EXAMINATION CENTRE OVERALL PERFORMANCE</th>
    </tr></thead>
        <tbody>
            <tr>
            ...
            <tr>
                ...
                <td style="text-align:left;font-weight:bold;padding:4px 8px;border:1px solid #333;background:${schoolGPABg};color:${schoolGPABg&&['#006400','#00AA00'].includes(schoolGPABg)?'#fff':'#000'};-webkit-print-color-adjust:exact;print-color-adjust:exact;">
    ...
    const thStyle = 'border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#1a3a5c;color:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
    ...
    const sTHStyle='border:1px solid #333;padding:3px 4px;text-align:center;font-weight:bold;background:#1a3a5c;color:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;';
    ...
    Object.keys(subjectStats).forEach(sub=>{
        ...
        // Grade cell — background coloured by GPA
        ...
        const grFg = (gradeLabel==='A'||gradeLabel==='B') ? '#fff' : '#000';
        tdC.textContent=gt;
        tdC.style.cssText=`${sTDBase}font-weight:bold;background:${grBg};color:${grFg};min-width:40px;-webkit-print-color-adjust:exact;print-color-adjust:exact;`;
        row.appendChild(tdC);
...
function renderStudentReports(students, container) {
    ...
    function render(index) {
        ...
        const reqs         = _getAllReqs().map(r=>`<li style="margin-bottom:1px;">${escapeHtml(r)}</li>`).join('');
        ...
        if (!eligStudent) {
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;min-height:auto;margin:0 auto;background:#fff;padding:8mm 10mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;page-break-after:always;">
                            ...
                            <table style="width:100%;border-collapse:collapse;font-size:9.5px;">
                                ...
                                <tbody>
                                    ${Object.entries(student.scores||{}).map(([sub,sc],idx)=>{
                                        if(sc.mark==null||sc.mark==='') return '';
                                        const bg='#fff'; // no alternating rows
    ...
    function renderTemplateB(index) {
        ...
        if (!eligStudent) {
            ...
            const subjectRows = Object.entries(student.scores||{})
                ...
                .map(([sub,sc]) => {
                    ...
                    return `<tr style="background:#fff;">
                        <td style="border:0.5px solid #bbb;padding:3px 6px;font-size:9.5px;background:#fff;">${escapeHtml(sub)}</td>
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;min-height:auto;margin:0 auto;background:#fff;padding:6mm 8mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;page-break-after:always;">
...
function renderStudentCertificates(students) {
    ...
    function renderCert(index) {
        ...
        certDiv.innerHTML = `
            <div style="width:297mm;height:210mm;margin:0;padding:0;background:#fff;position:relative;font-family:'Georgia',serif;box-sizing:border-box;overflow:hidden;display:flex;flex-direction:column;">
                ...
                <!-- Content area: fixed layout — no overlap -->
                <div style="position:relative;z-index:2;padding:8mm 18mm 6mm 18mm;box-sizing:border-box;height:calc(210mm - 22mm);display:flex;flex-direction:column;gap:0;">

                    <!-- SCHOOL IDENTITY (top) -->
                    <div style="text-align:center;flex-shrink:0;padding-bottom:3mm;border-bottom:1px solid #e0d9c4;">
                        ${schoolLogo
                            ? `<img src="${schoolLogo}" style="width:44px;height:44px;object-fit:contain;display:block;margin:0 auto 2px;">`
                            : `<div style="width:44px;height:44px;border-radius:50%;background:#1a3a5c;margin:0 auto 2px;display:flex;align-items:center;justify-content:center;-webkit-print-color-adjust:exact;print-color-adjust:exact;"><span style="color:#c8a84b;font-size:18px;font-weight:bold;">${(schoolName||'S')[0]}</span></div>`}
                        <div style="font-size:6.5px;letter-spacing:2.5px;color:#999;text-transform:uppercase;">THE UNITED REPUBLIC OF TANZANIA</div>
                        <div style="font-size:11px;font-weight:bold;color:#1a3a5c;text-transform:uppercase;letter-spacing:1px;margin-top:1px;">${escapeHtml((schoolName||'').toUpperCase())}</div>
                        <div style="font-size:7px;color:#888;">${escapeHtml(schoolIndex.toUpperCase())}</div>
                    </div>

                    <!-- CERTIFICATE TITLE (middle-upper) -->
                    <div style="text-align:center;flex-shrink:0;padding:4mm 0 2mm;">
                        <div style="display:flex;align-items:center;gap:6px;margin:0 auto 3px;width:60%;">
                            <div style="flex:1;height:1px;background:#c8a84b;"></div>
                            <i class="bi bi-diamond-fill" style="color:#c8a84b;font-size:7px;"></i>
                            <div style="flex:1;height:1px;background:#c8a84b;"></div>
                        </div>
                        <div style="font-size:7px;letter-spacing:4px;color:#888;text-transform:uppercase;margin-bottom:2px;">– ${lang.presentedTo} –</div>
                        <div style="font-size:20px;font-weight:bold;color:#1a3a5c;letter-spacing:3px;text-transform:uppercase;line-height:1.2;">${lang.certificate}</div>
                        <div style="width:60px;height:2.5px;background:linear-gradient(90deg,transparent,#c8a84b,transparent);margin:3px auto 0;"></div>
                    </div>

                    <!-- RECIPIENT NAME (centre) -->
                    <div style="text-align:center;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:2mm 0;">
                        <div style="font-size:8px;letter-spacing:2px;color:#aaa;font-style:italic;margin-bottom:3mm;">${lang.presentedTo}</div>
                        <div style="font-size:20px;font-weight:bold;color:#1a3a5c;letter-spacing:2px;text-transform:uppercase;line-height:1.3;">${escapeHtml(student.name)}</div>
                        <div style="display:flex;align-items:center;gap:7px;margin:3mm auto;width:45%;">
                            <div style="flex:1;height:1.5px;background:linear-gradient(90deg,transparent,#c8a84b);"></div>
                            <i class="bi bi-stars" style="color:#c8a84b;font-size:7px;"></i>
                            <div style="flex:1;height:1.5px;background:linear-gradient(90deg,#c8a84b,transparent);"></div>
                        </div>
                        <div style="font-size:9px;color:#444;font-style:italic;line-height:1.8;max-width:130mm;text-align:center;">
                            ${lang.recognition}<br>
                            <span style="color:#888;font-size:8px;">${lang.conducted} <strong style="color:#1a3a5c;">${escapeHtml((schoolName||'').toUpperCase())}</strong></span>
                        </div>
                    </div>

                    <!-- SIGNATURES (bottom) -->
                    <div style="flex-shrink:0;padding-top:3mm;border-top:1px solid #e0d9c4;">
                        <div style="display:flex;justify-content:space-between;align-items:flex-end;">
                            <div style="text-align:center;flex:1;">
                                <div style="height:8mm;border-bottom:1.5px solid #333;width:35mm;margin:0 auto 2mm;"></div>
                                <div style="font-size:8.5px;font-weight:bold;color:#222;">${escapeHtml(classTeacher||'___________')}</div>
                                <div style="font-size:7px;color:#777;letter-spacing:0.5px;">${lang.classTeacher}</div>
                                <div style="font-size:6.5px;color:#aaa;margin-top:1mm;">${lang.date}: ${escapeHtml(closingDate)}</div>
                            </div>
                            <div style="text-align:center;flex:1;display:flex;flex-direction:column;align-items:center;">
                                <div style="width:18mm;height:18mm;border-radius:50%;border:2px solid #c8a84b;display:flex;align-items:center;justify-content:center;background:rgba(200,168,75,0.06);-webkit-print-color-adjust:exact;print-color-adjust:exact;flex-direction:column;">
                                    <i class="bi bi-patch-check-fill" style="font-size:16px;color:#c8a84b;"></i>
                                    <div style="font-size:5px;color:#1a3a5c;font-weight:bold;letter-spacing:0.4px;margin-top:1px;">${lang.seal||'OFFICIAL SEAL'}</div>
                                </div>
                            </div>
                            <div style="text-align:center;flex:1;">
                                <div style="height:8mm;border-bottom:1.5px solid #333;width:35mm;margin:0 auto 2mm;"></div>
                                <div style="font-size:8.5px;font-weight:bold;color:#222;">${escapeHtml(headmaster||'___________')}</div>
                                <div style="font-size:7px;color:#777;letter-spacing:0.5px;">${lang.headTeacher}</div>
                                <div style="font-size:6.5px;color:#aaa;margin-top:1mm;">${lang.date}: ${escapeHtml(closingDate)}</div>
                            </div>
                        </div>
                    </div>
    ...
    document.getElementById('axpCertPreview').onclick = () => {
        ...
        html2pdf().set(_pdfOpt(`Certificate_${currentIndex+1}.pdf`)).from(certDiv).outputPdf('dataurlnewwindow').then(restore).catch(restore);
    };
    document.getElementById('axpCertDownload').onclick = () => {
        const restore = _certBtnLoad('axpCertDownload','Downloading…');
        html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_Certificate_${_sanitize(allStudents[currentIndex].name)}.pdf`)).from(certDiv).save().then(restore).catch(restore);
    };
