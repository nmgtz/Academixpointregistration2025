    ...
    if (isSuccess) {
      if (action === "login") {
        ...
        _showProcessingResult(true, "Login successful!", () => {
          ...
          /* Step 2: fade out the auth overlay smoothly instead of abrupt close */
          const authOv = document.getElementById("axp-auth-overlay");
          if (authOv) {
            authOv.style.transition = "opacity 0.6s ease";
          ...
          /* Step 4: show loading screen after a brief moment (let fade start) */
          setTimeout(() => {
            if (authOv) authOv.style.display = "none";
            _showLoadingScreen(() => {
              /* Dashboard data is already loading/loaded — just show it */
