...
Axp results replacement · JS
Copy

Syntax highlighting has been disabled due to code size.
// =============================================================================
// AcademixPoint — Results, Reports & Certificates Replacement Block
// =============================================================================
//
// PURPOSE:
//   Replace the old results/reports rendering functions in File 1 with the
//   new display + calculation logic from File 2.
//
// WHAT IS NOT TOUCHED:
//   - scriptURL, _apiGet, _apiPost        → File 1 data fetching stays as-is
//   - _appScriptSchoolId, _schoolMeta     → File 1 globals stay as-is
//   - globalUsername, _dashboardData      → File 1 globals stay as-is
//   - _axpGrade()                         → keep in File 1 (Demo + Task Progress use it)
//   - _axpExtractMark()                   → keep in File 1 (Task Progress uses it)
//   - navigateToSection()                 → keep in File 1, update routing cases below
//   - _axpToast(), escapeHtml()           → File 1 helpers used throughout here
//   - axp-section-card, axp-btn-* etc.    → File 1 CSS classes used throughout here
//
// DELETE FROM FILE 1 (before pasting this block):
//   renderResultsReportsSection()
//   _axpRRFetchAndRender()
//   _axpRRRenderResults()
//   _axpRRRenderReport()
//   _axpRRRenderAnalysis()
//   _axpRRRenderRanking()
//   _axpRRRenderGeneral()
//   _axpRRRenderSubjectWise()
//   _axpRRRenderSubjectRanking()
//   axpRROpenClass()
//   axpRRCloseClass()
//   axpRRRefreshClass()
//   axpRRSwitchTab()
...
//   axpLoadTeacherBehaviour()
//   renderStudentsReportSection()
//   axpLoadStudentsReport()
//   _renderStudentsReport()
//
// PASTE THIS BLOCK right where those deleted functions were.
//
// ROUTING — NO CHANGES NEEDED in navigateToSection().
//   The new functions use the same names as the deleted ones, so all existing
//   case statements in File 1 will call the new code automatically:
//     case 'results-reports':  renderResultsReportsSection(); break;  ← works as-is
//     case 'analytics':        renderAnalyticsSection();      break;  ← works as-is
//     case 'students-report':  renderStudentsReportSection(); break;  ← works as-is
//
// OPTIONAL IMPROVEMENT — pass the container element for reliability:
//     case 'results-reports':  renderResultsReportsSection(sectionEl); break;
// =============================================================================


// ─────────────────────────────────────────────────────────────────────────────
// SECTION 1: School Info Storage (per-class localStorage)
// Used by the wizard, report renderer, and certificate renderer to read
// schoolName, classTeacher, headmaster, dates, requirements.
// ─────────────────────────────────────────────────────────────────────────────
...
function renderResultsReportsSection(containerEl) {
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        // _FORCED_W = the exact pixel width we lock the container to.
        // windowWidth MUST equal _FORCED_W so html2canvas fills its virtual
        // browser 100% with content — any mismatch leaves blank space on the right.
        // html2pdf then scales that image to fit (297 - 2×margin)mm on the page.
        const _MARGIN   = 6;     // mm each side (top/right/bottom/left)
        const _FORCED_W = Math.round((297 - 2 * _MARGIN) * 96 / 25.4); // 285mm→≈1077px
        const opt = {
        ...
        // ── CAPTURE CONTAINER ──────────────────────────────────────────────────
        // Clone cont's innerHTML into a fresh off-screen div attached directly to
        // document.body. This completely bypasses any parent width constraints
        // (axp-section-card padding, axpSectionWrapper max-width, etc.) that would
        // otherwise prevent cont from rendering at full _FORCED_W width.
        // The clone is removed immediately after the PDF is built.
        const captureDiv = document.createElement('div');
        captureDiv.style.cssText = [
            'position:absolute',
            'top:0',
            'left:-9999px',         // off-screen left — html2canvas can still render it
            `width:${_FORCED_W}px`,
            `max-width:${_FORCED_W}px`,
            'overflow:visible',
        ...
        captureDiv.innerHTML = cont.innerHTML;
        ...
        // Simple reliable pipeline — same pattern as report card and certificate
        let p;
        if (action === 'download') {
            p = html2pdf().set(opt).from(captureDiv).save();
        } else {
            p = html2pdf().set(opt).from(captureDiv).outputPdf('dataurlnewwindow');
        }
...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        ...
        table.summary-table td { text-align: center; border: 0.5px solid #999; padding: 3px 5px; background: #fff; color: #000; }
...
function buildDivisionTable(students) {
    ...
    [document.createElement('th'), ...Object.keys(divCount).map(d => {
        ...
        th.style.cssText = 'border:0.5px solid #999;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
        hRow.appendChild(th);
    });
    ...
    ['F','M'].forEach(g => {
        ...
        td0.textContent = g;
        td0.style.cssText = 'border:0.5px solid #999;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
        row.appendChild(td0);
        Object.keys(divCount).forEach(d => {
            const td = document.createElement('td');
            td.textContent = divGender[d][g];
            td.style.cssText = 'border:0.5px solid #999;padding:4px 6px;text-align:center;background:#fff;color:#000;';
            row.appendChild(td);
        });
    ...
    td0t.textContent = 'T';
    td0t.style.cssText = 'border:0.5px solid #999;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    totRow.appendChild(td0t);
    Object.keys(divCount).forEach(d => {
        const td = document.createElement('td');
        td.textContent = divCount[d];
        td.style.cssText = 'border:0.5px solid #999;padding:4px 6px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    ...
    if (schoolGPA) {
        ...
        gpaCell.style.cssText = `text-align:center;font-weight:bold;padding:6px;border:0.5px solid #999;font-size:11px;letter-spacing:0.5px;background:${gpaBg};color:${gpaFg};-webkit-print-color-adjust:exact;print-color-adjust:exact;`;
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const thStyle = 'border:0.5px solid #999;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    ...
    const tdStyle    = 'border:0.5px solid #999;padding:3px 4px;text-align:center;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdNameStyle= 'border:0.5px solid #999;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:nowrap;vertical-align:middle;';
    const tdLastStyle= 'border:0.5px solid #999;padding:3px 4px;text-align:left;background:#fff;color:#000;overflow:hidden;white-space:normal;word-break:break-word;vertical-align:middle;';
...
function buildStudentTableB(students, displayMode, showPosition) {
    ...
    const thSt = 'border:0.5px solid #999;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;vertical-align:middle;';
    const fixedHeaders = ["CAND'S NO","CAND'S NAME","SEX","AGG","DIV",...(showPosition?["POS"]:[])];
    fixedHeaders.forEach((h,i) => {
        const th = document.createElement('th');
        th.textContent = h;
        th.style.cssText = thSt + (i===1?'text-align:left;':'') + 'vertical-align:middle;border:0.5px solid #999;';
        th.rowSpan = 2;
        hr1.appendChild(th);
    });
    ...
    if (subjects.length) {
        ...
        subGrpTh.style.cssText = thSt + 'border:0.5px solid #999;';
        hr1.appendChild(subGrpTh);

        subjects.forEach(sub => {
            const th = document.createElement('th');
            th.textContent = sub;
            th.style.cssText = thSt + 'font-size:clamp(5px,0.65vw,9px);white-space:normal;word-break:break-word;font-weight:bold;color:#000;background:#fff;border:0.5px solid #999;';
            hr2.appendChild(th);
        });
    }
    ...
    // tbody
    const tbody = document.createElement('tbody');
    const tdSt  = 'border:0.5px solid #999;padding:2px 3px;text-align:center;background:#fff;color:#000;vertical-align:middle;white-space:nowrap;overflow:hidden;';
...
function buildSubjectTable(students) {
    ...
    csT.innerHTML = `<thead><tr>
        <th colspan="2" style="text-align:center;background:#fff;color:#000;font-weight:bold;padding:5px 8px;border:0.5px solid #999;">EXAMINATION CENTRE OVERALL PERFORMANCE</th>
    </tr></thead>
        <tbody>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:0.5px solid #999;width:65%;">TOTAL PASSED CANDIDATES (DIV I–IV)</td>
                <td style="text-align:left;font-weight:bold;padding:4px 8px;border:0.5px solid #999;">${passed}</td>
            </tr>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:0.5px solid #999;">PERFORMANCE PERCENTAGE</td>
                <td style="text-align:left;font-weight:bold;padding:4px 8px;border:0.5px solid #999;">${perfPct}%</td>
            </tr>
            <tr>
                <td style="font-weight:bold;padding:4px 8px;border:0.5px solid #999;">SCHOOL GPA</td>
                <td style="text-align:left;font-weight:bold;padding:5px 8px;border:0.5px solid #999;font-size:11px;background:${schoolGPABg||'#fff'};color:${schoolGPABg&&['#006400','#00AA00','#FF0000'].includes(schoolGPABg)?'#fff':schoolGPABg?'#000':'#000'};-webkit-print-color-adjust:exact;print-color-adjust:exact;">
    ...
    const thStyle = 'border:0.5px solid #999;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    const thFirst = document.createElement('th');
    thFirst.textContent = 'CANDIDATES'; thFirst.rowSpan=2;
    thFirst.style.cssText = thStyle + 'text-align:left;vertical-align:middle;border:0.5px solid #999;';
    ...
    ['F','M','T'].concat(...Array(9).fill(['F','M','T'])).forEach(l=>{
        ...
        th.style.cssText=thStyle+'font-size:9px;padding:2px 1px;border:0.5px solid #999;';
        oHR2.appendChild(th);
    });
    ...
    catTd.style.cssText='border:0.5px solid #999;padding:3px 6px;font-weight:bold;text-align:left;background:#fff;color:#000;';
    oDR.appendChild(catTd);
    const tdSubStyle='border:0.5px solid #999;padding:2px 2px;text-align:center;vertical-align:middle;background:#fff;color:#000;font-size:10px;';
    ...
    const sTHStyle='border:0.5px solid #999;padding:3px 4px;text-align:center;font-weight:bold;background:#fff;color:#000;';
    const thS=document.createElement('th'); thS.textContent='SUBJECT'; thS.rowSpan=2; thS.style.cssText=sTHStyle+'text-align:left;vertical-align:middle;border:0.5px solid #999;'; sHR1.appendChild(thS);
    const satCols = [{t:'SAT-F',rs:2},{t:'SAT-M',rs:2},{t:'TOTAL',rs:2}];
    satCols.forEach(({t,rs})=>{ const th=document.createElement('th'); th.textContent=t; th.rowSpan=rs; th.style.cssText=sTHStyle+'font-size:9px;vertical-align:middle;border:0.5px solid #999;'; sHR1.appendChild(th); });
    ...
    ['AVER','GPA','POS','GRADE'].forEach(h=>{ const th=document.createElement('th'); th.textContent=h; th.rowSpan=2; th.style.cssText=sTHStyle+'vertical-align:middle;border:0.5px solid #999;'; sHR1.appendChild(th); });
    ['A','B','C','D','F'].forEach(()=>['F','M','T'].forEach(l=>{
        const th=document.createElement('th'); th.textContent=l; th.style.cssText=sTHStyle+'font-size:8px;padding:2px 1px;border:0.5px solid #999;'; sHR2.appendChild(th);
    }));
    sThead.appendChild(sHR1); sThead.appendChild(sHR2); sT.appendChild(sThead);
    const sTbody=document.createElement('tbody');
    const sTDBase='border:0.5px solid #999;padding:2px 3px;text-align:center;vertical-align:middle;background:#fff;color:#000;';
    Object.keys(subjectStats).forEach(sub=>{
...
function _axpTH(text, opts) {
    ...
    const th = document.createElement('th');
    th.textContent = text;
    th.style.cssText = `border:0.5px solid #999;padding:4px 6px;text-align:${opts.align||'center'};font-weight:bold;background:#fff;color:#000;-webkit-print-color-adjust:exact;print-color-adjust:exact;${opts.extra||''}`;
...
function _axpTD(text, opts) {
    ...
    td.style.cssText = `border:0.5px solid #999;padding:3px 5px;text-align:${opts.align||'center'};background:#fff;color:#000;${opts.extra||''}`;
...
function renderStudentReports(students, container) {
                                        ...
                                        <tbody>
                                            ${Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                                                .map(([key,val])=>{const bBg={A:'#006400',B:'#00AA00',C:'#ADFF2F',D:'#FFA500',F:'#FF0000'};const bFg={A:'#fff',B:'#fff',C:'#000',D:'#000',F:'#fff'};return `<tr>
                                                    ...
                                                    <td style="border:0.5px solid #ccd;padding:1.5px 3px;text-align:center;font-weight:bold;background:${bBg[val]||'#fff'};color:${bFg[val]||'#000'};-webkit-print-color-adjust:exact;print-color-adjust:exact;">${val}</td>
                                                </tr>`;}).join('')}
                                            <tr>
    ...
    // Portrait A4: 210mm × 297mm → 794 × 1123px at 96dpi.
    // windowWidth must equal _REP_W so html2canvas fills its virtual browser 100%.
    const _REP_W = 794;
    ...
    // Clone reportDiv into a fresh body-level div so html2canvas is never
    // constrained by axp-section-card padding or any ancestor max-width.
    // The clone is exactly _REP_W wide, position:fixed, invisible to the user.
    function _repCapturePdf(filename, action) {
        const captureDiv = document.createElement('div');
        captureDiv.style.cssText = [
            'position:absolute',
            'top:0',
        ...
        captureDiv.innerHTML = reportDiv.innerHTML;
        document.body.appendChild(captureDiv);
        const cleanup = () => { if (captureDiv.parentNode) document.body.removeChild(captureDiv); };
        const opt = _pdfOpt(filename);
        if (action === 'preview') {
            return html2pdf().set(opt).from(captureDiv).outputPdf('dataurlnewwindow')
                .then(cleanup).catch(cleanup);
        } else {
            return html2pdf().set(opt).from(captureDiv).save()
                .then(cleanup).catch(cleanup);
        }
    }
            ...
            if (count === 0) {
                ...
                await html2pdf().set({
                    ...
                    html2canvas: {
                        ...
                        allowTaint: true,
                        logging: false,
                        windowWidth: _REP_W,   // matches container width exactly
                        scrollX: 0,
                        scrollY: 0
                    },
...
function renderStudentCertificates(students) {
    ...
    // windowWidth must equal _CERT_W so html2canvas fills its virtual browser 100%.
    const _CERT_W = 1122;
    const _certPdfOpt = filename=>({
        margin: 0,
        filename,
        image: { type:'jpeg', quality:0.98 },
        html2canvas: {
    ...
    // Clone certDiv into a fresh body-level div so html2canvas is never
    // constrained by any ancestor width or padding.
    function _certCapturePdf(filename, action) {
        const captureDiv = document.createElement('div');
        captureDiv.style.cssText = [
            'position:absolute',
            'top:0',
        ...
        captureDiv.innerHTML = certDiv.innerHTML;
        document.body.appendChild(captureDiv);
        const cleanup = () => { if (captureDiv.parentNode) document.body.removeChild(captureDiv); };
        const opt = _certPdfOpt(filename);
        if (action === 'preview') {
            return html2pdf().set(opt).from(captureDiv).outputPdf('dataurlnewwindow')
                .then(cleanup).catch(cleanup);
        } else {
