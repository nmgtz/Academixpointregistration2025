...
function updateDashboardUI(data) {
    ...
    // ✨ CRITICAL: Setup dynamic menu links based on school index from login data
    if (data.schoolindex && typeof window.setupDynamicMenuLinks === 'function') {
        const schoolCode = data.schoolindex;
        console.log('🔄 Re-setting up menu links with school code from login:', schoolCode);
        
        // Re-setup all menu links dynamically
        window.setupDynamicMenuLinks(schoolCode);
        
        // Update shareable link
        const baseUrl = `https://www.academixpoint.com/p/${schoolCode.toLowerCase()}`;
        const shareableLink = `${baseUrl}-teachers-feeding-area.html`;
    ...
    // 🔔 Display Notifications Section (after plan div)
    displayNotifications(data);
    
    // 📢 Display Announcements Section (after notifications)
    displayAnnouncements(data);
}

function displayNotifications(data) {
    // Find the plan div to insert notifications after it
    const planDiv = document.querySelector('.plan, [class*="plan"]');
    
    if (!planDiv) {
        console.warn('Plan div not found. Notifications cannot be displayed.');
        return;
    }
    
    // Remove existing notifications section if it exists
    const existingNotifications = document.getElementById('notificationsSection');
    ...
    notificationsSection.style.cssText = `
    ...
    notificationsSection.innerHTML = `
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 15px;">
            <i class="bi bi-bell-fill" style="font-size: 24px; color: #ffd700;"></i>
            <h3 style="margin: 0; font-size: 18px; font-weight: 600;">Important Notification</h3>
        </div>
        <div style="background: rgba(255, 255, 255, 0.1); border-radius: 8px; padding: 15px; white-space: pre-wrap; line-height: 1.6;">
            ${notification}
        </div>
        <div style="margin-top: 12px; font-size: 12px; opacity: 0.8; display: flex; align-items: center; gap: 5px;">
            <i class="bi bi-clock"></i>
            <span>This notification expires 24 hours after registration</span>
        </div>
    `;
    
    // Insert after plan div
    planDiv.insertAdjacentElement('afterend', notificationsSection);
}

function displayAnnouncements(data) {
    // Find the notifications section or plan div to insert announcements after it
    const insertAfter = document.getElementById('notificationsSection') || 
                        document.querySelector('.plan, [class*="plan"]');
    
    if (!insertAfter) {
        console.warn('Cannot find element to insert announcements after.');
        return;
    }
    
    // Remove existing announcements section if it exists
    const existingAnnouncements = document.getElementById('announcementsSection');
    if (existingAnnouncements) {
        existingAnnouncements.remove();
    }
    ...
    announcementsSection.id = 'announcementsSection';
    announcementsSection.className = 'announcements-section';
    announcementsSection.style.cssText = `
        background: #ffffff;
        border-radius: 12px;
        padding: 20px;
        margin: 20px 0;
    ...
    `;
    header.innerHTML = `
        <i class="bi bi-megaphone-fill" style="font-size: 24px; color: #4ecca3;"></i>
        <h3 style="margin: 0; font-size: 18px; font-weight: 600; color: #2c3e50;">Announcements</h3>
        <span style="margin-left: auto; background: #4ecca3; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">
            ${announcements.length} Active
        </span>
    `;
    
    announcementsSection.appendChild(header);
    
    // Create announcements list
    const announcementsList = document.createElement('div');
    announcementsList.style.cssText = `
        display: flex;
    ...
    announcements.forEach((announcement, index) => {
        ...
        const priorityColor = priorityColors[announcement.priority] || '#95a5a6';
        ...
        // Add hover effect
        announcementCard.addEventListener('mouseenter', function() {
        ...
        announcementCard.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 10px;">
                ...
                <span style="background: ${priorityColor}; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px; font-weight: 600; white-space: nowrap;">
            ...
            <div style="display: flex; align-items: center; gap: 8px; font-size: 12px; color: #7f8c8d;">
                <i class="bi bi-calendar3"></i>
                <span>${formatAnnouncementDate(announcement.date)}</span>
                <span style="margin-left: auto; color: #95a5a6;">ID: ${announcement.id}</span>
            </div>
        `;
    ...
    announcementsSection.appendChild(announcementsList);
    
    // Insert after notifications or plan div
    insertAfter.insertAdjacentElement('afterend', announcementsSection);
}
