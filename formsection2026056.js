...
function renderResultsReportsSection(containerEl) {
    ...
    // OR scan for the active/visible section panel File 1 created.
    ...
    if (!cont) {
        console.error('[AXP] renderResultsSection: could not find main content container');
        return;
    }
    ...
    cont.innerHTML = `
        <div class="axp-section-card">
            ...
            <div id="axpRRTabs" style="display:none;margin-bottom:1em;border-bottom:2px solid #2a2a3e;overflow-x:auto;white-space:nowrap;">
                ${['Results','Report','Analysis','Top 10','Least 10','General','Subject Wise','Subject Ranking']
            ...
            <!-- Action buttons (hidden until data loads) -->
            <div id="axpRRActions" style="display:none;margin-bottom:1em;display:none;flex-wrap:wrap;gap:0.6em;">
                <button id="axpRRExcelBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
                    <i class="bi bi-file-earmark-excel"></i> Export Excel
                </button>
                <button id="axpRRPdfBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
                    <i class="bi bi-file-earmark-pdf"></i> Download PDF
                </button>
                <button id="axpRRPreviewBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
                    <i class="bi bi-eye"></i> Preview PDF
                </button>
                <button id="axpRRReportBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
                    <i class="bi bi-file-person"></i> Student Reports
                </button>
                <button id="axpRRCertBtn" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.5em;">
    ...
    function _axpRRShowTabs() {
        const tabs    = document.getElementById('axpRRTabs');
        const actions = document.getElementById('axpRRActions');
        const cls      = document.getElementById('axpRRClassSel').value;
        const examType = document.getElementById('axpRRExamSel').value;
        ...
        tabs.querySelectorAll('.axp-rr-tab').forEach(btn => {
            btn.onclick = () => {
                tabs.querySelectorAll('.axp-rr-tab').forEach(b => b.classList.remove('axp-rr-tab--active'));
        ...
        document.getElementById('axpRRReportBtn').onclick  = () => _axpRRStartReports(_currentStudents);
    ...
    function _axpRRRenderTab(tab) {
        ...
        const showPosition  = document.getElementById('axpRRPosSel').value === 'show';

        switch (tab) {
            case 'results':        cont.innerHTML = ''; cont.appendChild(_buildSchoolHeader()); cont.appendChild(buildDivisionTable(students)); cont.appendChild(buildStudentTable(students, displayMode, showPosition)); break;
            case 'report':         cont.innerHTML = ''; cont.appendChild(buildSubjectTable(students)); break;
    ...
    function _buildSchoolHeader() {
        ...
        const examType = document.getElementById('axpRRExamSel').value;
        const schoolTitle = sd?.indexNumber && sd?.schoolName ? `${sd.indexNumber} - ${sd.schoolName}` : 'School';
            ...
            div.innerHTML = `
                <div style="display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:0 1rem;">
                    ...
                    <div style="text-align:center;flex:1;">
                        <h1 contenteditable="true">${escapeHtml(schoolTitle)}</h1>
                        <p contenteditable="true">Academic Year: ${_schoolMeta.year || '—'}</p>
                        <p contenteditable="true">Exam: ${escapeHtml(examType)}</p>
                    </div>
                    <img src="${schoolLogo}" style="width:100px;height:100px;object-fit:contain;flex-shrink:0;">
                </div>`;
        } else {
            div.innerHTML = `
                <h1 contenteditable="true">${escapeHtml(schoolTitle)}</h1>
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        const cont    = document.getElementById('axpRRContent');
        const sd      = window.currentSchoolData;
        const sName   = sd?.schoolName || 'School';
        ...
        action === 'download' ? worker.save() : worker.outputPdf('dataurlnewwindow');
    }

    function _axpRRStartReports(students) {
        // Hands off to Section 6 (renderStudentReports)
        const wrapper1 = document.querySelector('.scroll-wrapper1') || document.getElementById('axpRRContent');
        renderStudentReports(students, wrapper1);
    }
...
function _axpRRTabIcon(tab) {
    const map = {
        'Results': 'bi-table', 'Report': 'bi-file-earmark-text',
...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        ...
        /* Alternating rows — very light for readability */
        table.axp-results-table tbody tr:nth-child(even),
        table.axp-rr-table tbody tr:nth-child(even) {
            background: #f0f4f8;
        }
        table.axp-results-table tbody tr:nth-child(odd),
        table.axp-rr-table tbody tr:nth-child(odd) {
            background: #fff;
        }
        ...
        table.summary-table td { text-align: center; }
...
function buildDivisionTable(students) {
    ...
    })].forEach(th => hRow.appendChild(th));
    ...
    ['F','M'].forEach(g => {
        ...
        const td0 = document.createElement('td'); td0.textContent = g; row.appendChild(td0);
        Object.keys(divCount).forEach(d => { const td = document.createElement('td'); td.textContent = divGender[d][g]; row.appendChild(td); });
        tbody.appendChild(row);
    });
    const totRow = document.createElement('tr');
    const td0 = document.createElement('td'); td0.textContent = 'T'; td0.style.fontWeight = 'bold'; totRow.appendChild(td0);
    Object.keys(divCount).forEach(d => { const td = document.createElement('td'); td.textContent = divCount[d]; td.style.fontWeight = 'bold'; totRow.appendChild(td); });
    ...
    if (schoolGPA) {
        let grade='F', comment='Fail', bg='#ffcccc';
        if (schoolGPA <= 1.6) { grade='A'; comment='Excellent';    bg='#b2dfdb'; }
        else if (schoolGPA <= 2.6) { grade='B'; comment='Very Good'; bg='#c8e6c9'; }
        else if (schoolGPA <= 3.6) { grade='C'; comment='Good';       bg='#fff9c4'; }
        else if (schoolGPA <= 4.6) { grade='D'; comment='Satisfactory'; bg='#ffe0b2'; }
        const gpaRow = document.createElement('tr');
        const gpaCell = document.createElement('td');
        gpaCell.colSpan = Object.keys(divCount).length + 1;
        gpaCell.style.cssText = `text-align:center;font-weight:bold;background:${bg};padding:3px;`;
...
function buildStudentTable(students, displayMode, showPosition) {
    ...
    const sd = window.currentSchoolData;
    const schoolIdx = sd?.indexNumber || (document.getElementById('schoolIndexInput') ? document.getElementById('schoolIndexInput').value.trim().toUpperCase() : '');
...
function buildSubjectTable(students) {
    ...
    function mkTbl(className) {
        ...
        t.style.cssText = 'width:100%;border-collapse:collapse;font-size:clamp(7px,0.80vw,12px);margin-top:10px;table-layout:fixed;';
        return t;
    }
    ...
    csT.innerHTML = `<thead><tr><th colspan="2" style="text-align:center;">EXAMINATION CENTRE OVERALL PERFORMANCE</th></tr></thead>
        <tbody>
            ...
            <tr><td style="font-weight:bold;">SCHOOL GPA</td>
                <td style="text-align:center;font-weight:bold;background:${schoolGPABg};color:#000;">${schoolGPA} - ${schoolGradeComment}</td></tr>
        </tbody>`;
    ...
    Object.keys(subjectStats).forEach(sub=>{
        ...
        tdC.style.cssText=`background:${grBg};font-weight:bold;color:#000;`; row.appendChild(tdC);
        sTbody.appendChild(row);
    });
    sT.appendChild(sTbody);

    const wrapper=document.createElement('div');
    wrapper.appendChild(csT); wrapper.appendChild(oT); wrapper.appendChild(sT);
    return wrapper;
}
...
function _buildAnalysisView(students) {
    const wrap = document.createElement('div');
    wrap.className = 'axp-section-card';

    // Gender breakdown
    let mCount=0, fCount=0, mTotal=0, fTotal=0;
    students.forEach(s => {
        if (s.gender==='M') { mCount++; mTotal+=Number(s.average||0); }
        else { fCount++; fTotal+=Number(s.average||0); }
    });
    const mAvg = mCount ? (mTotal/mCount).toFixed(2) : 'N/A';
    const fAvg = fCount ? (fTotal/fCount).toFixed(2) : 'N/A';

    // Subject averages
    const subjTot={}, subjCnt={};
    students.forEach(s => Object.entries(s.scores||{}).forEach(([sub,sc]) => {
        if (typeof sc.mark==='number') { subjTot[sub]=(subjTot[sub]||0)+sc.mark; subjCnt[sub]=(subjCnt[sub]||0)+1; }
    }));
    const subjRows = Object.keys(subjTot).map(sub => {
        const avg = (subjTot[sub]/subjCnt[sub]).toFixed(2);
        const { grade, color } = _axpGrade(parseFloat(avg));
        return { sub, avg, grade, color };
    }).sort((a,b) => b.avg - a.avg);

    wrap.innerHTML = `
        <h3 class="axp-section-title"><i class="bi bi-graph-up"></i> Analysis</h3>
        <div class="axp-form-row" style="gap:1.5em;flex-wrap:wrap;margin-bottom:1.5em;">
            <div class="axp-section-card" style="flex:1;min-width:140px;text-align:center;padding:1.2em;">
                <i class="bi bi-gender-male" style="font-size:2em;color:var(--axp-primary,#4ecca3);"></i>
                <div style="font-size:1.8em;font-weight:bold;color:#fff;">${mCount}</div>
                <div style="color:#888;font-size:0.85em;">Male Students</div>
                <div style="color:var(--axp-primary,#4ecca3);font-size:0.9em;">Avg: ${mAvg}</div>
            </div>
            <div class="axp-section-card" style="flex:1;min-width:140px;text-align:center;padding:1.2em;">
                <i class="bi bi-gender-female" style="font-size:2em;color:var(--axp-primary,#4ecca3);"></i>
                <div style="font-size:1.8em;font-weight:bold;color:#fff;">${fCount}</div>
                <div style="color:#888;font-size:0.85em;">Female Students</div>
                <div style="color:var(--axp-primary,#4ecca3);font-size:0.9em;">Avg: ${fAvg}</div>
            </div>
            <div class="axp-section-card" style="flex:1;min-width:140px;text-align:center;padding:1.2em;">
                <i class="bi bi-people" style="font-size:2em;color:var(--axp-primary,#4ecca3);"></i>
                <div style="font-size:1.8em;font-weight:bold;color:#fff;">${students.length}</div>
                <div style="color:#888;font-size:0.85em;">Total Students</div>
            </div>
        </div>
        <h4 style="color:var(--axp-primary,#4ecca3);margin-bottom:0.8em;"><i class="bi bi-bar-chart"></i> Subject Averages</h4>
        <table class="axp-rr-table" style="width:100%;border-collapse:collapse;">
            <thead><tr>
                <th>Subject</th><th>Avg Mark</th><th>Grade</th><th>Performance</th>
            </tr></thead>
            <tbody>${subjRows.map(r=>`
                <tr>
                    <td style="font-weight:500;">${escapeHtml(r.sub)}</td>
                    <td style="text-align:center;">${r.avg}</td>
                    <td style="text-align:center;font-weight:bold;color:${r.color};">${r.grade}</td>
                    <td>
                        <div class="axp-progress-bar" style="height:10px;background:#2a2a3e;border-radius:5px;overflow:hidden;">
                            <div class="axp-progress-fill" style="height:100%;width:${Math.min(parseFloat(r.avg),100)}%;background:${r.color};border-radius:5px;"></div>
                        </div>
                    </td>
                </tr>`).join('')}
            </tbody>
...
function _buildRankingView(students, direction, limit) {
    ...
    const ranked = direction==='top'
        ...
        : eligible.slice(-limit).reverse();

    const title = direction==='top' ? `<i class="bi bi-trophy"></i> Top ${limit} Students` : `<i class="bi bi-arrow-down-circle"></i> Least ${limit} Students`;

    const wrap = document.createElement('div');
    wrap.className = 'axp-section-card';
    const sd = window.currentSchoolData;
    const schoolIdx = sd?.indexNumber || '';

    wrap.innerHTML = `
        <h3 class="axp-section-title">${title}</h3>
        <table class="axp-rr-table" style="width:100%;border-collapse:collapse;">
            <thead><tr>
                <th>Pos</th><th>Cand No.</th><th>Name</th><th>Sex</th>
                <th>Points</th><th>Division</th><th>Average</th>
            </tr></thead>
            <tbody>${ranked.map((s,i) => {
                const candNum = schoolIdx ? `${schoolIdx}-${String(students.indexOf(s)+1).padStart(4,'0')}` : String(i+1);
                return `<tr>
                    <td style="text-align:center;font-weight:bold;color:var(--axp-primary,#4ecca3);">${s.position}</td>
                    <td>${escapeHtml(candNum)}</td>
                    ...
                    <td style="text-align:center;">${escapeHtml(s.division||'-')}</td>
...
function _buildGeneralView(students) {
    ...
    const divBreak = {};
    students.forEach(s => { divBreak[s.division]=(divBreak[s.division]||0)+1; });

    const wrap = document.createElement('div');
    wrap.className = 'axp-section-card';
    wrap.innerHTML = `
        <h3 class="axp-section-title"><i class="bi bi-clipboard-data"></i> General Result</h3>
        <div class="axp-form-row" style="gap:1em;flex-wrap:wrap;margin-bottom:1.5em;">
            ${[
                {label:'Registered', val:students.length,       icon:'bi-people'},
                {label:'Passed',     val:eligible.length,        icon:'bi-check-circle'},
                {label:'Failed',     val:students.length-eligible.length, icon:'bi-x-circle'},
                {label:'Pass Rate',  val:`${students.length ? ((eligible.length/students.length)*100).toFixed(1) : 0}%`, icon:'bi-percent'}
            ].map(item=>`
                <div class="axp-section-card" style="flex:1;min-width:120px;text-align:center;padding:1em;">
                    <i class="bi ${item.icon}" style="font-size:1.8em;color:var(--axp-primary,#4ecca3);"></i>
                    <div style="font-size:1.6em;font-weight:bold;color:#fff;">${item.val}</div>
                    <div style="color:#888;font-size:0.85em;">${item.label}</div>
                </div>`).join('')}
        </div>
        <h4 style="color:var(--axp-primary,#4ecca3);margin-bottom:0.8em;"><i class="bi bi-pie-chart"></i> Division Breakdown</h4>
        <table class="axp-rr-table" style="width:100%;border-collapse:collapse;max-width:400px;">
            <thead><tr><th>Division</th><th>Count</th><th>Percentage</th></tr></thead>
            <tbody>${['I','II','III','IV','O'].map(d=>`
                <tr>
                    <td style="font-weight:bold;text-align:center;">DIV ${d==='O'?'0':d}</td>
                    <td style="text-align:center;">${divBreak[d]||0}</td>
                    <td style="text-align:center;">${students.length?((divBreak[d]||0)/students.length*100).toFixed(1):0}%</td>
                </tr>`).join('')}
            </tbody>
        </table>`;
    return wrap;
...
function _buildSubjectWiseView(students) {
    ...
    students.forEach(s => Object.keys(s.scores||{}).forEach(sub=>subjects.add(sub)));

    const wrap = document.createElement('div');
    wrap.className = 'axp-section-card';
    wrap.innerHTML = `<h3 class="axp-section-title"><i class="bi bi-book"></i> Subject Wise</h3>`;

    subjects.forEach(sub => {
        const subStudents = students.filter(s => s.scores&&s.scores[sub]&&s.scores[sub].mark!=null);
        const sorted = [...subStudents].sort((a,b) => (b.scores[sub].mark||0)-(a.scores[sub].mark||0));

        const tbl = document.createElement('table');
        tbl.className = 'axp-rr-table';
        tbl.style.cssText = 'width:100%;border-collapse:collapse;margin-bottom:1.5em;';
        tbl.innerHTML = `
            <thead>
                <tr><th colspan="4" style="text-align:left;background:#0d0d1a;">${escapeHtml(sub)}</th></tr>
                <tr><th>#</th><th>Name</th><th>Mark</th><th>Grade</th></tr>
            </thead>
            <tbody>${sorted.map((s,i) => `
                <tr>
                    <td style="text-align:center;">${i+1}</td>
                    <td>${escapeHtml(s.name)}</td>
                    <td style="text-align:center;font-weight:bold;">${s.scores[sub].mark}</td>
                    <td style="text-align:center;font-weight:bold;color:${_axpGrade(s.scores[sub].mark).color};">${s.scores[sub].grade||_axpGrade(s.scores[sub].mark).grade}</td>
                </tr>`).join('')}
            </tbody>`;
...
function _buildSubjectRankingView(students) {
    const subjGPAs={}, subjCnts={};
    const gv={A:1,B:2,C:3,D:4,F:5};
    students.forEach(s=>Object.entries(s.scores||{}).forEach(([sub,sc])=>{
        if(gv[sc.grade]){ subjGPAs[sub]=(subjGPAs[sub]||0)+gv[sc.grade]; subjCnts[sub]=(subjCnts[sub]||0)+1; }
    }));
    const ranked = Object.keys(subjGPAs)
        .map(sub=>({ sub, gpa:(subjGPAs[sub]/subjCnts[sub]).toFixed(4), cnt:subjCnts[sub] }))
        .sort((a,b)=>a.gpa-b.gpa);

    const wrap = document.createElement('div');
    wrap.className = 'axp-section-card';
    wrap.innerHTML = `
        <h3 class="axp-section-title"><i class="bi bi-bar-chart"></i> Subject Ranking</h3>
        <table class="axp-rr-table" style="width:100%;border-collapse:collapse;">
            <thead><tr><th>Rank</th><th>Subject</th><th>GPA</th><th>Students</th><th>Grade</th></tr></thead>
            <tbody>${ranked.map((r,i)=>{
                let comment='F (Fail)',bg='#FF0000';
                const g=parseFloat(r.gpa);
                if(g<1.6){comment='A (Excell)';bg='#006400';}
                else if(g<2.6){comment='B (V.Good)';bg='#00FF00';}
                else if(g<3.6){comment='C (Good)';bg='#ADFF2F';}
                else if(g<4.6){comment='D (Satisf)';bg='#FFA500';}
                return `<tr>
                    <td style="text-align:center;font-weight:bold;color:var(--axp-primary,#4ecca3);">${i+1}</td>
                    <td style="font-weight:500;">${escapeHtml(r.sub)}</td>
                    <td style="text-align:center;">${r.gpa}</td>
                    <td style="text-align:center;">${r.cnt}</td>
                    <td style="background:${bg};color:#000;font-weight:bold;text-align:center;">${comment}</td>
                </tr>`;
            }).join('')}
            </tbody>
        </table>`;
...
function renderStudentReports(students, container) {
    ...
    // Nav bar — using axp-btn-secondary like rest of File 1
    ...
    nav.innerHTML = `
        ...
        <button id="axpRepNext"    class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.4em;"><i class="bi bi-chevron-right"></i> Next</button>
        <button id="axpRepPreview" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.4em;"><i class="bi bi-eye"></i> Preview PDF</button>
        <button id="axpRepDownload" class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.4em;"><i class="bi bi-download"></i> Download PDF</button>
        <button id="axpRepDownAll"  class="axp-btn-secondary" style="display:flex;align-items:center;gap:0.4em;"><i class="bi bi-cloud-download"></i> Download All PDFs</button>
        ...
        <span id="axpRepCounter" style="color:#888;font-size:0.9em;align-self:center;margin-left:0.5em;"></span>`;
    ...
    function render(index) {
        ...
        const schoolName   = _getVal('schoolNameInput');
        const schoolIndex  = _getVal('schoolIndexInput');
        ...
        const schoolDisplay = (sd?.schoolName&&sd?.indexNumber)
            ? `${sd.schoolName.toUpperCase()} - ${sd.indexNumber.toUpperCase()}`
            : `${schoolName.toUpperCase()} - ${schoolIndex.toUpperCase()}`;

        const candNum = sd?.indexNumber
            ? `${sd.indexNumber.toUpperCase()}-${String(index+1).padStart(4,'0')}`
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;min-height:297mm;margin:0 auto;background:#fff;padding:10mm 12mm;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;">
                    ...
                    <div style="text-align:center;padding-top:6mm;margin-bottom:3mm;">
                        ...
                        <h2 style="margin:2px 0;font-size:13px;font-weight:bold;color:#1a3a5c;text-transform:uppercase;">${escapeHtml(schoolDisplay)}</h2>
                        <p style="margin:1px 0;font-size:9px;color:#555;">${escapeHtml(schoolIndex)}</p>
                    ...
                    <p style="font-size:10px;line-height:1.6;margin:2mm 0 3mm;color:#222;">
                        ...
                        <strong style="color:#1a3a5c;">${escapeHtml(sd?.indexNumber||schoolDisplay)}</strong>.
    ...
    const _pdfOpt = (filename, landscape=false) => ({
        margin: 0,
        filename,
    ...
    document.getElementById('axpRepPreview').onclick = () =>
        html2pdf().set(_pdfOpt(`Report_${String(currentIndex+1).padStart(4,'0')}.pdf`)).from(reportDiv).outputPdf('dataurlnewwindow');

    document.getElementById('axpRepDownload').onclick = () => {
        const repNum = String(currentIndex+1).padStart(4,'0');
        html2pdf().set(_pdfOpt(`${_sanitize(_schoolIdx())}_${_sanitize(_examClass())}_${_sanitize(_examType())}_Report_${repNum}.pdf`)).from(reportDiv).save();
    };

    document.getElementById('axpRepDownAll').addEventListener('click', async () => {
        if (isDownloadingAll) {
            isDownloadingAll = false;
            document.getElementById('axpRepDownAll').innerHTML = '<i class="bi bi-cloud-download"></i> Download All PDFs';
            document.getElementById('axpRepDownAll').style.background = '';
            return;
        }
        isDownloadingAll = true;
        const origIdx = currentIndex;
        document.getElementById('axpRepDownAll').innerHTML = '<i class="bi bi-stop-circle"></i> Stop Download';
        document.getElementById('axpRepDownAll').style.background = '#ef4444';
        ...
        try {
            for (let i=0; i<eligible.length; i++) {
                ...
                if (!isDownloadingAll) break;
                if (reportDiv.innerHTML.includes('Student Not Found')||reportDiv.innerHTML.includes('Mwanafunzi Hajapatikana')) { skipped++; continue; }
            ...
            alert(isDownloadingAll?`Done! Downloaded: ${downloaded}, Skipped: ${skipped}`:`Stopped. Downloaded: ${downloaded}, Skipped: ${skipped}`);
        } catch(e) { console.error('[AXP] Batch report download error:',e); }
        finally {
            isDownloadingAll=false;
            document.getElementById('axpRepDownAll').innerHTML='<i class="bi bi-cloud-download"></i> Download All PDFs';
            document.getElementById('axpRepDownAll').style.background='';
...
function renderStudentCertificates(students) {
    ...
    certDiv.className = 'axp-section-card';
    ...
    function renderCert(index) {
        ...
        const sd           = window.currentSchoolData;
        const schoolName   = sd?.schoolName || _getVal('schoolNameInput');
        const schoolIndex  = sd?.indexNumber || _getVal('schoolIndexInput');
        const headmaster   = _getVal('headmasterInput');
        const classTeacher = _getVal('classTeacherInput');
        const closingDate  = _getVal('closingDateInput');
        const schoolLogo   = window.getSchoolLogo ? window.getSchoolLogo() : null;
        ...
        certDiv.innerHTML = `
            <div style="width:297mm;min-height:210mm;margin:0 auto;background:#fff;position:relative;font-family:'Georgia',serif;box-sizing:border-box;overflow:hidden;">
                ...
                <!-- Content area -->
                <div style="position:relative;z-index:2;padding:11mm 22mm 9mm 22mm;min-height:210mm;box-sizing:border-box;display:flex;flex-direction:column;align-items:center;justify-content:space-between;">
    ...
    const _pdfOpt = filename=>({ margin:0, filename, image:{type:'jpeg',quality:0.98}, html2canvas:{scale:3,useCORS:true,allowTaint:true}, jsPDF:{unit:'mm',format:'a4',orientation:'landscape'} });
