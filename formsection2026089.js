...
const scriptURL = 'https://script.google.com/macros/s/AKfycbxpdQmMcuOwYWU6xyRB6Ke3il1t21P5Vl0D4EvtVndleYpYOQQtqz1HxxRt1mTC6IFa/exec';
...
function _axpFeedUpdateProgress() {
  ...
  if(fill){fill.style.width=pct+"%";fill.style.background=pct===100?"#10b981":"#4ecca3";}
  _setText("axpFeedProgressPct",pct+"%");
  _setText("axpFeedProgressLabel",`${filled} of ${total} students`);
  _setText("axpFeedProgressSublabel",pct===100
...
window.axpBulkMode=function(mode){
  ...
  _setText("axpBulkModeHint",hints[mode]||"");
...
function _axpBulkRenderGrid(students){
  ...
  _setText("axpBulkTotal",students.length);
...
function _axpBulkUpdateStats(){
  ...
  _setText("axpBulkFilled",filled);_setText("axpBulkEmpty",total-filled);_setText("axpBulkPct",pct+"%");
}
...
window.axpBulkReadFile=function(input){
  const file=input.files[0]; if(!file)return;
  _setText("axpBulkFileName",file.name);
  const reader=new FileReader();
  reader.onload=function(e){
...
window.axpCorLoadStudents = async function() {
  ...
  try{
    ...
    _setText("axpCorStudentCount", `${students.length} students`);
...
window.axpPrintSubjectSheets = async function() {
  ...
  try {
    ...
    for (const subjectName of checked) {
      ...
      const ROW_H        = 8;
      const PAGE_USABLE  = _PDF.H - _PDF.MT - _PDF.MB - 30; /* 30 = header height */
      ...
      students.forEach((s, idx) => {
        /* Need a new page? */
        if (y + ROW_H > _PDF.H - _PDF.MB - 10) {
          _PDF.drawFooter(doc, pageNum, totalPages, examType, _PDF.H, _PDF.W, schoolName);
          doc.addPage();
          pageNum++;
          y = drawPageHeader();
          y = drawTableHeader(y);
...
window.axpPrintGeneralSheet = async function() {
  ...
  try {
    ...
    */
    ...
    const SIG_W   = 22;
    const REMAIN  = _PDF.UW - FIXED - SIG_W; /* 60 */
    ...
    const drawPageHeader = () => {
      /* Override _PDF.UW for landscape */
      const savedUW = _PDF.UW;
      Object.defineProperty(_PDF,"UW",{get:()=>UW,configurable:true});
      const y = _PDF.drawHeader(doc, {
        schoolName,
        schoolIndex: (_dashboardData&&_dashboardData.schoolindex)||"",
        examType, cls, year,
      ...
      Object.defineProperty(_PDF,"UW",{get:()=>savedUW,configurable:true});
      return y;
    };
    ...
    const drawTableHeader = (y) => {
      ...
      doc.setDrawColor(220,228,236);
      doc.setLineWidth(0.2);
      doc.rect(_PDF.ML, y, UW, HEADER_ROW_H, "S");
    ...
    allStudents.forEach((student, idx) => {
      /* New page check */
      if (y + ROW_H > pageH - _PDF.MB - 10) {
        _PDF.drawFooter(doc, pageNum, totalPages, examType, pageH, pageW, schoolName);
        doc.addPage();
        pageNum++;
        y = drawPageHeader();
        y = drawTableHeader(y);
      ...
      subjects.forEach(sub => {
        ...
        if (student.subjects.has(sub)) {
          _PDF.drawTick(doc, cx + subWFinal/2, y + ROW_H/2, 2.8);
        }
        cx += subWFinal;
      });
    ...
    if (!allStudents.length) {
      ...
      doc.text("No students found.", _PDF.ML+UW/2, y+8, {align:"center"});
    }
