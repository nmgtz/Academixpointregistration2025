...
function renderStudentReports(students, container) {
    ...
    async function _repCapturePdf(filename, action) {
        try {
            ...
            // ── Requirements ──
            if(reqs.length){
                const reqTxt=reqs.map((r,i)=>`${i+1}. ${r}`).join('  |  ');
                const reqLines=doc.splitTextToSize(reqTxt,CW-4);
                rect(ML,curY,CW,reqLines.length*4+4,[255,255,240],[200,180,0]);
                doc.setFontSize(7.5); doc.setFont('helvetica','bold'); setTxt([120,100,0]);
                doc.text((lang.requirements||'Requirements for Next Term')+':',ML+2,curY+3.5);
                doc.setFont('helvetica','normal'); setTxt(black);
                reqLines.forEach((ln,i)=>doc.text(ln,ML+2,curY+7+i*4));
                curY+=reqLines.length*4+6;
            }

            // ── Term dates ──
            curY+=1;
            setDraw([170,196,224]); doc.setLineWidth(0.2); doc.line(ML,curY,ML+CW,curY); curY+=3;
            ...
            // ── Class Teacher comment + signature block ──
            setDraw(navy); doc.setLineWidth(0.4); doc.line(ML,curY,ML+CW,curY); curY+=2;
            ...
            const ctRemark=lang.classCommentIntro?
                lang.classCommentIntro(student.name)+' '+(pt<=12?lang.performanceLevels?.strong:pt<=21?lang.performanceLevels?.moderate:lang.performanceLevels?.weak)+' '+(lang.academicPerformance||'academic performance')+' '+(lang.thisTerm||'this term.')+' '+(pt<=28?lang.encouragement?.encouraged:lang.encouragement?.necessary)
                :remark;
            ...
            // Signature line (left 55%) | Date (right 40%)
            const ctSigW=CW*0.52, ctDateX=ML+CW*0.6, ctDateW=CW*0.4;
            // Left — signature area
            setDraw(navy); doc.setLineWidth(0.5);
            doc.line(ML,curY,ML+ctSigW,curY);                        // signature line
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(classTeacher||'',ML,curY-1);                     // name sits on the line
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text(lang.classComment?.replace(/:.*/,'')||'Class Teacher',ML,curY+4);
            // Right — date area
            doc.setLineWidth(0.5);
            doc.line(ctDateX,curY,ctDateX+ctDateW,curY);             // date line
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text((lang.date||'Date')+':',ctDateX,curY+4);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(closingDate||'',ctDateX,curY-1);                // date sits on the line
            curY+=9;

            // ── Head of School comment + signature block ──
            setDraw(navy); doc.setLineWidth(0.4); doc.line(ML,curY,ML+CW,curY); curY+=2;
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text((lang.headComment||"Head of School's Comment")+':',ML,curY+3.5);
            curY+=6;
            const htRemark=(pt<=13?lang.remarksList.outstanding:pt<=17?lang.remarksList.commendable:pt<=24?lang.remarksList.satisfactory:lang.remarksList.followup)+' '+(lang.headSupportNote||'');
            const htLines=doc.splitTextToSize(htRemark,CW-4);
            doc.setFontSize(8.5); doc.setFont('helvetica','normal'); setTxt([40,40,40]);
            doc.text(htLines,ML,curY+3);
            curY+=htLines.length*4+4;
            // Signature line (left 55%) | Date (right 40%)
            const htSigW=CW*0.52, htDateX=ML+CW*0.6, htDateW=CW*0.4;
            setDraw(navy); doc.setLineWidth(0.5);
            doc.line(ML,curY,ML+htSigW,curY);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(headmaster||'',ML,curY-1);
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text(lang.headComment?.replace(/:.*/,'')||'Head of School',ML,curY+4);
            doc.setLineWidth(0.5);
            doc.line(htDateX,curY,htDateX+htDateW,curY);
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text((lang.date||'Date')+':',htDateX,curY+4);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(closingDate||'',htDateX,curY-1);
            curY+=9;
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        try {
            ...
            const _drawReportPage = async (doc, idx) => {
                ...
                if(reqs2.length){const rqt=reqs2.map((r,i)=>`${i+1}. ${r}`).join('  |  ');const rql=doc.splitTextToSize(rqt,CW-4);rc(ML,cy,CW,rql.length*4+4,[255,255,240],[200,180,0]);doc.setFontSize(7.5);doc.setFont('helvetica','bold');st([120,100,0]);doc.text((lang2.requirements||'Requirements')+':',ML+2,cy+3.5);doc.setFont('helvetica','normal');st(black);rql.forEach((ln,i)=>doc.text(ln,ML+2,cy+7+i*4));cy+=rql.length*4+6;}

                cy+=1;sd([170,196,224]);doc.setLineWidth(0.2);doc.line(ML,cy,ML+CW,cy);cy+=3;
                doc.setFontSize(8.5);doc.setFont('helvetica','normal');st([60,60,60]);doc.text(`${lang2.closes||'School Closes'}: ${closingDate2||'—'}`,ML,cy+3);doc.text(`${lang2.reopens||'School Opens'}: ${openingDate2||'—'}`,ML+CW/2,cy+3);cy+=6;

                // ── Class Teacher comment + signature ──
                sd(navy);doc.setLineWidth(0.4);doc.line(ML,cy,ML+CW,cy);cy+=2;
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);doc.text((lang2.classComment||"Class Teacher's Comment")+':',ML,cy+3.5);cy+=6;
                ...
                const ctl2=doc.splitTextToSize(ctr2,CW-4);doc.setFontSize(8.5);doc.setFont('helvetica','normal');st([40,40,40]);doc.text(ctl2,ML,cy+3);cy+=ctl2.length*4+4;
                {const sw2=CW*0.52,dx2=ML+CW*0.6,dw2=CW*0.4;
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy,ML+sw2,cy);
                doc.setFontSize(8.5);doc.setFont('helvetica','bold');st(navy);doc.text(classTeacher2||'',ML,cy-1);
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([100,100,100]);doc.text(lang2.classComment?.replace(/:.*/,'')||'Class Teacher',ML,cy+4);
                doc.setLineWidth(0.5);doc.line(dx2,cy,dx2+dw2,cy);
                doc.setFontSize(7.5);st([100,100,100]);doc.text((lang2.date||'Date')+':',dx2,cy+4);
                doc.setFontSize(8.5);doc.setFont('helvetica','bold');st(navy);doc.text(closingDate2||'',dx2,cy-1);}
                cy+=9;

                // ── Head of School comment + signature ──
                sd(navy);doc.setLineWidth(0.4);doc.line(ML,cy,ML+CW,cy);cy+=2;
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);doc.text((lang2.headComment||"Head of School's Comment")+':',ML,cy+3.5);cy+=6;
                ...
                const htl2=doc.splitTextToSize(htr2,CW-4);doc.setFontSize(8.5);doc.setFont('helvetica','normal');st([40,40,40]);doc.text(htl2,ML,cy+3);cy+=htl2.length*4+4;
                {const sw2=CW*0.52,dx2=ML+CW*0.6,dw2=CW*0.4;
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy,ML+sw2,cy);
                doc.setFontSize(8.5);doc.setFont('helvetica','bold');st(navy);doc.text(headmaster2||'',ML,cy-1);
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([100,100,100]);doc.text(lang2.headComment?.replace(/:.*/,'')||'Head of School',ML,cy+4);
                doc.setLineWidth(0.5);doc.line(dx2,cy,dx2+dw2,cy);
                doc.setFontSize(7.5);st([100,100,100]);doc.text((lang2.date||'Date')+':',dx2,cy+4);
                doc.setFontSize(8.5);doc.setFont('helvetica','bold');st(navy);doc.text(closingDate2||'',dx2,cy-1);}
                return true;
...
function renderStudentCertificates(students) {
    ...
    async function _certCapturePdf(filename, action) {
        try {
            ...
            const doc = new jsPDFCtor({unit:'mm',format:[297,210],orientation:'landscape',compress:false,precision:4,putOnlyUsedFonts:true});
            const PW=297, PH=210, cx=PW/2, cy=PH/2;

            const navy=[26,58,92], gold=[200,168,75], lgrey=[230,230,230], white=[255,255,255], black=[0,0,0];
            ...
            // ── Gold outer border ──
            setDraw(...gold); doc.setLineWidth(3);
            doc.rect(4,4,PW-8,PH-8,'D');
            setDraw(...gold); doc.setLineWidth(0.5);
            doc.rect(6,6,PW-12,PH-12,'D');

            // ── Navy header band ──
            setFill(...navy); doc.rect(0,0,PW,28,'F');

            // School name in header
            doc.setFontSize(14); doc.setFont('helvetica','bold'); setTxt(...white);
            doc.text(si.displayLine1.toUpperCase(),cx,11,{align:'center'});
            doc.setFontSize(8); doc.setFont('helvetica','normal');
            doc.text(si.displayLine2||'',cx,17,{align:'center'});
            doc.setFontSize(9); doc.setFont('helvetica','bold');
            doc.text((si.governmentHeader||'PRESIDENT\'S OFFICE').toUpperCase(),cx,23,{align:'center'});

            // ── Certificate title ──
            doc.setFontSize(22); doc.setFont('helvetica','bold'); setTxt(...gold);
            doc.text(lang.certificate||'CERTIFICATE OF COMPLETION',cx,45,{align:'center'});

            // Decorative line under title
            setDraw(...gold); doc.setLineWidth(1);
            doc.line(cx-50,49,cx+50,49);
            setDraw(...gold); doc.setLineWidth(0.3);
            doc.line(cx-60,51,cx+60,51);

            // ── "Presented to" ──
            doc.setFontSize(11); doc.setFont('helvetica','normal'); setTxt(...navy);
            doc.text(lang.presentedTo||'This certificate is proudly presented to',cx,62,{align:'center'});

            // ── Student name (big) ──
            doc.setFontSize(26); doc.setFont('times','bolditalic'); setTxt(...navy);
            doc.text(student.name,cx,76,{align:'center'});

            // Underline name
            const nw=doc.getTextWidth(student.name);
            setDraw(...gold); doc.setLineWidth(0.7);
            doc.line(cx-nw/2,79,cx+nw/2,79);

            // ── Recognition line ──
            doc.setFontSize(10); doc.setFont('helvetica','normal'); setTxt([60,60,60]);
            doc.text(lang.recognition||'For successfully completing the academic program',cx,90,{align:'center'});

            // ── Class info only (no exam type — certificates are occasion-based) ──
            doc.setFontSize(11); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(clsLabel,cx,100,{align:'center'});

            // ── "Conducted at" school ──
            doc.setFontSize(9); doc.setFont('helvetica','normal'); setTxt([60,60,60]);
            doc.text(`${lang.conducted||'At'}: ${si.displayLine1}`,cx,110,{align:'center'});

            // ── Points + Division badge ──
            setFill(245,245,220); setDraw(...gold); doc.setLineWidth(0.5);
            doc.roundedRect(cx-35,115,70,14,2,2,'FD');
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(`${currentLang==='sw'?'Pointi':'Points'}: ${student.point||'-'}   |   ${currentLang==='sw'?'Daraja':'Division'}: ${student.division||'-'}`,cx,124,{align:'center'});

            // ── Date ──
            doc.setFontSize(9); doc.setFont('helvetica','normal'); setTxt([80,80,80]);
            doc.text(`${lang.date||'Date'}: ${closingDate||'—'}`,cx,140,{align:'center'});

            // ── Signature row ──
            // Layout: [Class Teacher block] .... [Seal] .... [Head of School block]
            // Each block: signature line, printed name (auto-filled), role label, date line
            const sig1X=55, sig2X=PW-55, sigLineW=70, sigY=158;

            // ── Left: Class Teacher ──
            // Label
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text(lang.classTeacher||'Class Teacher',sig1X,sigY-2,{align:'center'});
            // Signature line
            setDraw(...navy); doc.setLineWidth(0.5);
            doc.line(sig1X-sigLineW/2,sigY+2,sig1X+sigLineW/2,sigY+2);
            // Printed name (auto-filled from class details) sits on the line
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(classTeacher||'',sig1X,sigY+1,{align:'center'});
            // Name printed label below line
            doc.setFontSize(7); doc.setFont('helvetica','normal'); setTxt([80,80,80]);
            doc.text(classTeacher?classTeacher.toUpperCase():'',sig1X,sigY+6,{align:'center'});
            // Date label + value
            doc.setFontSize(7.5); setTxt([100,100,100]);
            doc.text(lang.date||'Date',sig1X,sigY+11,{align:'center'});
            setDraw(...navy); doc.setLineWidth(0.4);
            doc.line(sig1X-28,sigY+12,sig1X+28,sigY+12);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(closingDate||'',sig1X,sigY+11,{align:'center'});

            // ── Centre: Seal ──
            setDraw(...gold); doc.setLineWidth(1.5);
            doc.circle(cx,sigY+5,14,'D');
            setDraw(...gold); doc.setLineWidth(0.4);
            doc.circle(cx,sigY+5,11,'D');
            doc.setFontSize(6.5); doc.setFont('helvetica','bold'); setTxt(...gold);
            doc.text(lang.seal||'OFFICIAL SEAL',cx,sigY+3,{align:'center'});
            const shortName=si.displayLine1.split(' ').slice(0,3).join(' ');
            const sealLines=doc.splitTextToSize(shortName,20);
            sealLines.forEach((ln,i)=>doc.text(ln,cx,sigY+7+i*3.5,{align:'center'}));

            // ── Right: Head of School ──
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); setTxt([100,100,100]);
            doc.text(lang.headTeacher||'Head of School',sig2X,sigY-2,{align:'center'});
            setDraw(...navy); doc.setLineWidth(0.5);
            doc.line(sig2X-sigLineW/2,sigY+2,sig2X+sigLineW/2,sigY+2);
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(headmaster||'',sig2X,sigY+1,{align:'center'});
            doc.setFontSize(7); doc.setFont('helvetica','normal'); setTxt([80,80,80]);
            doc.text(headmaster?headmaster.toUpperCase():'',sig2X,sigY+6,{align:'center'});
            doc.setFontSize(7.5); setTxt([100,100,100]);
            doc.text(lang.date||'Date',sig2X,sigY+11,{align:'center'});
            setDraw(...navy); doc.setLineWidth(0.4);
            doc.line(sig2X-28,sigY+12,sig2X+28,sigY+12);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(...navy);
            doc.text(closingDate||'',sig2X,sigY+11,{align:'center'});

            // ── Navy footer band ──
            setFill(...navy); doc.rect(0,PH-18,PW,18,'F');
            doc.setFontSize(8); doc.setFont('helvetica','normal'); setTxt(...gold);
            doc.text(si.displayLine1+' | '+si.displayLine2,cx,PH-8,{align:'center'});
