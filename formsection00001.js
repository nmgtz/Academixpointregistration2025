...
function updateDashboardUI(data) {
    ...
    // === NOTIFICATION SECTION (24-hour welcome message) ===
    if (alertedNotification) {
        if (data.notification && data.notification.trim() !== "") {
            // Show 24-hour welcome notification
    ...
    // === ANNOUNCEMENTS SECTION (System-wide announcements) ===
    ...
    if (alertedLink) {
        alertedLink.innerHTML = `
            <div style="background: #e6fffa; border-left: 4px solid #38b2ac; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
                <strong>🔗 Share with your team:</strong><br>
                <a href="${schoolLinkH}${data.schoolindex.toLowerCase()}-teachers-feeding-area.html" 
                   style="color: #2c7a7b; text-decoration: underline; font-weight: 600;">
                    ${data.schoolindex}-TEACHER'S FEEDING AREA
                </a>
            </div>
        `;
    }
    ...
    if (notificationList) {
        ...
        // Add system announcements to notifications
        if (data.announcements && data.announcements.length > 0) {
    ...
    if (messageList) {
        ...
        if (messages.length > 0 && messages[0] !== "No message available") {
            messages.forEach(msg => {
                const li = document.createElement('li');
                li.textContent = `${data.username}: ${msg}`;
                li.style.cssText = "padding: 10px; border-bottom: 1px solid #e2e8f0;";
