...
window._axpShowModal = function({ icon, iconColor, title, body, buttons }) {
...
/* ═══════════════════════════════════════════════════════════
   HELPERS
═══════════════════════════════════════════════════════════ */
window._axpGrade = function(score) {
...
window._axpCandNo = function(idx) {
...
═══════════════════════════════════════════════════════════ */
window._axpBuildSelector = function({ containerId, stepNo, label, items, onSelect,
                              iconFn, locked = false, selectedValue = null }) {
  ...
  wrap.innerHTML = `
    <div class="axp-step-head">
      ...
      ${selectedValue
        ? `<div class="axp-step-value">
             <i class="bi bi-check2" style="color:#4ecca3;margin-right:4px;"></i>
             ${escapeHtml(selectedValue)}</div>`
        : `<div style="font-size:11px;color:#94a3b8;margin-left:auto;">
             tap to select</div>`}
    </div>
    <div class="axp-chip-row" id="${containerId}_chips">
      ${items.length === 0
        ? `<span style="font-size:12px;color:#94a3b8;padding:4px 0;">
             No options available</span>`
        : items.map(item => {
            const val  = typeof item === "string" ? item : item.value;
            const disp = typeof item === "string" ? item : item.label;
            const icon = iconFn ? iconFn(val) : "";
            const active = val === selectedValue;
            return `
              <button class="axp-chip${active?" axp-chip-active":""}"
                data-val="${escapeHtml(val)}"
                onclick="_axpChipClick(this,'${containerId}','${escapeHtml(val).replace(/'/g,"\\'")}',${stepNo})">
...
window._axpChipClick = function(btn, containerId, val, stepNo) {
  const wrap = document.getElementById(containerId);
  if (!wrap) return;
  /* Deactivate siblings */
  wrap.querySelectorAll(".axp-chip").forEach(c => {
    c.classList.remove("axp-chip-active");
    c.innerHTML = c.innerHTML.replace(/<i class="bi bi-check2[^"]*"[^>]*><\/i>/g,"").trim();
  });
  /* Activate clicked */
  btn.classList.add("axp-chip-active");
  btn.innerHTML += ` <i class="bi bi-check2" style="margin-left:2px;"></i>`;

  /* Update step header */
  const head = wrap.querySelector(".axp-step-head");
  if (head) {
    const old = head.querySelector(".axp-step-value, div[style*='tap to select']");
  ...
  if (wrap._onSelect) wrap._onSelect(val);
};
...
═══════════════════════════════════════════════════════════ */
window.renderTeacherFeedingSection = function() {
  const sw = document.getElementById("axpSectionWrapper");
...
window.axpFeedTab = function(tab) {
  ["entry","bulk","correction"].forEach(t => {
...
═══════════════════════════════════════════════════════════ */
window._axpGetSubjects = function(cls) {
  return (_schoolMeta&&_schoolMeta.subjects&&cls&&_schoolMeta.subjects[cls])
    ? _schoolMeta.subjects[cls] : [];
}
window._axpGetClasses = function()   { return (_schoolMeta&&_schoolMeta.classes)   ? _schoolMeta.classes   : []; }
window._axpGetExamTypes = function() { return (_schoolMeta&&_schoolMeta.examTypes) ? _schoolMeta.examTypes : []; }

/* ─── ENTRY wizard ─── */
window._axpBootEntryWizard = function() {
...
/* ─── BULK wizard ─── */
window._axpBootBulkWizard = function() {
  const state = { examType:"", cls:"", subject:"" };
...
/* ─── CORRECTION wizard ─── */
window._axpBootCorWizard = function() {
  const state = { examType:"", cls:"", subject:"" };
...
/* ═══════════════════════════════════════════════════════════
   TEACHER BANNER
═══════════════════════════════════════════════════════════ */
window._axpFeedRenderTeacherBanner = function(bannerId, teacher, cls, subject) {
...
window._axpFeedRenderCard = function() {
...
window._axpFeedUpdateProgress = function() {
...
window._axpFeedSaveCurrentInput = function() {
...
window._axpFeedShowSummary = function(submitted=false) {
  const {students,marks,cls,examType,subject}=window._axpFeed;
...
window._axpBulkRenderGrid = function(students){
...
window._axpBulkUpdateStats = function(){
...
window._axpCorRenderList = function(students) {
...
window._axpShowPrintResult = function(containerId, info, error) {
...
window._axpBootPrintWizard = function() {
...
window._axpPDF = window._axpPDF || {}; var _PDF = {
  /* A4 dimensions in mm */
  W : 210,
  H : 297,
  ML: 12,   /* margin left */
  MR: 12,   /* margin right */
  MT: 14,   /* margin top */
  MB: 18,   /* margin bottom — enough for 7mm footer band */

  /* Usable width */
  get UW() { return this.W - this.ML - this.MR; },

  /* ── Page header — clean white, black borders only ── */
  drawHeader(doc, { schoolName, examType, cls, year, title, subTitle }) {
    ...
    doc.rect(x, y, this.UW, 22, "S");

    /* Divider between school name row and title row */
    doc.setLineWidth(0.3);
    doc.line(x, y+12, x+this.UW, y+12);

    /* School name */
    doc.setFont("helvetica","bold");
    doc.setFontSize(11);
    doc.setTextColor(0,0,0);
    doc.text((schoolName||"School").toUpperCase(), x+4, y+8);
    ...
    doc.text(meta, this.ML + this.UW - 4, y+8, {align:"right"});

    /* Title */
    doc.setFont("helvetica","bold");
    doc.setFontSize(8.5);
    doc.setTextColor(0,0,0);
    doc.text(title.toUpperCase(), x+4, y+18.5);

    if (subTitle) {
      doc.setFont("helvetica","normal");
      doc.setFontSize(7);
      doc.setTextColor(80,80,80);
      doc.text(subTitle, this.ML+this.UW-4, y+18.5, {align:"right"});
    }

    return y + 22 + 3; /* return Y after header */
  },

  /* ── Page footer — plain text, no background ── */
  ...
  /* ── Table row — white background, black borders only ── */
  drawRow(doc, cols, y, rowH, isHeader, isAlt) {
    /* All rows white — no colour fills */
    doc.setFillColor(255,255,255);
    doc.rect(this.ML, y, this.UW, rowH, "F");

    /* Border */
    doc.setDrawColor(0,0,0);
    doc.setLineWidth(isHeader ? 0.4 : 0.2);
  ...
  try {
    ...
    */
    ...
    allStudents.forEach((student, idx) => {
      ...
      /* Fixed cells — all black text */
      const fixedCols = [
        {text: idx+1,                              width:8,  align:"center"},
        {text: student.candidateNo,                width:28, align:"left"},
        {text: student.name,                       width:58, align:"left"},
        {text: student.gender==="F"?"F":"M",       width:10, align:"center"},
      ];
