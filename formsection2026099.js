...
// ─────────────────────────────────────────────────────────────────────────────
// FIXED _repCapturePdf — Reliable data reading + correct exam label
// Drop-in replacement for the existing _repCapturePdf function inside
// renderStudentReports(). Replace the entire async function _repCapturePdf block.
// ─────────────────────────────────────────────────────────────────────────────
...
async function _repCapturePdf(filename, action) {
    try {
        ...
        // Uses the FULL label function (Weekly Test / Monthly Test / etc.)
        // NOT the generic "Examination" fallback
        const examLabel = _axpExamLabel(examType, currentLang);
        const clsLabel  = _axpClassLabel(examClass, currentLang);
        const year      = _schoolMeta?.year || new Date().getFullYear();

        // ── Class Info — read from EVERY possible source in priority order ─────
        // Priority: live panel DOM > SchoolInfoStorage > legacy DOM inputs > fallback
        const _saved = SchoolInfoStorage.getClassData(examClass) || {};

        const _gv = (key, fallback) => {
            // 1. Live panel (user may be editing right now)
            const panel = document.getElementById('axpClassInfoPanel');
            if (panel) {
                const el = panel.querySelector(`[data-field="${key}"]`);
                if (el && el.value && el.value.trim()) return el.value.trim();
            }
            // 2. SchoolInfoStorage (persisted across sessions)
            if (_saved[key] && String(_saved[key]).trim()) return String(_saved[key]).trim();
            // 3. Legacy DOM inputs (old wizard)
            const dom = document.getElementById(key);
            if (dom && dom.value && dom.value.trim()) return dom.value.trim();
        ...
        const classTeacher = _gv('classTeacherInput',
                                 currentLang === 'sw' ? 'Mwalimu wa Darasa' : 'Class Teacher');
        const headmaster   = _gv('headmasterInput',
                                 currentLang === 'sw' ? 'Mkuu wa Shule' : 'Head of School');

        // Requirements — base 3 + extras
        const reqBase  = [_gv('requirement1'), _gv('requirement2'), _gv('requirement3')]
                           .filter(Boolean);
        ...
        // ── Behaviour grades (derived from points) ────────────────────────────
        ...
        const ctComment = lang.classCommentIntro
            ? lang.classCommentIntro(student.name) + ' '
              + (pt <= 12 ? lang.performanceLevels?.strong
                          : pt <= 21 ? lang.performanceLevels?.moderate
                                     : lang.performanceLevels?.weak)
              + ' ' + (lang.academicPerformance || '')
              + ' ' + (lang.thisTerm || '')
              + ' ' + (pt <= 28 ? lang.encouragement?.encouraged
                                 : lang.encouragement?.necessary)
            : remark;

        const hosComment = (pt <= 13 ? lang.remarksList.outstanding
                          : pt <= 17 ? lang.remarksList.commendable
                          : pt <= 24 ? lang.remarksList.satisfactory
        ...
        // ── Zone heights (bottom-pinned fixed blocks) ─────────────────────────
        const BOT_MARGIN = 9;
        const SIG_H      = 22;   // each signature block height
        const DATES_H    = 7;
        const REQS_H     = reqs.length ? Math.min(reqs.length * 4 + 5, 18) : 0;
        ...
        [[4,4,16,4,4,16],[PW-16,4,PW-4,4,PW-4,16],
         [4,PH-16,4,PH-4,16,PH-4],[PW-4,PH-16,PW-4,PH-4,PW-16,PH-4]]
            .forEach(([x1,y1,x2,y2,x3,y3]) => {
                doc.line(x1,y1,x2,y2); doc.line(x2,y2,x3,y3);
            });
        ...
        // ═════════════════════════════════════════════════════════════════════
        // HEADER  — reads header lines from live panel / storage
        // ═════════════════════════════════════════════════════════════════════
        let cy = 10;
        const HEADER_H = 34;

        // Build 5 header lines from live data (same as _buildSchoolHeader)
        ...
        const dist2 = _dd2.district
            ? _dd2.district.toUpperCase() + ' DISTRICT COUNCIL'
            : (hLines[2] || 'DISTRICT COUNCIL');

        // Override lines 0-4 from live server data every time
        ...
        hLines[3] = (si.displayLine2 ? si.displayLine2.toUpperCase() + ' — ' : '')
                  + si.displayName.toUpperCase();
        // Line 5: class + FULL exam label (not "Examination")
        hLines[4] = `${clsLabel.toUpperCase()} ${examLabel.toUpperCase()} — `
                  + (currentLang === 'sw' ? 'RIPOTI YA MAENDELEO' : 'PROGRESS REPORT')
                  + ` — ${year}`;
        ...
        const infoCols = [
            { lbl:(lang.studentName||'Name')+':',  val:student.name,        w:CW*0.36 },
            { lbl:(lang.class||'Class')+':',         val:clsLabel,            w:CW*0.20 },
            { lbl:(lang.sex||'Sex')+':',             val:student.gender||'—', w:CW*0.11 },
        ...
        const ptsLbl = lang.points   || (currentLang === 'sw' ? 'Pointi'  : 'Points');
        const divLbl = lang.division || (currentLang === 'sw' ? 'Daraja'  : 'Division');
        const posLbl = lang.position || (currentLang === 'sw' ? 'Nafasi'  : 'Position');
        ...
        // Show BOTH opening and closing dates + month if available
        const closesLbl  = (currentLang === 'sw' ? 'Imefungwa'   : 'Closed')  + ': ';
        const reopensLbl = (currentLang === 'sw' ? 'Itafunguliwa' : 'Reopens') + ': ';
        const monthLbl   = (currentLang === 'sw' ? 'Mwezi'        : 'Period')  + ': ';

        doc.setFontSize(8); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(closesLbl, ML+4, DATES_TOP + DATES_H/2 + 8*0.352/2);
        doc.setFont('helvetica','normal'); st(BLACK);
        const closesVal = T(closingDate);
        doc.text(closesVal, ML+4+doc.getTextWidth(closesLbl), DATES_TOP + DATES_H/2 + 8*0.352/2);

        doc.setFont('helvetica','bold'); st(NAVY);
        const reopensX = ML + CW * 0.38;
        doc.text(reopensLbl, reopensX, DATES_TOP + DATES_H/2 + 8*0.352/2);
        doc.setFont('helvetica','normal'); st(BLACK);
        doc.text(T(openingDate||'—'),
            reopensX + doc.getTextWidth(reopensLbl),
            DATES_TOP + DATES_H/2 + 8*0.352/2);

        // Month/period if provided
        if (month) {
            doc.setFont('helvetica','bold'); st(NAVY);
            const monthX = ML + CW * 0.72;
            doc.text(monthLbl, monthX, DATES_TOP + DATES_H/2 + 8*0.352/2);
            doc.setFont('helvetica','normal'); st(BLACK);
            doc.text(T(month),
                monthX + doc.getTextWidth(monthLbl),
                DATES_TOP + DATES_H/2 + 8*0.352/2);
        }
        ...
        if (reqs.length) {
            ...
            sf([255,255,245]); sd([200,210,180]); doc.setLineWidth(0.2);
            doc.rect(ML, REQS_TOP, CW, REQS_H, 'FD');

            const reqLabel = currentLang === 'sw'
                ? 'Mahitaji ya Muhula Ujao:'
                : 'Requirements for Next Term:';
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(reqLabel, ML+3, REQS_TOP + 4.5);

            // Distribute items across available width
            const itemW = (CW - 6) / reqs.length;
            reqs.forEach((r, i) => {
                doc.setFontSize(7); doc.setFont('helvetica','normal'); st([40,40,40]);
                const short = r.length > 32 ? r.slice(0,30) + '…' : r;
                doc.text('• ' + short, ML + 3 + i * itemW, REQS_TOP + 9);
            });
        }

        // ═════════════════════════════════════════════════════════════════════
        // CLASS TEACHER SIGNATURE BLOCK  (pinned)
        // ═════════════════════════════════════════════════════════════════════
        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, CT_TOP, ML+CW, CT_TOP);
        sf([248,250,255]); doc.rect(ML, CT_TOP, CW, SIG_H, 'F');

        const ctRole = currentLang === 'sw' ? 'MWALIMU WA DARASA' : 'CLASS TEACHER';
        doc.setFontSize(8); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(ctRole + ':', ML+2, CT_TOP + 5.5);

        const ctMaxW   = CW - doc.getTextWidth(ctRole + ': ') - 4;
        const ctShort  = doc.splitTextToSize(ctComment, ctMaxW)[0];
        doc.setFont('helvetica','normal'); st([40,40,40]);
        doc.text(ctShort, ML + 2 + doc.getTextWidth(ctRole + ': '), CT_TOP + 5.5);

        sd([200,215,230]); doc.setLineWidth(0.2);
        doc.line(ML+2, CT_TOP+9, ML+CW-2, CT_TOP+9);

        // 3-column: Name | Signature | Date
        const SY   = CT_TOP + 10;
        const NW   = CW * 0.36;
        const SW   = CW * 0.38;
        const sigLineW = 38;
        ...
        // Name column
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Jina:' : 'Name:', ML+2, SY+3);
        // Show the actual class teacher name clearly
        doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(T(classTeacher), ML+2, SY+9);
        // Underline
        doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([100,100,100]);
        doc.text(T(classTeacher), ML+2, SY+13);
        doc.setFontSize(7); st([150,150,150]);
        doc.text(ctRole, ML+2, SY+17);

        // Signature column (blank line)
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Sahihi:' : 'Signature:', ML+NW+2, SY+3);
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(ML+NW+2, SY+10, ML+NW+SW-4, SY+10);

        // Date column
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Tarehe:' : 'Date:', DX2+2, SY+3);
        doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(T(closingDate), DX2+2, SY+9);
        doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([100,100,100]);
        doc.text(T(closingDate), DX2+2, SY+13);

        // ═════════════════════════════════════════════════════════════════════
        // HEAD OF SCHOOL SIGNATURE BLOCK  (pinned)
        // ═════════════════════════════════════════════════════════════════════
        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, HOS_TOP, ML+CW, HOS_TOP);
        sf([248,252,248]); doc.rect(ML, HOS_TOP, CW, SIG_H, 'F');

        const hosRole = currentLang === 'sw' ? 'MKUU WA SHULE' : 'HEAD OF SCHOOL';
        doc.setFontSize(8); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(hosRole + ':', ML+2, HOS_TOP + 5.5);

        const hosMaxW  = CW - doc.getTextWidth(hosRole + ': ') - 4;
        const hosShort = doc.splitTextToSize(hosComment, hosMaxW)[0];
        doc.setFont('helvetica','normal'); st([40,40,40]);
        doc.text(hosShort, ML + 2 + doc.getTextWidth(hosRole + ': '), HOS_TOP + 5.5);

        sd([200,215,230]); doc.setLineWidth(0.2);
        doc.line(ML+2, HOS_TOP+9, ML+CW-2, HOS_TOP+9);

        const HOS_SY = HOS_TOP + 10;

        // Name column
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Jina:' : 'Name:', ML+2, HOS_SY+3);
        doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(T(headmaster), ML+2, HOS_SY+9);
        doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([100,100,100]);
        doc.text(T(headmaster), ML+2, HOS_SY+13);
        doc.setFontSize(7); st([150,150,150]);
        doc.text(hosRole, ML+2, HOS_SY+17);

        // Signature
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Sahihi:' : 'Signature:', ML+NW+2, HOS_SY+3);
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(ML+NW+2, HOS_SY+10, ML+NW+SW-4, HOS_SY+10);

        // Date
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st(DGREY);
        doc.text(currentLang === 'sw' ? 'Tarehe:' : 'Date:', DX2+2, HOS_SY+3);
        doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(T(closingDate), DX2+2, HOS_SY+9);
        doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([100,100,100]);
        doc.text(T(closingDate), DX2+2, HOS_SY+13);
