...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isLoggedIn) {
    ...
    if (savedUsername && savedPassword) {
      /* Hide page immediately on auto-login so homepage never shows */
...
function applyLandingStyles() {
  ...
  style.textContent = `
    ...
    /* Inner wrapper — pushed 23% from left, vertically centered */
    ...
    /* Mobile: full width, scrollable, centered */
...
function _startCanvasFallback() {
  ...
  /* Shared direction: all particles drift upper-right together */
  const DIR_X =  0.38;
  const DIR_Y = -0.42;

  function makeParticle(type) {
    const speed = type === "icon"
    ...
    return {
      ...
      size:  type === "icon"  ? Math.random() * 10 + 8   /* 8–18px, smaller */
           : type === "word"  ? Math.random() * 7  + 6   /* 6–13px word size */
           :                    Math.random() * 1.4 + 0.5,/* dot radius */
      dx:    DIR_X * speed * (0.8 + Math.random() * 0.4),
      dy:    DIR_Y * speed * (0.8 + Math.random() * 0.4),
      a:     type === "dot"
               ? Math.random() * 0.35 + 0.1
               : Math.random() * 0.18 + 0.06,
      pulse: Math.random() * Math.PI * 2,
      pSpd:  Math.random() * 0.018 + 0.006,
    };
  }

  /* 35 icons + 18 brand words + 80 dots */
  const ICON_N = 35, WORD_N = 18, DOT_N = 80;
  const particles = [
    ...Array.from({ length: ICON_N }, () => makeParticle("icon")),
    ...Array.from({ length: WORD_N }, () => makeParticle("word")),
    ...Array.from({ length: DOT_N  }, () => makeParticle("dot")),
  ];
  ...
  (function draw() {
    ...
    particles.forEach(p => {
      ...
      if (p.type === "dot") {
        ...
        ctx.save();
        ctx.font = `${p.size}px "bootstrap-icons"`;
        ...
        /* "ACADEMIXPOINT" broken into two tiers */
        const alpha = p.a + Math.sin(p.pulse) * 0.025;
        ctx.save();
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.translate(p.x, p.y);
        /* "ACADEMIX" in white, "POINT" in teal — inline */
        const fs = p.size;
        ctx.font = `700 ${fs}px "Arial Narrow", Arial, sans-serif`;
        const w1 = ctx.measureText("ACADEMIX").width;
        const w2 = ctx.measureText("POINT").width;
        const total = w1 + w2;
        ctx.fillStyle = `rgba(255,255,255,${alpha})`;
        ctx.fillText("ACADEMIX", -total/2 + w1/2, 0);
        ctx.fillStyle = `rgba(78,204,163,${alpha})`;
        ctx.fillText("POINT", -total/2 + w1 + w2/2, 0);
        ctx.restore();
      }
    });
  ...
  .then(data => {
    ...
    if (isSuccess) {
      if (action === "login") {
        ...
        /* Hide the form content immediately the moment success is confirmed —
           before the processing result message even appears */
        const loginFormCard = document.querySelector("#axp-auth-overlay .form-container");
        if (loginFormCard) {
          loginFormCard.style.transition = "opacity 0.25s ease";
          loginFormCard.style.opacity    = "0";
          loginFormCard.style.pointerEvents = "none";
        }
        ...
        _showProcessingResult(true, "Login successful!", () => {
          /* Step 1: hide page elements silently in background */
          ["#header","#main-id",".footer",".footer-bottom",
           "#home","#landing","#page-wrapper",".page-content",
           ".home-section",".hero","#hero","main","#content"].forEach(sel => {
            const el = document.querySelector(sel);
          ...
          /* Step 2: fade out the auth overlay smoothly */
          const authOv = document.getElementById("axp-auth-overlay");
          if (authOv) {
            authOv.style.transition = "opacity 0.5s ease";
            authOv.style.opacity    = "0";
          }

          /* Step 3: kick off dashboard data fetch immediately in background */
          const dashPromise = new Promise(resolve => {
            if (data.dashboardData) {
              _renderDashboard(data.dashboardData);
              resolve();
            } else {
              loadDashboardData(username, password).then(resolve).catch(resolve);
            }
          });

          /* Step 4: show loading screen after fade */
          setTimeout(() => {
            if (authOv) authOv.style.display = "none";
            _showLoadingScreen(() => {
              dashPromise.then(() => displayDashboard());
            });
          }, 350);
        });
        ...
        _showProcessingResult(true, extra, () => {
          ...
          showSuccessOverlay(extra, () => {
            /* Re-open overlay cleanly to login form — prevents blank/stale form showing */
            _openAuthForm("lg-form");
          });
        });
      }
...
function _showLoadingScreen(onDone) {
  ...
  /* Start dashboard prep in background immediately */
  if (typeof onDone === "function") {
    /* Small delay so screen renders first, then prep runs concurrently */
    setTimeout(onDone, 80);
    onDone = null; /* prevent double-call below */
  }
  ...
  /* Smooth 1→100 counter over ~3s */
  const TOTAL_MS = 3000;
  const INTERVAL = 28; /* ~35fps */
  let pct = 1;
  let lastMsg = "";

  /* Set progress bar transition to smooth per-tick */
  fill.style.transition = "width 0.03s linear";
  fill.style.width = "1%";
  pctEl.textContent = "1%";
