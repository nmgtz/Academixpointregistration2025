    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        const opt = {
            ...
            html2canvas: {
                ...
                windowWidth: 1122,   // A4 landscape ~297mm at 96dpi
                scrollX: 0,
                scrollY: 0
            },
        ...
        // Constrain content width so nothing bleeds off the right edge
...
function buildDivisionTable(students) {
    ...
    // Full-width wrapper like all other tables
    const outer = document.createElement('div');
    outer.style.cssText = 'width:100%;margin-bottom:10px;';
...
function renderStudentReports(students, container) {
            ...
            reportDiv.innerHTML = `
                <div style="width:210mm;height:297mm;margin:0 auto;background:#fff;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;overflow:hidden;page-break-after:always;page-break-inside:avoid;">
                    ...
                    <div style="position:absolute;inset:0;z-index:2;display:flex;flex-direction:column;padding:12mm 12mm 10mm;box-sizing:border-box;gap:0;">
                        ...
                        <!-- ── COMMENTS SECTION (fixed, ~12% of page height) ── -->
                        <div style="flex-shrink:0;border-top:1px solid #aac4e0;margin-top:2.5mm;padding-top:2mm;font-size:7.5px;color:#222;">
                            ...
                            <div style="font-size:7.5px;margin-bottom:1.5mm;">
                        ...
                        <div style="flex-shrink:0;display:flex;align-items:flex-end;gap:0;border-top:1.5px solid #1a3a5c;margin-top:2mm;padding-top:2.5mm;">
                            <div style="flex:1;text-align:center;font-size:7.5px;border-right:0.5px solid #ddd;padding:0 3mm;">
                                <div style="border-bottom:0.75px solid #333;margin-bottom:2px;height:8mm;"></div>
                                <div style="font-weight:bold;font-size:8px;">${escapeHtml(classTeacher||'_____________')}</div>
                                <div style="color:#555;">${lang.classComment.replace(/:.*/,'')}</div>
                                <div style="color:#777;">${lang.date}: ${escapeHtml(closingDate)}</div>
                            </div>
                            <div style="flex:0.7;text-align:center;font-size:7.5px;border-right:0.5px solid #ddd;padding:0 3mm;">
                                <div style="width:32px;height:32px;border:1px solid #aaa;border-radius:50%;margin:0 auto 2px;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:6px;">STAMP</div>
                            </div>
                            <div style="flex:1;text-align:center;font-size:7.5px;padding:0 3mm;">
                                <div style="border-bottom:0.75px solid #333;margin-bottom:2px;height:8mm;"></div>
                                <div style="font-weight:bold;font-size:8px;">${escapeHtml(headmaster||'_____________')}</div>
                                <div style="color:#555;">${lang.headComment.replace(/:.*/,'')}</div>
                                <div style="color:#777;">${lang.date}: ${escapeHtml(closingDate)}</div>
                            </div>
                        </div>
                ...
                <div style="width:210mm;height:297mm;margin:0 auto;background:#fff;box-sizing:border-box;font-family:Arial,sans-serif;color:#000;position:relative;overflow:hidden;page-break-after:always;page-break-inside:avoid;">
                    ...
                    <div style="position:absolute;inset:0;display:flex;flex-direction:column;padding:8mm 10mm 7mm;box-sizing:border-box;gap:0;">
                        ...
                        <div style="flex-shrink:0;border-top:1.5px solid #000;margin-top:3mm;padding-top:2.5mm;">
                            <div style="font-size:7.5px;margin-bottom:2mm;">
                            ...
                            <div style="display:flex;">
                                <div style="flex:1;text-align:center;font-size:7.5px;border-right:0.5px solid #ccc;padding:0 4mm;">
                                    <div style="border-bottom:0.5px solid #000;margin-bottom:2px;height:8mm;"></div>
                                    <div style="font-weight:bold;font-size:8px;">${escapeHtml(classTeacher||'_____________')}</div>
                                    <div style="color:#555;">Class Teacher</div>
                                    <div style="color:#777;font-size:7px;">Date: ${escapeHtml(closingDate)}</div>
                                </div>
                                <div style="flex:0.7;text-align:center;border-right:0.5px solid #ccc;padding:0 4mm;">
                                    <div style="width:30px;height:30px;border:1px solid #aaa;border-radius:50%;margin:0 auto 2px;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:6px;">STAMP</div>
                                </div>
                                <div style="flex:1;text-align:center;font-size:7.5px;padding:0 4mm;">
                                    <div style="border-bottom:0.5px solid #000;margin-bottom:2px;height:8mm;"></div>
                                    <div style="font-weight:bold;font-size:8px;">${escapeHtml(headmaster||'_____________')}</div>
                                    <div style="color:#555;">Head Teacher</div>
                                    <div style="color:#777;font-size:7px;">Date: ${escapeHtml(closingDate)}</div>
                                </div>
                            </div>
                        </div>
    ...
    const _pdfOpt = (filename) => ({
        ...
        html2canvas: {
            ...
            logging: false,
            imageTimeout: 0,
            windowWidth: 794,   // 210mm at 96dpi — prevents right-side overflow
            scrollX: 0,
            scrollY: 0
        },
        jsPDF: { unit:'mm', format:[210,297], orientation:'portrait' },
        pagebreak: { mode: 'avoid-all' }   // card is exactly one page — no breaks needed
    });
...
function renderStudentCertificates(students) {
    ...
    const _pdfOpt = filename=>({
        ...
        html2canvas: {
            ...
            logging: false,
            imageTimeout: 0,
            windowWidth: 1122,   // 297mm at 96dpi — exact landscape A4
            scrollX: 0,
            scrollY: 0
        },
...
function _axpExamLabel(examType, lang) {
    ...
    const MAP = [
        [/^(mid[\s-]?term|midterm|mid)$/,         'Midterm Examination',     'Mtihani wa Nusu Muhula'],
        [/^(terminal|end[\s-]?term|end[\s-]?of[\s-]?term|final)$/,
                                                    'Terminal Examination',    'Mtihani wa Mwisho wa Muhula'],
        ...
        [/^(mock|pre[\s-]?exam|pre[\s-]?necta)$/, 'Mock Examination',        'Mtihani wa Mazoezi'],
        [/^(annual|yearly|year[\s-]?end)$/,        'Annual Examination',      'Mtihani wa Mwaka'],
