...
function _renderDashboard(data) {
  ...
  setTimeout(() => {
    ...
    if (!localStorage.getItem(_getSchoolStorageKey()) && _dashboardData.status === "ACTIVE" && !_dashboardData.schoolId) {
...
function _loadSchoolSetupStatus() {
  ...
  if (storedId) {
    ...
    _updateStatCards();
    _updateSetupNotice();
  } else if (_dashboardData && _dashboardData.schoolId) {
    _appScriptSchoolId = _dashboardData.schoolId;
    _axpFetchAndRestoreMeta(_dashboardData.schoolId);
  } else {
    _updateSetupNotice();
  }
} 

async function _axpFetchAndRestoreMeta(schoolId) {
  try {
    const res = await _apiGet({ mode: "schoolMeta", schoolId });
    if (res.status === "success" && res.meta) {
      _saveSchoolSetup(schoolId, res.meta);
    } else {
      _updateSetupNotice();
    }
  } catch(e) {
...
window._axpPopSubjects = {};
window._axpPopActiveTab = null;
const _presetSubjectsList_popup = [
"B/MATH",
"MATH",
"ENGL",
"KISW",
...
function navigateToSection(sectionName) {
  ...
  switch (sectionName) {
    ...
    case "teacher-feeding":   renderTeacherFeedingSection();   break;
...
function _axpRRGeneratePDF(action, cls, examType) {
        ...
        try {
            ...
            const cell = (x,y,w,h,text,opts={}) => {
                ...
                if (opts.wrap && text) {
                    ...
                    const lineH = (opts.fontSize||8)*0.45;
            ...
            /* ── Fixed columns — wider name column ── */
            const fixedCols = showPosition
                ? [{l:"CAND'S NO",w:22},{l:"CAND'S NAME",w:50},{l:'SEX',w:9},{l:'AGG',w:11},{l:'DIV',w:11},{l:'POS',w:11}]
                : [{l:"CAND'S NO",w:22},{l:"CAND'S NAME",w:50},{l:'SEX',w:9},{l:'AGG',w:11},{l:'DIV',w:11}];

            const fixedW = fixedCols.reduce((a,c)=>a+c.w,0);
            const subjColW = CW - fixedW;

            /* ── Row and header heights ── */
            const SRH = 9;   /* student row min height */
            ...
            hLines.forEach((ln,i)=>{
                const f=hFonts[i]||{size:10,bold:false};
            ...
            ['DIVISION',...divKeys].forEach((h,i)=>{
                cell(DX+i*DCW, curY, DCW, RH, h==='O'?'0':h, {bold:true, fontSize:9});
            });
            curY+=RH;
            ['F','M'].forEach(g=>{
                cell(DX, curY, DCW, RH, g, {bold:true, fontSize:9});
                divKeys.forEach((d,i)=>cell(DX+(i+1)*DCW, curY, DCW, RH, divCount[d][g]||0, {fontSize:9}));
                curY+=RH;
            });
            cell(DX, curY, DCW, RH, 'T', {bold:true, fontSize:9});
            divKeys.forEach((d,i)=>cell(DX+(i+1)*DCW, curY, DCW, RH, (divCount[d].F||0)+(divCount[d].M||0), {bold:true, fontSize:9}));
            curY+=RH;

            if (schoolGPA) {
                const gc = gpaToColour(schoolGPA);
                cell(DX, curY, DW, RH+2,
                    `SCHOOL GPA: ${schoolGPA.toFixed(4)}   Grade: ${gc.grade}   (${gc.label})`,
                    {bold:true, bg:gc.bg, fg:gc.fg, fontSize:9});
                curY+=RH+2;
            }
            curY += 5;
            ...
            const drawStudentHeader = () => {
                let x = ML;
                fixedCols.forEach(c=>{ cell(x,curY,c.w,THH,c.l,{bold:true,fontSize:8}); x+=c.w; });
                cell(x,curY,subjColW,THH,'DETAILED SUBJECTS',{bold:true,fontSize:8});
                curY+=THH;
            };
            ...
            students.forEach((s,i)=>{
                ...
                const rowH = Math.max(SRH, subLines.length * 4.2 + 2);

                cell(x,curY,fixedCols[0].w,rowH,cNum,{fontSize:7}); x+=fixedCols[0].w;
                cell(x,curY,fixedCols[1].w,rowH,s.name||'',{fontSize:8,align:'left'}); x+=fixedCols[1].w;
                cell(x,curY,fixedCols[2].w,rowH,s.gender||'-',{fontSize:8}); x+=fixedCols[2].w;
                cell(x,curY,fixedCols[3].w,rowH,s.point,{fontSize:8}); x+=fixedCols[3].w;
                cell(x,curY,fixedCols[4].w,rowH,s.division||'-',{fontSize:8,bold:true}); x+=fixedCols[4].w;
                if (showPosition && fixedCols[5]) {
                    cell(x,curY,fixedCols[5].w,rowH,s._pos||'',{fontSize:8});
                    x+=fixedCols[5].w;
                }
                ...
                subLines.forEach((ln,li)=>doc.text(ln, x+1.5, curY+3.5+li*4.2));
            ...
            const csRH=8, csHdrH=10;
            const csLW=CW*0.65, csVW=CW*0.35;

            cell(ML,curY,CW,csHdrH,'EXAMINATION CENTRE OVERALL PERFORMANCE',{bold:true,fontSize:10});
            curY+=csHdrH;
            ...
            csRows.forEach(([lbl,val])=>{
                cell(ML,curY,csLW,csRH,lbl,{bold:true,align:'left',fontSize:9});
                cell(ML+csLW,curY,csVW,csRH,val,{bold:true,fontSize:9});
                curY+=csRH;
            });
            if (schoolGPA) {
                const gc=gpaToColour(schoolGPA);
                cell(ML,curY,csLW,csRH,'SCHOOL GPA',{bold:true,align:'left',fontSize:9});
                cell(ML+csLW,curY,csVW,csRH,`${schoolGPA.toFixed(4)} — ${gc.grade} (${gc.label})`,{bold:true,fontSize:9,bg:gc.bg,fg:gc.fg});
                curY+=csRH;
            }
            curY+=6;
            ...
            const oFW=28, oSubW=(CW-oFW)/oCols.length/3;
            const oRH=7, oHH=7;

            cell(ML,curY,oFW,oHH*2,'CANDIDATES',{bold:true,fontSize:8,align:'left'});
            let ox=ML+oFW;
            oCols.forEach(h=>{ cell(ox,curY,oSubW*3,oHH,h,{bold:true,fontSize:7}); ox+=oSubW*3; });
            curY+=oHH;
            ox=ML+oFW;
            oCols.forEach(()=>['F','M','T'].forEach(l=>{ cell(ox,curY,oSubW,oHH,l,{bold:true,fontSize:7}); ox+=oSubW; }));
            curY+=oHH;
            cell(ML,curY,oFW,oRH,'No. of Candidates',{bold:true,align:'left',fontSize:8});
            ox=ML+oFW;
            ['registered','sat','clean','absent','INC'].forEach(k=>{
                const f=overall[k].F,m=overall[k].M;
                [f,m,f+m].forEach(v=>{ cell(ox,curY,oSubW,oRH,v,{fontSize:8}); ox+=oSubW; });
            });
            ...
            const subStats={};
            subjects.forEach(sub=>{
               subStats[sub]={counts:{A:{F:0,M:0},B:{F:0,M:0},C:{F:0,M:0},D:{F:0,M:0},F:{F:0,M:0}},satF:0,satM:0,tot:0,gpaSum:0,markSum:0};
            });
          students.forEach(s=>{
    const g=s.gender==='F'?'F':'M';
    Object.entries(s.scores||{}).forEach(([sub,sc])=>{
        ...
        // ADD THIS LINE:
        if(sc.mark!=null&&sc.mark!=='') subStats[sub].markSum+=Number(sc.mark);
    });
});
            ...
            /* Subject table — readable font sizes */
            const sTH=7, sTR=7;
            const sSubW=Math.round(CW*0.15);
            const sSatW=Math.round(CW*0.035);
            const sGrFMT=Math.round(CW*0.025);
            const sGrpW=sGrFMT*3;
            const sAverW=Math.round(CW*0.055);
            const sGpaW=Math.round(CW*0.075);
            const sPosW=Math.round(CW*0.045);
            const sGradeW=CW-sSubW-sSatW*3-sGrpW*5-sAverW-sGpaW-sPosW;
            const sX=ML;

            let sx=sX;
            cell(sx,curY,sSubW,sTH*2,'SUBJECT',{bold:true,fontSize:8,align:'left'}); sx+=sSubW;
            ['SAT-F','SAT-M','TOTAL'].forEach(h=>{ cell(sx,curY,sSatW,sTH*2,h,{bold:true,fontSize:7}); sx+=sSatW; });
            ['A','B','C','D','F'].forEach(gr=>{ cell(sx,curY,sGrpW,sTH,gr,{bold:true,fontSize:8}); sx+=sGrpW; });
            ['AVER','GPA','POS','GRADE'].forEach(h=>{
                const w = h==='GPA'?sGpaW:h==='GRADE'?sGradeW:h==='POS'?sPosW:sAverW;
                cell(sx,curY,w,sTH*2,h,{bold:true,fontSize:7.5});
                sx+=w;
            });
            curY+=sTH;
            sx=sX+sSubW+sSatW*3;
            ['A','B','C','D','F'].forEach(()=>['F','M','T'].forEach(l=>{ cell(sx,curY,sGrFMT,sTH,l,{bold:true,fontSize:7}); sx+=sGrFMT; }));
            curY+=sTH;
            ...
            subjects.forEach(sub=>{
               ...
               const aver=st.tot?(st.markSum/st.tot).toFixed(1):'0.0';
                sx=sX;
                cell(sx,curY,sSubW,sTR,sub,{bold:true,fontSize:8,align:'left'}); sx+=sSubW;
                [st.satF,st.satM,st.tot].forEach(v=>{ cell(sx,curY,sSatW,sTR,v,{fontSize:8}); sx+=sSatW; });
                ['A','B','C','D','F'].forEach(gr=>{
                    const f=st.counts[gr].F,m=st.counts[gr].M;
                    [f,m,f+m].forEach(v=>{ cell(sx,curY,sGrFMT,sTR,v,{fontSize:7.5}); sx+=sGrFMT; });
                });
                cell(sx,curY,sAverW,sTR,aver,{fontSize:8}); sx+=sAverW;
                cell(sx,curY,sGpaW,sTR,gpaVal.toFixed(4),{bold:true,fontSize:8,bg:gc.bg,fg:gc.fg}); sx+=sGpaW;
                cell(sx,curY,sPosW,sTR,subPos[sub],{fontSize:8}); sx+=sPosW;
                cell(sx,curY,sGradeW,sTR,`${gc.grade} (${gc.label})`,{bold:true,fontSize:8,bg:gc.bg,fg:gc.fg});
                curY+=sTR;
            });
...
function _axpExamLabelSwBody(examType) {
    const t = (examType || '').trim().toUpperCase().replace(/[-_]+/g, ' ').replace(/\s+/g, ' ');
    const MAP = {
        'WEEKLY':     'JARIBIO LA KILA WIKI',
        'MONTHLY':    'JARIBIO LA KILA MWEZI',
        'MIDTERM':    'ROBO MUHULA',
        'MIDTERM2':   'ROBO MUHULA WA PILI',
        'MID TERM':   'ROBO MUHULA',
        'TERMINAL':   'NUSU MUHULA',
        'JOINT':      'MTIHANI WA PAMOJA',
        'ANNUAL':     'MWAKA MZIMA',
        'PREMOCK':    'MAANDALIZI YA MTIHANI WA DHIHAKA',
        'PRE MOCK':   'MAANDALIZI YA MTIHANI WA DHIHAKA',
        'MOCK':       'DHIHAKA',
        'PRENECTA':   'MAANDALIZI YA MTIHANI WA TAIFA',
        'PRE NECTA':  'MAANDALIZI YA MTIHANI WA TAIFA',
        'PRENECTA2':  'MAANDALIZI YA MTIHANI WA TAIFA (II)',
        'NECTA':      'MTIHANI WA TAIFA',
    };
    return MAP[t] || (examType || '').replace(/\b\w/g, c => c.toUpperCase());
}

// ── Swahili full heading label (e.g. "MTIHANI WA ROBO MUHULA")
function _axpExamLabelSwHeading(examType) {
    const body = _axpExamLabelSwBody(examType);
    // If the body label already starts with "MTIHANI" don't prepend again
    if (body.toUpperCase().startsWith('MTIHANI')) return body;
    return 'MTIHANI WA ' + body;
}

// ── Abbreviate teacher/HOS name: "J. Mwangi"  (first letter + surname capitalised)
function _axpAbbrName(fullName) {
    if (!fullName || !fullName.trim()) return '';
    const parts = fullName.trim().split(/\s+/);
    if (parts.length === 1) return parts[0];
    const abbr = parts[0][0].toUpperCase() + '.';
    const rest  = parts.slice(1).map(p => p.charAt(0).toUpperCase() + p.slice(1).toLowerCase()).join(' ');
    return abbr + ' ' + rest;
}

// ── Convert month input to Swahili month name
function _axpSwMonth(monthStr) {
    if (!monthStr) return '';
    // Try parsing as a date or plain month number
    const SW_MONTHS = ['Januari','Februari','Machi','Aprili','Mei','Juni',
                       'Julai','Agosti','Septemba','Oktoba','Novemba','Disemba'];
    // If it's a date string like "2026-04-01" or "April 2026"
    const parsed = new Date(monthStr);
    if (!isNaN(parsed)) return SW_MONTHS[parsed.getMonth()] + ', ' + parsed.getFullYear();
    // If it already contains a recognised English month name
    const EN = ['january','february','march','april','may','june',
                'july','august','september','october','november','december'];
    const lower = monthStr.toLowerCase();
    for (let i = 0; i < EN.length; i++) {
        if (lower.includes(EN[i])) {
            // extract year if present
            const yearMatch = monthStr.match(/\d{4}/);
            return SW_MONTHS[i] + (yearMatch ? ', ' + yearMatch[0] : '');
        }
    }
    // If it's a number 1-12
    const num = parseInt(monthStr, 10);
    if (num >= 1 && num <= 12) return SW_MONTHS[num - 1];
    return monthStr; // fallback: return as-is
}

// ── Division colour map — works in both colour and black-and-white print
// B&W: each div has a distinct fill density; text is always bold black
const DIV_STYLE = {
    'I'  : { fill:[0,140,60],   text:[255,255,255], bwFill:[220,220,220], label:'I'  },
    'II' : { fill:[144,238,144],text:[0,0,0],       bwFill:[180,180,180], label:'II' },
    'III': { fill:[255,215,0],  text:[0,0,0],       bwFill:[140,140,140], label:'III'},
    'IV' : { fill:[255,140,0],  text:[255,255,255], bwFill:[100,100,100], label:'IV' },
    'O'  : { fill:[200,0,0],    text:[255,255,255], bwFill:[60,60,60],    label:'0'  },
    '0'  : { fill:[200,0,0],    text:[255,255,255], bwFill:[60,60,60],    label:'0'  },
};

async function _repCapturePdf(filename, action) {
    try {
        const jsPDFCtor = await _axpGetJsPDF().catch(e => {
            _axpToast('jsPDF not available: ' + e.message, 'error'); return null;
        });
        if (!jsPDFCtor) return;
        ...
        const elig    = eligible.find(s => s.name === student.name && s.point === student.point);
        const si      = _axpSchoolInfo(currentLang || 'en');

        // ── Exam / Class selectors ────────────────────────────────────────────
        const examClass = document.getElementById('axpRepClassSel')?.value ||
                          document.getElementById('axpRRClassSel')?.value  || '';
        const examType  = document.getElementById('axpRepExamSel')?.value  ||
                          document.getElementById('axpRRExamSel')?.value   || '';

        // ── Language-aware labels ─────────────────────────────────────────────
        const isSw       = (currentLang === 'sw');
        // Heading label: "MTIHANI WA ROBO MUHULA" (Sw) or "MID-TERM EXAMINATION" (En)
        const examLabelHeading = isSw
            ? _axpExamLabelSwHeading(examType)
            : _axpExamLabel(examType, 'en');
        // Body label (short, no "MTIHANI WA" prefix): "ROBO MUHULA" (Sw) or "Mid-Term" (En)
        const examLabelBody = isSw
            ? _axpExamLabelSwBody(examType)
            : _axpExamLabel(examType, 'en');

        const clsLabel  = _axpClassLabel(examClass, currentLang) || examClass || '';
        const year      = _schoolMeta?.year || new Date().getFullYear();

        // ── School name: replace DAY→KUTWA in Swahili ─────────────────────────
        let schoolDisplayName = si.displayLine1 || '';
        if (isSw) {
            schoolDisplayName = schoolDisplayName
                .replace(/\bDAY\b/gi, 'KUTWA')
                .replace(/\bday\b/gi, 'kutwa');
        }

        // ── Class Info — read from every possible source ──────────────────────
        const _saved = SchoolInfoStorage.getClassData(examClass) || {};
        const _gv = (key, fallback) => {
            const panel = document.getElementById('axpClassInfoPanel');
            if (panel) { const el = panel.querySelector(`[data-field="${key}"]`); if (el && el.value && el.value.trim()) return el.value.trim(); }
            if (_saved[key] && String(_saved[key]).trim()) return String(_saved[key]).trim();
            const dom = document.getElementById(key); if (dom && dom.value && dom.value.trim()) return dom.value.trim();
            return fallback || '';
        };
        ...
        const classTeacher = _gv('classTeacherInput', isSw ? 'Mwalimu wa Darasa' : 'Class Teacher');
        const headmaster   = _gv('headmasterInput',   isSw ? 'Mkuu wa Shule'     : 'Head of School');

        // Requirements — base 3 + extras
        const reqBase  = [_gv('requirement1'), _gv('requirement2'), _gv('requirement3')].filter(Boolean);
        const reqExtra = (_saved._extraReqs || []).filter(Boolean);
        const reqs     = [...reqBase, ...reqExtra];

        // ── Candidate number ──────────────────────────────────────────────────
        const pt      = Number(student.point);
        const posInfo = elig ? `${elig.position} / ${elig.total}` : '—';
        const ci      = (si.rawIndex || '').replace(/[\./\\\s]/g, '').toUpperCase();
        const candNo  = ci
            ? `${ci}-${String(currentIndex + 1).padStart(4, '0')}`
            : `S000-${String(currentIndex + 1).padStart(4, '0')}`;

        // ── Behaviour grades ──────────────────────────────────────────────────
        let disc='C', coop='C', learn='C', sports='B', part='C',
            punct='C', lead='C', neat='C', resp='C', creat='C',
            remark = lang.remarksList.followup;
        if      (pt <= 17) { disc='A';coop='A';learn='A';sports='C';part='A';punct='A';lead='A';neat='A';resp='A';creat='B'; remark=lang.remarksList.outstanding; }
        else if (pt <= 21) { disc='B';coop='B';learn='B';sports='C';part='B';punct='B';lead='B';neat='B';resp='B';creat='B'; remark=lang.remarksList.commendable; }
        ...
        let descParagraph = '';
        if (isSw) {
            // Swahili format: "...aliyefanya mtihani wa ROBO MUHULA, Kidato cha Tatu mwaka wa masomo 2026."
            // "alipata" not "amepata", "wanafunzi N" not just "N"
            const divisionNoun = isSw ? 'Daraja' : 'Division';
            descParagraph =
                `Ripoti hii inathibitisha utendaji wa kitaaluma wa ${student.name} aliyefanya mtihani wa ` +
                `${examLabelBody}, ${clsLabel} mwaka wa masomo ${year}. ` +
                `Mwanafunzi alipata jumla ya pointi ${student.point}, akipata ${divisionNoun} ${student.division}` +
                `${elig ? `, na alikuwa wa ${elig.position} kati ya wanafunzi ${elig.total} darasani` : ''}. ` +
                `Daraja I (pointi 7–17) ni Bora Sana, Daraja II (18–21) ni Nzuri, Daraja III (22–25) ni Ya Kati, ` +
                `Daraja IV (26–33) ni Hafifu, Daraja 0 (34–35) ni Kushindwa. ` +
                `Matokeo ya kitaaluma na tathmini ya tabia ni kama ifuatavyo hapa chini:`;
        } else {
            descParagraph = lang.reportDescription
                ? lang.reportDescription(student.name, examLabelBody, clsLabel, year,
                    student.division, student.point, elig?.position || null, elig?.total || null)
                    .replace(/<[^>]+>/g, '').replace(/\s+/g, ' ').trim()
                : `This report certifies the academic performance of ${student.name} who sat for the ${examLabelBody} examination in ${clsLabel}, ${year}.`;
        }

        // ── jsPDF setup ───────────────────────────────────────────────────────
        // compress:false + precision:6 = maximum quality / larger file for print
        const doc = new jsPDFCtor({
            unit:'mm', format:'a4', orientation:'portrait',
            compress:false, precision:6, putOnlyUsedFonts:false
        });
        ...
        const BLACK=[0,0,0];
        const RED  =[183,28,28];
        const LGREY=[235,241,248];
        const DGREY=[68,68,68];
        const gradeColor = { A:[0,120,60], B:[0,160,90], C:[180,130,0], D:[200,90,0], F:[200,0,0] };
        const gradeFg    = { A:WHITE, B:WHITE, C:WHITE, D:WHITE, F:WHITE };
        ...
        // Vertically centred cell text helper
        const cellText = (text, x, y, w, h, opts={}) => {
            const { fs=8, bold=false, col=BLACK, align='center', italic=false } = opts;
            doc.setFontSize(fs);
            doc.setFont('helvetica', italic ? 'italic' : bold ? 'bold' : 'normal');
            st(col);
            const ascent = fs * 0.352;
            const cy2    = y + (h / 2) + (ascent / 2);
            const maxW   = w - (align === 'center' ? 2 : 3);
            const str    = doc.splitTextToSize(T(text), maxW)[0];
            const tx     = align === 'left'  ? x + 2
                         : align === 'right' ? x + w - 2
                         :                     x + w / 2;
            doc.text(str, tx, cy2, { align });
        };

        // ── Zone heights (bottom-pinned) ──────────────────────────────────────
        const BOT_MARGIN = 9;
        const SIG_H      = 20;   // each signature block height (reduced — no repeated text)
        const DATES_H    = 8;
        const REQS_LINE_H = 5;   // per requirement row
        const REQS_H     = reqs.length ? (reqs.length * REQS_LINE_H + 7) : 0;

        const HOS_TOP    = PH - BOT_MARGIN - SIG_H;
        const CT_TOP     = HOS_TOP - 0.5 - SIG_H;
        const ABOVE_CT   = CT_TOP - 0.5;
        const REQS_TOP   = REQS_H ? ABOVE_CT - REQS_H : ABOVE_CT;
        const DATES_TOP  = (REQS_H ? REQS_TOP : ABOVE_CT) - 0.5 - DATES_H;
        const CONTENT_BOTTOM = DATES_TOP - 1;

        // ═════════════════════════════════════════════════════
        // OUTER DECORATIVE FRAME — FIX: L-bracket lines ONLY,
        // NO filled rectangle (green was projecting inside)
        // ═════════════════════════════════════════════════════
        // Outer navy border
        sd(NAVY); doc.setLineWidth(0.7); doc.rect(4, 4, PW-8, PH-8, 'S');
        // Inner teal border (stroke only, no fill)
        sd(TEAL); doc.setLineWidth(0.25); doc.rect(6.5, 6.5, PW-13, PH-13, 'S');

        // Corner L-brackets — LINES ONLY (no filled rect, no filled square)
        // Each corner: two lines forming a right-angle bracket, 12mm arms
        sd(TEAL); doc.setLineWidth(1.2);

        // Top-left corner
        doc.line(4, 16, 4, 4);   // vertical arm (from y=16 up to y=4)
        doc.line(4, 4, 16, 4);   // horizontal arm (from x=4 to x=16)

        // Top-right corner
        doc.line(PW-4, 16, PW-4, 4);
        doc.line(PW-16, 4, PW-4, 4);

        // Bottom-left corner
        doc.line(4, PH-16, 4, PH-4);
        doc.line(4, PH-4, 16, PH-4);

        // Bottom-right corner
        doc.line(PW-4, PH-16, PW-4, PH-4);
        doc.line(PW-16, PH-4, PW-4, PH-4);

        // Candidate number corners
        doc.setFontSize(6.5); doc.setFont('helvetica','bold'); st([150,150,150]);
        doc.text(candNo, ML, 8);
        doc.text(candNo, ML+CW, 8, {align:'right'});

        // ═════════════════════════════════════════════════════
        // HEADER — 5 lines, reads district in Swahili
        // ═════════════════════════════════════════════════════
        let cy = 10;
        const HEADER_H = 40; // +6 for sub-header band

        // Build header lines
        const panel2   = document.getElementById('axpClassInfoPanel');
        let hLines     = panel2
            ? [...panel2.querySelectorAll('[data-hdrline]')]
                .map(i => i.value.trim()).filter(Boolean)
            : SchoolInfoStorage.getHeaderLines(examClass).filter(Boolean);

        const _dd2  = (typeof _dashboardData !== 'undefined') ? _dashboardData : {};

        // Swahili district line
        let districtLine;
        if (isSw) {
            districtLine = _dd2.district
                ? 'HALMASHAURI YA WILAYA YA ' + _dd2.district.toUpperCase()
                : 'HALMASHAURI YA WILAYA';
        } else {
            districtLine = _dd2.district
                ? _dd2.district.toUpperCase() + ' DISTRICT COUNCIL'
                : 'DISTRICT COUNCIL';
        }

        // Lines 0-1 are ALWAYS English (government header)
        hLines[0] = isSw ? "OFISI YA RAIS": "PRESIDENT'S OFFICE";
        hLines[1] = isSw ? "TAWALA ZA MIKOA NA SERIKALI ZA MITAA": "REGIONAL ADMINISTRATION AND LOCAL GOVERNMENT";
        hLines[2] = districtLine;
        hLines[3] = (si.displayLine2 ? si.displayLine2.toUpperCase() + ' — ' : '')
                  + schoolDisplayName.toUpperCase();
        // Line 5: class + exam heading label + month (Swahili month if isSw)
        const _monthDisplay = month
            ? (isSw ? _axpSwMonth(month) : month)
            : '';
        hLines[4] = isSw
            ? `${clsLabel} — ${examLabelHeading}${_monthDisplay ? ' — ' + _monthDisplay : ''}`
            : `${clsLabel.toUpperCase()} ${examLabelHeading.toUpperCase()}${_monthDisplay ? ' — ' + _monthDisplay.toUpperCase() : ''} — ${year}`;

        // Sub-header line 5b: report type banner
        const subHeaderText = isSw
            ? `RIPOTI YA MAENDELEO YA MWANAFUNZI — ${examLabelHeading}`
            : `STUDENT PROGRESS REPORT — ${examLabelHeading.toUpperCase()}`;

        const hFS  = [7, 7.5, 8.5, 12.5, 9];
        const hFW  = ['normal','normal','bold','bold','bold'];
        const hGap = [3.5, 3.5, 4.5, 6, 5];

        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, cy+HEADER_H, ML+CW, cy+HEADER_H);

        const logo = window.getSchoolLogo ? window.getSchoolLogo() : null;
        if (logo) {
            try { doc.addImage(logo,'PNG', ML, cy, 15, 15, '', 'FAST'); } catch(e){}
            try { doc.addImage(logo,'PNG', ML+CW-15, cy, 15, 15, '', 'FAST'); } catch(e){}
        }

        const cx2 = ML + CW / 2;
        let hy = cy + 3;
        hLines.forEach((ln, i) => {
            doc.setFontSize(hFS[i] || 8);
            doc.setFont('helvetica', hFW[i] || 'normal');
            st(i >= 3 ? NAVY : DGREY);
            doc.text(T(ln), cx2, hy, {align:'center'});
            hy += hGap[i] || 4;
        });

        // Sub-header band: navy background, white bold text
        const SH_Y = cy + HEADER_H - 6;
        sf(NAVY); doc.rect(ML, SH_Y, CW, 6, 'F');
        doc.setFontSize(7); doc.setFont('helvetica','bold'); st([255,255,255]);
        doc.text(subHeaderText, cx2, SH_Y + 4, {align:'center'});

        cy += HEADER_H + 3;

        // ═════════════════════════════════════════════════════
        // STUDENT INFO BAR — Swahili field labels
        // ═════════════════════════════════════════════════════
        const INFO_H = 7;
        sf(LGREY); doc.rect(ML, cy, CW, INFO_H, 'F');
        sd([170,196,224]); doc.setLineWidth(0.2); doc.rect(ML, cy, CW, INFO_H, 'S');

        // Field labels: EN vs SW
        const nameLabel  = isSw ? 'JINA'   : (lang.studentName  || 'NAME');
        const classLabel = isSw ? 'DARASA' : (lang.class        || 'CLASS');
        const sexLabel   = isSw ? 'JINSI'  : (lang.sex          || 'SEX');

        const infoCols = [
            { lbl: nameLabel + ':',  val: student.name,        w: CW*0.36 },
            { lbl: classLabel + ':', val: clsLabel,            w: CW*0.20 },
            { lbl: sexLabel + ':',   val: student.gender||'—', w: CW*0.11 },
            { lbl: 'No:',            val: candNo,              w: CW*0.33 },
        ];
        let ix = ML;
        infoCols.forEach((col, i) => {
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
            const lblW = doc.getTextWidth(col.lbl);
            const midY = cy + INFO_H/2 + 7.5*0.352/2;
            doc.text(col.lbl, ix+2, midY);
            doc.setFont('helvetica','normal'); st(BLACK);
            doc.text(T(col.val), ix+2+lblW+1.5, midY);
            ix += col.w;
            if (i < infoCols.length-1) {
                sd([170,196,224]); doc.setLineWidth(0.2);
                doc.line(ix, cy, ix, cy+INFO_H);
            }
        });
        cy += INFO_H + 2;
        ...
        // ═════════════════════════════════════════════════════
        const descLines  = doc.splitTextToSize(descParagraph, CW-6);
        const DESC_H     = Math.min(descLines.length, 4) * 3.8 + 5;

        sf([247,250,255]); sd([210,225,240]); doc.setLineWidth(0.2);
        doc.rect(ML, cy, CW, DESC_H, 'FD');
        doc.setFont('helvetica','normal'); doc.setFontSize(7.5); st([40,40,40]);
        doc.text(descLines.slice(0,4), ML+3, cy+4);
        cy += DESC_H + 2;

        // ═════════════════════════════════════════════════════
        // MAIN TABLES (Academic left 57% | Behaviour right 40%)
        // ═════════════════════════════════════════════════════
        const TABLE_TOP = cy;
        const TABLE_H   = CONTENT_BOTTOM - TABLE_TOP;
        const ACW = CW * 0.57;
        const BHW = CW * 0.40;
        const BHX = ML + ACW + CW * 0.03;

        const SBJ_W = ACW * 0.35;
        const MRK_W = ACW * 0.14;
        const GR_W  = ACW * 0.13;
        const CMT_W = ACW - SBJ_W - MRK_W - GR_W;
        const HDR_H      = 6;
        const AC_FOOT_H  = 6;

        // Academic section title
        sf(LGREY); doc.rect(ML, TABLE_TOP, ACW, HDR_H, 'F');
        sd(BLACK);  doc.setLineWidth(0.4); doc.rect(ML, TABLE_TOP, ACW, HDR_H, 'S');
        cellText((lang.performanceTitle||'ACADEMIC PERFORMANCE').toUpperCase(),
            ML, TABLE_TOP, ACW, HDR_H, {fs:7.5, bold:true, col:NAVY, align:'center'});

        // Column headers
        const acHdrCols = [
            { x:ML,                    w:SBJ_W, t:lang.subject||'Subject',  a:'left'   },
            { x:ML+SBJ_W,              w:MRK_W, t:lang.mark||'Mark',        a:'center' },
            { x:ML+SBJ_W+MRK_W,       w:GR_W,  t:'Gr',                      a:'center' },
            { x:ML+SBJ_W+MRK_W+GR_W,  w:CMT_W, t:lang.comment||'Remark',   a:'center' },
        ];
        acHdrCols.forEach(c => {
            sf(LGREY); doc.rect(c.x, TABLE_TOP+HDR_H, c.w, HDR_H, 'F');
            sd(BLACK); doc.setLineWidth(0.3); doc.rect(c.x, TABLE_TOP+HDR_H, c.w, HDR_H, 'S');
            cellText(c.t, c.x, TABLE_TOP+HDR_H, c.w, HDR_H, {fs:7, bold:true, col:NAVY, align:c.a});
        });

        let acRowY = TABLE_TOP + HDR_H * 2;

        // Behaviour section title
        sf(LGREY); doc.rect(BHX, TABLE_TOP, BHW, HDR_H, 'F');
        sd(BLACK);  doc.setLineWidth(0.4); doc.rect(BHX, TABLE_TOP, BHW, HDR_H, 'S');
        cellText((lang.behavior||'BEHAVIOUR').toUpperCase(),
            BHX, TABLE_TOP, BHW, HDR_H, {fs:7.5, bold:true, col:NAVY, align:'center'});

        const bhHdrCols = [
            { x:BHX,          w:BHW*0.76, t:lang.aspect||'Aspect', a:'left'   },
            { x:BHX+BHW*0.76, w:BHW*0.24, t:lang.rating||'Rate',   a:'center' },
        ];
        bhHdrCols.forEach(c => {
            sf(LGREY); doc.rect(c.x, TABLE_TOP+HDR_H, c.w, HDR_H, 'F');
            sd(BLACK); doc.setLineWidth(0.3); doc.rect(c.x, TABLE_TOP+HDR_H, c.w, HDR_H, 'S');
        ...
        let bhRowY = TABLE_TOP + HDR_H * 2;

        // Subject rows
        const subjectEntries = Object.entries(student.scores||{})
            .filter(([, sc]) => sc.mark != null && sc.mark !== '');

        const availAc = TABLE_H - HDR_H*2 - AC_FOOT_H;
        const acRowH  = Math.max(4.5, Math.min(7.5, availAc / Math.max(subjectEntries.length, 1)));

        subjectEntries.forEach(([sub, sc]) => {
            const gBg = gradeColor[sc.grade] || [120,120,120];
            const gFg = gradeFg[sc.grade]    || WHITE;

            const acRowCols = [
                { x:ML,                    w:SBJ_W, t:sub,                           opts:{fs:7.5, col:BLACK, align:'left'}  },
                { x:ML+SBJ_W,              w:MRK_W, t:sc.mark,                       opts:{fs:8.5, col:BLACK, align:'center', bold:true} },
                { x:ML+SBJ_W+MRK_W+GR_W,  w:CMT_W, t:lang.comments?.[sc.grade]||'—',opts:{fs:7,   col:[90,90,90], align:'left', italic:true} },
            ];
            acRowCols.forEach(rc => {
                sf([255,255,255]); sd(BLACK); doc.setLineWidth(0.2);
                doc.rect(rc.x, acRowY, rc.w, acRowH, 'FD');
                cellText(rc.t, rc.x, acRowY, rc.w, acRowH, rc.opts);
            });

            sf(gBg); doc.rect(ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH, 'F');
            sd(BLACK); doc.setLineWidth(0.2);
            doc.rect(ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH, 'S');
            cellText(sc.grade||'—', ML+SBJ_W+MRK_W, acRowY, GR_W, acRowH,
                {fs:9, bold:true, col:gFg, align:'center'});

            acRowY += acRowH;
        });

        // Academic footer — division coloured row
        // Shows: "Daraja I ya Pointi 17 — 78/100" with division background colour
        const divKey    = String(student.division || '').toUpperCase().trim();
        const divStyle  = DIV_STYLE[divKey] || { fill:LGREY, text:BLACK };
        // Compute a representative total mark out of 100 (average × subjects or direct avg)
        const subScores   = subjectEntries.map(([,sc]) => Number(sc.mark) || 0);
        const avgMark     = subScores.length ? Math.round(subScores.reduce((a,b)=>a+b,0) / subScores.length) : 0;
        const totalMark   = subScores.reduce((a,b)=>a+b,0);
         const studentElig  = eligible.find(s=>s.name===student.name&&s.point===student.point);
        const studentPOS      = studentElig ?  `${studentElig.position}/${studentElig.total}` : 'N/A';
        const divLabelDisp = divKey === 'O' ? '0' : divKey;
        const footerText  = isSw
            ? `Daraja ${divLabelDisp} ya Pointi ${student.point} — Nafasi : ${studentPOS} — Wastani : ${avgMark}`
            : `Division ${divLabelDisp} — ${student.point} Points — Position : ${studentPOS} — Average : ${avgMark}`;

        sf(divStyle.fill); doc.rect(ML, acRowY, ACW, AC_FOOT_H, 'F');
        sd(BLACK);  doc.setLineWidth(0.4); doc.rect(ML, acRowY, ACW, AC_FOOT_H, 'S');
        cellText(footerText, ML, acRowY, ACW, AC_FOOT_H,
            {fs:8, bold:true, col:divStyle.text, align:'center'});

        // Behaviour rows
        const behavItems = [
            ['discipline',disc], ['cooperation',coop], ['learning',learn],
            ['sports',sports],   ['participation',part], ['punctuality',punct],
            ['leadership',lead], ['neatness',neat],      ['respect',resp],
            ['creativity',creat]
        ];
        const BH_REM_H = 6;
        const availBh  = TABLE_H - HDR_H*2 - BH_REM_H;
        const bhRowH   = Math.max(4, Math.min(6.5, availBh / behavItems.length));

        behavItems.forEach(([key, val]) => {
            const bBg = gradeColor[val] || [120,120,120];
            const bFg = gradeFg[val]    || WHITE;

            sf([255,255,255]); sd(BLACK); doc.setLineWidth(0.2);
            doc.rect(BHX, bhRowY, BHW*0.76, bhRowH, 'FD');
            cellText(lang.behaviorAspects?.[key]||key, BHX, bhRowY, BHW*0.76, bhRowH,
                {fs:7.5, col:BLACK, align:'left'});

            sf(bBg); doc.rect(BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, 'F');
            sd(BLACK); doc.setLineWidth(0.2); doc.rect(BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, 'S');
            cellText(val, BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH,
                {fs:9, bold:true, col:bFg, align:'center'});

            bhRowY += bhRowH;
        });

        // Behaviour remark — black border
        sf([247,250,255]); sd(BLACK); doc.setLineWidth(0.2);
        doc.rect(BHX, bhRowY, BHW, BH_REM_H, 'FD');
        const remarkShort = remark.length > 62 ? remark.slice(0,60) + '…' : remark;
        cellText(remarkShort, BHX, bhRowY, BHW, BH_REM_H,
            {fs:7.5, col:[60,60,60], align:'center', italic:true});

        // ═════════════════════════════════════════════════════
        // FIXED BOTTOM ZONES
        // ═════════════════════════════════════════════════════
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(ML, CONTENT_BOTTOM, ML+CW, CONTENT_BOTTOM);

        // ── TERM DATES — spaced, with title above and value below ─────────────
        sf([245,248,253]); sd([210,225,240]); doc.setLineWidth(0.2);
        doc.rect(ML, DATES_TOP, CW, DATES_H, 'FD');

        const closesLbl  = isSw ? 'Imefungwa:' : 'Closed:';
        const reopensLbl = isSw ? 'Itafunguliwa:' : 'Reopens:';

        // Format date as DD/MM/YYYY if possible, otherwise leave as-is
        const _formatDate = (d) => {
            if (!d) return '—';
        ...
        // Left group: closes label + formatted date
        doc.setFontSize(7); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(closesLbl, ML+3, DATES_TOP + 3);
        doc.setFont('helvetica','normal'); st(BLACK); doc.setFontSize(8);
        doc.text(_formatDate(closingDate), ML+3, DATES_TOP + 6.5);

        // Right group: reopens label + formatted date
        const midX = ML + CW * 0.45;
        doc.setFontSize(7); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(reopensLbl, midX, DATES_TOP + 3);
        doc.setFont('helvetica','normal'); st(BLACK); doc.setFontSize(8);
        doc.text(_formatDate(openingDate || '—'), midX, DATES_TOP + 6.5);
        // NOTE: month is intentionally omitted here — it appears in heading line 5

        // ── REQUIREMENTS — numbered rows ──────────────────────────────────────
        if (reqs.length) {
            sd(NAVY); doc.setLineWidth(0.3);
            doc.line(ML, REQS_TOP, ML+CW, REQS_TOP);

            sf([255,255,247]); sd([200,210,180]); doc.setLineWidth(0.2);
            doc.rect(ML, REQS_TOP, CW, REQS_H, 'FD');

            const reqLabel = isSw ? 'Mahitaji ya Muhula Ujao:' : 'Requirements for Next Term:';
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(reqLabel, ML+3, REQS_TOP + 5);

            reqs.forEach((r, i) => {
                doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([40,40,40]);
                const rowY = REQS_TOP + 5 + (i + 1) * REQS_LINE_H;
                const short = r.length > 80 ? r.slice(0,78) + '…' : r;
                doc.text(`${i + 1}. ${short}`, ML + 4, rowY);
            });
        }

        // ═════════════════════════════════════════════════════
        // CLASS TEACHER SIGNATURE BLOCK
        // Signature: abbreviated name in cursive/italic bold font
        // NO repeated small text — just role label once, then sign line
        // ═════════════════════════════════════════════════════
        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, CT_TOP, ML+CW, CT_TOP);
        sf([248,250,255]); doc.rect(ML, CT_TOP, CW, SIG_H, 'F');

        const ctRole = isSw ? 'MWALIMU WA DARASA' : 'CLASS TEACHER';
        // Brief auto-comment line
        const ctComment = (isSw
            ? `${student.name} ameonyesha utendaji ${pt<=12?'mzuri sana':pt<=21?'wa wastani':'unaohitaji kujitahidi zaidi'} muhula huu.`
            : `${student.name} has shown ${pt<=12?'excellent':pt<=21?'satisfactory':'an improving'} performance this term.`
        );

        doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(ctRole + ':', ML+2, CT_TOP + 4);
        doc.setFont('helvetica','normal'); st([40,40,40]); doc.setFontSize(7);
        doc.text(doc.splitTextToSize(ctComment, CW - 4)[0], ML+2, CT_TOP + 8);

        // Horizontal rule under comment
        sd([200,215,230]); doc.setLineWidth(0.2);
        doc.line(ML+2, CT_TOP+10, ML+CW-2, CT_TOP+10);
        ...
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([130,130,130]);
        doc.text(ctRole, ML+2, SY + 1.5);

        // Signature in cursive-like italic bold — abbreviated name
        const ctAbbr = _axpAbbrName(classTeacher);
        doc.setFontSize(10); doc.setFont('helvetica','bolditalic'); st([20,20,80]);
        doc.text(ctAbbr || '____________________', ML + 2, SY + 7.5);

        // Signature underline
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(ML + 2, SY + 9, ML + NW - 4, SY + 9);

        // Date column: label on top, value below
        doc.setFontSize(6.5); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(isSw ? 'Tarehe:' : 'Date:', DX + 2, SY + 1.5);
        doc.setFontSize(8); doc.setFont('helvetica','normal'); st(BLACK);
        doc.text(_formatDate(closingDate), DX + 2, SY + 7.5);
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(DX + 2, SY + 9, DX + DW - 2, SY + 9);

        // ═════════════════════════════════════════════════════
        // HEAD OF SCHOOL SIGNATURE BLOCK
        // Same clean layout — no repeated text
        // ═════════════════════════════════════════════════════
        sd(NAVY); doc.setLineWidth(0.5); doc.line(ML, HOS_TOP, ML+CW, HOS_TOP);
        sf([248,252,248]); doc.rect(ML, HOS_TOP, CW, SIG_H, 'F');

        const hosRole = isSw ? 'MKUU WA SHULE' : 'HEAD OF SCHOOL';
        const hosComment = (isSw
            ? `Matokeo ya ${student.name} ${pt<=13?'ni bora sana':pt<=17?'yanaridhisha':pt<=24?'yanaonyesha maendeleo':'yanahitaji juhudi zaidi'} muhula huu.`
            : `${student.name}'s performance is ${pt<=13?'outstanding':pt<=17?'commendable':pt<=24?'satisfactory':'in need of improvement'} this term.`
        );

        doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(hosRole + ':', ML+2, HOS_TOP + 4);
        doc.setFont('helvetica','normal'); st([40,40,40]); doc.setFontSize(7);
        doc.text(doc.splitTextToSize(hosComment, CW - 4)[0], ML+2, HOS_TOP + 8);

        sd([200,215,230]); doc.setLineWidth(0.2);
        doc.line(ML+2, HOS_TOP+10, ML+CW-2, HOS_TOP+10);

        const HOS_SY = HOS_TOP + 11;

        // Role label
        doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([130,130,130]);
        doc.text(hosRole, ML+2, HOS_SY + 1.5);

        // Signature in cursive-like italic bold — abbreviated name
        const hosAbbr = _axpAbbrName(headmaster);
        doc.setFontSize(10); doc.setFont('helvetica','bolditalic'); st([20,20,80]);
        doc.text(hosAbbr || '____________________', ML + 2, HOS_SY + 7.5);

        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(ML + 2, HOS_SY + 9, ML + NW - 4, HOS_SY + 9);

        // Date column
        doc.setFontSize(6.5); doc.setFont('helvetica','bold'); st(NAVY);
        doc.text(isSw ? 'Tarehe:' : 'Date:', DX + 2, HOS_SY + 1.5);
        doc.setFontSize(8); doc.setFont('helvetica','normal'); st(BLACK);
        doc.text(_formatDate(closingDate), DX + 2, HOS_SY + 7.5);
        sd(NAVY); doc.setLineWidth(0.5);
        doc.line(DX + 2, HOS_SY + 9, DX + DW - 2, HOS_SY + 9);

        // Bottom border line
        sd(NAVY); doc.setLineWidth(0.35);
        doc.line(ML, PH - BOT_MARGIN, ML+CW, PH - BOT_MARGIN);

        // Dense watermark grid — repeated student name at low opacity across full page
        // This also substantially increases file size for print quality
        if (doc.setGState) {
            doc.setGState(new doc.GState({opacity:0.04}));
            st([26,58,92]); doc.setFont('helvetica','bold'); doc.setFontSize(9);
            const wmText = student.name.toUpperCase();
            const wmCols = 3, wmRows = 14;
            const wmXStep = CW / wmCols;
            const wmYStep = (PH - 20) / wmRows;
            for (let row = 0; row < wmRows; row++) {
                for (let col = 0; col < wmCols; col++) {
                    const wx = ML + col * wmXStep + wmXStep * 0.5;
                    const wy = 14 + row * wmYStep + wmYStep * 0.5;
                    try { doc.text(wmText, wx, wy, {align:'center', angle:30}); } catch(e){}
                }
            }
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        try {
            ...
            const mergeDoc = new jsPDFCtor({unit:'mm',format:'a4',orientation:'portrait',compress:false,precision:6,putOnlyUsedFonts:true});
            ...
            const _drawReportPage = async (doc, idx) => {
                ...
                const closingDate2 = _gv2('closingDateInput');
                const openingDate2 = _gv2('openingDateInput');
                const classTeacher2= _gv2('classTeacherInput');
                const headmaster2  = _gv2('headmasterInput');
                ...
                // ── Requirements ──
                if(reqs2.length){
                    cy+=2;sd([170,196,224]);doc.setLineWidth(0.2);doc.line(ML,cy,ML+CW,cy);cy+=3;
                    doc.setFontSize(8);doc.setFont('helvetica','bold');st(navy);
                    doc.text((lang2.requirements||'Requirements for Next Term')+':',ML,cy+3);cy+=5;
                    reqs2.forEach((r,i)=>{
                        const wt=doc.splitTextToSize(`${i+1}. ${r}`,CW-5);
                        doc.setFontSize(8);doc.setFont('helvetica','normal');st([40,40,40]);
                        wt.forEach((ln,j)=>doc.text(ln,ML+3,cy+j*4));
                        cy+=wt.length*4+1;
                    });
                    cy+=2;
                }
                ...
                doc.setFontSize(9);doc.setFont('helvetica','normal');st([60,60,60]);
                doc.text((lang2.closes||'School Closes')+': ',ML,cy+3);
                doc.setFont('helvetica','bold');st(black);
                doc.text(closingDate2||'—',ML+doc.getTextWidth((lang2.closes||'School Closes')+': '),cy+3);
                const rx2=ML+CW*0.5;
                doc.setFont('helvetica','normal');st([60,60,60]);doc.text((lang2.reopens||'School Opens')+': ',rx2,cy+3);
                doc.setFont('helvetica','bold');st(black);doc.text(openingDate2||'—',rx2+doc.getTextWidth((lang2.reopens||'School Opens')+': '),cy+3);
                cy+=7;

                // ── Class Teacher — comment + signature (mirrors web) ──
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy,ML+CW,cy);cy+=3;
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);
                doc.text((lang2.classComment||"Class Teacher's Comment")+':',ML,cy+3);cy+=5;
                ...
                doc.setFontSize(8.5);doc.setFont('helvetica','normal');st([40,40,40]);doc.text(ctl2,ML,cy+3);cy+=ctl2.length*4+4;
                {const ctR2=currentLang==='sw'?'Mwalimu wa Darasa':'Class Teacher';const dx2=ML+CW*0.62;const dw2=CW*0.38;
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([130,130,130]);
                doc.text(ctR2+':',ML,cy);doc.text((lang2.date||'Date')+':',dx2,cy);cy+=3;
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy+6,ML+CW*0.54,cy+6);doc.line(dx2,cy+6,dx2+dw2,cy+6);
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);
                if(classTeacher2)doc.text(classTeacher2,ML+1,cy+5.5);
                if(closingDate2)doc.text(closingDate2,dx2+1,cy+5.5);cy+=8;
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([100,100,100]);
                doc.text(classTeacher2||'___________________________',ML,cy);doc.text(closingDate2||'_______________',dx2,cy);cy+=3.5;
                doc.setFontSize(7);st([150,150,150]);doc.text(ctR2,ML,cy);cy+=6;}

                // ── Head of School — comment + signature (mirrors web) ──
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy,ML+CW,cy);cy+=3;
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);
                doc.text((lang2.headComment||"Head of School's Comment")+':',ML,cy+3);cy+=5;
                ...
                doc.setFontSize(8.5);doc.setFont('helvetica','normal');st([40,40,40]);doc.text(htl2,ML,cy+3);cy+=htl2.length*4+4;
                {const htR2=currentLang==='sw'?'Mkuu wa Shule':'Head of School';const dx2=ML+CW*0.62;const dw2=CW*0.38;
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([130,130,130]);
                doc.text(htR2+':',ML,cy);doc.text((lang2.date||'Date')+':',dx2,cy);cy+=3;
                sd(navy);doc.setLineWidth(0.5);doc.line(ML,cy+6,ML+CW*0.54,cy+6);doc.line(dx2,cy+6,dx2+dw2,cy+6);
                doc.setFontSize(9);doc.setFont('helvetica','bold');st(navy);
                if(headmaster2)doc.text(headmaster2,ML+1,cy+5.5);
                if(closingDate2)doc.text(closingDate2,dx2+1,cy+5.5);cy+=8;
                doc.setFontSize(7.5);doc.setFont('helvetica','normal');st([100,100,100]);
                doc.text(headmaster2||'___________________________',ML,cy);doc.text(closingDate2||'_______________',dx2,cy);cy+=3.5;
                doc.setFontSize(7);st([150,150,150]);doc.text(htR2,ML,cy);}
                return true;
            };
...
function renderStudentCertificates(students) {
    ...
    async function _certCapturePdf(filename, action) {
        try {
            ...
            const _siC       = _axpSchoolInfo(currentLang||'en');
            const schoolName = _siC.displayLine1 || '';
            const schoolIdx  = _siC.displayLine2 || '';

            const examClass    = (document.getElementById('axpRRClassSel')||document.getElementById('axpRepClassSel')||{}).value||'';
            const _savedC      = SchoolInfoStorage.getClassData(examClass)||{};
            const _gvC = k => {
                const p=document.getElementById('axpClassInfoPanel');
                if(p){const e=p.querySelector(`[data-field="${k}"]`);if(e&&e.value)return e.value;}
                if(_savedC[k])return _savedC[k];
                const d=document.getElementById(k);if(d&&d.value)return d.value;
                return '';
            };
            const classTeacher = _gvC('classTeacherInput');
            const headmaster   = _gvC('headmasterInput');
            const closingDate  = _gvC('closingDateInput');
            const clsLabel     = _axpClassLabel(examClass, currentLang)||examClass||'';

            // Safe text — jsPDF throws on empty string
            const T = v => (v==null||String(v).trim()==='')?' ':String(v);

            // ── Document: A4 landscape, high precision, no compression ───────
            const doc = new jsPDFCtor({
                unit:'mm', format:'a4', orientation:'landscape',
                compress:false, precision:6, putOnlyUsedFonts:true
            });
            const PW=297, PH=210, cx=PW/2;

            // ── Colour helpers ───────────────────────────────────────────────
            const GOLD =[200,168,75];  // #c8a84b
            const NAVY =[26,58,92];    // #1a3a5c
            const WHITE=[255,255,255];
            const GREY =[153,153,153]; // #999
            const DGREY=[68,68,68];    // #444
            const sf=c=>doc.setFillColor(c[0],c[1],c[2]);
            const sd=c=>doc.setDrawColor(c[0],c[1],c[2]);
            const st=c=>doc.setTextColor(c[0],c[1],c[2]);
            ...
            // ── 2. Outer gold border (3px) ───────────────────────────────────
            sd(GOLD); doc.setLineWidth(1.1); doc.rect(4,4,PW-8,PH-8,'D');

            // ── 3. Inner navy border (1px) ───────────────────────────────────
            sd(NAVY); doc.setLineWidth(0.35); doc.rect(6.5,6.5,PW-13,PH-13,'D');

            // ── 4. Corner ornaments — 12×12mm L-brackets in gold ─────────────
            // Each corner: two lines forming an L (top-left, top-right, bottom-left, bottom-right)
            sd(GOLD); doc.setLineWidth(1.4);
            // top-left
            doc.line(4,4,16,4); doc.line(4,4,4,16);
            // top-right
            doc.line(PW-16,4,PW-4,4); doc.line(PW-4,4,PW-4,16);
            // bottom-left
            doc.line(4,PH-4,16,PH-4); doc.line(4,PH-16,4,PH-4);
            // bottom-right
            doc.line(PW-16,PH-4,PW-4,PH-4); doc.line(PW-4,PH-16,PW-4,PH-4);

            // ── 5. Left & right vertical bars (gradient simulated in 10 steps) ─
            // gold → navy → gold  (top 9mm, bottom 9mm, 3.5mm wide)
            const barX1=9, barX2=PW-12.5, barW=3.5;
            const barTop=9, barBot=PH-9, barH=barBot-barTop;
            const barSteps=20;
            for(let i=0;i<barSteps;i++){
                const t=i/barSteps;
                // gold[0,0]→navy[50%]→gold[100%]
                let r,g,b;
                if(t<0.5){const p=t*2;r=Math.round(200+(26-200)*p);g=Math.round(168+(58-168)*p);b=Math.round(75+(92-75)*p);}
                else{const p=(t-0.5)*2;r=Math.round(26+(200-26)*p);g=Math.round(58+(168-58)*p);b=Math.round(92+(75-92)*p);}
                doc.setFillColor(r,g,b);
                const y=barTop+i*(barH/barSteps);
                const h=barH/barSteps+0.5;
                doc.rect(barX1,y,barW,h,'F');
                doc.rect(barX2,y,barW,h,'F');
            }

            // ── 6. Watermarks (student name, very faint gold, 3 positions) ───
            const wmName = student.name.toUpperCase();
            doc.setGState && doc.setGState(new doc.GState({opacity:0.04}));
            st([200,168,75]);
            doc.setFontSize(28); doc.setFont('helvetica','bold');
            try{ doc.text(wmName,cx,PH/2,{align:'center',angle:25}); }catch(e){}
            doc.setFontSize(15);
            try{ doc.text(wmName,PW*0.3,PH*0.22,{align:'center',angle:25}); }catch(e){}
            try{ doc.text(wmName,PW*0.7,PH*0.75,{align:'center',angle:25}); }catch(e){}
            doc.setGState && doc.setGState(new doc.GState({opacity:1}));

            // ── Content area: padding 10mm top, 17mm left/right, 8mm bottom ──
            const PL=17, PT=10, PR=17, PB=8;
            const CL=PL, CT=PT, CW=PW-PL-PR, CH=PH-PT-PB;
            let cy=CT;

            // ── 7. Header band: logo left | school info centre | logo right ──
            const hdrH=22; // approx height of header block
            // Left placeholder circle (logo)
            const logoR=7, logoX=CL+8, logoY=cy+hdrH/2;
            sf(NAVY); doc.circle(logoX,logoY,logoR,'F');
            st(GOLD); doc.setFontSize(12); doc.setFont('helvetica','bold');
            tx((schoolName||'S')[0].toUpperCase(), logoX, logoY+4, {align:'center'});

            // Right placeholder circle
            const logoX2=CL+CW-8;
            sf(NAVY); doc.circle(logoX2,logoY,logoR,'F');
            st(GOLD); doc.setFontSize(12); doc.setFont('helvetica','bold');
            tx((schoolName||'S')[0].toUpperCase(), logoX2, logoY+4, {align:'center'});

            // Centre school identity
            const centreX=cx;
            doc.setFontSize(5.5); doc.setFont('helvetica','normal'); st([153,153,153]);
            tx('THE UNITED REPUBLIC OF TANZANIA', centreX, cy+4, {align:'center'});
            doc.setFontSize(6); st([136,136,136]);
            tx("PRESIDENT'S OFFICE — REGIONAL ADMINISTRATION AND LOCAL GOVERNMENT", centreX, cy+8, {align:'center'});
            doc.setFontSize(13); doc.setFont('helvetica','bold'); st(NAVY);
            const nameLines = doc.splitTextToSize(schoolName.toUpperCase(), CW-50);
            nameLines.forEach((ln,i)=>tx(ln, centreX, cy+14+i*5, {align:'center'}));
            doc.setFontSize(6); doc.setFont('helvetica','normal'); st([119,119,119]);
            tx(schoolIdx.toUpperCase(), centreX, cy+14+nameLines.length*5, {align:'center'});

            // Separator line below header (gold, 1.5px)
            cy += hdrH + 3;
            sd(GOLD); doc.setLineWidth(0.55);
            doc.line(CL, cy, CL+CW, cy);
            cy += 5;

            // ── 8. Decorative rule above title ───────────────────────────────
            // Simulates:  ————◆—◆—◆————   (gradient lines + diamonds)
            const ruleW=120, ruleX=cx-ruleW/2;
            // Left gradient line (gold fade from centre)
            const lineSteps=12;
            for(let i=0;i<lineSteps;i++){
                ...
                doc.line(ruleX+t*(ruleW/2-8),cy+1,ruleX+(t+1/lineSteps)*(ruleW/2-8),cy+1);
            }
            for(let i=0;i<lineSteps;i++){
                const t=i/lineSteps;
                doc.setDrawColor(200,168,75);
                doc.setLineWidth(0.3);
                doc.line(cx+8+t*(ruleW/2-8),cy+1,cx+8+(t+1/lineSteps)*(ruleW/2-8),cy+1);
            }
            // Diamonds at centre
            sf(GOLD); doc.setFontSize(5);
            [cx-4,cx,cx+4].forEach(dx=>{
                // small diamond via rect rotated — use circle as fallback
                doc.circle(dx,cy+1,1,'F');
            });
            cy+=4;

            // ── 9. "Presented to" sub-label ──────────────────────────────────
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([153,153,153]);
            tx('— ' + (lang.presentedTo||'This certificate is proudly presented to') + ' —', cx, cy+3, {align:'center'});
            cy+=6;

            // ── 10. CERTIFICATE title ─────────────────────────────────────────
            doc.setFontSize(20); doc.setFont('helvetica','bold'); st(NAVY);
            tx(lang.certificate||'CERTIFICATE OF COMPLETION', cx, cy+4, {align:'center'});
            cy+=8;

            // Gradient underline bar (50mm wide, gold → transparent → gold)
            const barUW=50, barUX=cx-barUW/2, barUY=cy;
            const uSteps=20;
            for(let i=0;i<uSteps;i++){
                const t=i/uSteps;
                const alpha=t<0.5?t*2:(1-t)*2;
                const r=Math.round(200*alpha+255*(1-alpha));
                const g=Math.round(168*alpha+255*(1-alpha));
                const b=Math.round(75*alpha+255*(1-alpha));
                doc.setFillColor(r,g,b);
                doc.rect(barUX+i*(barUW/uSteps),barUY,barUW/uSteps+0.3,0.9,'F');
            }
            cy+=4;

            // ── 11. Recipient block (centred) ─────────────────────────────────
            // "presented to" italic label
            doc.setFontSize(7.5); doc.setFont('helvetica','normal'); st([187,187,187]);
            tx(lang.presentedTo||'This certificate is proudly presented to', cx, cy+4, {align:'center'});
            cy+=8;

            // Student name (large)
            doc.setFontSize(22); doc.setFont('helvetica','bold'); st(NAVY);
            tx(student.name, cx, cy+5, {align:'center'});
            cy+=10;

            // Name underline (decorative rule — same as title rule)
            const nrW=doc.getTextWidth(T(student.name))+20;
            sd(GOLD); doc.setLineWidth(0.55);
            doc.line(cx-nrW/2, cy, cx+nrW/2, cy);
            // Star accent centre
            sf(GOLD); doc.circle(cx,cy,1,'F');
            cy+=5;

            // Recognition text + school line
            doc.setFontSize(9); doc.setFont('helvetica','italic'); st([68,68,68]);
            tx(lang.recognition||'For successfully completing the academic program', cx, cy+3, {align:'center'});
            cy+=6;
            doc.setFontSize(8); doc.setFont('helvetica','normal'); st([136,136,136]);
            const conductedTxt=(lang.conducted||'At')+' ';
            const conductedW=doc.getTextWidth(conductedTxt);
            tx(conductedTxt, cx-conductedW/2-doc.getTextWidth(schoolName.toUpperCase())/2, cy+3);
            doc.setFont('helvetica','bold'); st(NAVY);
            tx(schoolName.toUpperCase(), cx-conductedW/2-doc.getTextWidth(schoolName.toUpperCase())/2+conductedW, cy+3);
            cy+=6;

            // Class label (if present)
            if(clsLabel&&clsLabel.trim()){
                doc.setFontSize(9); doc.setFont('helvetica','bold'); st(NAVY);
                tx(clsLabel, cx, cy+3, {align:'center'});
                cy+=6;
            }

            // Points / Division badge
            const bdW=80, bdH=9, bdX=cx-bdW/2, bdY=cy;
            sf([248,244,215]); sd(GOLD); doc.setLineWidth(0.4);
            doc.rect(bdX,bdY,bdW,bdH,'FD');
            doc.setFontSize(8); doc.setFont('helvetica','bold'); st(NAVY);
            const ptLbl =currentLang==='sw'?'Pointi':'Points';
            const divLbl=currentLang==='sw'?'Daraja':'Division';
            tx(ptLbl+': '+T(student.point)+'     '+divLbl+': '+T(student.division), cx, bdY+6, {align:'center'});
            cy+=bdH+4;

            // ── 12. Signature row ─────────────────────────────────────────────
            // Gold separator line above signatures
            const sigTop=PH-PB-28; // pin to ~28mm from bottom
            cy=sigTop;
            sd(GOLD); doc.setLineWidth(0.55);
            doc.line(CL,cy,CL+CW,cy);
            cy+=5;

            // Columns: CT left (flex:1 = CW*0.35), Seal centre (CW*0.3), HT right (CW*0.35)
            const colW=CW*0.32, colCTx=CL+CW*0.04, colHTx=CL+CW*0.64;
            const sealCx=cx;

            // ─ Class Teacher block ─
            // Signature underline (dark grey, 38mm wide, centred in column)
            const ctCX=colCTx+colW/2;
            const sigLineW=38, sigLineY=cy+7;
            sd([51,51,51]); doc.setLineWidth(0.55);
            doc.line(ctCX-sigLineW/2, sigLineY, ctCX+sigLineW/2, sigLineY);
            // Name on the line (bold, dark)
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            if(classTeacher) tx(classTeacher, ctCX, sigLineY-0.5, {align:'center'});
            // Name fallback text below
            doc.setFontSize(8); doc.setFont('helvetica','bold'); st([34,34,34]);
            tx(classTeacher||'_______________', ctCX, sigLineY+4, {align:'center'});
            // Role label
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([136,136,136]);
            tx(lang.classTeacher||'Class Teacher', ctCX, sigLineY+8, {align:'center'});
            // Date
            doc.setFontSize(6); st([170,170,170]);
            tx((lang.date||'Date')+': '+(closingDate||'—'), ctCX, sigLineY+12, {align:'center'});

            // ─ Official Seal (centre) ─
            // Double circle
            sd(GOLD); doc.setLineWidth(0.7); doc.circle(sealCx, cy+10, 9,'D');
            sd(GOLD); doc.setLineWidth(0.3); doc.circle(sealCx, cy+10, 7,'D');
            // Check icon simulation (star)
            sf(GOLD); doc.circle(sealCx,cy+8,1.5,'F');
            sf(GOLD); doc.circle(sealCx-2,cy+11,0.8,'F');
            sf(GOLD); doc.circle(sealCx+2,cy+11,0.8,'F');
            // Seal text
            doc.setFontSize(4); doc.setFont('helvetica','bold'); st(NAVY);
            tx(lang.seal||'OFFICIAL SEAL', sealCx, cy+16, {align:'center'});
            // School abbreviation inside seal
            const sealAbbr=schoolName.split(' ').slice(0,2).map(w=>w[0]||'').join('');
            doc.setFontSize(7); doc.setFont('helvetica','bold'); st(NAVY);
            tx(sealAbbr, sealCx, cy+11.5, {align:'center'});

            // ─ Head Teacher block ─
            const htCX=colHTx+colW/2;
            sd([51,51,51]); doc.setLineWidth(0.55);
            doc.line(htCX-sigLineW/2, sigLineY, htCX+sigLineW/2, sigLineY);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            if(headmaster) tx(headmaster, htCX, sigLineY-0.5, {align:'center'});
            doc.setFontSize(8); doc.setFont('helvetica','bold'); st([34,34,34]);
            tx(headmaster||'_______________', htCX, sigLineY+4, {align:'center'});
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([136,136,136]);
            tx(lang.headTeacher||'Head of School', htCX, sigLineY+8, {align:'center'});
            doc.setFontSize(6); st([170,170,170]);
            tx((lang.date||'Date')+': '+(closingDate||'—'), htCX, sigLineY+12, {align:'center'});
...
function _axpInjectFeedStyles() {
  if (document.getElementById("axpFeedStyles")) return;
  var s = document.createElement("style");
  s.id = "axpFeedStyles";
  s.textContent = `
 
  /* ── Chip/pill selector button ── */
  .axp-chip {
    display: inline-flex;
    align-items: center;
    gap: 6px;
  ...
  .axp-chip.axp-chip-active {
    background: #060c1c;
    border-color: #4ecca3;
    color: #4ecca3;
    box-shadow: 0 0 0 1px #4ecca3 inset;
  }
  .axp-chip.axp-chip-active .axp-chip-icon {
    color: #4ecca3;
  }
  ...
  .axp-step-block.axp-step-done {
    border-left-color: #4ecca3;
  }
  .axp-step-block.axp-step-locked {
    opacity: .45;
    pointer-events: none;
  }
  ...
  .axp-load-btn {
    ...
    gap: 8px;
    padding: 12px 24px;
    background: linear-gradient(135deg,#060c1c,#0f2248);
    color: #4ecca3;
    font-size: 13px;
    font-weight: 800;
  ...
  .axp-load-btn:disabled {
    opacity: .5;
    cursor: not-allowed;
  }
 
  /* Modal keyframe */
  @keyframes axpModalIn {
    from { opacity:0; transform:scale(.93) }
    to   { opacity:1; transform:scale(1)   }
  }
  @keyframes axpChipPop {
    0%   { transform: scale(1) }
    40%  { transform: scale(1.12) }
    100% { transform: scale(1) }
  }
  .axp-chip-active { animation: axpChipPop .2s ease; }
  ...
  .axp-table tbody tr:nth-child(even) {
    background: #f8fafc;
  }
  `;
  document.head.appendChild(s);
...
/* ═══════════════════════════════════════════════════════════
   MODAL POPUP SYSTEM  (replaces alert / confirm)
═══════════════════════════════════════════════════════════ */
function _axpInjectModal() {
  if (document.getElementById("axpModalShell")) return;
  var shell = document.createElement("div");
  shell.id = "axpModalShell";
  shell.style.cssText = `
  ...
  shell.innerHTML = `
    <div id="axpModalBox" style="
      background:#fff;max-width:460px;width:100%;
      box-shadow:0 28px 90px rgba(6,12,28,.45);
      animation:axpModalIn .18s ease;overflow:hidden;
      position:relative;z-index:2147483647;">
      <div id="axpModalHead" style="
        background:linear-gradient(135deg,#060c1c,#0f2248);
        padding:16px 20px;display:flex;align-items:center;gap:10px;">
        <div id="axpModalIcon" style="font-size:22px;"></div>
        <div id="axpModalTitle" style="font-size:15px;font-weight:800;color:#fff;flex:1;"></div>
        <button onclick="axpModalClose()" style="
          background:none;border:none;color:rgba(255,255,255,.45);
          font-size:22px;cursor:pointer;line-height:1;padding:0;">×</button>
      </div>
      <div id="axpModalBody" style="padding:20px 22px;font-size:13.5px;color:#1a1a2e;line-height:1.75;"></div>
      <div id="axpModalFoot" style="padding:10px 22px 18px;display:flex;gap:8px;justify-content:flex-end;"></div>
    </div>`;
  document.body.appendChild(shell);
}
 
window._axpModalCb = null;
window.axpModalClose = function() {
  const s = document.getElementById("axpModalShell");
  if (s) s.style.display = "none";
  window._axpModalCb = null;
};
 
function _axpShowModal({ icon, iconColor, title, body, buttons }) {
  _axpInjectModal(); // ensure shell exists before trying to get elements
  const get = id => document.getElementById(id);
  get("axpModalIcon").innerHTML   = icon  || "";
  get("axpModalIcon").style.color = iconColor || "#4ecca3";
  get("axpModalTitle").textContent = title || "";
  get("axpModalBody").innerHTML    = body  || "";
  get("axpModalFoot").innerHTML    = (buttons||[]).map(b =>
    `<button onclick="${b.action}" class="${b.cls||"axp-btn-secondary"}"
       style="${b.style||""}">${b.label}</button>`).join("");
  const s = document.getElementById("axpModalShell");
  s.style.display = "flex";
}
 
window.axpAlert = function(title, bodyHtml, type = "info") {
  const map = {
    info   : { icon:"<i class='bi bi-info-circle-fill'></i>",    color:"#4ecca3" },
    success: { icon:"<i class='bi bi-check-circle-fill'></i>",   color:"#10b981" },
    danger : { icon:"<i class='bi bi-x-octagon-fill'></i>",      color:"#ef4444" },
    warning: { icon:"<i class='bi bi-exclamation-triangle-fill'></i>", color:"#f59e0b" }
  };
  const t = map[type] || map.info;
  _axpShowModal({ icon:t.icon, iconColor:t.color, title, body:bodyHtml,
    buttons:[{ label:"OK", cls:"axp-btn-primary", action:"axpModalClose()" }] });
};
 
window.axpConfirm = function(title, bodyHtml, onYes, yesLabel="Yes, Proceed", yesStyle="") {
  window._axpModalCb = onYes;
  _axpShowModal({ icon:"<i class='bi bi-question-circle-fill'></i>", iconColor:"#f59e0b",
    title, body:bodyHtml,
    buttons:[
      { label:"Cancel",   cls:"axp-btn-secondary", action:"axpModalClose()" },
      { label:yesLabel,   cls:"axp-btn-primary",
        style: yesStyle||"background:#10b981;",
        action:"axpModalClose();if(window._axpModalCb){window._axpModalCb();window._axpModalCb=null;}" }
    ] });
};
 
window._axpToast = window._axpToast || function(msg, type) {
  const colors = { success:"#10b981", danger:"#ef4444", warning:"#f59e0b", info:"#4ecca3" };
  const t = document.createElement("div");
  t.style.cssText = `position:fixed;bottom:24px;right:24px;z-index:999999999999;
    background:${colors[type]||"#4ecca3"};color:#fff;
    padding:11px 20px;font-size:13px;font-weight:700;
    box-shadow:0 8px 28px rgba(0,0,0,.3);animation:axpModalIn .2s ease;`;
  t.textContent = msg;
...
═══════════════════════════════════════════════════════════ */
function _axpGrade(score) {
  if (score===undefined||score===null||score==="") return { grade:"—", color:"#94a3b8" };
  const n = parseInt(score);
  if (isNaN(n)) return { grade:"—", color:"#94a3b8" };
  if (n>=75) return { grade:"A", color:"#10b981" };
  if (n>=65) return { grade:"B", color:"#4ecca3" };
  if (n>=45) return { grade:"C", color:"#f59e0b" };
  if (n>=30) return { grade:"D", color:"#f97316" };
  return { grade:"F", color:"#ef4444" };
}
 
function _axpCandNo(idx) {
  const sid = String((_dashboardData&&_dashboardData.schoolindex)||"0000");
  return `${sid}-${String(idx+1).padStart(4,"0")}`;
}
 
/* ── Gender-aware alphabetical sort ──────────────────────────────────
   Females (F) first, sorted A→Z. Then males, sorted A→Z.
   Example: AZIZA, MARIAM, ZUBEDA, ABDUL, EMANUEL
─────────────────────────────────────────────────────────────────── */
function _axpSortStudents(arr) {
...
function _axpSetText(id, val) {
  const el = document.getElementById(id); if (el) el.textContent = val;
}
 
 
/* ═══════════════════════════════════════════════════════════
   TAP-SELECTOR BUILDER
   Creates a step block with tap-to-select chips.
   onSelect(value) is called when a chip is tapped.
═══════════════════════════════════════════════════════════ */
/* Callback registry — keyed by containerId */
var _axpSelectorCbs = {};
 
function _axpBuildSelector({ containerId, stepNo, label, items, onSelect,
                              iconFn, locked, selectedValue }) {
  var wrap = document.getElementById(containerId);
  if (!wrap) return;
 
  /* Store callback in registry */
  if (onSelect) _axpSelectorCbs[containerId] = onSelect;
 
  wrap.className = "axp-step-block" +
    (locked ? " axp-step-locked" : "") +
    (selectedValue ? " axp-step-done" : "");
 
  var chipsHtml;
  if (!items || items.length === 0) {
    chipsHtml = '<span style="font-size:12px;color:#94a3b8;padding:4px 0;">No options available</span>';
  } else {
    chipsHtml = items.map(function(item) {
      var val  = typeof item === "string" ? item : item.value;
      var disp = typeof item === "string" ? item : item.label;
      var icon = iconFn ? iconFn(val) : "";
      var active = val === selectedValue;
      /* Use data attributes — no inline JS string escaping issues */
      return '<button class="axp-chip' + (active ? " axp-chip-active" : "") + '"' +
        ' data-container="' + containerId + '"' +
        ' data-val="' + escapeHtml(val) + '"' +
        ' data-step="' + stepNo + '"' +
        ' onclick="_axpChipClick(this)">' +
        (icon ? '<i class="' + icon + ' axp-chip-icon"></i>' : '') +
        escapeHtml(disp) +
        (active ? '<i class="bi bi-check2" style="margin-left:2px;"></i>' : '') +
        '</button>';
    }).join("");
  }
 
  wrap.innerHTML =
    '<div class="axp-step-head">' +
      '<div class="axp-step-no">' + stepNo + '</div>' +
      '<div class="axp-step-label">' + label + '</div>' +
      (selectedValue
        ? '<div class="axp-step-value"><i class="bi bi-check2" style="color:#4ecca3;margin-right:4px;"></i>' + escapeHtml(selectedValue) + '</div>'
        : '<div style="font-size:11px;color:#94a3b8;margin-left:auto;">tap to select</div>') +
    '</div>' +
    '<div class="axp-chip-row" id="' + containerId + '_chips">' + chipsHtml + '</div>';
}
 
window._axpChipClick = function(btn) {
  var containerId = btn.getAttribute("data-container");
  var val         = btn.getAttribute("data-val");
  var stepNo      = btn.getAttribute("data-step");
  var wrap        = document.getElementById(containerId);
  if (!wrap) return;
 
  /* Deactivate siblings */
  wrap.querySelectorAll(".axp-chip").forEach(function(c) {
    c.classList.remove("axp-chip-active");
    /* Remove any existing check icon */
    var chk = c.querySelector(".bi-check2:not(.axp-chip-icon)");
    if (chk) chk.remove();
  });
 
  /* Activate clicked */
  btn.classList.add("axp-chip-active");
  var chkI = document.createElement("i");
  chkI.className = "bi bi-check2";
  chkI.style.marginLeft = "2px";
  btn.appendChild(chkI);
 
  /* Update step header value badge */
  var head = wrap.querySelector(".axp-step-head");
  if (head) {
    var old = head.querySelector(".axp-step-value") ||
              head.querySelector("div[style*='tap to select']");
    if (old) {
      old.className = "axp-step-value";
      old.innerHTML = '<i class="bi bi-check2" style="color:#4ecca3;margin-right:4px;"></i>' + escapeHtml(val);
    }
  }
  wrap.classList.add("axp-step-done");
  wrap.classList.remove("axp-step-locked");
 
  /* Fire callback from registry */
  var cb = _axpSelectorCbs[containerId];
  if (cb) cb(val);
};
...
═══════════════════════════════════════════════════════════ */
function renderTeacherFeedingSection() {
  _axpInjectFeedStyles();
  _axpInjectModal();
  var sw = document.getElementById("axpSectionWrapper");
 
  if (_dashboardData && _dashboardData.status !== "ACTIVE") {
    sw.innerHTML = _axpInactiveBlock(); return;
  }
  if (!_appScriptSchoolId) {
    sw.innerHTML = `
      <div class="axp-section-card" style="text-align:center;padding:40px 24px;">
        ...
        <h3 style="font-size:16px;font-weight:800;color:#1a1a2e;margin:0 0 8px;">Feature Locked</h3>
        <p style="font-size:13px;color:#64748b;max-width:360px;margin:0 auto 20px;line-height:1.6;">
          Configure your school first to unlock Marks Feeding.</p>
        <button onclick="axpOpenSetupPopup()" class="axp-btn-primary">
          <i class="bi bi-gear-fill"></i> Setup School Now</button>
      </div>`; return;
  }
  ...
  window._axpFeed = {
    students:[], teacher:null, marks:{},
    currentIdx:0, cls:"", examType:"", subject:"", submitted:false
  };
  ...
  sw.innerHTML = `
    <div class="axp-section-card" style="padding:0;">
      ...
      <!-- Header band -->
      <div style="background:linear-gradient(135deg,#060c1c,#0f2248);padding:18px 22px;color:#fff;">
        <div style="display:flex;align-items:center;gap:10px;">
          <div style="width:38px;height:38px;background:rgba(78,204,163,.15);
                      display:flex;align-items:center;justify-content:center;">
            <i class="bi bi-pencil-fill" style="font-size:18px;color:#4ecca3;"></i>
          </div>
          <div>
            <div style="font-size:16px;font-weight:800;letter-spacing:.3px;">Marks Feeding</div>
            <div style="font-size:11.5px;opacity:.5;">Enter exam marks per class, subject, and exam type</div>
          </div>
        </div>
      </div>
 
      <!-- Tab bar -->
      <div style="display:flex;background:#fff;border-bottom:2px solid #e2e8f0;overflow-x:auto;">
        <button onclick="axpFeedTab('entry')" id="axpFeedTabEntry"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#4ecca3;border-bottom:3px solid #4ecca3;margin-bottom:-2px;cursor:pointer;white-space:nowrap;">
          <i class="bi bi-person-badge"></i> One by One
        </button>
        <button onclick="axpFeedTab('bulk')" id="axpFeedTabBulk"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#94a3b8;border-bottom:3px solid transparent;margin-bottom:-2px;cursor:pointer;white-space:nowrap;">
          <i class="bi bi-table"></i> Bulk Entry
        </button>
        <button onclick="axpFeedTab('correction')" id="axpFeedTabCorrection"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#94a3b8;border-bottom:3px solid transparent;margin-bottom:-2px;cursor:pointer;white-space:nowrap;">
          <i class="bi bi-pencil-square" style="color:#f59e0b;"></i> Corrections
        </button>
        <button onclick="axpFeedTab('print')" id="axpFeedTabPrint"
          style="padding:12px 20px;border:none;background:none;font-size:13px;font-weight:700;
                 color:#94a3b8;border-bottom:3px solid transparent;margin-bottom:-2px;cursor:pointer;white-space:nowrap;">
          <i class="bi bi-printer-fill" style="color:#6366f1;"></i> Print Sheets
        </button>
      </div>
 
      <!-- ══════════ PRINT SHEETS TAB ══════════ -->
      <div id="axpFeedPanelPrint" style="display:none;padding:20px;background:#f8fafc;">
 
        <div style="background:linear-gradient(135deg,#0f172a,#1e1b4b);color:#fff;
                    padding:16px 20px;margin-bottom:20px;">
          <div style="display:flex;align-items:center;gap:10px;">
            <i class="bi bi-printer-fill" style="font-size:20px;color:#a5b4fc;"></i>
            <div>
              <div style="font-size:14px;font-weight:800;">Print Sheets Generator</div>
              <div style="font-size:11.5px;opacity:.5;">
                Generate mark-entry sheets (per subject) and general attendance sheets (all subjects)</div>
            </div>
          </div>
        </div>
 
        <!-- Wizard -->
        <div id="axpPrintWizard" style="margin-bottom:18px;">
          <div id="axpPrintStepExam"></div>
          <div id="axpPrintStepClass"></div>
          <div id="axpPrintLoadWrap" style="display:none;padding-top:4px;">
            <button class="axp-load-btn" id="axpPrintLoadBtn"
              style="border-color:#a5b4fc;color:#a5b4fc;"
              onclick="axpPrintLoadRoster()">
              <i class="bi bi-cloud-download"></i> Load Class Roster
            </button>
          </div>
        </div>
 
        <!-- Roster loaded panel -->
        <div id="axpPrintRosterPanel" style="display:none;">
 
          <!-- Info strip -->
          <div id="axpPrintInfoStrip" style="background:#f8fafc;padding:12px 16px;
               margin-bottom:18px;display:flex;gap:16px;flex-wrap:wrap;align-items:center;">
          </div>
 
          <!-- Two download cards -->
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:18px;">
 
            <!-- Card 1: Subject mark sheet -->
            <div style="background:#fff;border:1.5px solid #e2e8f0;padding:20px;">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                <div style="width:36px;height:36px;background:#f0fdf9;
                            display:flex;align-items:center;justify-content:center;">
                  <i class="bi bi-file-earmark-ruled" style="font-size:18px;color:#4ecca3;"></i>
                </div>
                <div>
                  <div style="font-size:13px;font-weight:800;color:#1a1a2e;">Subject Mark Sheet</div>
                  <div style="font-size:11px;color:#94a3b8;">One PDF per subject — mark entry columns</div>
                </div>
              </div>
              <div style="font-size:12px;color:#64748b;margin-bottom:14px;line-height:1.6;">
                Each subject gets its own sheet with candidate no., name, gender, a blank score
                column and a signature column. Select subjects to include:
              </div>
              <div id="axpPrintSubjectCheckboxes"
                style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px;
                       max-height:150px;overflow-y:auto;"></div>
              <button onclick="axpPrintSubjectSheets()" class="axp-btn-primary"
                style="width:100%;justify-content:center;" id="axpPrintSubjectBtn">
                <i class="bi bi-download"></i> Download Subject Sheets
              </button>
              <div id="axpPrintSubjectResult"></div>
            </div>
 
            <!-- Card 2: General attendance/overview sheet -->
            <div style="background:#fff;border:1.5px solid #e2e8f0;padding:20px;">
              <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
                <div style="width:36px;height:36px;background:#f0f0ff;
                            display:flex;align-items:center;justify-content:center;">
                  <i class="bi bi-file-earmark-check" style="font-size:18px;color:#6366f1;"></i>
                </div>
                <div>
                  <div style="font-size:13px;font-weight:800;color:#1a1a2e;">General Overview Sheet</div>
                  <div style="font-size:11px;color:#94a3b8;">All subjects in one — sign-off sheet</div>
                </div>
              </div>
              <div style="font-size:12px;color:#64748b;margin-bottom:14px;line-height:1.6;">
                One sheet listing every student with a column per subject.
                Subjects the student is enrolled in show <strong>✓</strong>;
                unenrolled cells are blank. Final column is for signature.
              </div>
              <div style="background:#f8fafc;padding:10px 12px;margin-bottom:14px;font-size:12px;color:#64748b;">
                <i class="bi bi-info-circle" style="color:#6366f1;"></i>
                Covers all subjects for the selected class and exam.
              </div>
              <button onclick="axpPrintGeneralSheet()" class="axp-btn-primary"
                style="width:100%;justify-content:center;background:#6366f1;border-color:#6366f1;"
                id="axpPrintGeneralBtn">
                <i class="bi bi-download"></i> Download General Sheet
              </button>
              <div id="axpPrintGeneralResult"></div>
            </div>
          </div>
 
          <div id="axpPrintMsg" style="margin-top:8px;"></div>
        </div>
 
        <div id="axpPrintEmptyState">
          <div class="axp-empty-state" style="padding:50px 16px;">
            <i class="bi bi-printer" style="font-size:40px;color:#6366f1;"></i>
            <p>Choose Exam Type and Class above — then tap <strong>Load Class Roster</strong>.</p>
          </div>
        </div>
      ...
      <!-- ══════════ ONE BY ONE TAB ══════════ -->
      <div id="axpFeedPanelEntry" style="padding:20px;">
 
        <!-- Wizard selector -->
        <div id="axpFeedWizard" style="margin-bottom:18px;">
          <div id="axpFeedStepExam"></div>
          <div id="axpFeedStepClass"></div>
          <div id="axpFeedStepSubject"></div>
          <div id="axpFeedLoadWrap" style="display:none;padding-top:4px;">
            <button class="axp-load-btn" id="axpFeedLoadBtn" onclick="axpFeedLoadRoster()">
              <i class="bi bi-cloud-download"></i> Load Roster
            </button>
          </div>
        </div>
 
        <div id="axpFeedTeacherBanner" style="display:none;margin-bottom:14px;"></div>
 
        <!-- Progress bar -->
        <div id="axpFeedProgressWrap" style="display:none;background:#060c1c;
             padding:12px 16px;margin-bottom:14px;color:#fff;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
            <span style="font-size:12px;opacity:.6;" id="axpFeedProgressLabel">0 of 0 students</span>
            <span style="font-size:20px;font-weight:800;color:#4ecca3;" id="axpFeedProgressPct">0%</span>
          </div>
          <div class="axp-progress-bar" style="height:6px;background:rgba(255,255,255,.1);">
            <div class="axp-progress-fill" id="axpFeedProgressFill"
                 style="width:0%;transition:width .4s;"></div>
          </div>
          <div style="font-size:11px;opacity:.4;margin-top:4px;" id="axpFeedProgressSublabel"></div>
        </div>
 
        <!-- Student card -->
        <div id="axpFeedStudentCard" style="display:none;">
          <div style="border-left:4px solid #4ecca3;padding:16px 18px;
                      background:#f0fdf9;margin-bottom:16px;">
            <div style="display:flex;align-items:center;gap:0;flex-wrap:wrap;">
              <div style="min-width:130px;padding-right:18px;border-right:2px solid #d1fae5;margin-right:18px;">
                <div style="font-size:9.5px;font-weight:700;color:#4ecca3;
                            text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">
                  Candidate No.</div>
                <div style="font-size:14px;font-weight:900;color:#060c1c;
                            letter-spacing:1px;font-family:monospace;" id="axpFeedCardCandNo">—</div>
              </div>
              <div style="flex:1;min-width:140px;padding-right:18px;
                          border-right:2px solid #d1fae5;margin-right:18px;">
                <div style="font-size:9.5px;font-weight:700;color:#94a3b8;
                            text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">
                  Student Name</div>
                <div style="font-size:16px;font-weight:700;color:#1a1a2e;" id="axpFeedCardName">—</div>
              </div>
              <div style="min-width:60px;padding-right:18px;
                          border-right:2px solid #d1fae5;margin-right:18px;">
                <div style="font-size:9.5px;font-weight:700;color:#94a3b8;
                            text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">Sex</div>
                <div style="font-size:14px;font-weight:700;color:#1a1a2e;" id="axpFeedCardGender">—</div>
              </div>
              <div style="min-width:80px;padding-right:18px;
                          border-right:2px solid #d1fae5;margin-right:18px;">
                <div style="font-size:9.5px;font-weight:700;color:#94a3b8;
                            text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">
                  Prev. Mark</div>
                <div style="font-size:14px;font-weight:700;" id="axpFeedPrevMark">—</div>
              </div>
              <div style="text-align:center;margin-left:auto;">
                <div style="font-size:9.5px;font-weight:700;color:#94a3b8;
                            text-transform:uppercase;letter-spacing:.5px;margin-bottom:3px;">Grade</div>
                <div style="font-size:40px;font-weight:900;line-height:1;"
                     id="axpFeedGradeBadge">—</div>
              </div>
            </div>
          </div>
 
          <div style="margin-bottom:16px;">
            <label style="font-size:10px;font-weight:700;color:#555;text-transform:uppercase;
                          letter-spacing:.5px;display:block;margin-bottom:6px;">
              Score (0 – 100)
            </label>
            <div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;">
              <input id="axpFeedScoreInput" type="number" min="0" max="100"
                class="axp-input"
                style="font-size:30px;font-weight:900;text-align:center;
                       letter-spacing:2px;width:150px;padding:12px;"
                placeholder="—"
                oninput="axpFeedOnScoreInput(this.value)"
                onkeydown="if(event.key==='Enter'||event.key==='ArrowRight') axpFeedNext();
                           if(event.key==='ArrowLeft') axpFeedPrev();" />
              <div style="font-size:12px;color:#64748b;line-height:2.2;">
                <span style="color:#10b981;font-weight:700;">A</span> ≥75 &nbsp;
                <span style="color:#4ecca3;font-weight:700;">B</span> ≥65 &nbsp;
                <span style="color:#f59e0b;font-weight:700;">C</span> ≥45 &nbsp;
          ...
          <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
            <button onclick="axpFeedPrev()" class="axp-btn-secondary" id="axpFeedPrevBtn">
              <i class="bi bi-chevron-left"></i> Prev
            </button>
            <button onclick="axpFeedNext()" class="axp-btn-primary" id="axpFeedNextBtn">
              Next <i class="bi bi-chevron-right"></i>
            </button>
            <button onclick="axpFeedSkip()" class="axp-btn-secondary"
              style="color:#f59e0b;border-color:#f59e0b;">
              <i class="bi bi-skip-forward"></i> Skip
            </button>
            <button onclick="axpFeedSubmitAll()" class="axp-btn-primary"
              style="background:#10b981;margin-left:auto;" id="axpFeedSubmitBtn">
              <i class="bi bi-send-fill"></i> Submit All
            </button>
          </div>
          <div id="axpFeedEntryMsg" style="margin-top:10px;"></div>
        </div>
 
        <div id="axpFeedSummary" style="display:none;margin-top:18px;"></div>
 
        <div id="axpFeedEmptyState">
          <div class="axp-empty-state" style="padding:40px 16px;">
            <i class="bi bi-person-badge" style="font-size:40px;"></i>
            <p>Choose Exam Type, Class and Subject above — then tap <strong>Load Roster</strong>.</p>
          </div>
        </div>
      </div>
 
      <!-- ══════════ BULK TAB ══════════ -->
      <div id="axpFeedPanelBulk" style="display:none;padding:20px;">
 
        <div id="axpBulkWizard" style="margin-bottom:16px;">
          <div id="axpBulkStepExam"></div>
          <div id="axpBulkStepClass"></div>
          <div id="axpBulkStepSubject"></div>
          <div id="axpBulkLoadWrap" style="display:none;padding-top:4px;">
            <button class="axp-load-btn" id="axpBulkLoadBtn" onclick="axpBulkLoadRoster()">
              <i class="bi bi-cloud-download"></i> Load Students
            </button>
          </div>
        </div>
 
        <div id="axpBulkTeacherBanner" style="display:none;margin-bottom:14px;"></div>
 
        <!-- Mode bar -->
        <div id="axpBulkModeBar" style="display:none;margin-bottom:16px;">
          <div style="display:flex;gap:0;border:1.5px solid #e2e8f0;overflow:hidden;width:fit-content;">
            <button onclick="axpBulkMode('grid')" id="axpBulkModeGrid"
              style="padding:9px 16px;border:none;background:#060c1c;color:#4ecca3;
                     font-size:12.5px;font-weight:700;cursor:pointer;border-right:1.5px solid #e2e8f0;">
              <i class="bi bi-grid-3x3-gap"></i> Grid
            </button>
            <button onclick="axpBulkMode('paste')" id="axpBulkModePaste"
              style="padding:9px 16px;border:none;background:#f8fafc;color:#64748b;
                     font-size:12.5px;font-weight:700;cursor:pointer;border-right:1.5px solid #e2e8f0;">
              <i class="bi bi-clipboard2-data"></i> Paste
            </button>
            <button onclick="axpBulkMode('excel')" id="axpBulkModeExcel"
              style="padding:9px 16px;border:none;background:#f8fafc;color:#64748b;
                     font-size:12.5px;font-weight:700;cursor:pointer;border-right:1.5px solid #e2e8f0;">
              <i class="bi bi-file-earmark-spreadsheet"></i> Excel
            </button>
            <button onclick="axpBulkMode('scan')" id="axpBulkModeScan"
              style="padding:9px 16px;border:none;background:#f8fafc;color:#64748b;
                     font-size:12.5px;font-weight:700;cursor:pointer;">
              <i class="bi bi-camera-fill"></i> Scan
            </button>
          </div>
          <span style="font-size:11.5px;color:#94a3b8;display:block;margin-top:6px;"
                id="axpBulkModeHint"></span>
        </div>
 
        <!-- Grid panel -->
        <div id="axpBulkPanelGrid" style="display:none;">
          <div style="display:flex;gap:12px;flex-wrap:wrap;background:#f8fafc;
                      padding:10px 14px;margin-bottom:12px;align-items:center;">
            <span style="font-size:12px;color:#64748b;">
              <i class="bi bi-people-fill" style="color:#4ecca3;"></i>
              <strong id="axpBulkTotal">0</strong> students
            </span>
            <span style="font-size:12px;color:#64748b;">
              <i class="bi bi-pencil-fill" style="color:#10b981;"></i>
              <strong id="axpBulkFilled">0</strong> filled
            </span>
            <span style="font-size:12px;color:#f59e0b;">
              <i class="bi bi-dash-circle"></i>
              <strong id="axpBulkEmpty">0</strong> empty
            </span>
            <div style="flex:1;min-width:80px;">
              <div class="axp-progress-bar" style="height:5px;">
                <div class="axp-progress-fill" id="axpBulkProgressFill"
                     style="width:0%;transition:width .3s;"></div>
              </div>
            </div>
            <strong style="font-size:15px;color:#4ecca3;" id="axpBulkPct">0%</strong>
          </div>
          <div style="overflow-x:auto;">
            <table class="axp-table" style="min-width:460px;">
              <thead>
                <tr>
                  <th style="width:120px;">Cand. No.</th>
                  <th>Name</th>
              ...
              <tbody id="axpBulkTableBody"></tbody>
            </table>
          </div>
          <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;">
            <button onclick="axpBulkSubmitAll()" class="axp-btn-primary"
              style="background:#10b981;" id="axpBulkSubmitBtn">
              <i class="bi bi-send-fill"></i> Submit All Marks
            </button>
            <button onclick="axpBulkClearAll()" class="axp-btn-secondary">
              <i class="bi bi-eraser"></i> Clear All
            </button>
          </div>
          <div id="axpBulkMsg" style="margin-top:10px;"></div>
        </div>
 
        <!-- Paste panel -->
        <div id="axpBulkPanelPaste" style="display:none;">
          <div class="axp-alert axp-alert-info" style="margin-bottom:12px;">
            <i class="bi bi-info-circle"></i>
            <span>Paste tab/comma-separated data from any spreadsheet.
            Each row: <code style="background:rgba(0,0,0,.06);padding:1px 5px;">
            Candidate No. or Name [tab/comma] Score</code></span>
          </div>
          <textarea id="axpBulkPasteArea"
            style="width:100%;height:220px;font-family:monospace;font-size:13px;
                   padding:12px;border:1.5px solid #e2e8f0;resize:vertical;
                   box-sizing:border-box;outline:none;"
            placeholder="S0001-0001&#9;78&#10;S0001-0002&#9;64&#10;…"></textarea>
          <div style="margin-top:10px;display:flex;gap:8px;">
            <button onclick="axpBulkParsePaste()" class="axp-btn-primary">
              <i class="bi bi-check2-square"></i> Parse & Preview
            </button>
            <button onclick="document.getElementById('axpBulkPasteArea').value=''"
              class="axp-btn-secondary"><i class="bi bi-eraser"></i> Clear</button>
          </div>
          <div id="axpBulkPastePreview" style="margin-top:14px;"></div>
        </div>
 
        <!-- Excel panel -->
        <div id="axpBulkPanelExcel" style="display:none;">
          <div style="background:#f0fdf9;border-left:4px solid #4ecca3;padding:16px 18px;margin-bottom:14px;">
            <div style="font-size:13px;font-weight:700;color:#1a1a2e;margin-bottom:8px;">
              <i class="bi bi-1-circle-fill" style="color:#4ecca3;"></i>
              Download template → fill in Score column → re-upload
            </div>
            <button onclick="axpBulkDownloadTemplate()" class="axp-btn-primary">
              <i class="bi bi-file-earmark-arrow-down"></i> Download CSV Template
            </button>
          </div>
          <div style="background:#f8fafc;border-left:4px solid #4ecca3;padding:16px 18px;">
            <div style="font-size:13px;font-weight:700;color:#1a1a2e;margin-bottom:8px;">
              <i class="bi bi-2-circle-fill" style="color:#4ecca3;"></i>
              Upload completed file (.csv)
            </div>
            <input type="file" id="axpBulkFileInput" accept=".csv,.xlsx,.xls"
              style="display:none;" onchange="axpBulkReadFile(this)" />
            <button onclick="document.getElementById('axpBulkFileInput').click()"
              class="axp-btn-secondary">
              <i class="bi bi-upload"></i> Choose File
            </button>
            <span id="axpBulkFileName" style="font-size:12px;color:#64748b;margin-left:10px;">
              No file chosen</span>
          </div>
          <div id="axpBulkExcelPreview" style="margin-top:10px;"></div>
        </div>
 
        <!-- Scan / OCR panel -->
        <div id="axpBulkPanelScan" style="display:none;">
 
          <!-- Quality feedback bar -->
          <div id="axpScanQualityBar" style="display:none;margin-bottom:14px;
               padding:12px 16px;border-left:4px solid #94a3b8;background:#f8fafc;">
            <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
              <span id="axpScanQualityIcon" style="font-size:20px;">📷</span>
              <div style="flex:1;">
                <div style="font-size:13px;font-weight:700;color:#1a1a2e;" id="axpScanQualityTitle">Ready</div>
                <div style="font-size:12px;color:#64748b;" id="axpScanQualityMsg"></div>
              </div>
              <div style="display:flex;gap:6px;">
                <span style="font-size:11px;padding:2px 8px;background:#f1f5f9;color:#64748b;font-weight:600;"
                      id="axpScanQualityBrightness">💡 —</span>
                <span style="font-size:11px;padding:2px 8px;background:#f1f5f9;color:#64748b;font-weight:600;"
                      id="axpScanQualitySharpness">🔍 —</span>
              </div>
            </div>
            <!-- Quality bar -->
            <div style="margin-top:8px;height:4px;background:#e2e8f0;overflow:hidden;">
              <div id="axpScanQualityFill" style="height:4px;width:0%;transition:width .4s;background:#94a3b8;"></div>
            </div>
          </div>
 
          <!-- Camera / Upload area -->
          <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:14px;">
            <button onclick="axpScanOpenCamera()" class="axp-btn-primary"
                    style="background:#060c1c;" id="axpScanCameraBtn">
              <i class="bi bi-camera-fill"></i> Open Camera
            </button>
            <button onclick="document.getElementById('axpScanFileInput').click()"
                    class="axp-btn-secondary" id="axpScanUploadBtn">
              <i class="bi bi-image"></i> Upload Photo(s)
            </button>
            <input type="file" id="axpScanFileInput" accept="image/*" multiple
                   style="display:none;" onchange="axpScanHandleFiles(this.files)" />
            <span style="font-size:11.5px;color:#94a3b8;align-self:center;">
              Single or multiple sheets supported</span>
          </div>
 
          <!-- Camera viewfinder -->
          <div id="axpScanViewfinder" style="display:none;margin-bottom:14px;position:relative;">
            <video id="axpScanVideo" autoplay playsinline muted
                   style="width:100%;max-width:520px;border:2px solid #e2e8f0;display:block;"></video>
            <!-- Overlay guides -->
            <div style="position:absolute;top:0;left:0;right:0;bottom:0;pointer-events:none;">
              <!-- corner brackets -->
              <div style="position:absolute;top:12px;left:12px;width:30px;height:30px;
                          border-top:3px solid #4ecca3;border-left:3px solid #4ecca3;"></div>
              <div style="position:absolute;top:12px;right:12px;width:30px;height:30px;
                          border-top:3px solid #4ecca3;border-right:3px solid #4ecca3;"></div>
              <div style="position:absolute;bottom:56px;left:12px;width:30px;height:30px;
                          border-bottom:3px solid #4ecca3;border-left:3px solid #4ecca3;"></div>
              <div style="position:absolute;bottom:56px;right:12px;width:30px;height:30px;
                          border-bottom:3px solid #4ecca3;border-right:3px solid #4ecca3;"></div>
            </div>
            <!-- Live quality indicator overlay -->
            <div id="axpScanLiveHint" style="position:absolute;bottom:0;left:0;right:0;
                 background:rgba(0,0,0,.6);color:#fff;font-size:12px;font-weight:600;
                 padding:6px 12px;text-align:center;">Analysing…</div>
            <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap;">
              <button onclick="axpScanCapture()" class="axp-btn-primary"
                      id="axpScanCaptureBtn" style="background:#10b981;" disabled>
                <i class="bi bi-camera"></i> Capture
              </button>
              <button onclick="axpScanCloseCamera()" class="axp-btn-secondary">
                <i class="bi bi-x-circle"></i> Cancel
              </button>
            </div>
          </div>
          ...
          <canvas id="axpScanCanvas" style="display:none;"></canvas>
 
          <!-- Queue of images to process -->
          <div id="axpScanQueue" style="display:none;margin-bottom:14px;">
            <div style="font-size:12.5px;font-weight:700;color:#1a1a2e;margin-bottom:8px;">
              <i class="bi bi-images"></i> Images queued: <span id="axpScanQueueCount">0</span>
            </div>
            <div id="axpScanThumbs" style="display:flex;gap:8px;flex-wrap:wrap;"></div>
            <div style="margin-top:10px;display:flex;gap:8px;flex-wrap:wrap;">
              <button onclick="axpScanProcessAll()" class="axp-btn-primary"
                      id="axpScanProcessBtn" style="background:#4ecca3;color:#060c1c;">
                <i class="bi bi-cpu"></i> Scan All with OCR
              </button>
              <button onclick="axpScanClearQueue()" class="axp-btn-secondary">
          ...
          <!-- OCR Progress -->
          <div id="axpScanOcrProgress" style="display:none;margin-bottom:14px;
               padding:14px 16px;background:#f0fdf9;border-left:4px solid #4ecca3;">
            <div style="font-size:13px;font-weight:700;color:#1a1a2e;margin-bottom:6px;">
              <span class="axp-spinner-sm"></span>
              <span id="axpScanOcrStatus">Initialising OCR engine…</span>
            </div>
            <div style="height:5px;background:#e2e8f0;overflow:hidden;margin-top:6px;">
              <div id="axpScanOcrFill" style="height:5px;background:#4ecca3;width:0%;transition:width .3s;"></div>
            </div>
            <div style="font-size:11px;color:#64748b;margin-top:4px;" id="axpScanOcrSub"></div>
          </div>
 
          <!-- Results / confirmation table -->
          <div id="axpScanResults" style="display:none;"></div>
        </div>
 
        <div id="axpBulkEmptyState">
          <div class="axp-empty-state" style="padding:40px 16px;">
            <i class="bi bi-table" style="font-size:38px;"></i>
            <p>Choose Exam, Class, and Subject above — then tap <strong>Load Students</strong>.</p>
          </div>
        </div>
      </div>
 
      <!-- ══════════ CORRECTIONS TAB ══════════ -->
      <div id="axpFeedPanelCorrection" style="display:none;padding:20px;">
 
        <div class="axp-alert axp-alert-warning" style="margin-bottom:18px;">
          <i class="bi bi-exclamation-triangle-fill"></i>
          <span>Corrections limited to <strong>5 per teacher per day</strong>.
          Each correction is logged in the audit trail.</span>
        </div>
 
        <!-- Correction wizard -->
        <div id="axpCorWizard" style="margin-bottom:16px;">
          <div id="axpCorStepExam"></div>
          <div id="axpCorStepClass"></div>
          <div id="axpCorStepSubject"></div>
          <div id="axpCorLoadWrap" style="display:none;padding-top:4px;">
            <button class="axp-load-btn" onclick="axpCorLoadStudents()">
              <i class="bi bi-search"></i> Load Students
            </button>
          </div>
        </div>
 
        <!-- Student picker (searchable list) -->
        <div id="axpCorStudentPicker" style="display:none;margin-bottom:16px;">
          <div class="axp-step-block axp-step-done">
            <div class="axp-step-head">
              <div class="axp-step-no">4</div>
              <div class="axp-step-label">Find Student</div>
              <div id="axpCorStudentCount"
                style="margin-left:auto;font-size:11px;color:#94a3b8;"></div>
            </div>
            ...
            <div style="position:relative;margin-bottom:8px;">
              ...
              <input id="axpCorSearchInput"
                class="axp-input"
                type="text"
                placeholder="Type name or candidate number…"
                autocomplete="off"
                style="padding-left:34px;font-size:13px;"
                oninput="axpCorFilterStudents(this.value)" />
            </div>
 
            <!-- Scrollable result list -->
            <div id="axpCorStudentList"
              style="max-height:240px;overflow-y:auto;border:1.5px solid #e2e8f0;
                     background:#fff;">
              <!-- filled by axpCorLoadStudents -->
            </div>
 
            <!-- Selected indicator -->
            <div id="axpCorSelectedBadge" style="display:none;margin-top:10px;
              background:#060c1c;color:#fff;padding:10px 14px;
              display:none;align-items:center;gap:10px;">
              <i class="bi bi-person-check-fill" style="color:#4ecca3;font-size:16px;"></i>
              <span style="flex:1;font-size:13px;font-weight:700;"
                    id="axpCorSelectedName">—</span>
              <button onclick="axpCorClearStudent()"
        ...
        <div id="axpCorForm" style="display:none;background:#f8fafc;padding:16px;">
          <!-- Fetched score indicator -->
          <div id="axpCorScoreStatus" style="display:none;margin-bottom:10px;
               padding:8px 12px;font-size:12px;font-weight:600;border-left:3px solid #4ecca3;
               background:#f0fdf9;color:#1a1a2e;"></div>
          <div class="axp-form-row">
            <div class="axp-field-group">
              <label>Current (Old) Score
                <span id="axpCorOldFetchSpinner" style="display:none;">
                  <span class="axp-spinner-sm" style="margin-left:6px;"></span>
                </span>
              </label>
              <input id="axpCorOldValue" class="axp-input" type="number"
                min="0" max="100" placeholder="Fetching…"
                readonly
                style="background:#f1f5f9;color:#475569;cursor:not-allowed;font-weight:700;" />
              <div style="font-size:11px;color:#94a3b8;margin-top:3px;">
                Auto-fetched from sheet — read only</div>
            </div>
            <div class="axp-field-group">
              <label>New Correct Score</label>
              <input id="axpCorNewValue" class="axp-input" type="number"
                min="0" max="100" placeholder="e.g. 54"
                oninput="axpCorPreviewGrade()" />
            </div>
          </div>
          <div style="margin-bottom:10px;" id="axpCorGradePreview"></div>
          <div class="axp-field-group" style="margin-bottom:12px;">
            <label>Reason for Correction</label>
            <input id="axpCorReason" class="axp-input"
              placeholder="e.g. Transposition error — 54 entered as 45" />
          </div>
          <div style="display:flex;gap:8px;">
            <button onclick="axpSubmitCorrection()" class="axp-btn-primary" id="axpCorSubmitBtn">
              <i class="bi bi-check2-circle"></i> Submit Correction
            </button>
            <button onclick="axpCorReset()" class="axp-btn-secondary">
              <i class="bi bi-x-circle"></i> Cancel
            </button>
          </div>
          <div id="axpCorMsg" style="margin-top:10px;"></div>
        </div>
 
        <div id="axpCorEmptyState">
          <div class="axp-empty-state" style="padding:40px 16px;">
            <i class="bi bi-pencil-square" style="font-size:38px;"></i>
            <p>Choose exam, class, and subject above to start a correction.</p>
          </div>
        </div>
      </div>
 
    </div>`; /* end axp-section-card */
...
/* ═══════════════════════════════════════════════════════════
   TAB SWITCHER
═══════════════════════════════════════════════════════════ */
window.axpFeedTab = function(tab) {
  ["entry","bulk","correction","print"].forEach(t => {
    const key   = t.charAt(0).toUpperCase()+t.slice(1);
    const panel = document.getElementById("axpFeedPanel"+key);
    const btn   = document.getElementById("axpFeedTab"+key);
    var active  = t===tab;
    if (panel) panel.style.display = active?"block":"none";
    if (btn) {
      btn.style.color             = active?"#4ecca3":"#94a3b8";
      btn.style.borderBottomColor = active?"#4ecca3":"transparent";
    }
  });
};
...
/* ═══════════════════════════════════════════════════════════
   WIZARD BOOT HELPERS
   Each wizard: step 1 Exam → step 2 Class → step 3 Subject → Load button
═══════════════════════════════════════════════════════════ */
function _axpGetSubjects(cls) {
  return (_schoolMeta&&_schoolMeta.subjects&&cls&&_schoolMeta.subjects[cls])
    ? _schoolMeta.subjects[cls] : [];
}
function _axpGetClasses()   { return (_schoolMeta&&_schoolMeta.classes)   ? _schoolMeta.classes   : []; }
function _axpGetExamTypes() { return (_schoolMeta&&_schoolMeta.examTypes) ? _schoolMeta.examTypes : []; }
 
/* ─── ENTRY wizard ─── */
function _axpBootEntryWizard() {
  const state = { examType:"", cls:"", subject:"" };
 
  function buildStep1() {
    _axpBuildSelector({
      containerId:"axpFeedStepExam", stepNo:1, label:"Exam Type",
      items: _axpGetExamTypes(),
      iconFn: () => "bi bi-journal-text",
      selectedValue: state.examType,
      onSelect(val) {
        state.examType = val; state.cls = ""; state.subject = "";
        buildStep2(); buildStep3(true);
        _syncLoadBtn();
      }
    });
  }
  ...
  function buildStep2() {
    _axpBuildSelector({
      containerId:"axpFeedStepClass", stepNo:2, label:"Class",
      items: _axpGetClasses(),
      iconFn: () => "bi bi-people",
      locked: !state.examType,
      selectedValue: state.cls,
      onSelect(val) {
        state.cls = val; state.subject = "";
        buildStep3(false);
        _syncLoadBtn();
      }
    });
  ...
  function buildStep3(locked) {
    _axpBuildSelector({
      containerId:"axpFeedStepSubject", stepNo:3, label:"Subject",
      items: locked ? [] : _axpGetSubjects(state.cls),
      iconFn: () => "bi bi-book",
      locked: locked || !state.cls,
  ...
  function _syncLoadBtn() {
    const wrap = document.getElementById("axpFeedLoadWrap");
    if (!wrap) return;
    wrap.style.display = (state.examType && state.cls && state.subject) ? "block" : "none";
    /* expose to outer scope */
    window._axpFeedWizardState = state;
  }
 
  buildStep1(); buildStep2(); buildStep3(true);
}
 
/* ─── BULK wizard ─── */
function _axpBootBulkWizard() {
  const state = { examType:"", cls:"", subject:"" };
 
  function buildStep1() {
    _axpBuildSelector({
      containerId:"axpBulkStepExam", stepNo:1, label:"Exam Type",
      items: _axpGetExamTypes(),
      iconFn: () => "bi bi-journal-text",
      selectedValue: state.examType,
      onSelect(val) {
        state.examType = val; state.cls=""; state.subject="";
        buildStep2(); buildStep3(true); sync();
      }
    });
  }
  function buildStep2() {
    _axpBuildSelector({
      containerId:"axpBulkStepClass", stepNo:2, label:"Class",
      items: _axpGetClasses(),
      iconFn: () => "bi bi-people",
      locked: !state.examType,
      selectedValue: state.cls,
      onSelect(val) { state.cls=val; state.subject=""; buildStep3(false); sync(); }
    });
  }
  function buildStep3(locked) {
    _axpBuildSelector({
      containerId:"axpBulkStepSubject", stepNo:3, label:"Subject",
      items: locked ? [] : _axpGetSubjects(state.cls),
      iconFn: () => "bi bi-book",
      locked: locked||!state.cls,
      selectedValue: state.subject,
      onSelect(val) { state.subject=val; sync(); }
    });
  }
  function sync() {
    const wrap = document.getElementById("axpBulkLoadWrap");
    if (wrap) wrap.style.display = (state.examType&&state.cls&&state.subject) ? "block":"none";
    window._axpBulkWizardState = state;
  }
  buildStep1(); buildStep2(); buildStep3(true);
}
 
/* ─── CORRECTION wizard ─── */
function _axpBootCorWizard() {
  const state = { examType:"", cls:"", subject:"" };
 
  function buildStep1() {
    _axpBuildSelector({
      containerId:"axpCorStepExam", stepNo:1, label:"Exam Type",
      items: _axpGetExamTypes(),
      iconFn: () => "bi bi-journal-text",
      selectedValue: state.examType,
      onSelect(val) { state.examType=val; state.cls=""; state.subject=""; buildStep2(); buildStep3(true); sync(); }
    });
  }
  function buildStep2() {
    _axpBuildSelector({
      containerId:"axpCorStepClass", stepNo:2, label:"Class",
      items: _axpGetClasses(),
      iconFn: () => "bi bi-people",
      locked: !state.examType,
      selectedValue: state.cls,
  ...
  function buildStep3(locked) {
    _axpBuildSelector({
      containerId:"axpCorStepSubject", stepNo:3, label:"Subject",
      items: locked ? [] : _axpGetSubjects(state.cls),
      iconFn: () => "bi bi-book",
      locked: locked||!state.cls,
      selectedValue: state.subject,
  ...
  function sync() {
    const wrap = document.getElementById("axpCorLoadWrap");
    if (wrap) wrap.style.display = (state.examType&&state.cls&&state.subject) ? "block":"none";
    window._axpCorWizardState = state;
  }
  buildStep1(); buildStep2(); buildStep3(true);
}
...
/* ═══════════════════════════════════════════════════════════
   TEACHER BANNER
═══════════════════════════════════════════════════════════ */
function _axpFeedRenderTeacherBanner(bannerId, teacher, cls, subject) {
  ...
  if (!teacher) {
    ...
    el.innerHTML=`
      <div class="axp-alert axp-alert-warning">
        <i class="bi bi-person-x-fill" style="font-size:16px;flex-shrink:0;"></i>
        <div>
          <strong>No teacher assigned</strong> to <em>${escapeHtml(cls)}</em>
          — <em>${escapeHtml(subject)}</em>.<br>
          <span style="font-size:12px;opacity:.85;">
            You can still enter marks. Assign in
            <a href="#" onclick="navigateToSection('assign-tasks');return false;"
               style="color:#f59e0b;font-weight:700;">Assign Tasks</a>.
          </span>
        </div>
      </div>`; return;
  }
  el.style.display="block";
  el.innerHTML=`
    <div style="background:#060c1c;color:#fff;padding:12px 16px;
                display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      <div style="width:36px;height:36px;min-width:36px;background:rgba(78,204,163,.15);
                  display:flex;align-items:center;justify-content:center;">
        <i class="bi bi-person-fill" style="font-size:17px;color:#4ecca3;"></i>
      </div>
      <div style="flex:1;min-width:0;">
        <div style="font-size:13.5px;font-weight:700;">${escapeHtml(teacher.name)}</div>
        <div style="font-size:11.5px;opacity:.55;">${escapeHtml(teacher.email||"No email")}
          ${teacher.phone?` · ${escapeHtml(teacher.phone)}`:""}
        </div>
      </div>
      <div style="display:flex;gap:6px;flex-wrap:wrap;">
        <span style="background:rgba(78,204,163,.12);color:#4ecca3;
...
window.axpFeedLoadRoster = async function() {
  const state = window._axpFeedWizardState || {};
  const { examType, cls, subject } = state;
  if (!examType||!cls||!subject) {
    axpAlert("Selection Required",
      "Please complete all three steps above before loading.","warning"); return;
  }
 
  const btn = document.getElementById("axpFeedLoadBtn");
  if (btn) { btn.disabled=true; btn.innerHTML='<span class="axp-spinner-sm"></span> Loading…'; }
 
  document.getElementById("axpFeedStudentCard").style.display="none";
  document.getElementById("axpFeedSummary").style.display="none";
  document.getElementById("axpFeedEmptyState").innerHTML=`
    <div class="axp-empty-state" style="padding:40px 16px;">
      <div class="axp-spinner-sm" style="margin:0 auto 10px;width:28px;height:28px;border-width:3px;"></div>
  ...
  try {
    const res = await _apiGet({
      mode:"feedingRoster", schoolId:_appScriptSchoolId,
      year:_schoolMeta.year, examType, class:cls, subject
    });
    ...
    if (!res.students||!res.students.length) {
      document.getElementById("axpFeedEmptyState").innerHTML=`
        <div class="axp-empty-state" style="padding:40px 16px;">
          <i class="bi bi-inbox"></i>
          <p>No students enrolled in <strong>${escapeHtml(subject)}</strong>
             for <strong>${escapeHtml(cls)}</strong>.</p>
        </div>`; return;
    }
 
    window._axpFeed = {
      students:_axpSortStudents(res.students), teacher:res.teacher||null,
      cls, examType, subject, currentIdx:0, marks:{}, submitted:false
    };
 
    _axpFeedRenderTeacherBanner("axpFeedTeacherBanner",res.teacher,cls,subject);
    document.getElementById("axpFeedProgressWrap").style.display="block";
    _axpFeedUpdateProgress();
    document.getElementById("axpFeedStudentCard").style.display="block";
    document.getElementById("axpFeedEmptyState").innerHTML="";
    _axpFeedRenderCard();
    ...
    document.getElementById("axpFeedEmptyState").innerHTML=`
      <div class="axp-alert axp-alert-danger">
        <i class="bi bi-exclamation-circle"></i>
        <span>Failed to load roster. Check your connection and try again.</span>
      </div>`;
  } finally {
    if (btn) { btn.disabled=false; btn.innerHTML='<i class="bi bi-cloud-download"></i> Load Roster'; }
  }
};
 
function _axpFeedRenderCard() {
  const { students, currentIdx, marks } = window._axpFeed;
  const total = students.length;
  if (currentIdx>=total) {
    document.getElementById("axpFeedStudentCard").style.display="none";
    _axpFeedShowSummary(); return;
  }
  const s = students[currentIdx];
  const existing = marks[s.name];
 
  document.getElementById("axpFeedCardCandNo").textContent = s.candidateNo||_axpCandNo(currentIdx);
  document.getElementById("axpFeedCardName").textContent   = s.name;
  document.getElementById("axpFeedCardGender").textContent = s.gender==="F"?"Female":"Male";
 
  const prevEl = document.getElementById("axpFeedPrevMark");
  if (existing!==undefined) {
    const {grade,color}=_axpGrade(existing);
    prevEl.innerHTML=`<span style="color:#64748b;">${existing}</span>
      <strong style="color:${color};margin-left:4px;">(${grade})</strong>`;
  } else { prevEl.textContent="—"; prevEl.style.color="#94a3b8"; }
 
  const inp = document.getElementById("axpFeedScoreInput");
  inp.value = existing!==undefined ? existing : "";
  axpFeedOnScoreInput(inp.value);
 
  const prevBtn=document.getElementById("axpFeedPrevBtn");
  const nextBtn=document.getElementById("axpFeedNextBtn");
  if (prevBtn) prevBtn.disabled = currentIdx===0;
  if (nextBtn) nextBtn.innerHTML = currentIdx===total-1
    ? '<i class="bi bi-check2"></i> Finish'
    : 'Next <i class="bi bi-chevron-right"></i>';
...
window.axpFeedOnScoreInput = function(val) {
  const b=document.getElementById("axpFeedGradeBadge"); if(!b)return;
  if(val===""||val===null||val===undefined){b.textContent="—";b.style.color="#94a3b8";return;}
  const {grade,color}=_axpGrade(parseInt(val));
  b.textContent=grade; b.style.color=color;
};
 
function _axpFeedUpdateProgress() {
  const {students,marks}=window._axpFeed;
  const total=students.length;
  const filled=students.filter(s=>marks[s.name]!==undefined).length;
  const pct=total>0?Math.round(filled/total*100):0;
  const fill=document.getElementById("axpFeedProgressFill");
  if(fill){fill.style.width=pct+"%";fill.style.background=pct===100?"#10b981":"#4ecca3";}
  _axpSetText("axpFeedProgressPct",pct+"%");
  _axpSetText("axpFeedProgressLabel",`${filled} of ${total} students`);
  _axpSetText("axpFeedProgressSublabel",pct===100
    ?"All students filled — click Submit All to save."
    :`${total-filled} student${total-filled!==1?"s":""} remaining`);
}
 
function _axpFeedSaveCurrentInput() {
  const {students,currentIdx}=window._axpFeed;
  if(currentIdx>=students.length)return;
  const inp=document.getElementById("axpFeedScoreInput");
  const val=inp?inp.value.trim():"";
  if(val!==""){const n=parseInt(val);
    if(!isNaN(n)&&n>=0&&n<=100) window._axpFeed.marks[students[currentIdx].name]=n;}
  _axpFeedUpdateProgress();
}
 
window.axpFeedNext = function() {
  const {students,currentIdx}=window._axpFeed;
  const inp=document.getElementById("axpFeedScoreInput");
  const val=inp?inp.value.trim():"";
  if(val!==""){const n=parseInt(val);
    if(isNaN(n)||n<0||n>100){
      _showSectionMsg("axpFeedEntryMsg","Score must be 0–100.","danger");return;}
    window._axpFeed.marks[students[currentIdx].name]=n;
    _axpFeedUpdateProgress();
  }
  if(currentIdx>=students.length-1){
    document.getElementById("axpFeedStudentCard").style.display="none";
    _axpFeedShowSummary();
  } else { window._axpFeed.currentIdx++; _axpFeedRenderCard(); }
  document.getElementById("axpFeedEntryMsg").innerHTML="";
};
 
window.axpFeedPrev = function() {
  _axpFeedSaveCurrentInput();
  if(window._axpFeed.currentIdx>0){
    window._axpFeed.currentIdx--;
    document.getElementById("axpFeedStudentCard").style.display="block";
    document.getElementById("axpFeedSummary").style.display="none";
    _axpFeedRenderCard();
  }
...
window.axpFeedSkip = function() {
  const {students,currentIdx}=window._axpFeed;
  _showSectionMsg("axpFeedEntryMsg",`Skipped: ${students[currentIdx].name}`,"warning");
  if(currentIdx>=students.length-1){
    document.getElementById("axpFeedStudentCard").style.display="none";
    _axpFeedShowSummary();
  } else { window._axpFeed.currentIdx++; _axpFeedRenderCard(); }
...
window.axpFeedSubmitAll = function() {
  _axpFeedSaveCurrentInput();
  const {students,marks,cls,examType,subject}=window._axpFeed;
  const allPayload=students.map(s=>({name:s.name,score:marks[s.name]!==undefined?marks[s.name]:""}));
  const payload=allPayload.filter(p=>p.score!==""); // ← only students with a score entered
  const filled=payload.length;
  if(!filled){axpAlert("No Marks","Enter at least one score first.","warning");return;}
 
  axpConfirm("Confirm Submission",
    `Submit marks for <strong>${filled}</strong> student${filled!==1?"s":""}
     in <strong>${escapeHtml(subject)}</strong> —
     <strong>${escapeHtml(cls)}</strong> — <strong>${escapeHtml(examType)}</strong>?`,
    async()=>{
      const btn=document.getElementById("axpFeedSubmitBtn");
      if(btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Submitting…';}
      try{
        const res=await _apiPost({
          mode:"marks",
          teacherEmail:_dashboardData.email,
          schoolId:_appScriptSchoolId,year:_schoolMeta.year,
          class:cls,examType,subject,
          "data[studentName]":payload.map(p=>p.name).join(","),
          "data[marks]":payload.map(p=>String(p.score)).join(",")
        });
        if(res.status==="success"){
          const saved = res.saved !== undefined ? res.saved : filled;
          const notFound = res.notFound && res.notFound.length ? res.notFound : [];
          if(saved === 0){
            axpAlert("No Marks Saved",
              `<div style="text-align:center;padding:8px 0;">
                 <div style="font-size:40px;margin-bottom:8px;">⚠️</div>
                 <div style="font-size:14px;color:#f59e0b;font-weight:700;margin-bottom:10px;">
                   0 marks were saved — student names did not match the exam sheet.</div>
                 <div style="font-size:12px;color:#64748b;">
                   Not found: ${escapeHtml(notFound.slice(0,5).join(", "))}${notFound.length>5?" …":""}
                 </div>
               </div>`,"warning");
          } else {
            window._axpFeed.submitted=true;
            document.getElementById("axpFeedStudentCard").style.display="none";
            _axpFeedShowSummary(true);
            _axpToast(`${saved} mark${saved!==1?"s":""} saved!`,"success");
            axpAlert("Marks Submitted",
              `<div style="text-align:center;padding:8px 0;">
                 <div style="font-size:48px;margin-bottom:8px;">✅</div>
                 <div style="font-size:15px;font-weight:700;color:#10b981;margin-bottom:12px;">
                   ${saved} mark${saved!==1?"s":""} saved to the server</div>
                 ${notFound.length?`<div style="background:#fff7ed;padding:8px 12px;font-size:12px;color:#c2410c;margin-top:8px;">
                   ⚠️ ${notFound.length} student${notFound.length!==1?"s":""} not found in sheet:
                   ${escapeHtml(notFound.slice(0,5).join(", "))}${notFound.length>5?" …":""}
                 </div>`:""}
                 <div style="background:#f0fdf9;padding:10px 14px;font-size:13px;color:#1a1a2e;margin-top:8px;">
                   <strong>${escapeHtml(subject)}</strong><br>
                   ${escapeHtml(cls)} &nbsp;·&nbsp; ${escapeHtml(examType)}
                 </div>
               </div>`,"success");
          }
        } else axpAlert("Submission Failed",res.message||"Please try again.","danger");
      }catch(e){axpAlert("Network Error","Check your connection and try again.","danger");}
      finally{if(btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-send-fill"></i> Submit All';}}
    },
    "Submit Marks","background:#10b981;");
};
 
function _axpFeedShowSummary(submitted=false) {
  const {students,marks,cls,examType,subject}=window._axpFeed;
  const el=document.getElementById("axpFeedSummary"); if(!el)return;
  el.style.display="block";
  const gc={A:0,B:0,C:0,D:0,F:0,"—":0};
  const rows=students.map((s,i)=>{
    const score=marks[s.name];const {grade,color}=_axpGrade(score);
    gc[grade]=(gc[grade]||0)+1;
    return {cand:s.candidateNo||_axpCandNo(i),name:s.name,gender:s.gender,score,grade,color};
  });
  const gc2={A:"#10b981",B:"#4ecca3",C:"#f59e0b",D:"#f97316",F:"#ef4444","—":"#94a3b8"};
  el.innerHTML=`
    <div style="border-bottom:2px solid #4ecca3;padding-bottom:10px;margin-bottom:14px;
                display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
      <h4 style="font-size:14px;font-weight:800;color:#1a1a2e;margin:0;flex:1;">
        <i class="bi bi-table" style="color:#4ecca3;"></i>
        ${escapeHtml(subject)} · ${escapeHtml(cls)} · ${escapeHtml(examType)}
      </h4>
      ${submitted
        ?`<span style="background:#10b981;color:#fff;padding:3px 12px;font-size:11.5px;font-weight:800;">
            <i class="bi bi-check2-all"></i> SUBMITTED</span>`
        :`<span style="background:#f59e0b;color:#fff;padding:3px 12px;font-size:11.5px;font-weight:800;">
            PREVIEW — Not yet saved</span>`}
    </div>
    <div style="display:grid;grid-template-columns:repeat(6,1fr);gap:7px;margin-bottom:16px;">
      ${["A","B","C","D","F","—"].map(g=>`
        <div style="background:#f8fafc;padding:10px 6px;text-align:center;
                    border-top:3px solid ${gc2[g]};">
          <div style="font-size:22px;font-weight:900;color:${gc2[g]};">${gc[g]||0}</div>
          <div style="font-size:10.5px;font-weight:700;color:${gc2[g]};margin-top:1px;">Grade ${g}</div>
        </div>`).join("")}
    </div>
    <div style="overflow-x:auto;">
      <table class="axp-table" style="min-width:400px;">
        <thead><tr>
        ...
        <tbody>
          ${rows.map((r,i)=>`
            <tr>
              <td style="font-family:monospace;font-size:12px;color:#4ecca3;font-weight:700;">
                ${escapeHtml(r.cand)}</td>
              <td style="font-weight:500;">${escapeHtml(r.name)}</td>
              <td style="text-align:center;color:#64748b;">${r.gender==="F"?"F":"M"}</td>
              <td style="text-align:center;font-weight:700;">${r.score!==undefined?r.score:"—"}</td>
              <td style="text-align:center;">
                <strong style="font-size:15px;font-weight:900;color:${r.color};">
                  ${r.grade}</strong></td>
            </tr>`).join("")}
        </tbody>
      </table>
    ...
    <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap;">
      ${!submitted?`<button onclick="axpFeedSubmitAll()" class="axp-btn-primary"
         style="background:#10b981;"><i class="bi bi-send-fill"></i> Submit Now</button>`:""}
      <button onclick="axpFeedReenter()" class="axp-btn-secondary">
        <i class="bi bi-arrow-counterclockwise"></i> Re-enter / Edit</button>
      <button onclick="axpFeedExportSummary()" class="axp-btn-secondary">
        <i class="bi bi-download"></i> Export CSV</button>
    </div>`;
}
 
window.axpFeedReenter=function(){
  document.getElementById("axpFeedSummary").style.display="none";
  document.getElementById("axpFeedStudentCard").style.display="block";
  window._axpFeed.currentIdx=0; _axpFeedRenderCard();
};
window.axpFeedExportSummary=function(){
  const {students,marks,cls,examType,subject}=window._axpFeed;
  const lines=["Candidate No.,Name,Gender,Score,Grade"];
  students.forEach((s,i)=>{
    const score=marks[s.name];const {grade}=_axpGrade(score);
    lines.push(`"${s.candidateNo||_axpCandNo(i)}","${s.name}","${s.gender}","${score!==undefined?score:""}","${grade}"`);
  });
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([lines.join("\n")],{type:"text/csv"}));
  a.download=`marks_${cls}_${subject}_${examType}.csv`; a.click();
};
...
/* ═══════════════════════════════════════════════════════════
   BULK LOGIC
═══════════════════════════════════════════════════════════ */
window.axpBulkLoadRoster = async function() {
  const state=window._axpBulkWizardState||{};
  const {examType,cls,subject}=state;
  if(!examType||!cls||!subject){
    axpAlert("Selection Required","Complete all three steps above.","warning");return;}
 
  const btn=document.getElementById("axpBulkLoadBtn");
  if(btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Loading…';}
  document.getElementById("axpBulkModeBar").style.display="none";
  ["axpBulkPanelGrid","axpBulkPanelPaste","axpBulkPanelExcel"]
    .forEach(id=>{document.getElementById(id).style.display="none";});
  document.getElementById("axpBulkEmptyState").innerHTML=`
    <div class="axp-empty-state" style="padding:30px;">
      <div class="axp-spinner-sm" style="margin:0 auto 8px;"></div>
  ...
  try{
    ...
    const students=_axpSortStudents(res.students||[]);
    if(!students.length){
      document.getElementById("axpBulkEmptyState").innerHTML=`
        <div class="axp-empty-state" style="padding:30px;">
          <i class="bi bi-inbox"></i>
          <p>No students for this selection.</p>
        </div>`; return;
    }
    window._axpBulk={students,marks:{},cls,examType,subject};
    _axpFeedRenderTeacherBanner("axpBulkTeacherBanner",res.teacher,cls,subject);
    _axpBulkRenderGrid(students);
    document.getElementById("axpBulkEmptyState").innerHTML="";
    document.getElementById("axpBulkModeBar").style.display="block";
    axpBulkMode("grid");
  }catch(e){
    document.getElementById("axpBulkEmptyState").innerHTML=`
      <div class="axp-alert axp-alert-danger">
        <i class="bi bi-exclamation-circle"></i>
        <span>Failed to load. Check connection and retry.</span>
      </div>`;
  }finally{
    if(btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-cloud-download"></i> Load Students';}
  }
};
 
window.axpBulkMode=function(mode){
  const hints={
    grid :"Grid: type directly — use ↑↓ or Enter to move between rows",
    paste:"Paste: paste spreadsheet data (tab or comma-separated)",
    excel:"Excel: download template → fill → re-upload",
    scan :"Scan: capture or upload a photo of a mark sheet — OCR reads names & scores automatically"
  };
  ["grid","paste","excel","scan"].forEach(m=>{
    const key=m.charAt(0).toUpperCase()+m.slice(1);
    const panel=document.getElementById(`axpBulkPanel${key}`);
    const btn=document.getElementById(`axpBulkMode${key}`);
    const active=m===mode;
    if(panel)panel.style.display=active?"block":"none";
    if(btn){btn.style.background=active?"#060c1c":"#f8fafc";
            btn.style.color=active?"#4ecca3":"#64748b";}
  });
  _axpSetText("axpBulkModeHint",hints[mode]||"");
  if(mode!=="scan") axpScanCloseCamera();
};
 
function _axpBulkRenderGrid(students){
  const tbody=document.getElementById("axpBulkTableBody"); if(!tbody)return;
  _axpSetText("axpBulkTotal",students.length);
  tbody.innerHTML=students.map((s,i)=>{
    const cand=s.candidateNo||_axpCandNo(i);
    return `
    <tr id="axpBulkRow_${i}">
      <td style="font-family:monospace;font-size:12px;color:#4ecca3;font-weight:700;">
        ${escapeHtml(cand)}</td>
      <td style="font-weight:500;">${escapeHtml(s.name)}</td>
      <td style="text-align:center;color:#64748b;">${s.gender==="F"?"F":"M"}</td>
      <td style="text-align:center;padding:4px 6px;">
        <input class="axp-input" id="axpBulkScore_${i}" type="number"
          min="0" max="100"
          style="text-align:center;font-size:14px;font-weight:700;padding:5px;width:90px;"
          placeholder="—" data-idx="${i}" data-name="${escapeHtml(s.name)}"
          oninput="axpBulkOnInput(this)"
          onkeydown="if(event.key==='Enter'||event.key==='ArrowDown'){
            var nx=document.getElementById('axpBulkScore_'+(${i}+1));
            if(nx){nx.focus();nx.select();}}
           else if(event.key==='ArrowUp'){
            var pv=document.getElementById('axpBulkScore_'+(${i}-1));
            if(pv){pv.focus();pv.select();}}" />
      </td>
      <td style="text-align:center;" id="axpBulkGrade_${i}">
        <span style="font-size:15px;font-weight:900;color:#94a3b8;">—</span>
      </td>
    </tr>`; }).join("");
}
 
window.axpBulkOnInput=function(input){
  const idx=parseInt(input.getAttribute("data-idx"));
  const name=input.getAttribute("data-name");
  const val=input.value.trim();
  const gEl=document.getElementById(`axpBulkGrade_${idx}`);
  if(val===""){
    if(gEl)gEl.innerHTML=`<span style="font-size:15px;font-weight:900;color:#94a3b8;">—</span>`;
    delete window._axpBulk.marks[name];
  } else {
    const n=parseInt(val);
    if(!isNaN(n)&&n>=0&&n<=100){
      window._axpBulk.marks[name]=n;
      const {grade,color}=_axpGrade(n);
      if(gEl)gEl.innerHTML=`<strong style="font-size:15px;font-weight:900;color:${color};">${grade}</strong>`;
    }
  }
  _axpBulkUpdateStats();
};
 
function _axpBulkUpdateStats(){
  if(!window._axpBulk)return;
  const total=window._axpBulk.students.length;
  const filled=Object.keys(window._axpBulk.marks).length;
  const pct=total>0?Math.round(filled/total*100):0;
  const fillEl=document.getElementById("axpBulkProgressFill");
  if(fillEl){fillEl.style.width=pct+"%";fillEl.style.background=pct===100?"#10b981":"#4ecca3";}
  _axpSetText("axpBulkFilled",filled);_axpSetText("axpBulkEmpty",total-filled);_axpSetText("axpBulkPct",pct+"%");
}
 
window.axpBulkClearAll=function(){
  if(!window._axpBulk)return;
  window._axpBulk.marks={};
  window._axpBulk.students.forEach((_,i)=>{
    const inp=document.getElementById(`axpBulkScore_${i}`);
    const grd=document.getElementById(`axpBulkGrade_${i}`);
    if(inp)inp.value="";
    if(grd)grd.innerHTML=`<span style="font-size:15px;font-weight:900;color:#94a3b8;">—</span>`;
  });
  _axpBulkUpdateStats();
};
 
window.axpBulkSubmitAll=function(){
  if(!window._axpBulk)return;
  const{students,marks,cls,examType,subject}=window._axpBulk;
  const allPayload=students.map(s=>({name:s.name,score:marks[s.name]!==undefined?marks[s.name]:""}));
  const payload=allPayload.filter(p=>p.score!==""); // ← only students with a score entered
  const filled=payload.length;
  if(!filled){axpAlert("No Marks","Enter at least one mark first.","warning");return;}
  axpConfirm("Confirm Bulk Submission",
    `Submit <strong>${filled}</strong> marks for
     <strong>${escapeHtml(subject)}</strong> —
     <strong>${escapeHtml(cls)}</strong> — <strong>${escapeHtml(examType)}</strong>?`,
    async()=>{
      const btn=document.getElementById("axpBulkSubmitBtn");
      if(btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Submitting…';}
      try{
        const res=await _apiPost({
          mode:"marks",
        ...
        if(res.status==="success"){
          ...
          if(saved === 0){
            ...
            _showSectionMsg("axpBulkMsg",`${saved} marks saved!`,"success");
            _axpToast(`Marks saved for ${subject} — ${cls}!`,"success");
            students.forEach((s,i)=>{
              if(marks[s.name]!==undefined){
                const row=document.getElementById(`axpBulkRow_${i}`);
                if(row){row.style.background="#ecfdf5";
                  setTimeout(()=>{if(row)row.style.background="";},2500);}
              }
            });
            axpAlert("Marks Submitted",
              `<div style="text-align:center;padding:8px 0;">
        ...
        } else axpAlert("Submission Failed",res.message||"Please retry.","danger");
      }catch(e){axpAlert("Network Error","Check your connection and try again.","danger");}
      finally{if(btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-send-fill"></i> Submit All Marks';}}
    },"Submit Marks","background:#10b981;");
};
 
window.axpBulkParsePaste=function(){
  if(!window._axpBulk)return;
  const raw=(document.getElementById("axpBulkPasteArea")||{}).value||"";
  const lines=raw.split(/\r?\n/).map(l=>l.trim()).filter(Boolean);
  const prev=document.getElementById("axpBulkPastePreview");
  const byName={};const byCand={};
  window._axpBulk.students.forEach((s,i)=>{
    byName[s.name.toLowerCase()]=s.name;
    byCand[(s.candidateNo||_axpCandNo(i)).toLowerCase()]=s.name;
  });
  ...
  lines.forEach(line=>{
    ...
    const keyRaw=parts[0];const score=parseInt(parts[parts.length-1]);
    if(isNaN(score)||score<0||score>100){skipped.push(keyRaw+" (bad score)");return;}
    const name=byCand[keyRaw.toLowerCase()]||byName[keyRaw.toLowerCase()];
    if(!name){skipped.push(keyRaw+" (not found)");return;}
    window._axpBulk.marks[name]=score;
    const idx=window._axpBulk.students.findIndex(s=>s.name===name);
    if(idx>=0){
      const inp=document.getElementById(`axpBulkScore_${idx}`);
      const grd=document.getElementById(`axpBulkGrade_${idx}`);
      if(inp)inp.value=score;
      if(grd){const{grade,color}=_axpGrade(score);
        grd.innerHTML=`<strong style="font-size:15px;font-weight:900;color:${color};">${grade}</strong>`;}
    }
    matched++;
  });
  _axpBulkUpdateStats();
  prev.innerHTML=`
    <div style="background:#f0fdf9;border-left:4px solid #10b981;padding:12px 16px;margin-bottom:10px;">
      <strong style="color:#10b981;">${matched} students</strong> matched.
      ${skipped.length?`<br><span style="color:#f59e0b;font-size:12px;">
        ${skipped.length} skipped: ${escapeHtml(skipped.slice(0,5).join(", "))}
        ${skipped.length>5?"…":""}</span>`:""}
    </div>
    ${matched?`<button onclick="axpBulkSubmitAll()" class="axp-btn-primary"
       style="background:#10b981;margin-right:8px;">
       <i class="bi bi-send-fill"></i> Submit ${matched} Marks</button>
     <button onclick="axpBulkMode('grid')" class="axp-btn-secondary">
       <i class="bi bi-grid-3x3-gap"></i> Review in Grid</button>`:""}`;
};
 
window.axpBulkDownloadTemplate=function(){
  if(!window._axpBulk)return;
  const{students,cls,examType,subject}=window._axpBulk;
  const lines=[`"Candidate No.","Student Name","Gender","Score (0-100)"`];
  students.forEach((s,i)=>lines.push(`"${s.candidateNo||_axpCandNo(i)}","${s.name}","${s.gender}",""`));
  const a=document.createElement("a");
  a.href=URL.createObjectURL(new Blob([lines.join("\n")],{type:"text/csv"}));
  a.download=`template_${cls}_${subject}_${examType}.csv`; a.click();
  _axpToast("Template downloaded — fill column D then re-upload.","info");
};
 
window.axpBulkReadFile=function(input){
  const file=input.files[0]; if(!file)return;
  _axpSetText("axpBulkFileName",file.name);
  const reader=new FileReader();
  reader.onload=function(e){
    const ta=document.getElementById("axpBulkPasteArea");
    if(ta)ta.value=e.target.result;
    axpBulkMode("paste"); axpBulkParsePaste();
    const prev=document.getElementById("axpBulkExcelPreview");
    if(prev)prev.innerHTML=`<div class="axp-alert axp-alert-success">
      <i class="bi bi-check-circle"></i>
      <span>File <strong>${escapeHtml(file.name)}</strong> parsed.</span></div>`;
  };
...
/* ═══════════════════════════════════════════════════════════
   SCAN / OCR MODE  (Tesseract.js powered)
   ── Image quality analysis (brightness + sharpness)
   ── Live camera viewfinder with real-time feedback
   ── Multi-image queue support
   ── OCR → name/score extraction → roster matching → confirm → submit
═══════════════════════════════════════════════════════════ */
 
var _axpScanState = {
  stream     : null,    // MediaStream from camera
  rafId      : null,    // requestAnimationFrame id for live analysis
  queue      : [],      // { dataUrl, thumb } objects waiting to process
  results    : [],      // matched { name, score, raw, matched } objects
  worker     : null     // Tesseract worker (reused)
};
 
/* ── Load Tesseract.js on demand ── */
function _axpLoadTesseract() {
...
/* ── Image quality analysis using canvas pixel data ──
   Returns { score:0-100, brightness, sharpness, issues:[] }
   brightness: mean luminance (0-255)
   sharpness : variance of Laplacian approximation (higher = sharper)
*/
function _axpAnalyseImage(canvas) {
  ...
  var brightness = totalLum / pixels;
 
  /* Sharpness — simplified Laplacian variance on grayscale */
  var gray = new Float32Array(pixels);
  ...
  var sharpness = Math.sqrt(Math.max(lapVar, 0));
 
  /* Score & issues */
  var issues = [];
  var score  = 100;
 
  if (brightness < 60)  { issues.push({ msg:"Too dark — move to a brighter area or turn on light", icon:"💡" }); score -= 35; }
  else if (brightness < 100) { issues.push({ msg:"Low light — try improving lighting", icon:"💡" }); score -= 15; }
  else if (brightness > 220) { issues.push({ msg:"Too bright / overexposed — reduce glare", icon:"☀️" }); score -= 20; }
 
  if (sharpness < 8)    { issues.push({ msg:"Very blurry — hold camera steady or move closer", icon:"📷" }); score -= 40; }
  else if (sharpness < 18) { issues.push({ msg:"Slightly blurry — hold steady", icon:"✋" }); score -= 15; }
 
  if (w < 400 || h < 300) { issues.push({ msg:"Image too small — move camera closer to the sheet", icon:"🔍" }); score -= 20; }
 
  score = Math.max(0, Math.min(100, score));
  return { score: score, brightness: Math.round(brightness), sharpness: Math.round(sharpness), issues: issues };
}
 
/* ── Update quality bar UI ── */
function _axpScanShowQuality(q) {
  var bar    = document.getElementById('axpScanQualityBar');
  var icon   = document.getElementById('axpScanQualityIcon');
  var title  = document.getElementById('axpScanQualityTitle');
  var msg    = document.getElementById('axpScanQualityMsg');
  var bri    = document.getElementById('axpScanQualityBrightness');
  var sharp  = document.getElementById('axpScanQualitySharpness');
  var fill   = document.getElementById('axpScanQualityFill');
  var capBtn = document.getElementById('axpScanCaptureBtn');
  var liveHint = document.getElementById('axpScanLiveHint');
  if (!bar) return;
 
  bar.style.display = 'block';
 
  var color, titleText, msgText, barBg;
  if (q.score >= 75) {
    color = '#10b981'; titleText = 'Good — ready to capture';
    msgText = 'Image quality is acceptable for OCR scanning.';
    barBg   = '#10b981';
  } else if (q.score >= 45) {
    color = '#f59e0b'; titleText = 'Fair — you may capture but quality could affect accuracy';
    msgText = q.issues.map(function(i){ return i.icon + ' ' + i.msg; }).join(' · ');
    barBg   = '#f59e0b';
  } else {
    color = '#ef4444'; titleText = 'Poor — please improve before capturing';
    msgText = q.issues.map(function(i){ return i.icon + ' ' + i.msg; }).join(' · ');
    barBg   = '#ef4444';
  }
 
  bar.style.borderLeftColor = color;
  if (icon)  icon.textContent  = q.score >= 75 ? '✅' : q.score >= 45 ? '⚠️' : '❌';
  if (title) { title.textContent = titleText; title.style.color = color; }
  if (msg)   msg.textContent   = msgText || '';
  if (bri)   bri.textContent   = '💡 ' + q.brightness;
  if (sharp) sharp.textContent = '🔍 ' + q.sharpness;
  if (fill)  { fill.style.width = q.score + '%'; fill.style.background = barBg; }
  if (capBtn) capBtn.disabled = q.score < 45;
  if (liveHint) {
    liveHint.textContent = q.score >= 75 ? '✅ Good — tap Capture'
      : q.score >= 45    ? '⚠️ ' + (q.issues[0] ? q.issues[0].msg : 'Fair quality')
      :                    '❌ ' + (q.issues[0] ? q.issues[0].msg : 'Poor quality');
    liveHint.style.background = q.score >= 75 ? 'rgba(16,185,129,.7)'
      : q.score >= 45 ? 'rgba(245,158,11,.8)' : 'rgba(239,68,68,.8)';
  }
}
 
/* ── Open camera ── */
window.axpScanOpenCamera = async function() {
  var vf = document.getElementById('axpScanViewfinder');
  var video = document.getElementById('axpScanVideo');
  if (!vf || !video) return;
  ...
  try {
    ...
    _axpScanState.stream = stream;
    video.srcObject = stream;
    vf.style.display = 'block';
 
    /* Live quality analysis every 600ms */
    var canvas  = document.getElementById('axpScanCanvas');
    var lastAnalysis = 0;
    function analyse() {
      _axpScanState.rafId = requestAnimationFrame(function(ts) {
        if (ts - lastAnalysis > 600 && video.readyState === 4) {
          lastAnalysis = ts;
          canvas.width  = video.videoWidth  || 640;
          canvas.height = video.videoHeight || 480;
          canvas.getContext('2d').drawImage(video, 0, 0);
          var q = _axpAnalyseImage(canvas);
          _axpScanShowQuality(q);
        }
        analyse();
      });
    }
    ...
    axpAlert('Camera Error', 'Could not access camera: ' + ex.message +
      '<br><br>Try <strong>Upload Photo</strong> instead.', 'danger');
  }
};
 
/* ── Close camera ── */
window.axpScanCloseCamera = function() {
  if (_axpScanState.stream) {
    _axpScanState.stream.getTracks().forEach(function(t){ t.stop(); });
    _axpScanState.stream = null;
  }
  if (_axpScanState.rafId) { cancelAnimationFrame(_axpScanState.rafId); _axpScanState.rafId = null; }
  var vf = document.getElementById('axpScanViewfinder');
  if (vf) vf.style.display = 'none';
};
 
/* ── Capture frame from camera ── */
window.axpScanCapture = function() {
  var video  = document.getElementById('axpScanVideo');
  var canvas = document.getElementById('axpScanCanvas');
  ...
  axpScanCloseCamera();
  _axpScanEnqueue(dataUrl);
  _axpToast('Photo captured — add more or tap Scan All.', 'success');
};
 
/* ── Handle file uploads ── */
window.axpScanHandleFiles = function(files) {
  ...
  Array.from(files).forEach(function(file) {
    ...
    reader.onload = function(ev) {
      ...
      img.onload = function() {
        ...
        _axpScanShowQuality(q);
        if (q.score < 40) {
          axpAlert('Poor Image Quality',
            '❌ <strong>' + escapeHtml(file.name) + '</strong><br><br>' +
            q.issues.map(function(i){ return i.icon + ' ' + i.msg; }).join('<br>') +
            '<br><br>Please retake or upload a clearer image.', 'warning');
        } else {
          _axpScanEnqueue(ev.target.result);
          if (q.score < 60) {
            _axpToast('⚠️ ' + file.name + ': ' + (q.issues[0]||{msg:'Fair quality'}).msg + ' — results may vary.', 'warning');
          }
        }
        pending--;
      };
  ...
  document.getElementById('axpScanFileInput').value = '';
};
 
/* ── Add image to queue ── */
function _axpScanEnqueue(dataUrl) {
  _axpScanState.queue.push(dataUrl);
  var qEl    = document.getElementById('axpScanQueue');
  var thumbs = document.getElementById('axpScanThumbs');
  var count  = document.getElementById('axpScanQueueCount');
  if (qEl)  qEl.style.display = 'block';
  if (count) count.textContent = _axpScanState.queue.length;
  if (thumbs) {
    var idx = _axpScanState.queue.length - 1;
    var div = document.createElement('div');
    div.style.cssText = 'position:relative;width:72px;height:72px;';
    div.innerHTML =
      '<img src="' + dataUrl + '" style="width:72px;height:72px;object-fit:cover;border:2px solid #e2e8f0;" />' +
      '<div style="position:absolute;top:2px;right:2px;background:#ef4444;color:#fff;' +
      'width:16px;height:16px;font-size:10px;font-weight:700;display:flex;align-items:center;' +
      'justify-content:center;cursor:pointer;" onclick="_axpScanRemoveFromQueue(' + idx + ')">✕</div>';
    thumbs.appendChild(div);
  }
}
 
/* ── Remove from queue ── */
window._axpScanRemoveFromQueue = function(idx) {
  _axpScanState.queue.splice(idx, 1);
  var thumbs = document.getElementById('axpScanThumbs');
  var count  = document.getElementById('axpScanQueueCount');
  if (count) count.textContent = _axpScanState.queue.length;
  if (thumbs) {
    /* Rebuild thumbs */
    thumbs.innerHTML = '';
    _axpScanState.queue.forEach(function(dataUrl, i) {
      var div = document.createElement('div');
      div.style.cssText = 'position:relative;width:72px;height:72px;';
      div.innerHTML =
        '<img src="' + dataUrl + '" style="width:72px;height:72px;object-fit:cover;border:2px solid #e2e8f0;" />' +
        '<div style="position:absolute;top:2px;right:2px;background:#ef4444;color:#fff;' +
        'width:16px;height:16px;font-size:10px;font-weight:700;display:flex;align-items:center;' +
        'justify-content:center;cursor:pointer;" onclick="_axpScanRemoveFromQueue(' + i + ')">✕</div>';
      thumbs.appendChild(div);
    });
  }
  if (!_axpScanState.queue.length) {
    var qEl = document.getElementById('axpScanQueue');
...
/* ── Clear queue ── */
window.axpScanClearQueue = function() {
  _axpScanState.queue = [];
  var qEl    = document.getElementById('axpScanQueue');
  var thumbs = document.getElementById('axpScanThumbs');
  var resEl  = document.getElementById('axpScanResults');
  if (qEl)    qEl.style.display = 'none';
  if (thumbs) thumbs.innerHTML  = '';
  if (resEl)  { resEl.style.display = 'none'; resEl.innerHTML = ''; }
  _axpScanState.results = [];
};
 
/* ── Process all queued images with Tesseract ── */
window.axpScanProcessAll = async function() {
  if (!_axpScanState.queue.length) {
    axpAlert('No Images', 'Add at least one photo first.', 'warning'); return;
  }
  if (!window._axpBulk || !window._axpBulk.students.length) {
    axpAlert('No Roster', 'Load students first before scanning.', 'warning'); return;
  }
 
  var prog   = document.getElementById('axpScanOcrProgress');
  var status = document.getElementById('axpScanOcrStatus');
  var fill   = document.getElementById('axpScanOcrFill');
  var sub    = document.getElementById('axpScanOcrSub');
  var btn    = document.getElementById('axpScanProcessBtn');
 
  if (prog)  prog.style.display  = 'block';
  if (btn)   { btn.disabled = true; btn.innerHTML = '<span class="axp-spinner-sm"></span> Scanning…'; }
 
  var allRawLines = [];
  var total = _axpScanState.queue.length;
 
  try {
    var Tesseract = await _axpLoadTesseract();
    if (status) status.textContent = 'Loading OCR engine…';
    if (fill)   fill.style.width   = '5%';
 
    /* Create / reuse worker */
    if (!_axpScanState.worker) {
      _axpScanState.worker = await Tesseract.createWorker('eng', 1, {
        logger: function(m) {
          if (m.status === 'recognizing text' && fill) {
            /* Will be overridden per-image below */
          }
        }
      });
      await _axpScanState.worker.setParameters({
        tessedit_char_whitelist: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789 .-/',
        preserve_interword_spaces: '1'
      });
    }
 
    for (var imgIdx = 0; imgIdx < total; imgIdx++) {
      var pct = Math.round(((imgIdx) / total) * 90) + 5;
      if (status) status.textContent = 'Scanning image ' + (imgIdx+1) + ' of ' + total + '…';
      if (fill)   fill.style.width   = pct + '%';
      if (sub)    sub.textContent    = 'Using Tesseract.js OCR engine (3-10× faster mode)';
 
      var { data } = await _axpScanState.worker.recognize(_axpScanState.queue[imgIdx]);
      var lines = data.text.split('\n').map(function(l){ return l.trim(); }).filter(Boolean);
      allRawLines = allRawLines.concat(lines);
    }
 
    if (fill)   fill.style.width   = '95%';
    if (status) status.textContent = 'Matching names with roster…';
 
    /* ── Parse OCR lines → { name, score } ── */
    var extracted = _axpScanParseLines(allRawLines);
 
    /* ── Match against loaded roster ── */
    var matched   = _axpScanMatchRoster(extracted);
    _axpScanState.results = matched;
 
    if (fill)   fill.style.width   = '100%';
    if (status) status.textContent = 'Done! ' + matched.filter(function(r){ return r.matched; }).length + ' students matched.';
 
    setTimeout(function() { if (prog) prog.style.display = 'none'; }, 1500);
 
    _axpScanShowResults(matched);
 
  } catch(ex) {
    if (prog) prog.style.display = 'none';
    axpAlert('OCR Failed', ex.message, 'danger');
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="bi bi-cpu"></i> Scan All with OCR'; }
  }
};
 
/* ── Parse raw OCR lines into { rawName, score } pairs ── */
function _axpScanParseLines(lines) {
  var results = [];
  lines.forEach(function(line) {
    /* Look for a number (0-100) at end or start of line — that is the score */
    var scoreMatch = line.match(/\b([0-9]{1,3})\b/g);
    if (!scoreMatch) return;
 
    /* Find score: last number in range 0-100 */
    var score = null;
    for (var s = scoreMatch.length - 1; s >= 0; s--) {
      var n = parseInt(scoreMatch[s], 10);
      if (n >= 0 && n <= 100) { score = n; break; }
    }
    if (score === null) return;
 
    /* Name = everything that is not the score number, stripped of extra symbols */
    var nameRaw = line
      .replace(new RegExp('\\b' + score + '\\b'), '')
      .replace(/[^A-Za-z\s\.\-]/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
 
    if (nameRaw.length >= 3) {
      results.push({ rawName: nameRaw.toUpperCase(), score: score });
    }
  });
  return results;
}
 
/* ── Match extracted names against the loaded roster (fuzzy) ── */
function _axpScanMatchRoster(extracted) {
  var students = window._axpBulk ? window._axpBulk.students : [];
 
  /* Build upper-cased roster map */
  var rosterMap = {}; /* upperName → original student object */
  students.forEach(function(s) {
    rosterMap[s.name.toUpperCase()] = s;
  });
 
  return extracted.map(function(item) {
    var rawUp = item.rawName.toUpperCase();
    var bestMatch = null, bestScore = 0;
 
    Object.keys(rosterMap).forEach(function(rosterName) {
      var sim = _axpStrSimilarity(rawUp, rosterName);
      if (sim > bestScore) { bestScore = sim; bestMatch = rosterMap[rosterName]; }
    });
 
    return {
      rawName : item.rawName,
      score   : item.score,
      matched : bestScore >= 0.55,
      student : bestMatch,
...
function _axpStrSimilarity(a, b) {
  if (a === b) return 1;
  if (a.length < 2 || b.length < 2) return 0;
  function bigrams(s) {
    var bg = {};
    for (var i = 0; i < s.length - 1; i++) {
      var bg2 = s.substr(i, 2);
      bg[bg2] = (bg[bg2] || 0) + 1;
    }
    return bg;
  }
  ...
  var intersection = 0;
  Object.keys(bgA).forEach(function(k) {
...
/* ── Show results confirmation table ── */
function _axpScanShowResults(results) {
  var el = document.getElementById('axpScanResults');
  if (!el) return;
 
  var matched   = results.filter(function(r){ return r.matched; });
  var unmatched = results.filter(function(r){ return !r.matched; });
 
  var rows = results.map(function(r, i) {
    var bg = r.matched ? '' : 'background:#fff5f5;';
    var confColor = r.confidence >= 80 ? '#10b981' : r.confidence >= 55 ? '#f59e0b' : '#ef4444';
    var studentName = r.matched && r.student ? escapeHtml(r.student.name) : '<span style="color:#ef4444;">No match</span>';
    var scoreValid  = r.score >= 0 && r.score <= 100;
    return '<tr style="' + bg + '">' +
      '<td style="font-size:12px;color:#94a3b8;">' + (i+1) + '</td>' +
      '<td style="font-size:12px;font-family:monospace;">' + escapeHtml(r.rawName) + '</td>' +
      '<td>' + studentName + '</td>' +
      '<td style="text-align:center;">' +
        '<input type="number" min="0" max="100" value="' + (scoreValid ? r.score : '') + '"' +
        ' style="width:70px;text-align:center;padding:4px;border:1.5px solid #e2e8f0;font-weight:700;"' +
        ' data-idx="' + i + '" oninput="window._axpScanResults[' + i + '].score=parseInt(this.value)||0;" />' +
      '</td>' +
      '<td style="text-align:center;font-size:11px;font-weight:700;color:' + confColor + ';">' +
        r.confidence + '%</td>' +
      '<td style="text-align:center;">' +
        (r.matched ? '<input type="checkbox" checked data-idx="' + i + '" id="axpScanChk_' + i + '" />'
                   : '<input type="checkbox" data-idx="' + i + '" id="axpScanChk_' + i + '" disabled />') +
      '</td>' +
    '</tr>';
  }).join('');
  ...
  el.innerHTML =
    '<div style="margin-bottom:12px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;">' +
      '<span style="font-size:13px;font-weight:700;color:#10b981;"><i class="bi bi-check-circle-fill"></i> ' +
        matched.length + ' matched</span>' +
      (unmatched.length ? '<span style="font-size:13px;font-weight:700;color:#ef4444;"><i class="bi bi-x-circle-fill"></i> ' +
        unmatched.length + ' unmatched</span>' : '') +
    '</div>' +
    '<div style="overflow-x:auto;">' +
    '<table class="axp-table" style="min-width:520px;margin-bottom:14px;">' +
      '<thead><tr>' +
    ...
    '</table></div>' +
    (unmatched.length ?
      '<div class="axp-alert axp-alert-warning" style="margin-bottom:12px;">' +
        '<i class="bi bi-exclamation-triangle-fill"></i>' +
        '<span>' + unmatched.length + ' name(s) could not be matched. ' +
        'They are unchecked. You can manually correct the score and re-run, ' +
        'or switch to Grid mode to fill them manually.</span></div>' : '') +
    '<div style="display:flex;gap:8px;flex-wrap:wrap;">' +
      '<button onclick="axpScanApplyToGrid()" class="axp-btn-primary" style="background:#10b981;">' +
        '<i class="bi bi-check2-all"></i> Apply to Grid & Submit</button>' +
      '<button onclick="axpScanClearQueue()" class="axp-btn-secondary">' +
        '<i class="bi bi-arrow-counterclockwise"></i> Rescan</button>' +
    '</div>';
 
  /* Expose results for inline score editing */
  window._axpScanResults = results;
}
 
/* ── Apply matched scores to the bulk grid, then switch to grid mode ── */
window.axpScanApplyToGrid = function() {
  var results = window._axpScanResults || [];
  var applied = 0;
 
  results.forEach(function(r, i) {
    var chk = document.getElementById('axpScanChk_' + i);
    if (!chk || !chk.checked) return;
    if (!r.matched || !r.student) return;
    var score = parseInt(r.score, 10);
    if (isNaN(score) || score < 0 || score > 100) return;
 
    /* Apply to _axpBulk marks */
    if (window._axpBulk) {
      window._axpBulk.marks[r.student.name] = score;
      /* Update grid input if visible */
      var idx = window._axpBulk.students.findIndex(function(s){ return s.name === r.student.name; });
      if (idx >= 0) {
        var inp = document.getElementById('axpBulkScore_' + idx);
        var grd = document.getElementById('axpBulkGrade_' + idx);
        if (inp) { inp.value = score; }
        if (grd) {
          var g = _axpGrade(score);
          grd.innerHTML = '<strong style="font-size:15px;font-weight:900;color:' + g.color + ';">' + g.grade + '</strong>';
        }
      }
      applied++;
    }
  ...
  _axpBulkUpdateStats();
  axpBulkMode('grid');
  _axpToast(applied + ' scores applied to grid — review and submit.', 'success');
};
 
/* ═══════════════════════════════════════════════════════════
   CORRECTIONS LOGIC
═══════════════════════════════════════════════════════════ */
window.axpCorLoadStudents = async function() {
  const state=window._axpCorWizardState||{};
  const {examType,cls,subject}=state;
  if(!examType||!cls||!subject){
    axpAlert("Selection Required","Complete all three steps above.","warning");return;}
  ...
  try{
    ...
    const students=_axpSortStudents(res.students||[]);
    if(!students.length){
      document.getElementById("axpCorEmptyState").innerHTML=`
        <div class="axp-alert axp-alert-warning">
          <i class="bi bi-exclamation-triangle"></i>
    ...
    window._axpCorStudents = students;
    window._axpCorSelectedStudent = "";
 
    _axpCorRenderList(students);
    _axpSetText("axpCorStudentCount", `${students.length} students`);
 
    /* Reset search & badge */
    const si=document.getElementById("axpCorSearchInput"); if(si) si.value="";
    const badge=document.getElementById("axpCorSelectedBadge");
    if(badge) badge.style.display="none";
 
    document.getElementById("axpCorStudentPicker").style.display="block";
    document.getElementById("axpCorForm").style.display="none";
    document.getElementById("axpCorEmptyState").innerHTML="";
 
    /* Focus search box */
    setTimeout(()=>{ const si=document.getElementById("axpCorSearchInput"); if(si) si.focus(); },80);
 
  }catch(e){axpAlert("Load Failed","Failed to load students.","danger");}
};
 
/* Render (filtered) list rows */
function _axpCorRenderList(students) {
  const list = document.getElementById("axpCorStudentList");
  if (!list) return;
  if (!students.length) {
    list.innerHTML=`<div style="padding:16px;text-align:center;font-size:13px;color:#94a3b8;">
      No matches found</div>`; return;
  }
  list.innerHTML = students.map((s,i)=>{
    const cand = s.candidateNo || _axpCandNo(
      window._axpCorStudents ? window._axpCorStudents.indexOf(s) : i);
    const isActive = window._axpCorSelectedStudent === s.name;
    return `
    <div onclick="axpCorSelectStudent('${escapeHtml(s.name).replace(/'/g,"\\'")}',this)"
      data-name="${escapeHtml(s.name)}"
      style="display:flex;align-items:center;gap:12px;padding:10px 14px;
             cursor:pointer;border-bottom:1px solid #f1f5f9;
             background:${isActive?"#060c1c":"#fff"};
             transition:background .1s;"
      onmouseenter="if(!this.classList.contains('axp-cor-active'))
        this.style.background='#f0fdf9'"
      onmouseleave="if(!this.classList.contains('axp-cor-active'))
        this.style.background='#fff'"
      ${isActive?`class="axp-cor-active"`:""}>
      <div style="width:32px;height:32px;min-width:32px;
                  background:${isActive?"rgba(78,204,163,.15)":"#f1f5f9"};
                  display:flex;align-items:center;justify-content:center;">
        <i class="bi bi-person-fill"
           style="font-size:14px;color:${isActive?"#4ecca3":"#94a3b8"};"></i>
      </div>
      <div style="flex:1;min-width:0;">
        ...
        <div style="font-size:11px;color:${isActive?"rgba(255,255,255,.5)":"#94a3b8"};
                    font-family:monospace;margin-top:1px;">${escapeHtml(cand)}</div>
      </div>
      ${isActive
        ? `<i class="bi bi-check-circle-fill" style="color:#4ecca3;font-size:16px;"></i>`
...
/* Live search filter */
window.axpCorFilterStudents = function(query) {
  if (!window._axpCorStudents) return;
  const q = query.trim().toLowerCase();
  const filtered = !q ? window._axpCorStudents :
    window._axpCorStudents.filter(s => {
      const cand = (s.candidateNo || "").toLowerCase();
      return s.name.toLowerCase().includes(q) || cand.includes(q);
    });
  _axpCorRenderList(filtered);
};
 
window.axpCorSelectStudent=function(name, rowEl){
  window._axpCorSelectedStudent = name;
 
  /* Highlight selected row — re-render filtered list */
  const list = document.getElementById("axpCorStudentList");
  if (list) {
    const q = (document.getElementById("axpCorSearchInput")||{}).value||"";
    axpCorFilterStudents(q);
  }
 
  /* Show selected badge */
  const badge = document.getElementById("axpCorSelectedBadge");
  const nameEl = document.getElementById("axpCorSelectedName");
  if (badge) { badge.style.display="flex"; }
  if (nameEl) nameEl.textContent = name;
 
  /* Show correction form — clear new score & reason, reset old score to loading state */
  const form=document.getElementById("axpCorForm");
  if(form) form.style.display="block";
 
  const oldInp = document.getElementById("axpCorOldValue");
  const newInp = document.getElementById("axpCorNewValue");
  const reason = document.getElementById("axpCorReason");
  const spinner= document.getElementById("axpCorOldFetchSpinner");
  const status = document.getElementById("axpCorScoreStatus");
  const submit = document.getElementById("axpCorSubmitBtn");
  const preview= document.getElementById("axpCorGradePreview");
 
  if(oldInp) { oldInp.value=""; oldInp.placeholder="Fetching…"; }
  ...
  setTimeout(()=>{ const f=document.getElementById("axpCorForm");
    if(f) f.scrollIntoView({behavior:"smooth",block:"nearest"}); },100);
  ...
  _apiGet({
    ...
    if(res.status==="success"){
      const existing = res.score;
      if(existing !== null && existing !== "" && existing !== undefined){
        if(oldInp){
          oldInp.value       = existing;
          oldInp.placeholder = "";
        }
        if(status){
          status.style.display    = "block";
          status.style.borderColor= "#4ecca3";
          status.style.background = "#f0fdf9";
          const g = _axpGrade(parseInt(existing,10));
          status.innerHTML = `<i class="bi bi-check-circle-fill" style="color:#4ecca3;margin-right:6px;"></i>
            Current score fetched: <strong>${existing}</strong>
            <span style="color:${g.color};font-weight:900;margin-left:6px;">${g.grade}</span>`;
        }
      } else {
        /* Student has no score yet — cannot correct what doesn't exist */
        if(oldInp){ oldInp.value=""; oldInp.placeholder="No score recorded"; }
        if(status){
          status.style.display    = "block";
          status.style.borderColor= "#f59e0b";
          status.style.background = "#fffbeb";
          status.innerHTML = `<i class="bi bi-exclamation-triangle-fill" style="color:#f59e0b;margin-right:6px;"></i>
            No score recorded for this student in <strong>${subject}</strong> yet.
            Submit marks first before correcting.`;
        }
        if(submit) submit.disabled=true;
      }
    } else {
      if(spinner) spinner.style.display="none";
      if(oldInp){ oldInp.value=""; oldInp.placeholder="Fetch failed"; }
      if(status){
        status.style.display    = "block";
        status.style.borderColor= "#ef4444";
        status.style.background = "#fff5f5";
        status.innerHTML = `<i class="bi bi-x-circle-fill" style="color:#ef4444;margin-right:6px;"></i>
          Could not fetch score: ${res.message||"Unknown error"}`;
      }
      if(submit) submit.disabled=true;
    }
  }).catch(function(err){
    if(spinner) spinner.style.display="none";
    if(oldInp){ oldInp.value=""; oldInp.placeholder="Network error"; }
    if(status){
      status.style.display    = "block";
      status.style.borderColor= "#ef4444";
      status.style.background = "#fff5f5";
...
/* Clear the selected student and hide the form */
window.axpCorClearStudent = function() {
  window._axpCorSelectedStudent = "";
  const badge = document.getElementById("axpCorSelectedBadge");
  if (badge) badge.style.display = "none";
  const form = document.getElementById("axpCorForm");
  if (form) form.style.display = "none";
  /* Re-render list without highlight */
  const q = (document.getElementById("axpCorSearchInput")||{}).value||"";
  axpCorFilterStudents(q);
};
 
window.axpCorPreviewGrade=function(){
  const val=(document.getElementById("axpCorNewValue")||{}).value;
  const prev=document.getElementById("axpCorGradePreview");if(!prev)return;
  if(!val){prev.innerHTML="";return;}
  const{grade,color}=_axpGrade(parseInt(val));
  prev.innerHTML=`<span style="font-size:12.5px;color:#64748b;">New grade: </span>
    <strong style="font-size:18px;font-weight:900;color:${color};">${grade}</strong>`;
};
 
window.axpSubmitCorrection=function(){
  const state=window._axpCorWizardState||{};
  const {examType,cls,subject}=state;
  const studentName=window._axpCorSelectedStudent||"";
  const oldValue=(document.getElementById("axpCorOldValue")||{}).value.trim();
  const newValue=(document.getElementById("axpCorNewValue")||{}).value.trim();
  const reason=(document.getElementById("axpCorReason")||{}).value.trim();
 
  if(!studentName){axpAlert("Missing","Select a student.","warning");return;}
  if(!oldValue){axpAlert("Missing","Enter the current (old) score.","warning");return;}
  if(!newValue){axpAlert("Missing","Enter the new correct score.","warning");return;}
  if(!reason){axpAlert("Missing","Reason is required.","warning");return;}
 
  const nOld=parseInt(oldValue),nNew=parseInt(newValue);
  if(isNaN(nOld)||nOld<0||nOld>100){axpAlert("Invalid","Old score must be 0–100.","danger");return;}
  if(isNaN(nNew)||nNew<0||nNew>100){axpAlert("Invalid","New score must be 0–100.","danger");return;}
  if(nOld===nNew){axpAlert("No Change","Old and new scores are identical.","warning");return;}
 
  const{grade:og,color:oc}=_axpGrade(nOld);const{grade:ng,color:nc}=_axpGrade(nNew);
 
  axpConfirm("Confirm Correction",
    `Correcting <strong>${escapeHtml(studentName)}</strong>:<br><br>
     <span style="font-size:22px;font-weight:900;color:${oc};">${nOld}</span>
     <span style="color:${oc};font-weight:700;"> (${og})</span>
     &nbsp;→&nbsp;
     <span style="font-size:22px;font-weight:900;color:${nc};">${nNew}</span>
     <span style="color:${nc};font-weight:700;"> (${ng})</span>
     <br><br><em style="font-size:12px;color:#64748b;">Reason: ${escapeHtml(reason)}</em>`,
    async()=>{
      const btn=document.getElementById("axpCorSubmitBtn");
      if(btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Submitting…';}
      try{
        const res=await _apiPost({
          mode:"correct",
          ...
          schoolId:_appScriptSchoolId,year:_schoolMeta.year,
          class:cls,examType,subject,
          studentName,   /* flat field — backend uses this */
          mark:nNew      /* backend reads e.parameter.mark */
        });
        if(res.status==="success"){
          axpAlert("Correction Saved",
            `Score for <strong>${escapeHtml(studentName)}</strong> updated:
             <strong>${nOld}</strong> → <strong>${nNew}</strong>.`,"success");
          axpCorReset();
          _axpToast("Correction submitted.","success");
        } else axpAlert("Failed",res.message||"Limit may have been reached.","danger");
      }catch(e){axpAlert("Network Error","Please try again.","danger");}
      finally{if(btn){btn.disabled=false;
        btn.innerHTML='<i class="bi bi-check2-circle"></i> Submit Correction';}}
    },"Confirm Correction","background:#f59e0b;");
};
 
window.axpCorReset=function(){
  ["axpCorForm","axpCorStudentPicker"].forEach(id=>{
    const el=document.getElementById(id);if(el)el.style.display="none";
  });
  ["axpCorOldValue","axpCorNewValue","axpCorReason"].forEach(id=>{
    const el=document.getElementById(id);if(el)el.value="";
  });
  window._axpCorSelectedStudent="";
  const gp=document.getElementById("axpCorGradePreview");if(gp)gp.innerHTML="";
  const cm=document.getElementById("axpCorMsg");if(cm)cm.innerHTML="";
  document.getElementById("axpCorEmptyState").innerHTML=`
    <div class="axp-empty-state" style="padding:40px 16px;">
      <i class="bi bi-pencil-square" style="font-size:38px;"></i>
      <p>Choose exam, class, and subject above to start a correction.</p>
    </div>`;
  /* Re-boot correction wizard */
  _axpBootCorWizard();
};
 
/* End of Teacher Feeding Area */
 
 
/* ── Print result response panel ── */
function _axpShowPrintResult(containerId, info, error) {
  const el = document.getElementById(containerId);
  if (!el) return;
  if (error) {
    el.innerHTML = `
      <div style="display:flex;align-items:flex-start;gap:12px;padding:14px 16px;
                  background:#fef2f2;border:1.5px solid #fca5a5;margin-top:14px;">
        <i class="bi bi-x-circle-fill" style="color:#ef4444;font-size:20px;margin-top:1px;"></i>
        <div>
          <div style="font-size:13px;font-weight:800;color:#dc2626;margin-bottom:3px;">
            PDF Generation Failed</div>
          <div style="font-size:12px;color:#7f1d1d;">${escapeHtml(error)}</div>
        </div>
      </div>`; return;
  }
  /* Success */
  const rows = info.subjects
    ? info.subjects.map(s=>`<span style="display:inline-block;padding:3px 8px;
        margin:2px;border:1px solid #d1d5db;font-size:11px;color:#374151;">
        <i class="bi bi-file-earmark-pdf" style="color:#10b981;"></i> ${escapeHtml(s)}</span>`).join("")
    : "";
  el.innerHTML = `
    <div style="display:flex;align-items:flex-start;gap:12px;padding:14px 16px;
                background:#f0fdf4;border:1.5px solid #86efac;margin-top:14px;">
      <i class="bi bi-check-circle-fill" style="color:#10b981;font-size:22px;margin-top:1px;"></i>
      <div style="flex:1;">
        <div style="font-size:13px;font-weight:800;color:#065f46;margin-bottom:4px;">
          PDF Downloaded Successfully</div>
        ${info.filename ? `
          <div style="font-size:12px;color:#064e3b;margin-bottom:6px;">
        ...
        ${info.count ? `
          <div style="font-size:12px;color:#064e3b;margin-bottom:6px;">
            <strong>${info.count}</strong> PDF${info.count!==1?"s":""} saved to your Downloads folder.
          </div>` : ""}
        ${rows ? `<div style="margin-top:4px;">${rows}</div>` : ""}
        <div style="font-size:11px;color:#6b7280;margin-top:6px;">
          <i class="bi bi-info-circle"></i>
          Check your browser's Downloads folder. If the file didn't open automatically,
          tap the download notification or open it from Downloads.
        </div>
      </div>
    </div>`;
}
...
/* ═══════════════════════════════════════════════════════════════
   PRINT SHEETS  —  WIZARD + jsPDF GENERATOR
   Requires jsPDF (loaded on demand from cdnjs)
   All drawing is done programmatically — no DOM/screenshot capture.
═══════════════════════════════════════════════════════════════ */
 
/* ── Boot print wizard (Exam + Class only — no subject needed for general sheet) ── */
function _axpBootPrintWizard() {
  const state = { examType:"", cls:"" };
 
  function buildStep1() {
    _axpBuildSelector({
      containerId:"axpPrintStepExam", stepNo:1, label:"Exam Type",
      items: _axpGetExamTypes(),
      iconFn: ()=>"bi bi-journal-text",
      selectedValue: state.examType,
      onSelect(val) {
        state.examType=val; state.cls="";
        buildStep2(); sync();
      }
    });
  }
  function buildStep2() {
    _axpBuildSelector({
      containerId:"axpPrintStepClass", stepNo:2, label:"Class",
      items: _axpGetClasses(),
      iconFn: ()=>"bi bi-people",
      locked: !state.examType,
      selectedValue: state.cls,
      onSelect(val) { state.cls=val; sync(); }
    });
  }
  function sync() {
    const wrap=document.getElementById("axpPrintLoadWrap");
    if(wrap) wrap.style.display=(state.examType&&state.cls)?"block":"none";
    window._axpPrintWizardState=state;
  }
  buildStep1(); buildStep2();
}
 
/* ── Load roster for print (all subjects for the class) ── */
window.axpPrintLoadRoster = async function() {
  const state = window._axpPrintWizardState||{};
  const {examType,cls} = state;
  if (!examType||!cls) {
    axpAlert("Selection Required","Choose Exam Type and Class first.","warning"); return;
  }
 
  const btn = document.getElementById("axpPrintLoadBtn");
  if (btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Loading…';}
 
  document.getElementById("axpPrintRosterPanel").style.display="none";
  ["axpPrintSubjectResult","axpPrintGeneralResult"].forEach(id=>{
    const el=document.getElementById(id); if(el) el.innerHTML="";
  });
  document.getElementById("axpPrintEmptyState").innerHTML=`
    <div class="axp-empty-state" style="padding:40px 16px;">
      <div class="axp-spinner-sm" style="margin:0 auto 10px;width:28px;height:28px;border-width:3px;"></div>
  ...
  try {
    ...
    const subjects = _axpGetSubjects(cls);
    if (!subjects.length) {
      document.getElementById("axpPrintEmptyState").innerHTML=`
        <div class="axp-alert axp-alert-warning">
          <i class="bi bi-exclamation-triangle"></i>
          <span>No subjects configured for <strong>${escapeHtml(cls)}</strong>.
          Add subjects in School Setup.</span>
        </div>`; return;
    }
 
    /* Fetch roster for each subject in parallel */
    const fetches = subjects.map(sub =>
      _apiGet({ mode:"feedingRoster", schoolId:_appScriptSchoolId,
                year:_schoolMeta.year, examType, class:cls, subject:sub })
        .then(r => ({ subject:sub, students:_axpSortStudents(r.students||[]) }))
        .catch(()=> ({ subject:sub, students:[] }))
    );
    const results = await Promise.all(fetches);
 
    /* Collect unique students (by name) and their subjects */
    const studentMap = {};   /* name → { name, gender, candidateNo, subjects:Set } */
    results.forEach(({subject, students}) => {
      students.forEach((s,i) => {
        if (!studentMap[s.name]) {
          studentMap[s.name] = {
            name: s.name,
            gender: s.gender,
            candidateNo: s.candidateNo || _axpCandNo(i),
            subjects: new Set()
          };
        }
        studentMap[s.name].subjects.add(subject);
      });
    });
 
    const allStudents = _axpSortStudents(Object.values(studentMap));
 
    window._axpPrintData = { examType, cls, subjects, allStudents, results };
 
    /* Info strip */
    document.getElementById("axpPrintInfoStrip").innerHTML=`
      <span style="font-size:12px;color:#64748b;">
        <i class="bi bi-people-fill" style="color:#4ecca3;"></i>
      ...
      <span style="font-size:12px;color:#64748b;">
        ...
        ${escapeHtml(examType)} · ${escapeHtml(cls)} · ${escapeHtml(_schoolMeta.year||"")}
      </span>`;
 
    /* Subject checkboxes */
    document.getElementById("axpPrintSubjectCheckboxes").innerHTML =
      subjects.map(s => `
        <label style="display:inline-flex;align-items:center;gap:5px;
          ...
          <input type="checkbox" value="${escapeHtml(s)}" checked
            style="accent-color:#4ecca3;width:13px;height:13px;">
          ${escapeHtml(s)}
        </label>`).join("");
 
    document.getElementById("axpPrintEmptyState").innerHTML="";
    document.getElementById("axpPrintRosterPanel").style.display="block";
 
  } catch(e) {
    document.getElementById("axpPrintEmptyState").innerHTML=`
      <div class="axp-alert axp-alert-danger">
        <i class="bi bi-exclamation-circle"></i>
        <span>Failed to load roster. Check connection and retry.</span>
      </div>`;
  } finally {
    if (btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-cloud-download"></i> Load Class Roster';}
  }
};
 
 
/* ═══════════════════════════════════════════════════════════
   jsPDF LOADER (CDN, loaded once)
═══════════════════════════════════════════════════════════ */
function _axpLoadJsPDF() {
  return new Promise((resolve, reject) => {
...
function _axpExamLabel(examType) {
  var map = {
    "WEEKLY"    : "WEEKLY TEST",
    "MONTHLY"   : "MONTHLY TEST",
    "MIDTERM"   : "MID TERM EXAMINATION",
    "MIDTERM2"  : "MID TERM EXAMINATION II",
    "TERMINAL"  : "TERMINAL EXAMINATION",
    "JOINT"     : "JOINT EXAMINATION",
    "ANNUAL"    : "ANNUAL EXAMINATION",
    "PREMOCK"   : "PRE-MOCK EXAMINATION",
    "MOCK"      : "MOCK EXAMINATION",
    "PRENECTA"  : "PRE-NECTA EXAMINATION",
    "PRENECTA2" : "PRE-NECTA EXAMINATION II"
  };
  var key = String(examType||"").toUpperCase().trim();
  return map[key] || (key + " EXAMINATION");
}
...
var _axpPDF = {
  ...
  /* Usable width */
  get UW() { return this.W - this.ML - this.MR; },
 
  /* ── Page header ── */
  drawHeader(doc, { schoolName, schoolIndex, examType, cls, year, title, subTitle }) {
    const x = this.ML;
    let y = this.MT;
 
    /* Top accent bar — border only, no fill */
    doc.setDrawColor(6, 12, 28);
    doc.setLineWidth(0.5);
    doc.rect(x, y, this.UW, 14, "S");
 
    /* School name */
    doc.setFont("helvetica","bold");
    doc.setFontSize(12);
    doc.setTextColor(6, 12, 28);
    doc.text((schoolName||"—").toUpperCase(), x+4, y+5.5);
 
    /* School index below name */
    if (schoolIndex) {
      doc.setFont("helvetica","normal");
      doc.setFontSize(7.5);
      doc.setTextColor(80,80,80);
      doc.text("Index: " + schoolIndex, x+4, y+11);
    }
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(80,80,80);
    const meta = `${examType}  ·  ${cls}  ·  ${year}`;
    doc.text(meta, this.ML + this.UW - 4, y+5.5, {align:"right"});
 
    /* Title row — border only, no fill */
    doc.setDrawColor(15, 34, 72);
    doc.setLineWidth(0.3);
    doc.rect(x, y+14, this.UW, 9, "S");
    doc.setFont("helvetica","bold");
    doc.setFontSize(9);
    doc.setTextColor(15, 34, 72);
    doc.text(title.toUpperCase(), x+4, y+20);
    if (subTitle) {
      doc.setFont("helvetica","normal");
      doc.setFontSize(7);
      doc.setTextColor(80,100,120);
      doc.text(subTitle, this.ML+this.UW-4, y+20, {align:"right"});
    }
 
    return y + 14 + 9 + 4; /* return Y after header */
  },
 
  /* ── Page footer — Page X of Y ── */
  drawFooter(doc, pageNum, totalPages, examType, pageH, pageW, schoolName) {
    const pH = pageH || this.H;
    const pW = pageW || this.W;
    const UW = pW - this.ML - this.MR;
    const y  = pH - this.MB + 2;
 
    doc.setDrawColor(200,200,200);
    doc.setLineWidth(0.3);
    doc.line(this.ML, y, this.ML + UW, y);
 
    doc.setFont("helvetica","bold");
    doc.setFontSize(7.5);
    doc.setTextColor(30,30,30);
    doc.text(`Page ${pageNum} of ${totalPages}`, this.ML + UW, y+4, {align:"right"});
 
    doc.setFont("helvetica","normal");
    doc.setFontSize(6.5);
    doc.setTextColor(150,150,150);
    doc.text(_axpExamLabel(examType), this.ML, y+4);
 
    doc.setFont("helvetica","italic");
    doc.setFontSize(6.5);
    doc.setTextColor(150,150,150);
    doc.text((schoolName||"").toUpperCase(), this.ML + UW/2, y+4, {align:"center"});
  },
 
  /* ── Table row ── (returns new Y) */
  drawRow(doc, cols, y, rowH, isHeader, isAlt, borderColor) {
    /* Border only — no background fill on any row */
    const bc = borderColor || [0,0,0];
    doc.setDrawColor(bc[0],bc[1],bc[2]);
    doc.setLineWidth(0.2);
    doc.rect(this.ML, y, this.UW, rowH, "S");
 
    /* Cells */
    let cx = this.ML;
    cols.forEach(({text, width, align, bold, color}) => {
      doc.setFont("helvetica", bold||isHeader ? "bold":"normal");
      doc.setFontSize(isHeader ? 7.5 : 8);
      const r = color ? color[0] : 30;
      const g = color ? color[1] : 30;
      const b = color ? color[2] : 30;
      doc.setTextColor(r,g,b);
 
      /* Cell divider */
      doc.setDrawColor(bc[0],bc[1],bc[2]);
      doc.line(cx+width, y, cx+width, y+rowH);
 
      const tx = align==="center" ? cx+width/2 :
                 align==="right"  ? cx+width-2  : cx+2;
      const fs = isHeader ? 7.5 : 8;
      const ty = y + rowH/2 + fs * 0.35;
      doc.text(String(text||"").toUpperCase(), tx, ty, { align: align||"left", maxWidth: width-4 });
      cx += width;
    });
  ...
  /* ── Tick mark ── */
  drawTick(doc, cx, cy, size=3) {
    doc.setDrawColor(16,185,129);
    doc.setLineWidth(0.6);
    doc.line(cx - size*0.5, cy, cx - size*0.1, cy + size*0.5);
    doc.line(cx - size*0.1, cy + size*0.5, cx + size*0.6, cy - size*0.4);
  }
};
...
/* ═══════════════════════════════════════════════════════════
   SUBJECT MARK SHEETS
   One PDF per selected subject. Each sheet:
     Header: school / exam / class / year / subject
     Table: # | Candidate No. | Name | Gender | Score (blank) | Signature
     Rows never split across pages.
═══════════════════════════════════════════════════════════ */
window.axpPrintSubjectSheets = async function() {
  const data = window._axpPrintData;
  if (!data) return;
 
  /* Selected subjects */
  const checked = [...document.querySelectorAll("#axpPrintSubjectCheckboxes input:checked")]
    .map(i=>i.value);
  if (!checked.length) {
    axpAlert("No Subject","Select at least one subject to print.","warning"); return;
  }
 
  const btn = document.getElementById("axpPrintSubjectBtn");
  if (btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Generating…';}
 
  try {
    const JsPDF  = await _axpLoadJsPDF();
    const { examType, cls, results } = data;
    const schoolName = (_dashboardData&&_dashboardData.schoolname)||(_dashboardData&&_dashboardData.schoolindex)||"School";
    const year       = (_schoolMeta&&_schoolMeta.year)||"";
    const generatedAt = new Date().toLocaleString();
 
    for (const subjectName of checked) {
      const subjectResult = results.find(r=>r.subject===subjectName);
      const students      = subjectResult ? subjectResult.students : [];
 
      /* Column widths (mm) — total = UW = 186 — no # column */
      const COL = {
        cand : 28,
        name : 86,
        sex  : 10,
      ...
      const ROW_H        = 8;
      const PAGE_USABLE  = _axpPDF.H - _axpPDF.MT - _axpPDF.MB - 30; /* 30 = header height */
 
      /* Count pages needed */
      const rowsPerPage = Math.floor((PAGE_USABLE - HEADER_ROW_H) / ROW_H);
 
      const doc = new JsPDF({ orientation:"portrait", unit:"mm", format:"a4" });
      let pageNum = 1;
      const totalPages = Math.ceil(students.length / rowsPerPage) || 1;
 
      const drawPageHeader = (yStart) => {
        return _axpPDF.drawHeader(doc, {
          schoolName,
          schoolIndex: (_dashboardData&&_dashboardData.schoolindex)||"",
          examType,
          cls,
          year,
          title: `Subject Mark Sheet — ${subjectName}`,
          subTitle: `${students.length} students`
        });
      };
 
      const drawTableHeader = (y) => {
        return _axpPDF.drawRow(doc, [
          {text:"Cand. No.",     width:COL.cand, align:"left",   bold:true, borderColor:[0,0,0]},
          {text:"Student Name",  width:COL.name, align:"left",   bold:true, borderColor:[0,0,0]},
          {text:"Sex",           width:COL.sex,  align:"center", bold:true, borderColor:[0,0,0]},
          {text:"Score",         width:COL.score,align:"center", bold:true, borderColor:[0,0,0]},
          {text:"Signature",     width:COL.sig,  align:"center", bold:true, borderColor:[0,0,0]},
        ], y, HEADER_ROW_H, true, false, [0,0,0]);
      };
      ...
      students.forEach((s, idx) => {
        /* Need a new page? */
        if (y + ROW_H > _axpPDF.H - _axpPDF.MB - 10) {
          _axpPDF.drawFooter(doc, pageNum, totalPages, examType, _axpPDF.H, _axpPDF.W, schoolName);
          doc.addPage();
          pageNum++;
          y = drawPageHeader();
          y = drawTableHeader(y);
        ...
        const cand = s.candidateNo || _axpCandNo(idx);
        const isAlt = idx % 2 === 1;
        y = _axpPDF.drawRow(doc, [
          {text: cand,        width:COL.cand, align:"left", color:[30,100,180]},
          {text: s.name,      width:COL.name, align:"left"},
          {text: s.gender==="F"?"F":"M", width:COL.sex, align:"center"},
          {text: "",          width:COL.score,align:"center"},
          {text: "",          width:COL.sig,  align:"center"},
        ], y, ROW_H, false, isAlt, [0,0,0]);
      });
 
      /* Empty-class fallback row */
      if (!students.length) {
        y = _axpPDF.drawRow(doc, [
          {text:"No students enrolled in this subject",
           width:_axpPDF.UW, align:"center"}
        ], y, ROW_H, false, false);
      }
 
      _axpPDF.drawFooter(doc, pageNum, totalPages, examType, _axpPDF.H, _axpPDF.W, schoolName);
      doc.save(`MarkSheet_${cls}_${subjectName}_${examType}_${year}.pdf`);
    }
 
    /* ── Response panel ── */
    _axpShowPrintResult("axpPrintSubjectResult", {
      count: checked.length,
      subjects: checked,
      type: "Subject Mark Sheet"
    });
    _axpToast(`${checked.length} subject sheet${checked.length!==1?"s":""} downloaded!`,"success");
 
  } catch(e) {
    console.error(e);
    _axpShowPrintResult("axpPrintSubjectResult", null, e.message);
    axpAlert("Error", "PDF generation failed: "+e.message, "danger");
  } finally {
    if (btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-download"></i> Download Subject Sheets';}
  }
};
 
 
/* ═══════════════════════════════════════════════════════════
   GENERAL OVERVIEW / ATTENDANCE SHEET
   One PDF for the entire class:
     Columns: # | Cand.No | Name | Sex | [Subject1] | [Subject2] … | Signature
     Enrolled cell = ✓ (drawn); unenrolled = blank.
     Rows never split across pages.
═══════════════════════════════════════════════════════════ */
window.axpPrintGeneralSheet = async function() {
  const data = window._axpPrintData;
  if (!data) return;
 
  const btn = document.getElementById("axpPrintGeneralBtn");
  if (btn){btn.disabled=true;btn.innerHTML='<span class="axp-spinner-sm"></span> Generating…';}
 
  try {
    const JsPDF  = await _axpLoadJsPDF();
    const { examType, cls, subjects, allStudents } = data;
    const schoolName = (_dashboardData&&_dashboardData.schoolname)||(_dashboardData&&_dashboardData.schoolindex)||"School";
    const year       = (_schoolMeta&&_schoolMeta.year)||"";
    const generatedAt = new Date().toLocaleString();
 
    /* ── Column width calculation ──
       Fixed cols: Cand(28) + Name(66) + Sex(10) = 104
       Remaining = 186 - 104 = 82, split among subjects + Signature(22)
       Subject cols fill remaining space, min 12, max 20
    */
    const FIXED   = 28 + 66 + 10;             /* 104 — no # col */
    const SIG_W   = 22;
    const REMAIN  = _axpPDF.UW - FIXED - SIG_W; /* 60 */
    const subW    = subjects.length > 0
      ? Math.max(10, Math.min(18, Math.floor(REMAIN / subjects.length)))
      : 14;
 
    /* If too many subjects, switch to landscape */
    const minNeeded = FIXED + subjects.length * 10 + SIG_W;
    const landscape = minNeeded > 190;
    const pageW     = landscape ? 297 : 210;
    const pageH     = landscape ? 210 : 297;
    const UW        = pageW - _axpPDF.ML - _axpPDF.MR;
    const subWFinal = Math.max(10, Math.min(20,
      Math.floor((UW - FIXED - SIG_W) / Math.max(subjects.length,1))));
 
    const HEADER_ROW_H = 7;
    const ROW_H        = 7;
    const PAGE_USABLE  = pageH - _axpPDF.MT - _axpPDF.MB - 32;
    const rowsPerPage  = Math.floor((PAGE_USABLE - HEADER_ROW_H) / ROW_H);
    const totalPages   = Math.ceil(allStudents.length / rowsPerPage) || 1;
    ...
    const buildCols = () => [
      {text:"Cand. No.",   width:28,        align:"left",   bold:true},
      {text:"Student Name",width:66,        align:"left",   bold:true},
      {text:"Sex",         width:10,        align:"center", bold:true},
      ...subjects.map(s=>({text:s, width:subWFinal, align:"center", bold:true})),
      {text:"Signature",   width:SIG_W,     align:"center", bold:true},
    ];
 
    const drawPageHeader = () => {
      /* Override _axpPDF.UW for landscape */
      const savedUW = _axpPDF.UW;
      Object.defineProperty(_axpPDF,"UW",{get:()=>UW,configurable:true});
      const y = _axpPDF.drawHeader(doc, {
        schoolName,
        schoolIndex: (_dashboardData&&_dashboardData.schoolindex)||"",
        examType, cls, year,
        title: "General Subject Overview & Attendance Sheet",
        subTitle: `${allStudents.length} students · ${subjects.length} subjects`
      });
      Object.defineProperty(_axpPDF,"UW",{get:()=>savedUW,configurable:true});
      return y;
    };
 
    const drawTableHeader = (y) => {
      const cols = buildCols();
      /* Header row — border only, no fill */
      doc.setDrawColor(0,0,0);
      doc.setLineWidth(0.2);
      doc.rect(_axpPDF.ML, y, UW, HEADER_ROW_H, "S");
      ...
      cols.forEach(col => {
        ...
        doc.setTextColor(30,30,30);
        doc.setDrawColor(0,0,0);
        doc.line(cx+col.width, y, cx+col.width, y+HEADER_ROW_H);
 
        /* Rotate subject names if narrow */
        if (col.width <= 14 && col.text !== "#" && !["Cand. No.","Student Name","Sex","Signature"].includes(col.text)) {
          doc.saveGraphicsState();
          doc.text(String(col.text||"").toUpperCase(), cx + col.width/2, y + HEADER_ROW_H - 1, {
            angle: 50, align:"left", maxWidth: col.width + 4
          });
          doc.restoreGraphicsState();
        } else {
          const tx = col.align==="center" ? cx+col.width/2 : cx+2;
          doc.text(String(col.text||"").toUpperCase(), tx, y + HEADER_ROW_H/2 + 6.5 * 0.35,
            {align: col.align||"left", maxWidth: col.width-2});
        }
        cx += col.width;
      });
    ...
    allStudents.forEach((student, idx) => {
      /* New page check */
      if (y + ROW_H > pageH - _axpPDF.MB - 10) {
        _axpPDF.drawFooter(doc, pageNum, totalPages, examType, pageH, pageW, schoolName);
        doc.addPage();
        pageNum++;
        y = drawPageHeader();
        y = drawTableHeader(y);
      ...
      doc.setDrawColor(0,0,0);
      doc.setLineWidth(0.2);
      doc.rect(_axpPDF.ML, y, UW, ROW_H, "S");
 
      /* Fixed cells — no # column */
      const fixedCols = [
        {text: student.candidateNo,                width:28, align:"left",   color:[30,100,180]},
        {text: student.name,                       width:66, align:"left",   color:[20,20,20]},
        {text: student.gender==="F"?"F":"M",       width:10, align:"center", color:[80,80,80]},
      ];
      ...
      fixedCols.forEach(col => {
        ...
        const tx = col.align==="center" ? cx+col.width/2 : cx+2;
        doc.text(String(col.text||"").toUpperCase(), tx, y + ROW_H/2 + 7.5 * 0.35,
          {align:col.align||"left", maxWidth:col.width-3});
        cx += col.width;
      });
      ...
      subjects.forEach(sub => {
        doc.setDrawColor(0,0,0);
        doc.line(cx+subWFinal, y, cx+subWFinal, y+ROW_H);
        if (student.subjects.has(sub)) {
          _axpPDF.drawTick(doc, cx + subWFinal/2, y + ROW_H/2, 2.8);
        }
        cx += subWFinal;
      });
    ...
    if (!allStudents.length) {
      ...
      doc.text("No students found.", _axpPDF.ML+UW/2, y+8, {align:"center"});
    }
 
    _axpPDF.drawFooter(doc, pageNum, totalPages, examType, pageH, pageW, schoolName);
    doc.save(`GeneralSheet_${cls}_${examType}_${year}.pdf`);
 
    /* ── Response panel ── */
    _axpShowPrintResult("axpPrintGeneralResult", {
      filename: `GeneralSheet_${cls}_${examType}_${year}.pdf`,
      pages: totalPages,
      students: allStudents.length,
      type: "General Overview Sheet"
    });
    _axpToast("General sheet downloaded!","success");
 
  } catch(e) {
    console.error(e);
    _axpShowPrintResult("axpPrintGeneralResult", null, e.message);
    axpAlert("Error","PDF generation failed: "+e.message,"danger");
  } finally {
    if (btn){btn.disabled=false;btn.innerHTML='<i class="bi bi-download"></i> Download General Sheet';}
  }
};
