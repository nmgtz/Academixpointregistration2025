...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* NOTE: We do NOT auto-open auth on page load for logged-out users.
     The homepage should scroll freely. Auth opens only via Login/Register clicks. */
  ...
  if (resetEmail && resetToken) {
    ...
    /* After overlay is built, fill the hidden inputs inside the inner form */
...
/* ─────────────────────────────────────────────────────
   ...
   on TOP of the page. The body is NEVER modified so the
   homepage scrolls freely at all times.
───────────────────────────────────────────────────── */
function _ensureAuthOverlay() {
  ...
  /* We target all .login-form wrappers AND any stale form-container divs
     that are NOT inside our overlay (which doesn't exist yet).           */
  document.querySelectorAll(".login-form, .form-container").forEach(el => {
    /* Only remove nodes that are NOT already inside our overlay           */
    if (!el.closest("#axp-auth-overlay")) el.remove();
  });
  /* Also nuke any stale IDs the template may have baked in */
  ...
  ov.style.cssText = [
    ...
    "width:100vw","height:100dvh","padding:0 16px","box-sizing:border-box",
    "background:#060c1c","overflow:hidden"
  ].join(";");
  document.body.appendChild(ov);

  /* ── login-form wrapper (the scrollable centering shell) ── */
  ...
  /* ════════════════════════════════════════════
     FORM 1 — LOGIN
     Container id  : lg-form
     Inner form id : login-form
  ════════════════════════════════════════════ */
  ...
  /* ════════════════════════════════════════════
     FORM 2 — SIGN UP
     Container id  : sp-form
     Inner form id : signup-form
  ════════════════════════════════════════════ */
  ...
  /* ════════════════════════════════════════════
     FORM 3 — FORGOT PASSWORD / SEND TOKEN
     Container id  : set-form
     Inner form id : reset-form
  ════════════════════════════════════════════ */
  ...
  /* ════════════════════════════════════════════
     ...
     Container id  : newSet-form   (matches template id so toggleForms still works)
     Inner form id : new-password-form   (what submitForm looks up)
  ════════════════════════════════════════════ */
...
function _openAuthForm(formId) {
  ...
  if (ov) {
    ...
    // Make sure video/canvas go inside or behind the overlay
...
function _closeAuthForm() {
  ...
  // Return video elements to body so they don't linger
  ["axp-video-bg","axp-video-overlay","axp-canvas-bg"].forEach(id => {
    const el = document.getElementById(id);
...
function applyLandingStyles() {
  ...
  style.textContent = `
    /* ── Auth styles: scoped to the overlay div, zero body interference ── */
    body::before { content: none; }

    /* Hide auth elements until the overlay is open */
    .login-form { display: none !important; }

    /* Inside the overlay the login-form wrapper is visible */
    ...
    /* Container fills the available height while staying capped at 440px wide */
    .form-container {
      ...
      width: 100%;
      max-width: 440px;
      /* Fill viewport height with comfortable padding so nothing clips */
      ...
      /* Uniform, tightly-scaled padding so all fields are visible */
      padding: clamp(12px, 2.2vh, 28px) clamp(16px, 4vw, 34px) clamp(10px, 1.8vh, 24px);
      animation: axpSlideUp 0.6s cubic-bezier(0.22,1,0.36,1) both;
    }
    ...
    /* ── LABELS: clearly readable teal-tinted white ── */
    .form-container label {
      display: block;
      font-size: 10px;
      font-weight: 600;
      color: #a8f0da;          /* readable teal-white, not invisible on dark bg */
    ...
    .axp-input-wrap select {
      ...
      border-radius: 4px;
      padding: clamp(7px, 1.1vh, 10px) 42px clamp(7px, 1.1vh, 10px) 13px;
...
function _injectVideoBackground() {
  if (document.getElementById("axp-video-bg")) return;

  const overlay = document.createElement("div");
  overlay.id = "axp-video-overlay";
  ...
  const videoSources = [
    "https://cdn.coverr.co/videos/coverr-students-working-in-library-5144/1080p.mp4",
    "https://cdn.coverr.co/videos/coverr-a-teacher-writes-on-a-chalkboard-2647/1080p.mp4",
    "https://www.pexels.com/video/1739010/download/",
    "https://storage.googleapis.com/coverr-main/mp4/Commute.mp4"
  ];
  ...
  setTimeout(() => {
    if (!videoLoaded) _startCanvasFallback();
  }, 3200);

  document.body.prepend(vid);
  vid.play().catch(() => { if (!videoLoaded) setTimeout(_startCanvasFallback, 800); });
}
...
function _startCanvasFallback() {
  ...
  (function draw() {
    ...
    g.addColorStop(0.5, "#060e26");
    g.addColorStop(1,   "#001f18");
    ctx.fillStyle = g;
    ...
    for (let i = 0; i < N; i++) {
      for (let j = i + 1; j < N; j++) {
        ...
        if (d < LINK) {
          ...
          ctx.strokeStyle = `rgba(78,204,163,${0.13 * (1 - d / LINK)})`;
          ctx.lineWidth   = 0.55;
          ctx.stroke();
        }
      }
    ...
    pts.forEach(p => {
      ...
      ctx.fillStyle = `rgba(78,204,163,${p.a})`;
      ctx.fill();
    });
...
function showFormPage(formId) {
  /* Don't hide any page chrome — auth sits in a fixed overlay above everything */
...
function toggleForms(formIdToShow) {
  ...
  if (sel) sel.style.display = "flex";   /* flex so internal column layout works */
}
...
/* ─────────────────────────────────────────────────────
   SUBMIT FORM  (now uses overlay instead of button text)
───────────────────────────────────────────────────── */
function submitForm(action) {
  ...
  /* NOTE: "new-password-form" is the <form> element inside the "newSet-form" container */
...
function showMessage(message, success = true) {
  const visibleForm = document.querySelector(".form-container[style*='display']");
...
function _ensureSpinnerStyle() {
  /* kept for backward compatibility – spinner now lives in overlay */
}
