...
// ═══════════════════════════════════════════════════════════════════════════════
// FULLY ADJUSTED _repCapturePdf FUNCTION
// Changes made:
//   1. Swahili exam labels map (ROBO MUHULA, NUSU MUHULA, MWAKA, etc.)
//   2. Description paragraph in Swahili — correct phrasing, "katika" not comma
//   3. "aliyefanya mtihani wa ROBO MUHULA, Kidato cha..." format
//   4. "wanafunzi N" not just "N"
//   5. "ni kama ifuatavyo hapa chini:" at end of Swahili description
//   6. District header → "HALMASHAURI YA WILAYA YA [DISTRICT]" in Swahili
//   7. DAY → KUTWA in school name (Swahili only)
//   8. Info bar: NAME→JINA, CLASS→DARASA, SEX→JINSI (Swahili)
//   9. Requirements in numbered rows (1. 2. 3.)
//  10. Date spacing: title above, date value on line below — no overlap
//  11. Removed repetitive small text labels under signatures
//  12. Signature in cursive/italic bold — abbreviated first name + surname
//  13. Corner ornament fix — bracket lines only, no green fill projecting inside
// ═══════════════════════════════════════════════════════════════════════════════

// ── Swahili-aware exam label map (short form for body text)
...
async function _repCapturePdf(filename, action) {
    try {
        ...
        // Line 5: class + exam heading label
        hLines[4] = isSw
            ? `${clsLabel.toUpperCase()} — ${examLabelHeading} — ${year}`
            : `${clsLabel.toUpperCase()} ${examLabelHeading.toUpperCase()} RESULTS — ${year}`;
        ...
        sd(NAVY);  doc.setLineWidth(0.3); doc.rect(ML, TABLE_TOP, ACW, HDR_H, 'S');
        ...
        acHdrCols.forEach(c => {
            ...
            sd([170,196,224]); doc.setLineWidth(0.2); doc.rect(c.x, TABLE_TOP+HDR_H, c.w, HDR_H, 'S');
        ...
        sd(NAVY);  doc.setLineWidth(0.3); doc.rect(BHX, TABLE_TOP, BHW, HDR_H, 'S');
        ...
        subjectEntries.forEach(([sub, sc]) => {
            ...
            acRowCols.forEach(rc => {
                sf([255,255,255]); sd([200,210,220]); doc.setLineWidth(0.2);
            ...
            sd([200,210,220]); doc.setLineWidth(0.2);
        ...
        // Academic footer
        sf(LGREY); doc.rect(ML, acRowY, ACW, AC_FOOT_H, 'F');
        sd(NAVY);  doc.setLineWidth(0.3); doc.rect(ML, acRowY, ACW, AC_FOOT_H, 'S');
        const ptsLbl = lang.points   || (isSw ? 'Pointi'  : 'Points');
        const divLbl = lang.division || (isSw ? 'Daraja'  : 'Division');
        const posLbl = lang.position || (isSw ? 'Nafasi'  : 'Position');
        cellText(`${ptsLbl}: ${student.point}   ${divLbl}: ${student.division}   ${posLbl}: ${posInfo}`,
            ML, acRowY, ACW, AC_FOOT_H, {fs:8, bold:true, col:RED, align:'center'});
        ...
        behavItems.forEach(([key, val]) => {
            ...
            sd([200,210,220]); doc.rect(BHX+BHW*0.76, bhRowY, BHW*0.24, bhRowH, 'S');
        ...
        // Behaviour remark
        sf([247,250,255]); sd([200,210,220]); doc.setLineWidth(0.2);
        ...
        // Month/period if provided
        if (month) {
            const monthLbl = isSw ? 'Kipindi:' : 'Period:';
            const monthX   = ML + CW * 0.75;
            doc.setFontSize(7); doc.setFont('helvetica','bold'); st(NAVY);
            doc.text(monthLbl, monthX, DATES_TOP + 3);
            doc.setFont('helvetica','normal'); st(BLACK); doc.setFontSize(8);
            doc.text(T(month), monthX, DATES_TOP + 6.5);
        }
        ...
        if (doc.setGState) {
            ...
            st([26,58,92]); doc.setFont('helvetica','bold'); doc.setFontSize(22);
            try { doc.text(student.name.toUpperCase(), PW/2, PH/2, {align:'center', angle:28}); } catch(e){}
