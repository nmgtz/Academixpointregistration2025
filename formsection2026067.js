...
function renderResultsReportsSection(containerEl) {
    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        const uiEls = [tabsEl, actionsEl, bannerEl, folderEl].filter(Boolean);
        uiEls.forEach(el => el.style.display = 'none');

        // ── Force cont to full A4 landscape width inline ───────────────────────
        const prevStyle = cont.getAttribute('style') || '';
        cont.style.width    = '1077px';
        cont.style.maxWidth = '1077px';
        cont.style.overflow = 'visible';

        const filename = `${sanitize(sName)}_${sanitize(cls)}_${sanitize(examType)}.pdf`;

        const restore = () => {
            uiEls.forEach(el => el.style.display = '');
            cont.setAttribute('style', prevStyle);
        };
...
function renderStudentReports(students, container) {
    ...
    // Capture reportDiv directly — force width inline, restore after.
    function _repCapturePdf(filename, action) {
        const prevStyle = reportDiv.getAttribute('style') || '';
        ...
        const cleanup = () => { reportDiv.setAttribute('style', prevStyle); };
        const opt = _pdfOpt(filename);
        if (action === 'preview') {
            return html2pdf().set(opt).from(reportDiv).outputPdf('dataurlnewwindow')
