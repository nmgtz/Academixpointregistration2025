...
function renderStudentReports(students, container) {
    ...
    async function _repCapturePdf(filename, action) {
        // Clone inner content onto body so html2canvas captures it fully
        // outside any scroll/overflow container
        const inner = reportDiv.firstElementChild;
        if (!inner) { _axpToast('Report not rendered yet', 'warning'); return; }
        const clone = inner.cloneNode(true);
        clone.style.cssText = `position:fixed;top:0;left:0;width:${_REP_W}px;height:1123px;z-index:99999;background:#fff;margin:0;padding:0;box-shadow:none;border:none;overflow:hidden;box-sizing:border-box;`;
    ...
    document.getElementById('axpRepMergeAll').addEventListener('click', async () => {
        ...
        mergeContainer.style.cssText = `position:fixed;top:0;left:0;width:${_REP_W}px;background:#fff;z-index:99998;margin:0;padding:0;box-shadow:none;border:none;pointer-events:none;overflow:hidden;`;
        ...
        try {
            for (let i=0; i<eligible.length; i++) {
                ...
                if (!inner) continue;
                const clone = inner.cloneNode(true);
                clone.style.cssText += `;width:${_REP_W}px;height:1123px;overflow:hidden;display:block;flex-shrink:0;`;
            ...
            if (count === 0) {
                ...
                for (let pi = 0; pi < pages.length; pi++) {
                    ...
                    const cvs = await html2canvas(pageEl, {
                        ...
                        windowWidth: _REP_W, width: _REP_W, height: 1123,
                        scrollX: 0, scrollY: 0
                    });
