  ...
  try {
    ...
    const rows = students.map((student, index) => {
      const candNumber = currentSchoolData?.indexNumber
        ? `${currentSchoolData.indexNumber}-${String(index + 1).padStart(4, '0')}` 
        : index + 1;
      const row = [
        candNumber, 
...
// 2. ADD: Student Report Generation Function
function renderStudentReports(students) {
  ...
  if (!navContainer) {
    ...
    const buttons = [
      ...
      { id: 'downloadAllBtn', text: 'Download All PDFs' },
  ...
  engBtn.addEventListener("click", () => switchLanguage("en"));
  swaBtn.addEventListener("click", () => switchLanguage("sw"));
  highlightLanguage();
  ...
  function updateReport(index) {
    ...
    // Get school info from localStorage or wizard
    const savedClassData = JSON.parse(localStorage.getItem('school_info_by_class') || '{}')[selectedClass] || {};
    
    const schoolName = savedClassData.schoolNameInput || currentSchoolData?.schoolName || 'School Name';
    const schoolIndex = savedClassData.schoolIndexInput || currentSchoolData?.indexNumber || '';
    const openingDate = savedClassData.openingDateInput || '';
    const month = savedClassData.monthInput || '';
    const closingDate = savedClassData.closingDateInput || '';
    const classTeacher = savedClassData.classTeacherInput || '';
    const headmaster = savedClassData.headmasterInput || '';
    const requirement1 = savedClassData.requirement1 || '';
    const requirement2 = savedClassData.requirement2 || '';
    const requirement3 = savedClassData.requirement3 || '';
    
    const requirements = [requirement1, requirement2, requirement3].filter(r => r !== "");
    const formattedRequirements = requirements.map(item => `<li>${item}</li>`).join('');
    ...
    if (!eligibleStudent) {
      reportDiv.innerHTML = `
        <div style="text-align: center; color: #c62828; padding: 20px;">
          <p>No report data available for ${student.name}</p>
        </div>
      `;
      return;
    }
    ...
    reportDiv.innerHTML = `
      <div style="position: relative; min-height: 100vh; background: #ffffff; padding: 20px;">
        <!-- Watermark -->
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-45deg); font-size: 60px; font-weight: bold; color: rgba(0, 0, 0, 0.02); white-space: nowrap; user-select: none; pointer-events: none;">
          ${student.name.toUpperCase()}
        </div>

        <!-- Content -->
        <div style="position: relative; z-index: 2;">
          <div style="position: absolute; top: 10px; left: 10px; font-size: 11px; font-weight: bold;">
            ${schoolIndex}-${String(index + 1).padStart(4, '0')}
          </div>
          
          ${schoolLogo ? `
            <div style="text-align: center; margin-bottom: 20px;">
              <img src="${schoolLogo}" style="width: 80px; height: 80px;">
            </div>
          ` : ''}

          <h2 style="text-align: center; margin: 10px 0; color: #1565c0;">${schoolName.toUpperCase()}</h2>
          <h4 style="text-align: center; margin: 10px 0;">${lang.reportTitle} - ${month.toUpperCase()}</h4>

          <p style="margin: 20px 0;">
            ${lang.examNotice} <strong>${student.name}</strong><br>
            ${lang.satFor} ${examType.toUpperCase()} ${lang.at} <strong>${schoolName}</strong>
          </p>

          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <thead>
              <tr style="background: #e3f2fd;">
                <th style="border: 1px solid #90caf9; padding: 8px;">${lang.subject}</th>
                <th style="border: 1px solid #90caf9; padding: 8px;">${lang.mark}</th>
                <th style="border: 1px solid #90caf9; padding: 8px;">${lang.comment}</th>
              </tr>
            </thead>
            <tbody>
              ${Object.entries(student.scores).map(([subject, s]) => {
                ...
                return `
                  <tr>
                    <td style="border: 1px solid #e0e0e0; padding: 6px;">${subject}</td>
                    <td style="border: 1px solid #e0e0e0; padding: 6px;">${s.mark} - '${s.grade}'</td>
                    <td style="border: 1px solid #e0e0e0; padding: 6px;">${lang.comments[s.grade] || '-'}</td>
                  </tr>
                `;
              }).join('')}
              <tr>
                <td colspan="3" style="border: 1px solid #e0e0e0; padding: 8px;">
                  ${lang.points}: <b>${student.point}</b> | ${lang.division}: <b>${student.division}</b>
                </td>
              </tr>
              <tr>
                <td colspan="3" style="border: 1px solid #e0e0e0; padding: 8px;">
                  ${lang.position}: ${positionInfo}
                </td>
              </tr>
            </tbody>
          </table>

          <p><strong>${lang.termDates}:</strong> ${lang.closes} ${closingDate} ${lang.reopens} ${openingDate}</p>
          
          ${formattedRequirements ? `
            <p><strong>${lang.requirements}:</strong></p>
            <ul>${formattedRequirements}</ul>
          ` : ''}

          <p style="margin-top: 20px;">
            <strong>${lang.classComment}:</strong> ${student.name} has shown good progress this term.<br>
            <strong>${lang.signature}:</strong> ${classTeacher} | <strong>${lang.date}:</strong> ${closingDate}
          </p>

          <p style="margin-top: 20px;">
            <strong>${lang.headComment}:</strong> Keep up the good work.<br>
            <strong>${lang.signature}:</strong> ${headmaster} | <strong>${lang.date}:</strong> ${closingDate}
          </p>
        </div>
      </div>
    `;
  ...
  downloadAllBtn.addEventListener("click", async () => {
    ...
    try {
      for (let i = 0; i < eligible.length; i++) {
        ...
        const hasNoData = reportContent.includes('No report data available');
        ...
        const filename = `${schoolIndex}_${selectedClass}_${examType}_Report_${repNumber}.pdf`;
  ...
  previewBtn.addEventListener("click", () => {
    const opt = {
      margin: 0.2,
      filename: `Report_${currentIndex + 1}.pdf`,
      image: { type: 'jpeg', quality: 0.98 },
      html2canvas: { scale: 4 },
      jsPDF: { unit: 'in', format: 'a4', orientation: 'portrait' }
    };
...
// 3. ADD: Certificate Generation Function
function renderStudentCertificates(students) {
  ...
  if (allStudents.length === 0) {
    reportContainer.innerHTML = '<div style="text-align: center; padding: 2em; color: #d32f2f;">No students found for certificate generation.</div>';
    return;
  }
  ...
  navContainer.innerHTML = `
    <button id="certPrevBtn">Back</button>
    <button id="certNextBtn">Next</button>
    <button id="certPreviewBtn">Preview</button>
    <button id="certDownloadBtn">Download Certificate</button>
    <button id="certDownloadAllBtn">Download All Certificates</button>
    <button id="certEngBtn">English</button>
    <button id="certSwaBtn">Swahili</button>
  `;
  reportContainer.insertAdjacentElement('beforebegin', navContainer);
  ...
  function updateCertificate(index) {
    ...
    certificateDiv.innerHTML = `
      <div style="width: 210mm; height: 280mm; margin: 0 auto; padding: 20mm; background: white; position: relative; font-family: 'Georgia', serif;">
        <!-- Decorative Corners -->
        <div style="position: absolute; top: 15mm; left: 15mm; width: 60px; height: 60px; border-top: 3px solid #DAA520; border-left: 3px solid #DAA520;"></div>
        <div style="position: absolute; top: 15mm; right: 15mm; width: 60px; height: 60px; border-top: 3px solid #DAA520; border-right: 3px solid #DAA520;"></div>
        <div style="position: absolute; bottom: 15mm; left: 15mm; width: 60px; height: 60px; border-bottom: 3px solid #DAA520; border-left: 3px solid #DAA520;"></div>
        <div style="position: absolute; bottom: 15mm; right: 15mm; width: 60px; height: 60px; border-bottom: 3px solid #DAA520; border-right: 3px solid #DAA520;"></div>
        
        <!-- Header -->
        <div style="text-align: center; margin-bottom: 40px;">
          ${schoolLogo ? `<img src="${schoolLogo}" style="width: 80px; height: 80px; margin-bottom: 15px;">` : ''}
          <h1 style="margin: 10px 0; font-size: 32px; color: #1565c0; text-transform: uppercase; letter-spacing: 3px;">
            ${lang.certificate}
          </h1>
          <div style="width: 180px; height: 3px; background: linear-gradient(90deg, transparent, #DAA520, transparent); margin: 15px auto;"></div>
        </div>
        
        <!-- School Info -->
        <div style="text-align: center; margin-bottom: 50px;">
          <h2 style="margin: 5px 0; font-size: 22px; color: #333;">${schoolName.toUpperCase()}</h2>
          <p style="margin: 5px 0; font-size: 14px; color: #666;">${schoolIndex.toUpperCase()}</p>
        </div>
        ...
        <div style="text-align: center; margin: 60px 0;">
          ...
          <h2 style="font-size: 32px; color: #1565c0; margin: 30px 0; font-weight: bold; text-transform: uppercase;">
            ${student.name}
          </h2>
          
          <div style="width: 280px; height: 2px; background: #DAA520; margin: 30px auto;"></div>
          ...
          <p style="font-size: 16px; color: #666; margin: 30px 0;">
            ${lang.conducted} <strong>${schoolName.toUpperCase()}</strong>
          </p>
        </div>
        
        <!-- Signatures -->
        <div style="display: flex; justify-content: space-around; margin-top: 80px; padding-top: 20px; border-top: 2px solid #f0f0f0;">
          <div style="text-align: center; flex: 1;">
            <div style="border-bottom: 2px solid #333; width: 180px; margin: 0 auto 10px; height: 35px;"></div>
            <div style="font-weight: bold; color: #333;">${classTeacher}</div>
            <div style="font-size: 12px; color: #666;">${lang.classTeacher}</div>
            <div style="font-size: 11px; color: #999; margin-top: 5px;">${lang.date}: ${closingDate}</div>
          </div>
  ...
  certDownloadBtn.addEventListener("click", () => {
    ...
    const schoolIndex = currentSchoolData?.indexNumber || '';
    const filename = `${schoolIndex}_Certificate_${student.name}.pdf`;
  ...
  certDownloadAllBtn.addEventListener("click", async () => {
    ...
    try {
      for (let i = 0; i < allStudents.length; i++) {
        ...
        const filename = `${schoolIndex}_Certificate_${certNumber}.pdf`;
      ...
      if (isDownloadingAll) {
        alert(`Download Complete! Downloaded: ${downloadedCount} certificates`);
      }
    } catch (error) {
      console.error('Error during batch download:', error);
    } finally {
...
// 4. ADD: Helper functions for calling reports/certificates
function fetchReport() {
  ...
  if (!masterRepResURL) {
    reportDiv.innerHTML = `<div style="color: red; padding: 1rem;">Access not available for ${selectedClass}</div>`;
    return;
  }
...
async function displaystdData(masterRepResURL) {
  ...
  try {
    ...
    if (data.status === 'success') {
      renderStudentReports(data.data);
    }
  } catch (error) {
...
// 5. REPLACE/UPDATE: Add buttons to your results display
// In your existing fetchScores or results rendering function, add these buttons:
/*
...
exportButton.style.cssText = 'background: #28a745; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin: 10px 0;';
exportButton.onclick = () => createExcelFile(students, examType, selectedClass);
buttonGroup.appendChild(exportButton);

// Student Reports Button
const reportBtn = document.createElement('button');
reportBtn.textContent = 'Generate Student Reports';
reportBtn.style.cssText = 'background: #007bff; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin: 10px 5px;';
reportBtn.onclick = () => fetchReport();
buttonGroup.appendChild(reportBtn);

// Certificates Button
const certificateBtn = document.createElement('button');
certificateBtn.textContent = 'Generate Certificates';
certificateBtn.style.cssText = 'background: #ff9800; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; margin: 10px 5px;';
certificateBtn.onclick = () => fetchCertificates();
buttonGroup.appendChild(certificateBtn);
