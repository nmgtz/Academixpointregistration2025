...
if (loginButton && registerButton) {
  loginButton.addEventListener("click", (e) => {
    ...
    // Check for stored credentials when login button is clicked
    const storedCredentials = getStoredCredentials();
    
    if (storedCredentials) {
      // Auto-login with stored credentials
      autoLogin(storedCredentials.username, storedCredentials.password);
    } else {
      // Show login form if no valid credentials
      applyStyles();
...
if (loginLink) {
  loginLink.addEventListener("click", (e) => {
    ...
    // Check for stored credentials when login link is clicked
    const storedCredentials = getStoredCredentials();
...
document.addEventListener("DOMContentLoaded", () => {
  // Don't check credentials on page load - let user stay on homepage
  // Only show login form if there are URL parameters for password reset
...
// Function to save credentials with expiration time (6 hours)
function saveCredentials(username, password) {
  const credentials = {
    username: username,
    password: password,
    expiry: Date.now() + (6 * 60 * 60 * 1000) // 6 hours in milliseconds
  };
  localStorage.setItem('userCredentials', JSON.stringify(credentials));
  localStorage.setItem('isLoggedIn', 'true');
}

// Function to get stored credentials if they haven't expired
...
// Function to auto-login with stored credentials
async function autoLogin(username, password) {
  try {
    ...
    // If auto-login fails, clear credentials and show login form
    clearStoredCredentials();
    applyStyles();
    document.getElementById("header").style.display = "none";
    document.getElementById("dashboard").style.display = "none";
