...
function _axpInjectResultsStyles() {
    ...
    s.textContent = `
        .axp-rr-tab {
            ...
            color: #888; padding: 0.6em 1em; cursor: pointer; font-size: 0.9em;
            transition: all 0.2s; white-space: nowrap;
        }
        .axp-rr-tab:hover { color: var(--axp-primary, #4ecca3); }
        .axp-rr-tab--active {
            color: var(--axp-primary, #4ecca3);
            border-bottom-color: var(--axp-primary, #4ecca3);
            font-weight: 600;
        }
        table.axp-results-table, table.axp-rr-table {
            width: 100%; border-collapse: collapse; font-size: clamp(7px, 0.80vw, 12px);
        ...
        table.axp-rr-table th, table.axp-rr-table td {
            border: 0.5px solid #2a2a3e; padding: 4px 6px;
            white-space: nowrap; overflow: hidden;
        }
        table.axp-results-table th, table.axp-rr-table th {
            background: #0d0d1a; color: var(--axp-primary, #4ecca3); font-weight: 600;
        }
        table.axp-results-table tr:nth-child(even),
        table.axp-rr-table tr:nth-child(even) { background: rgba(255,255,255,0.02); }
        @media print {
            table.axp-results-table td, table.axp-rr-table td {
...
function renderStudentReports(students, container) {
    ...
    function render(index) {
        ...
        if (!eligStudent) {
            ...
            reportDiv.innerHTML = `
                <div style="position:relative;min-height:100vh;background:#fff;overflow:hidden;">
                    <!-- Watermarks -->
                    ${[{t:'50%',l:'50%',fs:'60px'},{t:'20%',l:'20%',fs:'30px'},{t:'80%',l:'20%',fs:'30px'},{t:'20%',l:'80%',fs:'30px'},{t:'80%',l:'80%',fs:'30px'}]
                        .map(p=>`<div style="position:absolute;top:${p.t};left:${p.l};transform:translate(-50%,-50%) rotate(-45deg);font-size:${p.fs};font-weight:bold;color:rgba(0,0,0,0.02);white-space:nowrap;pointer-events:none;">${escapeHtml(student.name.toUpperCase())}</div>`).join('')}
                    <!-- Content -->
                    <div style="position:relative;z-index:2;">
                        <div style="position:absolute;top:10px;left:10px;font-size:11px;font-weight:bold;color:#333;">${escapeHtml(candNum)}</div>
                        <div style="position:absolute;top:10px;right:10px;font-size:11px;font-weight:bold;color:#333;">${escapeHtml(candNum)}</div>
                        <h2 style="text-align:center;margin:40px 0 0;color:#1565c0;">${lang.governmentHeader}</h2>
                        <h2 style="text-align:center;margin:0;color:#1976d2;">${lang.localGov}</h2>
                        <h4 style="text-align:center;margin:10px 0;color:#424242;">${escapeHtml(schoolDisplay)}</h4><br>
                        <h4 style="text-align:center;margin:5px 0;color:#0d47a1;border-top:2px solid #64b5f6;border-bottom:2px solid #64b5f6;padding:4px 0;">${escapeHtml(reportTitle)}</h4><br>
                        <p style="margin:2px 0;color:#424242;line-height:1.4;">
                            ${lang.examNotice} <strong style="color:#1565c0;">${escapeHtml(student.name)}</strong><br>
                            ${lang.satFor} ${escapeHtml(examType.toUpperCase())} ${lang.at}
                            <strong style="color:#1565c0;">${escapeHtml(sd?.indexNumber||schoolDisplay)}</strong>.<br>
                            ${lang.academicStanding}
                        </p>
                        <div style="display:flex;gap:20px;margin-top:8px;flex-wrap:wrap;">
                            <div style="flex:2;min-width:280px;">
                                <h4 style="color:#0d47a1;padding:8px 0;">${lang.performanceTitle}</h4>
                                <table style="width:100%;border-collapse:collapse;">
                                    <thead><tr style="background:#e3f2fd;color:#0d47a1;">
                                        <th style="padding:7px;border:1px solid #90caf9;">${lang.subject}</th>
                                        <th style="padding:7px;border:1px solid #90caf9;">${lang.mark}</th>
                                        <th style="padding:7px;border:1px solid #90caf9;">${lang.comment}</th>
                                    </tr></thead>
                                    <tbody>
                                        ${Object.entries(student.scores||{}).map(([sub,sc],idx)=>{
                                            if(sc.mark==null||sc.mark==='') return '';
                                            return `<tr style="background:${idx%2===0?'#f5f5f5':'#fff'};">
                                                <td style="padding:6px;border:1px solid #e0e0e0;">${escapeHtml(sub)}</td>
                                                <td style="padding:6px;border:1px solid #e0e0e0;color:#1565c0;font-weight:600;">${sc.mark} - '${sc.grade}'</td>
                                                <td style="padding:6px;border:1px solid #e0e0e0;color:#616161;font-style:italic;">(${lang.comments[sc.grade]||'-'})</td>
                                            </tr>`;
                                        }).join('')}
                                        <tr><td colspan="3" style="text-align:center;padding:3px;color:#ccc;">───────────────────────────────────</td></tr>
                                        <tr><td colspan="3" style="padding:7px;border:1px solid #e0e0e0;">
                                            ${lang.points}: <b style="color:#d84315;">${student.point}</b>
                                            &nbsp;&nbsp; ${lang.division}: <b style="color:#d84315;">${student.division}</b>
                                        </td></tr>
                                        <tr><td colspan="3" style="padding:7px;border:1px solid #e0e0e0;">${lang.position}: ${posInfo}</td></tr>
                                    </tbody>
                                </table>
                            </div>
                            <div style="flex:1;min-width:180px;">
                                <h4 style="color:#0d47a1;padding:8px 0;">${lang.behavior}</h4>
                                <table style="width:100%;border-collapse:collapse;">
                                    <thead><tr style="background:#e8eaf6;color:#0d47a1;">
                                        <th style="border:1px solid #9fa8da;padding:7px;">${lang.aspect}</th>
                                        <th style="border:1px solid #9fa8da;padding:7px;">${lang.rating}</th>
                                    </tr></thead>
                                    <tbody>
                                        ${Object.entries({discipline:disc,cooperation:coop,learning:learn,sports,participation:part,punctuality:punct,leadership:lead,neatness:neat,respect:resp,creativity:creat})
                                            ...
                                            <tr style="background:${idx%2===0?'#fafafa':'#fff'};">
                                                <td style="border:1px solid #e0e0e0;padding:5px;">${lang.behaviorAspects[key]}</td>
                                                <td style="border:1px solid #e0e0e0;padding:5px;text-align:center;font-weight:bold;">${val}</td>
                                            </tr>`).join('')}
                                        <tr><td colspan="2" style="border:1px solid #e0e0e0;padding:7px;">
                                            <strong>${lang.remarks}</strong> ${escapeHtml(remark)}
                                        </td></tr>
                                    </tbody>
                                </table>
                            </div>
                        </div>
                        <div style="margin-top:8px;font-size:13px;line-height:1.6;color:#424242;">
                            <p><strong style="color:#1565c0;">${lang.termDates}:</strong>
                                ${lang.closes} <strong style="color:#1565c0;">${escapeHtml(closingDate)}</strong>
                                ${lang.reopens} <strong style="color:#1565c0;">${escapeHtml(openingDate)}</strong>.
                            </p>
                            <p><strong style="color:#1565c0;">${lang.requirements}:</strong></p>
                            <ul style="margin-left:5px;margin-top:-12px;">${reqs}</ul>
                            <p>
                                <strong style="color:#1565c0;">${lang.classComment}:</strong>
                                ${lang.classCommentIntro(`<i>${escapeHtml(student.name.toLowerCase().replace(/\b\w/g,c=>c.toUpperCase()))}</i>`)}
                                ...
                                <br><strong style="color:#1565c0;">${lang.signature}:</strong> ${escapeHtml(classTeacher)}
                                &nbsp;|&nbsp; <strong style="color:#1565c0;">${lang.date}:</strong> ${escapeHtml(closingDate)}
                            </p>
                            <p>
                                ...
                                <br><strong style="color:#1565c0;">${lang.signature}:</strong> ${escapeHtml(headmaster)}
                                &nbsp;|&nbsp; <strong style="color:#1565c0;">${lang.date}:</strong> ${escapeHtml(closingDate)}
                            </p>
                        </div>
                    </div>
    ...
    const _pdfOpt = (filename, landscape=false) => ({
        ...
        html2canvas: { scale:4 },
        jsPDF: { unit:'in', format:'a4', orientation: landscape?'landscape':'portrait' }
    });
...
function renderStudentCertificates(students) {
    ...
    function renderCert(index) {
        ...
        certDiv.innerHTML = `
            <div style="width:210mm;min-height:280mm;margin:0 auto;padding:20mm 15mm;background:#fff;position:relative;font-family:'Georgia',serif;box-sizing:border-box;">
                <!-- Corner decorations -->
                <div style="position:absolute;top:12mm;left:12mm;width:55px;height:55px;border-top:3px solid #DAA520;border-left:3px solid #DAA520;"></div>
                <div style="position:absolute;top:12mm;right:12mm;width:55px;height:55px;border-top:3px solid #DAA520;border-right:3px solid #DAA520;"></div>
                <div style="position:absolute;bottom:12mm;left:12mm;width:55px;height:55px;border-bottom:3px solid #DAA520;border-left:3px solid #DAA520;"></div>
                <div style="position:absolute;bottom:12mm;right:12mm;width:55px;height:55px;border-bottom:3px solid #DAA520;border-right:3px solid #DAA520;"></div>

                <!-- Header -->
                <div style="text-align:center;margin-bottom:35px;">
                    ${schoolLogo?`<img src="${schoolLogo}" style="width:75px;height:75px;margin-bottom:12px;display:block;margin-left:auto;margin-right:auto;">`:''}
                    <h1 style="margin:10px 0;font-size:28px;color:#1565c0;text-transform:uppercase;letter-spacing:3px;font-weight:bold;">${lang.certificate}</h1>
                    <div style="width:160px;height:3px;background:linear-gradient(90deg,transparent,#DAA520,transparent);margin:12px auto;"></div>
                    <h2 style="margin:8px 0;font-size:18px;color:#333;">${escapeHtml(schoolName.toUpperCase())}</h2>
                    <p style="margin:4px 0;font-size:13px;color:#666;">${escapeHtml(schoolIndex.toUpperCase())}</p>
                </div>

                <!-- Main -->
                <div style="text-align:center;margin:50px 0;">
                    <p style="font-size:15px;font-style:italic;color:#666;margin-bottom:25px;">${lang.presentedTo}</p>
                    <h2 style="font-size:28px;color:#1565c0;margin:25px 0;font-weight:bold;text-transform:uppercase;letter-spacing:2px;">${escapeHtml(student.name)}</h2>
                    <div style="width:250px;height:2px;background:#DAA520;margin:25px auto;"></div>
                    <p style="font-size:15px;color:#666;margin:35px 0;line-height:1.8;">${lang.recognition}</p>
                    <p style="font-size:14px;color:#666;margin:25px 0;">${lang.conducted} <strong>${escapeHtml(schoolName.toUpperCase())}</strong></p>
                </div>

                <!-- Signatures -->
                <div style="display:flex;justify-content:space-around;margin-top:60px;padding-top:15px;border-top:1px solid #e0e0e0;">
                    ${[{name:classTeacher,role:lang.classTeacher},{name:headmaster,role:lang.headTeacher}].map(({name,role})=>`
                        <div style="text-align:center;flex:1;">
                            <div style="border-bottom:2px solid #333;width:160px;margin:0 auto 8px;height:30px;"></div>
                            <div style="font-weight:bold;color:#333;">${escapeHtml(name)}</div>
                            <div style="font-size:11px;color:#666;">${role}</div>
                            <div style="font-size:10px;color:#999;margin-top:4px;">${lang.date}: ${escapeHtml(closingDate)}</div>
                        </div>`).join('')}
                </div>

                <!-- Seal -->
                <div style="text-align:center;margin-top:35px;padding-top:8px;border-top:1px solid #e0e0e0;">
                    <div style="display:inline-block;padding:8px 18px;border:2px solid #DAA520;border-radius:50px;font-size:11px;color:#666;">
                        <i class="bi bi-award-fill" style="color:#DAA520;margin-right:4px;"></i> ${lang.seal}
                    </div>
                </div>
            </div>`;
    ...
    const _pdfOpt = filename=>({ margin:0, filename, image:{type:'jpeg',quality:0.98}, html2canvas:{scale:4}, jsPDF:{unit:'mm',format:'a4',orientation:'portrait'} });
