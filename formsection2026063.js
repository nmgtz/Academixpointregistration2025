    ...
    function _axpRRGeneratePDF(action, cls, examType) {
        ...
        const opt = {
            ...
            html2canvas: {
                ...
                backgroundColor: '#ffffff',  // explicit white — prevents transparent bg washing out colours
                windowWidth: _FORCED_W,   // must equal container width — no blank right side
                scrollX: 0,
                scrollY: 0   // captureDiv is position:fixed at top:0 — no scroll offset needed
            },
        ...
        captureDiv.style.cssText = [
            ...
            'box-shadow:none',
            'border:none',
            'z-index:-9999',        // behind everything — user never sees it
            'pointer-events:none'
        ].join(';');
        ...
        // Build runner header strings from live school/exam/class/year data
        const _pdfSi    = _axpSchoolInfo('en');
        const _pdfYear  = (typeof _schoolMeta!=='undefined'&&_schoolMeta&&_schoolMeta.year) ? _schoolMeta.year : new Date().getFullYear();
        const _hdrLeft  = _pdfSi.displayName || _pdfSi.rawName || '';
        const _hdrRight = `${_axpExamLabel(examType,'en')} | ${_axpClassLabel(cls,'en')} | ${_pdfYear}  \u00A9 ${_pdfSi.displayName || ''}`;

        // Build PDF with real per-page header (pages 2+) and footer (all pages).
        // Uses html2pdf worker pipeline to access the jsPDF instance directly.
        const worker = html2pdf().set(opt).from(captureDiv).toPdf();
        const p = worker.get('pdf').then(pdf => {
            ...
            for (let pg = 1; pg <= totalPages; pg++) {
                ...
                // ── HEADER: pages 2+ only (page 1 already has full school header) ──
                if (pg > 1) {
                    // Thin line below header strip
                    pdf.setDrawColor(51, 51, 51);
                    pdf.setLineWidth(0.3);
                    pdf.line(_MARGIN, _MARGIN + 5, pgW - _MARGIN, _MARGIN + 5);
                    // LEFT: school name — bold navy
                    pdf.setFontSize(8);
                    pdf.setFont('helvetica', 'bold');
                    pdf.setTextColor(26, 58, 92);   // #1a3a5c
                    pdf.text(_hdrLeft, _MARGIN, _MARGIN + 3.5);
                    // RIGHT: exam | class | year © school — small grey
                    pdf.setFontSize(6.5);
                    pdf.setFont('helvetica', 'normal');
                    pdf.setTextColor(85, 85, 85);
                    pdf.text(_hdrRight, pgW - _MARGIN, _MARGIN + 3.5, { align: 'right' });
                }

                // ── FOOTER: all pages — thin line + centred Page X of Y ──
                pdf.setDrawColor(180, 180, 180);
                pdf.setLineWidth(0.2);
                pdf.line(_MARGIN, pgH - _MARGIN + 1, pgW - _MARGIN, pgH - _MARGIN + 1);
                pdf.setFontSize(7.5);
                pdf.setFont('helvetica', 'normal');
                pdf.setTextColor(120, 120, 120);
                pdf.text(`Page ${pg} of ${totalPages}`, pgW / 2, pgH - _MARGIN + 3.5, { align: 'center' });
            }
            if (action === 'download') {
...
function renderStudentReports(students, container) {
    ...
    const _pdfOpt = (filename) => ({
        ...
        html2canvas: {
            ...
            windowWidth: _REP_W,
            scrollX: 0,
            scrollY: 0   // captureDiv is position:fixed at top:0 — no scroll offset
        },
            ...
            if (count === 0) {
                ...
                await html2pdf().set({
                    ...
                    html2canvas: {
                        ...
                        scrollX: 0,
                        scrollY: 0   // position:fixed at top:0 — no scroll offset
                    },
