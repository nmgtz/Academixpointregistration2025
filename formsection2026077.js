...
// ── QRCode loader (qrcode-generator) ─────────────────────────────────────────
function _axpGetQRCode() {
    return new Promise((resolve, reject) => {
        if (window.qrcode) { resolve(window.qrcode); return; }
        const s = document.createElement('script');
        s.src = 'https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js';
        s.onload = () => {
            // qrcodejs attaches to window.QRCode — wrap for our use
            if (window.QRCode) { resolve(window.QRCode); return; }
            reject(new Error('QRCode lib loaded but QRCode not found'));
        };
        s.onerror = () => reject(new Error('Failed to load QRCode lib'));
        document.head.appendChild(s);
    });
}
window._axpGetQRCode = _axpGetQRCode;

// ── Pure-JS QR matrix generator (no external lib needed for PDF) ─────────────
// Generates a simple QR-like dot matrix via a tiny inline implementation.
// For PDF we use a basic approach: encode data as Code-128 barcode + text QR fallback.
// We draw a real QR using the inline reed-solomon qr encoder below.
function _axpQRMatrix(text) {
    // Minimal QR matrix builder using a canvas-based approach
    // Returns a 2D boolean array (modules)
    try {
        const canvas = document.createElement('canvas');
        ...
        const step = Math.floor(size/25); // sample 25×25 grid
        const matrix = [];
        for(let row=0;row<25;row++){
            matrix[row]=[];
            for(let col=0;col<25;col++){
                const px = ((row*step)*size + col*step)*4;
                matrix[row][col] = imgData[px] < 128; // dark = true
            }
        }
        return matrix;
    } catch(e) { return null; }
...
function renderStudentReports(students, container) {
    ...
    async function _repCapturePdf(filename, action) {
        try {
            ...
            // ════════════════════════════════════════════════════════════════
            // BOTTOM SECTION — pinned layout:
            //   1. Term dates
            //   2. Requirements (from class info storage)
            //   3. CLASS TEACHER comment  → name (saved) + date (closing date)
            //   4. HEAD OF SCHOOL comment → name (saved) + date (closing date)
            //   5. QR code (right) + Barcode (left) — student data encoded
            // No blank lines for handwriting — all values pre-filled from storage
            // ════════════════════════════════════════════════════════════════

            // ── Term dates ────────────────────────────────────────────────────
            setDraw([170,196,224]); doc.setLineWidth(0.2);
            doc.line(ML, curY, ML+CW, curY); curY += 4;
            doc.setFontSize(8.5); doc.setFont('helvetica','normal'); setTxt([70,70,70]);
            const _cLbl=(lang.closes||'School Closes')+': ';
            const _rLbl=(lang.reopens||'School Opens')+': ';
            doc.text(_cLbl, ML, curY+3);
            doc.setFont('helvetica','bold'); setTxt(black);
            doc.text(closingDate||'—', ML+doc.getTextWidth(_cLbl), curY+3);
            doc.setFont('helvetica','normal'); setTxt([70,70,70]);
            doc.text(_rLbl, ML+CW*0.5, curY+3);
            doc.setFont('helvetica','bold'); setTxt(black);
            doc.text(openingDate||'—', ML+CW*0.5+doc.getTextWidth(_rLbl), curY+3);
            curY += 7;

            // ── Requirements (from locally saved class info) ──────────────────
            if (reqs.length) {
                // Thin navy top border
                setDraw(navy); doc.setLineWidth(0.3);
                doc.line(ML, curY, ML+CW, curY); curY += 2;
                // Header: gold badge label
                rect(ML, curY, CW, 6, [255,248,220], null);
                doc.setFontSize(8); doc.setFont('helvetica','bold'); setTxt([120,80,0]);
                doc.text((lang.requirements||'Requirements for Next Term')+':', ML+2, curY+4);
                curY += 7;
                // Each requirement — compact, no alternating background
                reqs.forEach((r, i) => {
                    const lines = doc.splitTextToSize(`${i+1}. ${r}`, CW-5);
                    doc.setFontSize(8); doc.setFont('helvetica','normal'); setTxt([30,30,30]);
                    lines.forEach((ln, j) => doc.text(ln, ML+3, curY+j*4));
                    curY += lines.length*4 + 1;
                });
                curY += 2;
            }

            // ── CLASS TEACHER comment + pre-filled signature ──────────────────
            setDraw(navy); doc.setLineWidth(0.3);
            doc.line(ML, curY, ML+CW, curY); curY += 3;

            // Heading
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text((lang.classComment||"Class Teacher's Comment")+':', ML, curY+3);
            curY += 5;
            ...
            curY += ctLines.length*4 + 4;

            // Signature block — name (left) | date (right) — NO blank lines, values pre-filled
            const ctRL = currentLang==='sw'?'Mwalimu wa Darasa':'Class Teacher';
            const _dtL = lang.date||'Date';
            const _dX  = ML+CW*0.65;
            // Role labels (tiny grey)
            doc.setFontSize(7); doc.setFont('helvetica','normal'); setTxt([150,150,150]);
            doc.text(ctRL+':', ML, curY);
            doc.text(_dtL+':', _dX, curY);
            curY += 4;
            // Name bold navy | Date bold navy — taken directly from saved class info
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(classTeacher||'—', ML, curY);
            doc.text(closingDate||'—', _dX, curY);
            curY += 5;

            // ── HEAD OF SCHOOL comment + pre-filled signature ─────────────────
            setDraw(navy); doc.setLineWidth(0.3);
            doc.line(ML, curY, ML+CW, curY); curY += 3;

            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text((lang.headComment||"Head of School's Comment")+':', ML, curY+3);
            curY += 5;
            ...
            curY += htLines.length*4 + 4;

            const htRL = currentLang==='sw'?'Mkuu wa Shule':'Head of School';
            doc.setFontSize(7); doc.setFont('helvetica','normal'); setTxt([150,150,150]);
            doc.text(htRL+':', ML, curY);
            doc.text(_dtL+':', _dX, curY);
            curY += 4;
            doc.setFontSize(9); doc.setFont('helvetica','bold'); setTxt(navy);
            doc.text(headmaster||'—', ML, curY);
            doc.text(closingDate||'—', _dX, curY);
            curY += 7;

            // ── QR CODE + BARCODE — pinned to bottom-right ────────────────────
            // QR encodes: name, class, exam, points, division, position, head comment
            const qrPayload = [
                ...
                'Position: '+posInfo,
                'Head: '+htRemark.replace(/[\r\n]+/g,' ')
            ].join(' | ');

            // ── Draw QR code (right side) ─────────────────────────────────────
            // We generate a QR visually using a self-contained micro-QR drawer
            const qrSize  = 22;  // mm
            const qrX     = ML+CW - qrSize;
            const qrY     = curY;

            // Draw QR using canvas → dataURL → jsPDF addImage
            (function(){
                try {
                    const cvs = document.createElement('canvas');
                    cvs.width = 110; cvs.height = 110;
                    const qrObj = new window.QRCode(cvs, {
                        text: qrPayload,
                        width:110, height:110,
                        colorDark:'#000000', colorLight:'#ffffff',
                        correctLevel: window.QRCode && window.QRCode.CorrectLevel
                            ? window.QRCode.CorrectLevel.M : 1
                    });
                    // QRCode lib draws async — read immediately (it draws sync for canvas)
                    const imgData = cvs.toDataURL('image/png',1.0);
                    doc.addImage(imgData,'PNG', qrX, qrY, qrSize, qrSize);
                } catch(e) {
                    // Fallback: draw a simple placeholder box with X
                    setDraw(navy); doc.setLineWidth(0.3);
                    doc.rect(qrX, qrY, qrSize, qrSize,'D');
                    doc.setFontSize(5); setTxt([120,120,120]);
                    doc.text('QR', qrX+qrSize/2, qrY+qrSize/2, {align:'center'});
                }
            })();

            // QR label below
            doc.setFontSize(5.5); doc.setFont('helvetica','normal'); setTxt([120,120,120]);
            doc.text('Scan for student data', qrX+qrSize/2, qrY+qrSize+2.5, {align:'center'});

            // ── Draw Barcode (left side, same row as QR) ─────────────────────
            // Code-128 style barcode drawn as alternating thin/thick bars
            // Encodes: candidateNum + points + division
            const bcText  = `${candNum} | Div:${student.division||'-'} | Pts:${student.point||'-'}`;
            const bcX     = ML;
            const bcY     = qrY;
            const bcW     = CW - qrSize - 4;  // full width minus QR
            const bcH     = 10;

            // Draw bars — simple representation using the text hash
            (function(){
                // Encode each char as a series of bars (simplified visual only)
                const chars = bcText.split('');
                const totalBars = chars.length * 4 + 6;
                const barW = bcW / totalBars;
                let bx = bcX;
                // Start quiet zone + start char
                doc.setFillColor(0,0,0);
                [1,0,1,0,1,1].forEach(dark => {
                    if(dark) doc.rect(bx, bcY, barW*0.8, bcH, 'F');
                    bx += barW;
                });
                // Data bars
                chars.forEach(ch => {
                    ...
                    const pattern = [
                        (code>>5)&1, (code>>4)&1, (code>>3)&1, (code>>2)&1
                    ];
                    pattern.forEach((dark, pi) => {
                        if(dark) doc.rect(bx, bcY, barW*(pi%2===0?0.6:0.9), bcH, 'F');
                        bx += barW;
                    });
                });
                // Stop char
                [1,1,0,1,0,1,1].forEach(dark => {
                    if(dark) doc.rect(bx, bcY, barW*0.8, bcH, 'F');
                    bx += barW;
                });
                ...
                doc.text(bcText, bcX+bcW/2, bcY+bcH+3, {align:'center'});
            })();
...
function renderStudentCertificates(students) {
    ...
    async function _certCapturePdf(filename, action) {
        try {
            ...
            // ── 12. Signature row ─────────────────────────────────────────────
            // Layout (landscape, bottom 30mm):
            //   LEFT col  : Class Teacher sig block
            //   CENTRE    : Official Seal  +  QR code (stacked)
            //   RIGHT col : Head Teacher sig block
            // Full-width barcode strip runs below everything along the very bottom edge

            // Pin signature row to bottom
            const sigTop = PH - PB - 30;
            cy = sigTop;
            ...
            // Column centres
            const colW   = CW * 0.30;
            const ctCX   = CL + colW * 0.5;                 // Class Teacher centre
            const htCX   = CL + CW - colW * 0.5;            // Head Teacher centre
            const midCX  = cx;                               // Centre column

            const sigLineW = 40;
            const sigLineY = cy + 7;

            // ─ Class Teacher ─
            sd([51,51,51]); doc.setLineWidth(0.45);
            doc.line(ctCX-sigLineW/2, sigLineY, ctCX+sigLineW/2, sigLineY);
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            if(classTeacher) tx(classTeacher, ctCX, sigLineY-0.8, {align:'center'});
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            tx(classTeacher||'_______________', ctCX, sigLineY+4, {align:'center'});
            doc.setFontSize(6.5); doc.setFont('helvetica','normal'); st([120,120,120]);
            tx(lang.classTeacher||'Class Teacher', ctCX, sigLineY+8, {align:'center'});
            doc.setFontSize(6); st([160,160,160]);
            ...
            doc.setFontSize(8.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            if(headmaster) tx(headmaster, htCX, sigLineY-0.8, {align:'center'});
            doc.setFontSize(7.5); doc.setFont('helvetica','bold'); st([34,34,34]);
            ...
            // ─ Centre: Official Seal (top) + QR code (below seal) ─
            // QR payload — full student record, no class/division/points on the face
            const certQRPayload = [
                'CERT:'+schoolName.toUpperCase(),
            ...
            // Official seal (small, above QR)
            const sealCy = cy + 3;
            sd(GOLD); doc.setLineWidth(0.6); doc.circle(midCX, sealCy+5, 6, 'D');
            sd(GOLD); doc.setLineWidth(0.25); doc.circle(midCX, sealCy+5, 4.5, 'D');
            const sealAbbr = schoolName.split(' ').slice(0,2).map(w=>w[0]||'').join('');
            doc.setFontSize(6); doc.setFont('helvetica','bold'); st(NAVY);
            tx(sealAbbr, midCX, sealCy+7, {align:'center'});
            doc.setFontSize(3.5); doc.setFont('helvetica','normal'); st(NAVY);
            tx(lang.seal||'OFFICIAL SEAL', midCX, sealCy+11, {align:'center'});

            // QR code — drawn below seal, centred in middle column
            const qrSzC = 18;  // mm — compact but scannable
            const qrXC  = midCX - qrSzC/2;
            const qrYC  = sealCy + 13;
            (function(){
                try {
                    const cvs = document.createElement('canvas');
                    cvs.width = 140; cvs.height = 140;
                    new window.QRCode(cvs, {
                        text: certQRPayload,
                        width:140, height:140,
                        colorDark:'#000000', colorLight:'#ffffff',
                        correctLevel: window.QRCode?.CorrectLevel?.M || 1
                    });
                    // Gold border frame around QR
                    sd(GOLD); doc.setLineWidth(0.4);
                    doc.rect(qrXC-1, qrYC-1, qrSzC+2, qrSzC+2, 'D');
                    doc.addImage(cvs.toDataURL('image/png',1.0),'PNG', qrXC, qrYC, qrSzC, qrSzC);
                } catch(e) {
                    sd(GOLD); doc.setLineWidth(0.4);
                    doc.rect(qrXC-1, qrYC-1, qrSzC+2, qrSzC+2, 'D');
                    doc.setFontSize(5); st([120,120,120]);
                    tx('QR', midCX, qrYC+qrSzC/2, {align:'center'});
                }
            })();
            // Tiny label under QR
            doc.setFontSize(4.5); doc.setFont('helvetica','normal'); st([140,140,140]);
            tx('Scan to verify', midCX, qrYC+qrSzC+3, {align:'center'});

            // ── Full-width barcode strip — pinned to very bottom ──────────────
            // Encodes: school index + student name + candidate sequence
            const _cleanIdxC = (schoolIdx||'SCH').replace(/[^A-Z0-9]/gi,'').toUpperCase();
            const bcCandNum  = _cleanIdxC+'-'+String(currentIndex+1).padStart(4,'0');
            const bcCertText = bcCandNum+' | '+student.name.toUpperCase();

            const bcCY  = PH - PB - 5;   // 5mm from bottom content edge
            const bcCX  = CL;
            const bcCW  = CW;
            const bcCH  = 7;

            (function(){
                const chars     = bcCertText.split('');
                const totalBars = chars.length * 4 + 8;
                const bw        = bcCW / totalBars;
                let bx          = bcCX;
                doc.setFillColor(0,0,0);
                // Start guard bars
                [1,0,1,0,1,1,0].forEach(dark => {
                    if(dark) doc.rect(bx, bcCY, bw*0.75, bcCH, 'F');
                    bx += bw;
                });
                // Data
                chars.forEach(ch => {
                    const c = ch.charCodeAt(0);
                    [(c>>5)&1,(c>>4)&1,(c>>3)&1,(c>>2)&1].forEach((dark,pi) => {
                        if(dark) doc.rect(bx, bcCY, bw*(pi%2===0?0.55:0.85), bcCH, 'F');
                        bx += bw;
                    });
                });
                // Stop guard bars
                [0,1,1,0,1,0,1].forEach(dark => {
                    if(dark) doc.rect(bx, bcCY, bw*0.75, bcCH, 'F');
                    bx += bw;
                });
                // Human-readable text centred below bars
                doc.setFontSize(5); doc.setFont('helvetica','normal');
                doc.setTextColor(60,60,60);
                doc.text(bcCertText, CL+CW/2, bcCY+bcCH+2.5, {align:'center'});
            })();
